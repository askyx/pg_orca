...
class CTranslatorUtils {
 private:
  ...
  static IMdIdArray *ResolvePolymorphicTypes(CMemoryPool *mp, IMdIdArray *return_arg_mdids, List *input_arg_types,
                                             FuncExpr *funcexpr);
 ...
 public:
  ...
  static EdxlIndexScanDirection ConvertToDXLIndexScanDirection(ScanDirection sd);

  // create a DXL index descriptor from an index MD id
  static CDXLIndexDescr *GetIndexDescr(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid);
  ...
  static CDXLColDescrArray *GetColumnDescriptorsFromRecord(CMemoryPool *mp, CIdGenerator *id_generator, List *col_names,
  ...
  static CDXLColDescrArray *GetColumnDescriptorsFromComposite(CMemoryPool *mp, CMDAccessor *md_accessor,
                                                              CIdGenerator *id_generator, const IMDType *md_type);
  ...
  static ULongPtrArray *GetOutputColIdsArray(CMemoryPool *mp, List *target_list, IntToUlongMap *attno_to_colid_map);

  // construct an array of column ids for the given group by set
  static ULongPtrArray *GetGroupingColidArray(CMemoryPool *mp, CBitSet *group_by_cols,
  ...
  static CDXLNode *CreateDXLProjElemConstNULL(CMemoryPool *mp, CMDAccessor *md_accessor, CIdGenerator *colid_generator,
                                              const IMDColumn *col);
