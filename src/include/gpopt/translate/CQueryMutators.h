...
class CQueryMutators {
  ...
  using CContextIncLevelsupMutator = struct SContextIncLevelsupMutator {
   public:
    ...
    // ctor
    SContextIncLevelsupMutator(ULONG current_query_level, BOOL should_fix_top_level_target_list)
 ...
 public:
  ...
  // normalize query
  static Query *NormalizeQuery(CMemoryPool *mp, CMDAccessor *md_accessor, const Query *query, ULONG query_level);
  ...
  static TargetEntry *GetTargetEntryForAggExpr(CMemoryPool *mp, CMDAccessor *md_accessor, Node *node, ULONG attno);
  ...
  static TargetEntry *MakeTopLevelTargetEntry(TargetEntry *target_entry, ULONG attno);
