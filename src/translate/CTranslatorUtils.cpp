...
//
//---------------------------------------------------------------------------
CDXLIndexDescr *CTranslatorUtils::GetIndexDescr(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid) {
...
//
//---------------------------------------------------------------------------
CDXLTableDescr *CTranslatorUtils::GetTableDescr(CMemoryPool *mp, CMDAccessor *md_accessor, CIdGenerator *id_generator,
  ...
  // look up table name
  CMDName *table_mdname = rte->alias ? GPOS_NEW(mp) CMDName(GPOS_NEW(mp) CWStringConst(mp, rte->alias->aliasname), true)
                                     : GPOS_NEW(mp) CMDName(mp, rel->Mdname().GetMDName());

  // uint32_t required_perms = static_cast<uint32_t>(rte->requiredPerms);
  uint32_t required_perms = 0;
  CDXLTableDescr *table_descr = GPOS_NEW(mp)
      CDXLTableDescr(mp, mdid, table_mdname, 0, rte->rellockmode, required_perms, assigned_query_id_for_target_rel);
  ...
  for (uint32_t ul = 0; ul < len; ul++) {
    ...
    CDXLColDescr *dxl_col_descr = GPOS_NEW(mp) CDXLColDescr(col, id_generator->next_id(), md_col->AttrNum(), col_type,
...
//
//---------------------------------------------------------------------------
bool CTranslatorUtils::IsSirvFunc(CMemoryPool *mp, CMDAccessor *md_accessor, OID func_oid) {
  ...
  if (GPDB_NEXTVAL == func_oid || GPDB_CURRVAL == func_oid || GPDB_SETVAL == func_oid) {
    return false;
  }
  ...
  bool is_sirv = (!func->ReturnsSet() && IMDFunction::EfsVolatile == func->GetFuncStability());
...
//
//---------------------------------------------------------------------------
bool CTranslatorUtils::HasSubquery(Node *node) {
  List *unsupported_list = ListMake1Int(T_SubLink);
  int32_t unsupported = gpdb::FindNodes(node, unsupported_list);
...
//
//---------------------------------------------------------------------------
CDXLLogicalTVF *CTranslatorUtils::ConvertToCDXLLogicalTVF(CMemoryPool *mp, CMDAccessor *md_accessor,
                                                          CIdGenerator *id_generator, const RangeTblEntry *rte) {
  /*
  ...
  if (IsA(rtfunc->funcexpr, Const)) {
    ...
    CMDIdGPDB *mdid_return_type = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, constExpr->consttype);

    const IMDType *type = md_accessor->RetrieveType(mdid_return_type);
    CDXLColDescrArray *column_descrs = GetColumnDescriptorsFromComposite(mp, md_accessor, id_generator, type);

    CMDName *func_name = CDXLUtils::CreateMDNameFromCharArray(mp, rte->eref->aliasname);

    // if TVF evaluates to const, pass invalid key as funcid
    CDXLLogicalTVF *tvf_dxl = GPOS_NEW(mp)
        CDXLLogicalTVF(mp, GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, 0), mdid_return_type, func_name, column_descrs);
  ...
  // get function id
  CMDIdGPDB *mdid_func = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, funcexpr->funcid);
  CMDIdGPDB *mdid_return_type = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, funcexpr->funcresulttype);
  const IMDType *type = md_accessor->RetrieveType(mdid_return_type);
  ...
  if (nullptr != rtfunc->funccoltypes) {
    ...
    column_descrs = GetColumnDescriptorsFromRecord(mp, id_generator, rte->eref->colnames, rtfunc->funccoltypes,
    ...
    column_descrs =
        GetColumnDescriptorsFromBase(mp, id_generator, mdid_return_type, default_type_modifier, &func_mdname);
  }
...
//
//---------------------------------------------------------------------------
IMdIdArray *CTranslatorUtils::ResolvePolymorphicTypes(CMemoryPool *mp, IMdIdArray *return_arg_mdids,
                                                      List *input_arg_types, FuncExpr *funcexpr) {
  uint32_t arg_index = 0;

  const uint32_t num_arg_types = gpdb::ListLength(input_arg_types);
  const uint32_t num_args_from_query = gpdb::ListLength(funcexpr->args);
  const uint32_t num_return_args = return_arg_mdids->Size();
  const uint32_t num_args = std::min(num_arg_types, num_args_from_query);
  const uint32_t total_args = num_args + num_return_args;
  ...
  for (uint32_t ul = 0; ul < num_return_args; ul++) {
  ...
  if (!gpdb::ResolvePolymorphicArgType(total_args, arg_types, arg_modes, funcexpr)) {
    GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiDXLUnrecognizedType,
  ...
  for (uint32_t ul = num_args; ul < total_args; ul++) {
    IMDId *resolved_mdid = nullptr;
    resolved_mdid = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, arg_types[ul]);
...
//
//---------------------------------------------------------------------------
bool CTranslatorUtils::ContainsPolymorphicTypes(IMdIdArray *mdid_array) {
  GPOS_ASSERT(nullptr != mdid_array);
  const uint32_t len = mdid_array->Size();
  for (uint32_t ul = 0; ul < len; ul++) {
...
//
//---------------------------------------------------------------------------
CDXLColDescrArray *CTranslatorUtils::GetColumnDescriptorsFromRecord(CMemoryPool *mp, CIdGenerator *id_generator,
                                                                    List *col_names, List *col_types,
  ...
  forthree(col_name, col_names, col_type, col_types, col_type_modifier, col_type_modifiers) {
    ...
    int32_t type_modifier = lfirst_int(col_type_modifier);
    ...
    CDXLColDescr *dxl_col_descr =
        GPOS_NEW(mp) CDXLColDescr(col_mdname, id_generator->next_id(), int32_t(ul + 1) /* attno */, col_type,
                                  type_modifier, false /* fColDropped */
        );
...
//---------------------------------------------------------------------------
CDXLColDescrArray *CTranslatorUtils::GetColumnDescriptorsFromRecord(CMemoryPool *mp, CIdGenerator *id_generator,
                                                                    List *col_names, IMdIdArray *out_arg_types) {
  GPOS_ASSERT(out_arg_types->Size() == (uint32_t)gpdb::ListLength(col_names));
  ListCell *col_name = nullptr;
  ...
  foreach (col_name, col_names) {
    ...
    CDXLColDescr *dxl_col_descr =
        GPOS_NEW(mp) CDXLColDescr(col_mdname, id_generator->next_id(), int32_t(ul + 1) /* attno */, col_type,
                                  default_type_modifier, false /* fColDropped */
        );
    column_descrs->Append(dxl_col_descr);
    ul++;
  }
...
//
//---------------------------------------------------------------------------
CDXLColDescrArray *CTranslatorUtils::GetColumnDescriptorsFromBase(CMemoryPool *mp, CIdGenerator *id_generator,
  ...
  CDXLColDescr *dxl_col_descr = GPOS_NEW(mp) CDXLColDescr(col_mdname, id_generator->next_id(), int32_t(1) /* attno */,
...
//
//---------------------------------------------------------------------------
CDXLColDescrArray *CTranslatorUtils::GetColumnDescriptorsFromComposite(CMemoryPool *mp, CMDAccessor *md_accessor,
  ...
  for (uint32_t ul = 0; ul < col_ptr_arr->Size(); ul++) {
...
//
//---------------------------------------------------------------------------
CMDColumnArray *CTranslatorUtils::ExpandCompositeType(CMemoryPool *mp, CMDAccessor *md_accessor, const IMDType *type) {
  ...
  for (uint32_t ul = 0; ul < rel->ColumnCount(); ul++) {
...
//
//---------------------------------------------------------------------------
EdxlJoinType CTranslatorUtils::ConvertToDXLJoinType(JoinType jt) {
...
//
//---------------------------------------------------------------------------
EdxlIndexScanDirection CTranslatorUtils::ConvertToDXLIndexScanDirection(ScanDirection sd) {
...
//
//---------------------------------------------------------------------------
const CDXLColDescr *CTranslatorUtils::GetColumnDescrAt(const CDXLTableDescr *table_descr, uint32_t pos) {
  GPOS_ASSERT(0 != pos);
...
//
//---------------------------------------------------------------------------
ScanDirection CTranslatorUtils::GetScanDirection(EdxlIndexScanDirection idx_scan_direction) {
...
//
//---------------------------------------------------------------------------
EdxlSetOpType CTranslatorUtils::GetSetOpType(SetOperation setop, bool is_all) {
...
//
//---------------------------------------------------------------------------
ULongPtrArray *CTranslatorUtils::GetGroupingColidArray(CMemoryPool *mp, CBitSet *group_by_cols,
  ...
  if (nullptr != group_by_cols) {
    ...
    while (bsi.Advance()) {
      const uint32_t colid = GetColId(bsi.Bit(), sort_group_cols_to_colid_map);
...
//
//---------------------------------------------------------------------------
CBitSetArray *CTranslatorUtils::GetColumnAttnosForGroupBy(
    CMemoryPool *mp, List *group_clause_list, List *grouping_set_list, uint32_t num_cols,
  ...
  foreach (cell, grouping_set_list) {
    ...
    switch (grouping_set->kind) {
      ...
      case GROUPING_SET_ROLLUP: {
        col_attnos_arr_current = CreateGroupingSetsForRollup(mp, grouping_set, num_cols, group_cols, group_col_pos);
        break;
      }
      case GROUPING_SET_CUBE: {
        col_attnos_arr_current = CreateGroupingSetsForCube(mp, grouping_set, num_cols, group_cols, group_col_pos);
        break;
      }
      case GROUPING_SET_SETS: {
        col_attnos_arr_current = CreateGroupingSetsForSets(mp, grouping_set, num_cols, group_cols, group_col_pos);
        break;
      }
      case GROUPING_SET_SIMPLE: {
        col_attnos_arr_current = CreateGroupingSetsForSimple(mp, grouping_set, num_cols, group_cols, group_col_pos);
        break;
      }
      default: {
        /* can't happen */
        GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLError, GPOS_WSZ_LIT("Unrecognized grouping set kind"));
      }
    }
    ...
    uint32_t col_attnos_arr_size = col_attnos_arr->Size();
    if (col_attnos_arr_size > 0) {
      CBitSetArray *col_attnos_arr_temp = GPOS_NEW(mp) CBitSetArray(mp);

      for (uint32_t ul = 0; ul < col_attnos_arr_size; ul++) {
        for (uint32_t ulInner = 0; ulInner < col_attnos_arr_current->Size(); ulInner++) {
          CBitSet *bset = GPOS_NEW(mp) CBitSet(mp, *(*col_attnos_arr)[ul]);
  ...
  foreach (cell, grouping_set->content) {
    ...
    switch (gs_current->kind) {
      ...
      case GROUPING_SET_SIMPLE:
        ...
        break;
      default:
        GPOS_RAISE(ExmaDXL, ExmiQuery2DXLUnsupportedFeature, GPOS_WSZ_LIT("nested grouping set"));
    }
...
//
//---------------------------------------------------------------------------
CBitSetArray *CTranslatorUtils::CreateGroupingSetsForSimple(CMemoryPool *mp, const GroupingSet *grouping_set,
                                                            uint32_t num_cols, CBitSet *group_cols,
                                                            UlongToUlongMap *group_col_pos) {
  GPOS_ASSERT(nullptr != grouping_set);
  ...
  CBitSet *bset = CreateAttnoSetForGroupingSet(mp, grouping_set->content, num_cols, group_col_pos, group_cols,
                                               false /* use_group_clause */);
  col_attnos_arr->Append(bset);
  return col_attnos_arr;
}
...
//
//---------------------------------------------------------------------------
CBitSetArray *CTranslatorUtils::CreateGroupingSetsForRollup(CMemoryPool *mp, const GroupingSet *grouping_set,
                                                            uint32_t num_cols, CBitSet *group_cols,
                                                            UlongToUlongMap *group_col_pos) {
  GPOS_ASSERT(nullptr != grouping_set);
...
//
//---------------------------------------------------------------------------
CBitSetArray *CTranslatorUtils::CreateGroupingSetsForCube(CMemoryPool *mp, const GroupingSet *grouping_set,
                                                          uint32_t num_cols, CBitSet *group_cols,
                                                          UlongToUlongMap *group_col_pos) {
  GPOS_ASSERT(nullptr != grouping_set);
  ...
  ListCell *lc = nullptr;
  foreach (lc, grouping_set->content) {
    uint32_t current_results_size = col_attnos_arr->Size();

    for (uint32_t ul = 0; ul < current_results_size; ul++) {
      CBitSet *current_result = GPOS_NEW(mp) CBitSet(mp, *(*col_attnos_arr)[ul]);
...
//
//---------------------------------------------------------------------------
CBitSet *CTranslatorUtils::CreateAttnoSetForGroupingSet(
    CMemoryPool *mp, List *group_elems, uint32_t num_cols,
    UlongToUlongMap *group_col_pos,  // mapping of grouping col positions to SortGroupRef ids
...
//
//---------------------------------------------------------------------------
ULongPtrArray *CTranslatorUtils::GenerateColIds(
    CMemoryPool *mp, List *target_list, IMdIdArray *input_mdid_arr, ULongPtrArray *input_colids,
    const bool *is_outer_ref,  // array of flags indicating if input columns are outer references
  ...
  foreach (target_entry_cell, target_list) {
    ...
    if (!target_entry->resjunk) {
      ...
      if (CMDIdGPDB::CastMdid(mdid)->Oid() != expr_type_oid || is_outer_ref[col_pos]) {
      ...
      colid_array->Append(GPOS_NEW(mp) uint32_t(colid));
...
//---------------------------------------------------------------------------
Query *CTranslatorUtils::FixUnknownTypeConstant(Query *old_query, List *output_target_list) {
  ...
  foreach (target_entry_cell, old_query->targetList) {
    ...
    if (!old_target_entry->resjunk) {
      if (IsA(old_target_entry->expr, Const) && (GPDB_UNKNOWN == gpdb::ExprType((Node *)old_target_entry->expr))) {
        ...
        new_target_entry->expr =
            (Expr *)gpdb::CoerceToCommonType(nullptr, /* pstate */
                                             (Node *)old_node, target_type_oid, "UNION/INTERSECT/EXCEPT");
...
//
//---------------------------------------------------------------------------
OID CTranslatorUtils::GetTargetListReturnTypeOid(List *target_list, uint32_t col_pos) {
  uint32_t col_idx = 0;
  ListCell *target_entry_cell = nullptr;
...
//
//---------------------------------------------------------------------------
CDXLColDescrArray *CTranslatorUtils::GetDXLColumnDescrArray(CMemoryPool *mp, List *target_list, ULongPtrArray *colids,
...
//		target list
//---------------------------------------------------------------------------
ULongPtrArray *CTranslatorUtils::GetPosInTargetList(CMemoryPool *mp, List *target_list, bool keep_res_junked) {
  ListCell *target_entry_cell = nullptr;
  ...
  foreach (target_entry_cell, target_list) {
    ...
    positions->Append(GPOS_NEW(mp) uint32_t(ul));
    ul++;
  }
...
//		identifier
//---------------------------------------------------------------------------
CDXLColDescr *CTranslatorUtils::GetColumnDescrAt(CMemoryPool *mp, TargetEntry *target_entry, uint32_t colid,
                                                 uint32_t pos) {
  ...
  CDXLColDescr *dxl_col_descr = GPOS_NEW(mp) CDXLColDescr(mdname, colid, pos,      /* attno */
...
//---------------------------------------------------------------------------
CDXLNode *CTranslatorUtils::CreateDummyProjectElem(CMemoryPool *mp, uint32_t colid_input, uint32_t colid_output,
                                                   CDXLColDescr *dxl_col_descr) {
  CMDIdGPDB *original_mdid = CMDIdGPDB::CastMdid(dxl_col_descr->MdidType());
  CMDIdGPDB *copy_mdid = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, original_mdid->Oid(),
  ...
  CDXLColRef *dxl_colref = GPOS_NEW(mp) CDXLColRef(mdname, colid_input, copy_mdid, dxl_col_descr->TypeModifier());
  ...
  CDXLNode *dxl_project_element = GPOS_NEW(mp) CDXLNode(
      mp,
      GPOS_NEW(mp) CDXLScalarProjElem(mp, colid_output, GPOS_NEW(mp) CMDName(mp, dxl_col_descr->MdName()->GetMDName())),
...
//
//---------------------------------------------------------------------------
ULongPtrArray *CTranslatorUtils::GetOutputColIdsArray(CMemoryPool *mp, List *target_list,
  ...
  foreach (target_entry_cell, target_list) {
    TargetEntry *target_entry = (TargetEntry *)lfirst(target_entry_cell);
    uint32_t resno = (uint32_t)target_entry->resno;
    int32_t attno = (int32_t)target_entry->resno;
    const uint32_t *ul = attno_to_colid_map->Find(&attno);

    if (nullptr == ul) {
      GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLAttributeNotFound, resno);
    }

    colids->Append(GPOS_NEW(mp) uint32_t(*ul));
  }
...
//
//---------------------------------------------------------------------------
uint32_t CTranslatorUtils::GetColId(int32_t index, IntToUlongMap *colid_map) {
  GPOS_ASSERT(0 < index);

  const uint32_t *ul = colid_map->Find(&index);
...
//
//---------------------------------------------------------------------------
uint32_t CTranslatorUtils::GetColId(uint32_t query_level, int32_t varno, int32_t var_attno, IMDId *mdid,
                                    CMappingVarColId *var_colid_mapping) {
  OID oid = CMDIdGPDB::CastMdid(mdid)->Oid();
  Var *var = gpdb::MakeVar(varno, var_attno, oid, -1, 0);
  uint32_t colid = var_colid_mapping->GetColId(query_level, var, EpspotNone);
  gpdb::GPDBFree(var);
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorUtils::CreateDXLProjElemFromInt8Const(CMemoryPool *mp, CMDAccessor *md_accessor, int32_t val) {
...
//---------------------------------------------------------------------------
bool CTranslatorUtils::HasOrderedAggRefInProjList(CDXLNode *proj_list_dxlnode) {
  GPOS_ASSERT(nullptr != proj_list_dxlnode &&
              EdxlopScalarProjectList == proj_list_dxlnode->GetOperator()->GetDXLOperator());
...
//---------------------------------------------------------------------------
TargetEntry *CTranslatorUtils::GetGroupingColumnTargetEntry(Node *node, List *group_clause, List *target_list) {
  GPOS_ASSERT(nullptr != node);
  List *target_list_subset = gpdb::FindMatchingMembersInTargetList(node, target_list);
...
//---------------------------------------------------------------------------
bool CTranslatorUtils::IsGroupingColumn(Node *node, List *group_clause, List *target_list) {
  GPOS_ASSERT(nullptr != node);
...
bool CTranslatorUtils::IsGroupingColumn(const TargetEntry *target_entry, List *group_clause) {
  ...
  foreach (group_clause_cell, group_clause) {
    ...
    if (IsGroupingColumn(target_entry, (SortGroupClause *)group_clause_node)) {
      return true;
    }
  }
...
//		Check if the TargetEntry is a grouping column
//---------------------------------------------------------------------------
bool CTranslatorUtils::IsGroupingColumn(const TargetEntry *target_entry, const SortGroupClause *grouping_clause) {
...
//
//---------------------------------------------------------------------------
int64_t CTranslatorUtils::GetLongFromStr(const CWStringBase *wcstr) {
...
//
//---------------------------------------------------------------------------
int32_t CTranslatorUtils::GetIntFromStr(const CWStringBase *wcstr) {
  return (int32_t)GetLongFromStr(wcstr);
}
...
//
//---------------------------------------------------------------------------
char *CTranslatorUtils::CreateMultiByteCharStringFromWCString(const wchar_t *wcstr) {
  GPOS_ASSERT(nullptr != wcstr);

  uint32_t max_len = GPOS_WSZ_LENGTH(wcstr) * GPOS_SIZEOF(wchar_t) + 1;
  char *str = (char *)gpdb::GPDBAlloc(max_len);
#ifdef GPOS_DEBUG
  int64_t li = (int32_t)
#endif
      clib::Wcstombs(str, const_cast<wchar_t *>(wcstr), max_len);
  GPOS_ASSERT(0 <= li);
...
//
//---------------------------------------------------------------------------
UlongToUlongMap *CTranslatorUtils::MakeNewToOldColMapping(CMemoryPool *mp, ULongPtrArray *old_colids,
  ...
  const uint32_t num_cols = old_colids->Size();
  for (uint32_t ul = 0; ul < num_cols; ul++) {
    uint32_t old_colid = *((*old_colids)[ul]);
    uint32_t new_colid = *((*new_colids)[ul]);
#ifdef GPOS_DEBUG
    bool result =
#endif  // GPOS_DEBUG
        old_new_col_mapping->Insert(GPOS_NEW(mp) uint32_t(old_colid), GPOS_NEW(mp) uint32_t(new_colid));
    GPOS_ASSERT(result);
  }
...
//
//---------------------------------------------------------------------------
bool CTranslatorUtils::HasProjElem(CDXLNode *project_list_dxlnode, Edxlopid dxl_opid) {
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorUtils::CreateDXLProjElemConstNULL(CMemoryPool *mp, CMDAccessor *md_accessor,
                                                       CIdGenerator *pidgtorCol, const IMDColumn *md_col) {
  GPOS_ASSERT(nullptr != md_col);
  GPOS_ASSERT(!md_col->IsSystemColumn());

  const wchar_t *col_name = md_col->Mdname().GetMDName()->GetBuffer();
  uint32_t colid = pidgtorCol->next_id();
...
//---------------------------------------------------------------------------
CDXLNode *CTranslatorUtils::CreateDXLProjElemConstNULL(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid,
                                                       uint32_t colid, const wchar_t *col_name) {
...
//---------------------------------------------------------------------------
CDXLNode *CTranslatorUtils::CreateDXLProjElemConstNULL(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid,
                                                       uint32_t colid, char *alias_name) {
  ...
  if (mdid->Equals(&CMDIdGPDB::m_mdid_int2)) {
    ...
    datum_dxl = GPOS_NEW(mp) CDXLDatumBool(mp, mdid, true /*fConstNull*/, false /*value*/);
    ...
    datum_dxl = CMDTypeGenericGPDB::CreateDXLDatumVal(mp, mdid, md_type, default_type_modifier, true /*fConstNull*/,
                                                      nullptr,                       /*pba */
...
//
//---------------------------------------------------------------------------
void CTranslatorUtils::CheckRTEPermissions(List *range_table_list) {
...
//
//---------------------------------------------------------------------------
void CTranslatorUtils::UpdateGrpColMapping(CMemoryPool *mp, UlongToUlongMap *group_col_pos, CBitSet *group_cols,
                                           uint32_t sort_group_ref) {
  GPOS_ASSERT(nullptr != group_col_pos);
  GPOS_ASSERT(nullptr != group_cols);

  if (!group_cols->Get(sort_group_ref)) {
    uint32_t num_unique_grouping_cols = group_cols->Size();
    group_col_pos->Insert(GPOS_NEW(mp) uint32_t(num_unique_grouping_cols), GPOS_NEW(mp) uint32_t(sort_group_ref));
...
//		given node
//---------------------------------------------------------------------------
void CTranslatorUtils::MarkOuterRefs(
    uint32_t *colids,      // array of column ids to be checked
  ...
  for (uint32_t ulCol = 0; ulCol < num_columns; ulCol++) {
    uint32_t colid = colids[ulCol];
...
//
//---------------------------------------------------------------------------
SubLinkType CTranslatorUtils::MapDXLSubplanToSublinkType(EdxlSubPlanType dxl_subplan_type) {
  GPOS_ASSERT(EdxlSubPlanTypeSentinel > dxl_subplan_type);
  uint32_t mapping[][2] = {{EdxlSubPlanTypeScalar, EXPR_SUBLINK},
  ...
  for (uint32_t ul = 0; ul < arity; ul++) {
    ...
    if ((uint32_t)dxl_subplan_type == elem[0]) {
...
//
//---------------------------------------------------------------------------
EdxlSubPlanType CTranslatorUtils::MapSublinkTypeToDXLSubplan(SubLinkType slink) {
  uint32_t mapping[][2] = {{EXPR_SUBLINK, EdxlSubPlanTypeScalar},
...
//
//---------------------------------------------------------------------------
bool CTranslatorUtils::RelHasConstraints(const IMDRelation *rel) {
  if (0 < rel->CheckConstraintCount()) {
    return true;
  }

  const uint32_t num_cols = rel->ColumnCount();
...
//
//---------------------------------------------------------------------------
List *CTranslatorUtils::GetAssertErrorMsgs(CDXLNode *assert_constraint_list) {
  ...
  const uint32_t num_constraints = assert_constraint_list->Arity();

  for (uint32_t ul = 0; ul < num_constraints; ul++) {
    CDXLNode *dxl_constraint_node = (*assert_constraint_list)[ul];
    CDXLScalarAssertConstraint *dxl_constraint_op =
        CDXLScalarAssertConstraint::Cast(dxl_constraint_node->GetOperator());
    CWStringBase *error_msg = dxl_constraint_op->GetErrorMsgStr();
    error_msgs_list = gpdb::LAppend(
        error_msgs_list, gpdb::MakeStringValue(CreateMultiByteCharStringFromWCString(error_msg->GetBuffer())));
  }
...
//
//---------------------------------------------------------------------------
uint32_t CTranslatorUtils::GetNumNonSystemColumns(const IMDRelation *rel) {
  GPOS_ASSERT(nullptr != rel);

  uint32_t num_non_system_cols = 0;
...
EdxlAggrefKind CTranslatorUtils::GetAggKind(char aggkind) {
  switch (aggkind) {
    case 'n': {
    ...
    default: {
      GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiExpr2DXLAttributeNotFound, GPOS_WSZ_LIT("Unknown aggkind value"));
    }
  }
}

char CTranslatorUtils::GetAggKind(EdxlAggrefKind aggkind) {
  switch (aggkind) {
    ...
    default: {
      GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiDXL2ExprAttributeNotFound, GPOS_WSZ_LIT("Unknown aggkind value"));
    }
  }
}
...
//---------------------------------------------------------------------------
bool CTranslatorUtils::IsCompositeConst(CMemoryPool *mp, CMDAccessor *md_accessor, const RangeTblFunction *rtfunc) {
  if (!IsA(rtfunc->funcexpr, Const)) {
    return false;
  }
...
bool CTranslatorUtils::RelContainsForeignPartitions(const IMDRelation *rel, CMDAccessor *md_accessor) {
  IMdIdArray *partition_mdids = rel->ChildPartitionMdids();
  for (uint32_t ul = 0; partition_mdids && ul < partition_mdids->Size(); ++ul) {
    IMDId *part_mdid = (*partition_mdids)[ul];
    const IMDRelation *partrel = md_accessor->RetrieveRel(part_mdid);
    if (partrel->RetrieveRelStorageType() == IMDRelation::ErelstorageForeign) {
      return true;
    }
  }
  return false;
