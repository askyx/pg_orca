...
static const OID lead_func_oids[] = {
    7011, 7074, 7075, 7310, 7312, 7314, 7316, 7318, 7320, 7322, 7324, 7326, 7328, 7330, 7332, 7334, 7336, 7338,
    7340, 7342, 7344, 7346, 7348, 7350, 7352, 7354, 7356, 7358, 7360, 7362, 7364, 7366, 7368, 7370, 7372, 7374,
    7376, 7378, 7380, 7382, 7384, 7386, 7388, 7390, 7392, 7394, 7396, 7398, 7400, 7402, 7404, 7406, 7408, 7410,
    7412, 7414, 7416, 7418, 7420, 7422, 7424, 7426, 7428, 7430, 7432, 7434, 7436, 7438, 7440, 7442, 7444, 7446,
    7448, 7450, 7452, 7454, 7456, 7458, 7460, 7462, 7464, 7466, 7468, 7470, 7472, 7474, 7476, 7478, 7480, 7482,
    7484, 7486, 7488, 7214, 7215, 7216, 7220, 7222, 7224, 7244, 7246, 7248, 7260, 7262, 7264};

// OIDs of variants of LAG window function
static const OID lag_func_oids[] = {
    7675, 7491, 7493, 7495, 7497, 7499, 7501, 7503, 7505, 7507, 7509, 7511, 7513, 7515, 7517, 7519, 7521, 7523,
    7525, 7527, 7529, 7531, 7533, 7535, 7537, 7539, 7541, 7543, 7545, 7547, 7549, 7551, 7553, 7555, 7557, 7559,
    7561, 7563, 7565, 7567, 7569, 7571, 7573, 7575, 7577, 7579, 7581, 7583, 7585, 7587, 7589, 7591, 7593, 7595,
    7597, 7599, 7601, 7603, 7605, 7607, 7609, 7611, 7613, 7615, 7617, 7619, 7621, 7623, 7625, 7627, 7629, 7631,
    7633, 7635, 7637, 7639, 7641, 7643, 7645, 7647, 7649, 7651, 7653, 7655, 7657, 7659, 7661, 7663, 7665, 7667,
    7669, 7671, 7673, 7211, 7212, 7213, 7226, 7228, 7230, 7250, 7252, 7254, 7266, 7268, 7270};
...
//
//---------------------------------------------------------------------------
CTranslatorQueryToDXL::CTranslatorQueryToDXL(CContextQueryToDXL *context, CMDAccessor *md_accessor,
  ...
  if (query->withCheckOptions) {
    GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature, GPOS_WSZ_LIT("View with WITH CHECK OPTION"));
  }
  ...
  if (nullptr != query_level_to_cte_map) {
    ...
    while (cte_list_hashmap_iter.Advance()) {
      uint32_t cte_query_level = *(cte_list_hashmap_iter.Key());
      ...
      if (cte_query_level < query_level && nullptr != cte_list_entry) {
        ...
        bool is_res GPOS_ASSERTS_ONLY =
            m_query_level_to_cte_map->Insert(GPOS_NEW(m_mp) uint32_t(cte_query_level), cte_list_entry);
        GPOS_ASSERT(is_res);
      }
    }
  }
  ...
  m_scalar_translator = GPOS_NEW(m_mp)
      CTranslatorScalarToDXL(m_context, m_md_accessor, m_query_level, m_query_level_to_cte_map, m_dxl_cte_producers);
}
...
//
//---------------------------------------------------------------------------
CTranslatorQueryToDXL *CTranslatorQueryToDXL::QueryToDXLInstance(CMemoryPool *mp, CMDAccessor *md_accessor,
                                                                 Query *query) {
  CContextQueryToDXL *context = GPOS_NEW(mp) CContextQueryToDXL(mp);

  return GPOS_NEW(context->m_mp) CTranslatorQueryToDXL(context, md_accessor,
...
//
//---------------------------------------------------------------------------
void CTranslatorQueryToDXL::CheckUnsupportedNodeTypes(Query *query) {
  return;
}
...
//
//---------------------------------------------------------------------------
void CTranslatorQueryToDXL::CheckSirvFuncsWithoutFromClause(Query *query) {
  // if there is a FROM clause or if target list is empty, look no further
  if ((nullptr != query->jointree && 0 < gpdb::ListLength(query->jointree->fromlist)) || NIL == query->targetList) {
    return;
  }
...
//
//---------------------------------------------------------------------------
bool CTranslatorQueryToDXL::HasSirvFunctions(Node *node) const {
...
//---------------------------------------------------------------------------
void CTranslatorQueryToDXL::CheckSupportedCmdType(Query *query) {
  ...
  static const SCmdNameElem unsupported_commands[] = {{CMD_UTILITY, GPOS_WSZ_LIT("UTILITY command")}};

  const uint32_t length = GPOS_ARRAY_SIZE(unsupported_commands);
...
void CTranslatorQueryToDXL::CheckRangeTable(Query *query) {
  ...
  foreach (lc, query->rtable) {
    ...
    if (!query->hasRowSecurity && nullptr != rte->securityQuals) {
      GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
                 GPOS_WSZ_LIT("Security quals present in RTE without row level security enabled"));
    }
    ...
    if (query->hasRowSecurity && query->hasSubLinks && 0 < gpdb::ListLength(rte->securityQuals) &&
      ...
      GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
                 GPOS_WSZ_LIT("Query has row level security enabled and security quals contain sublinks"));
    }
...
bool CTranslatorQueryToDXL::CheckSublinkInSecurityQuals(Node *node, void *context) {
  if (nullptr == node) {
    return false;
  }
  ...
  return gpdb::WalkExpressionTree(
      node, (bool (*)(Node *node, void *context))CTranslatorQueryToDXL::CheckSublinkInSecurityQuals, context);
}
...
//
//---------------------------------------------------------------------------
CDXLNodeArray *CTranslatorQueryToDXL::GetQueryOutputCols() const {
...
//
//---------------------------------------------------------------------------
CDXLNodeArray *CTranslatorQueryToDXL::GetCTEs() const {
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateSelectQueryToDXL() {
  ...
  // In Orca, we only keep range table entries for the base tables in the planned statement, but not for the view
  // itself. Since permissions are only checked during ExecutorStart, we lose track of the permissions required for the
  // view and the select goes through successfully. We therefore need to check permissions before we go into
  // optimization for all RTEs, including the ones not explicitly referred in the query, e.g. views.
  ...
  if (m_query->hasForUpdate) {
    ...
    for (int i = 0; i < rt_len; i++) {
      const RangeTblEntry *rte = (RangeTblEntry *)gpdb::ListNth(m_query->rtable, i);
  ...
  GPOS_ASSERT_IMP(m_dxl_cte_producers == nullptr || 0 < m_dxl_cte_producers->Size(),
                  nullptr != dxl_cte_anchor_top && nullptr != dxl_cte_anchor_bottom);

  GPOS_ASSERT_IMP(nullptr != m_query->setOperations, 0 == gpdb::ListLength(m_query->windowClause));
  if (nullptr != m_query->setOperations) {
    ...
    foreach (lc, target_list) {
      ...
      if (0 < target_entry->ressortgroupref) {
        uint32_t colid = ((*dxl_col_descr_array)[resno - 1])->Id();
  ...
  } else if (0 != gpdb::ListLength(m_query->windowClause))  // translate window clauses
  {
    ...
    child_dxlnode = TranslateWindowToDXL(dxlnode, m_query->targetList, m_query->windowClause, m_query->sortClause,
                                         sort_group_attno_to_colid_mapping, output_attno_to_colid_mapping);
  } else {
    child_dxlnode =
        TranslateGroupingSets(m_query->jointree, m_query->targetList, m_query->groupClause, m_query->groupingSets,
                              m_query->hasAggs, sort_group_attno_to_colid_mapping, output_attno_to_colid_mapping);
  }

  // translate limit clause
  CDXLNode *limit_dxlnode = TranslateLimitToDXLGroupBy(m_query->sortClause, m_query->limitCount, m_query->limitOffset,
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateSelectProjectJoinToDXL(List *target_list, FromExpr *from_expr,
  ...
  return TranslateTargetListToDXLProject(target_list, join_tree_dxlnode, sort_group_attno_to_colid_mapping,
...
CDXLNode *CTranslatorQueryToDXL::TranslateSelectProjectJoinForGrpSetsToDXL(
    List *target_list, FromExpr *from_expr, IntToUlongMap *sort_group_attno_to_colid_mapping,
    IntToUlongMap *output_attno_to_colid_mapping, List *group_clause) {
  CDXLNode *join_tree_dxlnode = TranslateFromExprToDXL(from_expr);

  // translate target list entries into a logical project
  return TranslateTargetListToDXLProject(target_list, join_tree_dxlnode, sort_group_attno_to_colid_mapping,
                                         output_attno_to_colid_mapping, group_clause, true /*is_expand_aggref_expr*/);
}
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateQueryToDXL() {
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateInsertQueryToDXL() {
  ...
  if (rte->relkind == 'f') {
    GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature, GPOS_WSZ_LIT("Inserts with foreign tables"));
  }
  CDXLTableDescr *table_descr = CTranslatorUtils::GetTableDescr(m_mp, m_md_accessor, m_context->m_colid_counter, rte,
                                                                m_query_id, &m_context->m_has_distributed_tables);
  ...
  bool contains_foreign_parts = CTranslatorUtils::RelContainsForeignPartitions(md_rel, m_md_accessor);
  if (contains_foreign_parts) {
    // Partitioned tables with external/foreign partitions
    GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
               GPOS_WSZ_LIT("Insert with External/foreign partition storage types"));
  }

  const uint32_t num_table_columns = CTranslatorUtils::GetNumNonSystemColumns(md_rel);
  const uint32_t target_list_length = gpdb::ListLength(m_query->targetList);
  ...
  const uint32_t num_system_cols = md_rel->ColumnCount() - num_table_columns;
  const uint32_t num_non_dropped_cols = md_rel->NonDroppedColsCount() - num_system_cols;
  ...
  uint32_t target_list_pos = 0;
  for (uint32_t ul = 0; ul < num_table_columns; ul++) {
    ...
    if (target_list_pos < target_list_length) {
      ...
      if (attno == resno) {
        ...
        source_array->Append(GPOS_NEW(m_mp) uint32_t(dxl_ident->GetDXLColRef()->Id()));
        target_list_pos++;
        continue;
      }
    }
    ...
    CDXLNode *project_elem_dxlnode =
        CTranslatorUtils::CreateDXLProjElemConstNULL(m_mp, m_md_accessor, m_context->m_colid_counter, mdcol);
    uint32_t colid = CDXLScalarProjElem::Cast(project_elem_dxlnode->GetOperator())->Id();
    project_list_dxlnode->AddChild(project_elem_dxlnode);
    source_array->Append(GPOS_NEW(m_mp) uint32_t(colid));
  }
...
//
//---------------------------------------------------------------------------
CWStringDynamic *CTranslatorQueryToDXL::ExtractStorageOptionStr(DefElem *def_elem) {
...
//
//---------------------------------------------------------------------------
void CTranslatorQueryToDXL::GetCtidAndSegmentId(uint32_t *ctid, uint32_t *segment_id) {
  const FormData_pg_attribute *att_tup_tupid = SystemAttributeDefinition(SelfItemPointerAttributeNumber);

  // ctid column id
  IMDId *mdid = GPOS_NEW(m_mp) CMDIdGPDB(IMDId::EmdidGeneral, att_tup_tupid->atttypid);
  *ctid = CTranslatorUtils::GetColId(m_query_level, m_query->resultRelation, SelfItemPointerAttributeNumber, mdid,
                                     m_var_to_colid_map);
  mdid->Release();
}
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateDeleteQueryToDXL() {
  ...
  const RangeTblEntry *rte = (RangeTblEntry *)gpdb::ListNth(m_query->rtable, m_query->resultRelation - 1);
  if (rte->relkind == 'f') {
    GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature, GPOS_WSZ_LIT("Deletes with foreign tables"));
  }
  CDXLTableDescr *table_descr = CTranslatorUtils::GetTableDescr(m_mp, m_md_accessor, m_context->m_colid_counter, rte,
                                                                m_query_id, &m_context->m_has_distributed_tables);
  ...
  if (contains_foreign_parts) {
    // Partitioned tables with external/foreign partitions
    GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
               GPOS_WSZ_LIT("Delete with External/foreign partition storage types"));
  }
  ...
  const uint32_t num_of_non_sys_cols = md_rel->ColumnCount();
  for (uint32_t ul = 0; ul < num_of_non_sys_cols; ul++) {
    const IMDColumn *mdcol = md_rel->GetMdCol(ul);
    if (mdcol->IsSystemColumn() || mdcol->IsDropped()) {
      continue;
    }

    uint32_t colid = CTranslatorUtils::GetColId(m_query_level, m_query->resultRelation, mdcol->AttrNum(),
                                                mdcol->MdidType(), m_var_to_colid_map);
    delete_colid_array->Append(GPOS_NEW(m_mp) uint32_t(colid));
  }

  CDXLLogicalDelete *delete_dxlop =
      GPOS_NEW(m_mp) CDXLLogicalDelete(m_mp, table_descr, ctid_colid, segid_colid, delete_colid_array);
...
CDXLNode *CTranslatorQueryToDXL::TranslateUpdateQueryToDXL() {
  ...
  if (rte->relkind == 'f') {
    GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature, GPOS_WSZ_LIT("Updates with foreign tables"));
  }
  CDXLTableDescr *table_descr = CTranslatorUtils::GetTableDescr(m_mp, m_md_accessor, m_context->m_colid_counter, rte,
                                                                m_query_id, &m_context->m_has_distributed_tables);
  ...
  if (contains_foreign_parts) {
    // Partitioned tables with external/foreign partitions
    GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
               GPOS_WSZ_LIT("Update with External/foreign partition storage types"));
  }

  uint32_t ctid_colid = 0;
  uint32_t segmentid_colid = 0;
  ...
  for (uint32_t ul = 0; ul < num_of_non_sys_cols; ul++) {
    ...
    int32_t attno = mdcol->AttrNum();
    uint32_t *updated_colid = update_column_map->Find(&attno);

    uint32_t colid = CTranslatorUtils::GetColId(m_query_level, m_query->resultRelation, attno, mdcol->MdidType(),
                                                m_var_to_colid_map);
    ...
    if (nullptr != updated_colid) {
      insert_colid_array->Append(GPOS_NEW(m_mp) uint32_t(*updated_colid));
    } else {
      insert_colid_array->Append(GPOS_NEW(m_mp) uint32_t(colid));
    }
  ...
  CDXLLogicalUpdate *pdxlopupdate = GPOS_NEW(m_mp)
      CDXLLogicalUpdate(m_mp, table_descr, ctid_colid, segmentid_colid, delete_colid_array, insert_colid_array);
...
IntToUlongMap *CTranslatorQueryToDXL::UpdatedColumnMapping() {
  ...
  foreach (lc, m_query->targetList) {
    ...
    if (!target_entry->resjunk) {
      ...
      CDXLScalarIdent *dxl_ident = CDXLScalarIdent::Cast(dxl_column->GetOperator());
      uint32_t colid = dxl_ident->GetDXLColRef()->Id();
...
//
//---------------------------------------------------------------------------
bool CTranslatorQueryToDXL::OIDFound(OID oid, const OID oids[], uint32_t size) {
  bool found = false;
...
//
//---------------------------------------------------------------------------
bool CTranslatorQueryToDXL::IsLeadWindowFunc(CDXLOperator *dxlop) {
...
//
//---------------------------------------------------------------------------
bool CTranslatorQueryToDXL::IsLagWindowFunc(CDXLOperator *dxlop) {
  bool is_lag = false;
  if (EdxlopScalarWindowRef == dxlop->GetDXLOperator()) {
    CDXLScalarWindowRef *winref_dxlop = CDXLScalarWindowRef::Cast(dxlop);
    const CMDIdGPDB *mdid_gpdb = CMDIdGPDB::CastMdid(winref_dxlop->FuncMdId());
...
//
//---------------------------------------------------------------------------
CDXLWindowFrame *CTranslatorQueryToDXL::CreateWindowFramForLeadLag(bool is_lead_func, CDXLNode *dxl_offset) const {
  ...
  if (nullptr == dxl_offset) {
    dxl_lead_edge = GPOS_NEW(m_mp)
        CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarWindowFrameEdge(m_mp, true /* fLeading */, dxl_frame_lead));
    dxl_trail_edge = GPOS_NEW(m_mp)
        CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarWindowFrameEdge(m_mp, false /* fLeading */, dxl_frame_trail));
  ...
  return GPOS_NEW(m_mp)
      CDXLWindowFrame(EdxlfsRow,     // frame specification
                      EdxlfesNulls,  // frame exclusion strategy is set to exclude NULLs in GPDB
                      dxl_lead_edge, dxl_trail_edge, InvalidOid, InvalidOid, InvalidOid, false, false);
}
...
//
//---------------------------------------------------------------------------
void CTranslatorQueryToDXL::UpdateLeadLagWinSpecPos(
  ...
  for (uint32_t ul = 0; ul < arity; ul++) {
    ...
    if (is_lead_func || is_lag) {
      ...
      CDXLWindowSpec *pdxlwsNew =
          GPOS_NEW(m_mp) CDXLWindowSpec(m_mp, window_spec_dxlnode->GetPartitionByColIdArray(), mdname,
...
//
//---------------------------------------------------------------------------
CDXLWindowSpecArray *CTranslatorQueryToDXL::TranslateWindowSpecToDXL(List *window_clause,
  ...
  foreach (lc, window_clause) {
    ...
    if (0 < gpdb::ListLength(wc->orderClause)) {
      ...
      const uint32_t size = dxl_sort_cols->Size();
    ...
    window_frame = m_scalar_translator->TranslateWindowFrameToDXL(
        wc->frameOptions, wc->startOffset, wc->endOffset, wc->startInRangeFunc, wc->endInRangeFunc, wc->inRangeColl,
        wc->inRangeAsc, wc->inRangeNullsFirst, m_var_to_colid_map, project_list_dxlnode_node);

    CDXLWindowSpec *window_spec_dxlnode =
        GPOS_NEW(m_mp) CDXLWindowSpec(m_mp, part_columns, mdname, sort_col_list_dxl, window_frame);
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateWindowToDXL(CDXLNode *child_dxlnode, List *target_list, List *window_clause,
    ...
    if (!target_entry->resjunk) {
      if (IsA(target_entry->expr, Var) || IsA(target_entry->expr, WindowFunc)) {
        ...
        CMDName *mdname_alias = GPOS_NEW(m_mp)
            CMDName(m_mp, CDXLScalarProjElem::Cast(project_elem_dxlnode->GetOperator())->GetMdNameAlias()->GetMDName());
        CDXLNode *new_project_elem_dxlnode =
            GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarProjElem(m_mp, colid, mdname_alias));
        CDXLNode *project_elem_new_child_dxlnode = GPOS_NEW(m_mp) CDXLNode(
            m_mp,
            GPOS_NEW(m_mp) CDXLScalarIdent(
                m_mp, GPOS_NEW(m_mp) CDXLColRef(
                          GPOS_NEW(m_mp) CMDName(m_mp, mdname_alias->GetMDName()), colid,
                          GPOS_NEW(m_mp) CMDIdGPDB(IMDId::EmdidGeneral, gpdb::ExprType((Node *)target_entry->expr)),
  ...
  forboth(lc, omitted_target_entries, lc_resno, resno_list) {
    TargetEntry *target_entry = (TargetEntry *)lfirst(lc);
    int32_t resno = (int32_t)lfirst_int(lc_resno);

    TargetEntry *te_window_spec =
        CTranslatorUtils::GetWindowSpecTargetEntry((Node *)target_entry->expr, window_clause, target_list);
    if (nullptr != te_window_spec) {
      const uint32_t colid =
          CTranslatorUtils::GetColId((int32_t)te_window_spec->ressortgroupref, sort_col_attno_to_colid_mapping);
      StoreAttnoColIdMapping(output_attno_to_colid_mapping, resno, colid);
      AddSortingGroupingColumn(target_entry, sort_col_attno_to_colid_mapping, colid);
    }
  }
  ...
  CDXLWindowSpecArray *window_spec_array =
      TranslateWindowSpecToDXL(window_clause, sort_col_attno_to_colid_mapping, new_child_project_list_dxlnode);
...
//
//---------------------------------------------------------------------------
ULongPtrArray *CTranslatorQueryToDXL::TranslatePartColumns(List *partition_by_clause,
                                                           IntToUlongMap *col_attno_colid_mapping) const {
  ...
  foreach (lc, partition_by_clause) {
    ...
    uint32_t colid = CTranslatorUtils::GetColId((int32_t)sort_group_clause->tleSortGroupRef, col_attno_colid_mapping);

    part_cols->Append(GPOS_NEW(m_mp) uint32_t(colid));
  }
...
//
//---------------------------------------------------------------------------
CDXLNodeArray *CTranslatorQueryToDXL::TranslateSortColumsToDXL(List *sort_clause,
                                                               IntToUlongMap *col_attno_colid_mapping) const {
  ...
  foreach (lc, sort_clause) {
    ...
    const uint32_t colid =
        CTranslatorUtils::GetColId((int32_t)sort_group_clause->tleSortGroupRef, col_attno_colid_mapping);
    ...
    CDXLScalarSortCol *sc_sort_col_dxlop = GPOS_NEW(m_mp) CDXLScalarSortCol(
        m_mp, colid, op_mdid, GPOS_NEW(m_mp) CWStringConst(str->GetBuffer()), sort_group_clause->nulls_first);
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateLimitToDXLGroupBy(List *sort_clause, Node *limit_count,
                                                            Node *limit_offset_node, CDXLNode *child_dxlnode,
                                                            IntToUlongMap *grpcols_to_colid_mapping) {
  if (0 == gpdb::ListLength(sort_clause) && nullptr == limit_count && nullptr == limit_offset_node) {
    return child_dxlnode;
  }
...
//
//---------------------------------------------------------------------------
void CTranslatorQueryToDXL::AddSortingGroupingColumn(TargetEntry *target_entry,
                                                     IntToUlongMap *sort_grpref_to_colid_mapping,
                                                     uint32_t colid) const {
  if (0 < target_entry->ressortgroupref) {
    int32_t *key = GPOS_NEW(m_mp) int32_t(target_entry->ressortgroupref);
    uint32_t *value = GPOS_NEW(m_mp) uint32_t(colid);

    // insert idx-colid mapping in the hash map
    bool is_res GPOS_ASSERTS_ONLY = sort_grpref_to_colid_mapping->Insert(key, value);

    GPOS_ASSERT(is_res);
  }
}

static bool ExpressionContainsMissingVars(const Expr *expr, CBitSet *grpby_cols_bitset) {
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::CreateSimpleGroupBy(List *target_list, List *group_clause, CBitSet *grpby_cols_bitset,
  ...
  if (nullptr == grpby_cols_bitset) {
    ...
    if (!has_grouping_sets) {
      ...
      while (mi.Advance()) {
        ...
        bool result =
#endif  // GPOS_DEBUG
            output_attno_to_colid_mapping->Insert(GPOS_NEW(m_mp) int32_t(*(mi.Key())),
                                                  GPOS_NEW(m_mp) uint32_t(*(mi.Value())));
        GPOS_ASSERT(result);
      }
    }
    // else:
  ...
  foreach (lc, target_list) {
    ...
    TargetEntry *te_equivalent =
        CTranslatorUtils::GetGroupingColumnTargetEntry((Node *)target_entry->expr, group_clause, target_list);

    bool is_grouping_col = grpby_cols_bitset->Get(target_entry->ressortgroupref) ||
                           (nullptr != te_equivalent && grpby_cols_bitset->Get(te_equivalent->ressortgroupref));
    uint32_t colid = 0;
    ...
    if (is_grouping_col) {
      ...
      if (IsA(target_entry->expr, Aggref) && ((Aggref *)target_entry->expr)->aggdistinct &&
  ...
  if (1 < num_dqa && !optimizer_enable_multiple_distinct_aggs) {
    GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
               GPOS_WSZ_LIT("Multiple Distinct Qualified Aggregates are disabled in the optimizer"));
  }

  // initialize the array of grouping columns
  ULongPtrArray *grouping_cols =
      CTranslatorUtils::GetGroupingColidArray(m_mp, grpby_cols_bitset, sort_grpref_to_colid_mapping);
  ...
  return GPOS_NEW(m_mp)
      CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLLogicalGroupBy(m_mp, grouping_cols), project_list_grpby_dxlnode, child_dxlnode);
}
...
//---------------------------------------------------------------------------
bool CTranslatorQueryToDXL::IsDuplicateDqaArg(List *dqa_list, Aggref *aggref) {
...
//---------------------------------------------------------------------------
static bool GroupingSetContainsValue(GroupingSet *group, int32_t value) {
  ListCell *lc = nullptr;
...
//
//		[1]
// https://www.postgresql.org/message-id/flat/CAHnPFjSdFx_TtNpQturPMkRSJMYaD5rGP2=8iFH9V24-OjHGiQ@mail.gmail.com
// [2] https://www.postgresql.org/message-id/flat/830269.1656693747@sss.pgh.pa.us
//---------------------------------------------------------------------------
void CTranslatorQueryToDXL::CheckNoDuplicateAliasGroupingColumn(List *target_list, List *group_clause,
                                                                List *grouping_set) {
  ...
  foreach (lc1, grouping_set) {
    ...
    while (bsiter.Advance()) {
      if (GroupingSetContainsValue(group, bsiter.Bit())) {
        if (group->kind == GROUPING_SET_CUBE || group->kind == GROUPING_SET_ROLLUP ||
        ...
        if (countNonSimple > 1 || (countNonSimple > 0 && countSimple > 0)) {
          GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
                     GPOS_WSZ_LIT("Multiple grouping sets specifications with duplicate aliased columns"));
        }
      }
    }
  }
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateGroupingSets(FromExpr *from_expr, List *target_list, List *group_clause,
                                                       List *grouping_set, bool has_aggs,
                                                       IntToUlongMap *sort_grpref_to_colid_mapping,
                                                       IntToUlongMap *output_attno_to_colid_mapping) {
  const uint32_t num_of_cols = gpdb::ListLength(target_list) + 1;
  ...
  if (nullptr == group_clause && nullptr == grouping_set) {
    ...
    CDXLNode *select_project_join_dxlnode = TranslateSelectProjectJoinToDXL(
        target_list, from_expr, sort_grpref_to_colid_mapping, child_attno_colid_mapping, group_clause);
    ...
    CDXLNode *result_dxlnode = CreateSimpleGroupBy(target_list, group_clause, bitset, has_aggs,
  ...
  CheckNoDuplicateAliasGroupingColumn(target_list, group_clause, grouping_set);
  ...
  CBitSetArray *bitset_array = CTranslatorUtils::GetColumnAttnosForGroupBy(
      m_mp, group_clause, grouping_set, num_of_cols, grpcol_index_to_colid_mapping, unique_grp_cols_bitset);

  const uint32_t num_of_grouping_sets = bitset_array->Size();
  ...
  if (1 == num_of_grouping_sets) {
    ...
    CDXLNode *groupby_dxlnode = CreateSimpleGroupBy(target_list, group_clause, bitset, has_aggs,
                                                    false,  // has_grouping_sets
                                                    select_project_join_dxlnode, sort_grpref_to_colid_mapping,
                                                    child_attno_colid_mapping, output_attno_to_colid_mapping);

    CDXLNode *result_dxlnode =
        CreateDXLProjectGroupingFuncs(target_list, groupby_dxlnode, bitset, output_attno_to_colid_mapping,
  ...
  CDXLNode *result_dxlnode = CreateDXLUnionAllForGroupingSets(
      from_expr, target_list, group_clause, has_aggs, bitset_array, sort_grpref_to_colid_mapping,
      output_attno_to_colid_mapping, grpcol_index_to_colid_mapping);
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::CreateDXLUnionAllForGroupingSets(
    FromExpr *from_expr, List *target_list, List *group_clause, bool has_aggs, CBitSetArray *bitset_array,
    IntToUlongMap *sort_grpref_to_colid_mapping, IntToUlongMap *output_attno_to_colid_mapping,
  ...
  const uint32_t cte_id = m_context->m_cte_id_counter->next_id();
  ...
  CDXLNode *select_project_join_dxlnode = TranslateSelectProjectJoinForGrpSetsToDXL(
      target_list, from_expr, sort_groupref_to_colid_producer_mapping, spj_output_attno_to_colid_mapping, group_clause);
  ...
  CDXLLogicalCTEProducer *cte_prod_dxlop =
      GPOS_NEW(m_mp) CDXLLogicalCTEProducer(m_mp, cte_id, op_colid_array_cte_producer);
  ...
  for (uint32_t ul = 0; ul < num_of_grouping_sets; ul++) {
    ...
    GPOS_DELETE(m_var_to_colid_map);
    m_var_to_colid_map =
        var_colid_orig_mapping->CopyRemapColId(m_mp, op_colid_array_cte_producer, colid_array_cte_consumer);

    IntToUlongMap *spj_consumer_output_attno_to_colid_mapping =
        RemapColIds(m_mp, spj_output_attno_to_colid_mapping, op_colid_array_cte_producer, colid_array_cte_consumer);
    IntToUlongMap *phmiulSortgrouprefColIdConsumer = RemapColIds(m_mp, sort_groupref_to_colid_producer_mapping,
                                                                 op_colid_array_cte_producer, colid_array_cte_consumer);

    // construct a CTE consumer
    CDXLNode *cte_consumer_dxlnode =
        GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLLogicalCTEConsumer(m_mp, cte_id, colid_array_cte_consumer));
    ...
    CDXLNode *groupby_dxlnode =
        CreateSimpleGroupBy(target_list_copy, group_clause, grouping_set_bitset, has_aggs,
                            true,  // has_grouping_sets
                            cte_consumer_dxlnode, phmiulSortgrouprefColIdConsumer,
                            spj_consumer_output_attno_to_colid_mapping, groupby_attno_to_colid_mapping);

    // add a project list for the NULL values
    CDXLNode *project_dxlnode = CreateDXLProjectNullsForGroupingSets(
        target_list_copy, groupby_dxlnode, grouping_set_bitset, phmiulSortgrouprefColIdConsumer,
        groupby_attno_to_colid_mapping, grpcol_index_to_colid_mapping);

    ULongPtrArray *colids_outer_array =
        CTranslatorUtils::GetOutputColIdsArray(m_mp, target_list_copy, groupby_attno_to_colid_mapping);
    if (nullptr != unionall_dxlnode) {
      GPOS_ASSERT(nullptr != colid_array_inner);
      CDXLColDescrArray *dxl_col_descr_array = CTranslatorUtils::GetDXLColumnDescrArray(
          m_mp, target_list_copy, colids_outer_array, true /* keep_res_junked */);
      ...
      CDXLLogicalSetOp *dxl_setop =
          GPOS_NEW(m_mp) CDXLLogicalSetOp(m_mp, EdxlsetopUnionAll, dxl_col_descr_array, input_colids, false);
    ...
    if (ul == num_of_grouping_sets - 1) {
      ...
      foreach (lc, target_list_copy) {
        ...
        int32_t sortgroupref = int32_t(target_entry->ressortgroupref);
        if (0 < sortgroupref && nullptr != phmiulSortgrouprefColIdConsumer->Find(&sortgroupref)) {
  ...
  foreach (lc, target_list) {
    ...
    const uint32_t colid = dxl_col_descr->Id();
    output_col_idx++;
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::DXLDummyConstTableGet() const {
  // construct the schema of the const table
  CDXLColDescrArray *dxl_col_descr_array = GPOS_NEW(m_mp) CDXLColDescrArray(m_mp);

  const CMDTypeBoolGPDB *md_type_bool =
      dynamic_cast<const CMDTypeBoolGPDB *>(m_md_accessor->PtMDType<IMDTypeBool>(m_sysid));
  ...
  CDXLColDescr *dxl_col_descr = GPOS_NEW(m_mp) CDXLColDescr(
      ...
      GPOS_NEW(m_mp) CMDIdGPDB(IMDId::EmdidGeneral, mdid->Oid()), default_type_modifier, false /* is_dropped */
  );
  ...
  CDXLLogicalConstTable *dxlop =
      GPOS_NEW(m_mp) CDXLLogicalConstTable(m_mp, dxl_col_descr_array, dispatch_identifier_datum_arrays);
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateSetOpToDXL(Node *setop_node, List *target_list,
                                                     IntToUlongMap *output_attno_to_colid_mapping) {
  ...
  CDXLNode *left_child_dxlnode =
      TranslateSetOpChild(psetopstmt->larg, leftchild_array, mdid_array_leftchild, target_list);
  CDXLNode *right_child_dxlnode =
      TranslateSetOpChild(psetopstmt->rarg, rightchild_array, mdid_array_rightchild, target_list);

  // mark outer references in input columns from left child
  uint32_t *colid = GPOS_NEW_ARRAY(m_mp, uint32_t, leftchild_array->Size());
  bool *outer_ref_array = GPOS_NEW_ARRAY(m_mp, bool, leftchild_array->Size());
  const uint32_t size = leftchild_array->Size();
  for (uint32_t ul = 0; ul < size; ul++) {
  ...
  ULongPtrArray *output_colids = CTranslatorUtils::GenerateColIds(
      m_mp, target_list, mdid_array_leftchild, leftchild_array, outer_ref_array, m_context->m_colid_counter);
  ...
  bool is_cast_across_input =
      SetOpNeedsCast(target_list, mdid_array_leftchild) || SetOpNeedsCast(target_list, mdid_array_rightchild);
  ...
  CDXLNode *dxlnode = CreateDXLSetOpFromColumns(setop_type, target_list, output_colids, input_colids, children_dxlnodes,
  ...
  foreach (lc, target_list) {
    ...
    if (!target_entry->resjunk) {
      ...
      uint32_t colid = dxl_col_descr_new->Id();
      StoreAttnoColIdMapping(output_attno_to_colid_mapping, resno, colid);
      output_col_idx++;
    }
  }
...
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::CreateDXLSetOpFromColumns(EdxlSetOpType setop_type, List *output_target_list,
  ...
  const uint32_t num_of_cols = output_colids->Size();
  ...
  for (uint32_t ul = 0; ul < num_of_cols; ul++) {
    uint32_t colid_output = *(*output_colids)[ul];
    uint32_t colid_input = *(*input_first_child_array)[ul];

    bool is_col_exists = bitset->Get(colid_input);
    bool is_casted_col = (colid_output != colid_input);

    uint32_t target_list_pos = *(*output_col_pos)[ul];
    ...
    if (!is_col_exists) {
      ...
      input_first_child_new_array->Append(GPOS_NEW(m_mp) uint32_t(colid_input));
      ...
      uint32_t colid_new = m_context->m_colid_counter->next_id();
      input_first_child_new_array->Append(GPOS_NEW(m_mp) uint32_t(colid_new));

      uint32_t colid_union_output = colid_new;
      if (is_casted_col) {
      ...
      CDXLNode *project_elem_dxlnode =
          CTranslatorUtils::CreateDummyProjectElem(m_mp, colid_input, colid_new, output_col_descr);
  ...
  CDXLLogicalSetOp *dxlop =
      GPOS_NEW(m_mp) CDXLLogicalSetOp(m_mp, setop_type, output_col_descrs, input_colids, is_cast_across_input);
...
//
//---------------------------------------------------------------------------
bool CTranslatorQueryToDXL::SetOpNeedsCast(List *target_list, IMdIdArray *input_col_mdids) {
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateSetOpChild(Node *child_node, ULongPtrArray *colids,
                                                     IMdIdArray *input_col_mdids, List *target_list) {
  GPOS_ASSERT(nullptr != colids);
  GPOS_ASSERT(nullptr != input_col_mdids);

  if (IsA(child_node, RangeTblRef)) {
    RangeTblRef *range_tbl_ref = (RangeTblRef *)child_node;
    const uint32_t rt_index = range_tbl_ref->rtindex;
    ...
    if (RTE_SUBQUERY == rte->rtekind) {
      ...
      CTranslatorQueryToDXL query_to_dxl_translator(m_context, m_md_accessor, m_var_to_colid_map, query_derived_tbl,
    ...
    const CDXLColDescrArray *dxl_col_descr_array =
        CDXLLogicalSetOp::Cast(dxlnode->GetOperator())->GetDXLColumnDescrArray();
    GPOS_ASSERT(nullptr != dxl_col_descr_array);
    const uint32_t length = dxl_col_descr_array->Size();
    for (uint32_t ul = 0; ul < length; ul++) {
      const CDXLColDescr *dxl_col_descr = (*dxl_col_descr_array)[ul];
      uint32_t *colid = GPOS_NEW(m_mp) uint32_t(dxl_col_descr->Id());
      colids->Append(colid);
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateFromExprToDXL(FromExpr *from_expr) {
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateFromClauseToDXL(Node *node) {
  GPOS_ASSERT(nullptr != node);

  if (IsA(node, RangeTblRef)) {
    RangeTblRef *range_tbl_ref = (RangeTblRef *)node;
    uint32_t rt_index = range_tbl_ref->rtindex;
    const RangeTblEntry *rte = (RangeTblEntry *)gpdb::ListNth(m_query->rtable, rt_index - 1);
    ...
    if (rte->funcordinality) {
      GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature, GPOS_WSZ_LIT("WITH ORDINALITY"));
    }
    ...
    switch (rte->rtekind) {
      ...
      case RTE_RELATION: {
        return TranslateRTEToDXLLogicalGet(rte, rt_index, m_query_level);
      }
      case RTE_VALUES: {
      ...
      case RTE_SUBQUERY: {
        return TranslateDerivedTablesToDXL(rte, rt_index, m_query_level);
      }
      case RTE_FUNCTION: {
...
//---------------------------------------------------------------------------
void CTranslatorQueryToDXL::UnsupportedRTEKind(RTEKind rtekind) {
  GPOS_ASSERT(!(RTE_RELATION == rtekind || RTE_CTE == rtekind || RTE_FUNCTION == rtekind || RTE_SUBQUERY == rtekind ||
                RTE_VALUES == rtekind));
  ...
  switch (rtekind) {
    ...
    case RTE_JOIN: {
      GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature, GPOS_WSZ_LIT("RangeTableEntry of type Join"));
    }
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateRTEToDXLLogicalGet(const RangeTblEntry *rte, uint32_t rt_index,
  ...
  uint32_t query_id_for_target_rel = UNASSIGNED_QUERYID;
  if (m_query->resultRelation > 0 && uint32_t(m_query->resultRelation) == rt_index) {
  ...
  CDXLTableDescr *dxl_table_descr =
      CTranslatorUtils::GetTableDescr(m_mp, m_md_accessor, m_context->m_colid_counter, rte, query_id_for_target_rel,
  ...
  if (IMDRelation::ErelstorageForeign == md_rel->RetrieveRelStorageType()) {
    ...
    dxl_op = GPOS_NEW(m_mp) CDXLLogicalGet(m_mp, dxl_table_descr, rteHasSecurityQuals);
  }
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateValueScanRTEToDXL(const RangeTblEntry *rte, uint32_t rt_index,
                                                            uint32_t current_query_level) {
  List *tuples_list = rte->values_lists;
  GPOS_ASSERT(nullptr != tuples_list);

  const uint32_t num_of_tuples = gpdb::ListLength(tuples_list);
    ...
    foreach (lc_column, tuple_list) {
      ...
      if (IsA(expr, Const)) {
        ...
        CDXLColDescr *dxl_col_descr =
            GPOS_NEW(m_mp) CDXLColDescr(mdname, colid, col_pos_idx + 1 /* attno */,
                                        GPOS_NEW(m_mp) CMDIdGPDB(IMDId::EmdidGeneral, const_expr->consttype),
        ...
        CDXLNode *project_elem_dxlnode =
            TranslateExprToDXLProject(expr, col_name_char_array, true /* insist_new_colids */);
        ...
        if (0 == tuple_pos) {
          ...
          CDXLColDescr *dxl_col_descr =
              GPOS_NEW(m_mp) CDXLColDescr(mdname, colid, col_pos_idx + 1 /* attno */,
                                          GPOS_NEW(m_mp) CMDIdGPDB(IMDId::EmdidGeneral, gpdb::ExprType((Node *)expr)),
      ...
      colid_array->Append(GPOS_NEW(m_mp) uint32_t(colid));
      col_pos_idx++;
    }
  ...
  if (fAllConstant) {
    ...
    CDXLLogicalConstTable *dxlop =
        GPOS_NEW(m_mp) CDXLLogicalConstTable(m_mp, dxl_col_descr_array, dxl_values_datum_array);
...
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateColumnValuesToDXL(CDXLDatumArray *dxl_datum_array_const_tbl_get,
  ...
  if (0 == dxl_datum_array_const_tbl_get->Size()) {
    ...
    CDXLLogicalConstTable *dxlop =
        GPOS_NEW(m_mp) CDXLLogicalConstTable(m_mp, dxl_column_descriptors, dxl_datum_arrays_const_tbl_get);
  ...
  CDXLNode *project_list_dxlnode =
      GPOS_NEW(m_mp) CDXLNode(GPOS_NEW(m_mp) CDXLScalarProjList(m_mp), project_elem_dxlnode_array);

  CDXLNode *project_dxlnode = GPOS_NEW(m_mp)
      CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLLogicalProject(m_mp), project_list_dxlnode, const_tbl_get_dxlnode);
...
//		Returns a CDXLNode representing a from relation range table entry
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateTVFToDXL(const RangeTblEntry *rte, uint32_t rt_index,
                                                   uint32_t  // current_query_level
) {
  /*
  ...
  bool is_composite_const = CTranslatorUtils::IsCompositeConst(m_mp, m_md_accessor, rtfunc);
  ...
  if (!IsA(rtfunc->funcexpr, FuncExpr) && !is_composite_const) {
    ...
    CDXLNode *project_elem_dxlnode =
        TranslateExprToDXLProject((Expr *)rtfunc->funcexpr, rte->eref->aliasname, true /* insist_new_colids */);
    project_list_dxlnode->AddChild(project_elem_dxlnode);
  ...
  CDXLLogicalTVF *tvf_dxlop =
      CTranslatorUtils::ConvertToCDXLLogicalTVF(m_mp, m_md_accessor, m_context->m_colid_counter, rte);
  ...
  if (IsA(rtfunc->funcexpr, Const)) {
    ...
    if (constant->constisnull) {
      GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature, GPOS_WSZ_LIT("Row-type variable"));
    }

    CDXLNode *constValue = m_scalar_translator->TranslateScalarToDXL((Expr *)(rtfunc->funcexpr), m_var_to_colid_map);
  ...
  CMDIdGPDB *mdid_func = GPOS_NEW(m_mp) CMDIdGPDB(IMDId::EmdidGeneral, funcexpr->funcid);
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateCTEToDXL(const RangeTblEntry *rte, uint32_t rt_index,
                                                   uint32_t current_query_level) {
  const uint32_t cte_query_level = current_query_level - rte->ctelevelsup;
  ...
  if (nullptr == cte_list_entry) {
    ...
    GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLError, GPOS_WSZ_LIT("No CTE"));
  }
  ...
  if (NIL == cte_producer_target_list) {
    GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature, GPOS_WSZ_LIT("Empty target list"));
  }
  ...
  uint32_t cte_id = cte_producer_dxlop->Id();
  ...
  m_var_to_colid_map->LoadCTEColumns(current_query_level, rt_index, colid_array_cte_consumer,
                                     const_cast<List *>(cte_producer_target_list));

  CDXLLogicalCTEConsumer *cte_consumer_dxlop =
      GPOS_NEW(m_mp) CDXLLogicalCTEConsumer(m_mp, cte_id, colid_array_cte_consumer);
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateDerivedTablesToDXL(const RangeTblEntry *rte, uint32_t rt_index,
                                                             uint32_t current_query_level) {
  ...
  CDXLNodeArray *cte_dxlnode_array = query_to_dxl_translator.GetCTEs();
  GPOS_ASSERT(nullptr != derived_tbl_dxlnode && query_output_cols_dxlnode_array != nullptr);

  CUtils::AddRefAppend(m_dxl_cte_producers, cte_dxlnode_array);

  // make note of new columns from derived table
  m_var_to_colid_map->LoadDerivedTblColumns(current_query_level, rt_index, query_output_cols_dxlnode_array,
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateExprToDXL(Expr *expr) {
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateJoinExprInFromToDXL(JoinExpr *join_expr) {
  ...
  GPOS_ASSERT(nullptr != left_child_dxlnode && nullptr != right_child_dxlnode);
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateTargetListToDXLProject(List *target_list, CDXLNode *child_dxlnode,
                                                                 IntToUlongMap *sort_grpref_to_colid_mapping,
                                                                 IntToUlongMap *output_attno_to_colid_mapping,
                                                                 List *plgrpcl, bool is_expand_aggref_expr) {
  bool is_groupby =
      (0 != gpdb::ListLength(m_query->groupClause) || 0 != gpdb::ListLength(m_query->groupingSets) || m_query->hasAggs);
  ...
  foreach (lc, target_list) {
    ...
    if (IsA(target_entry->expr, GroupingFunc)) {
      ...
      bool is_orderby_col = CTranslatorUtils::IsSortingColumn(target_entry, m_query->sortClause);
      bool insist_proj = IsA(target_entry->expr, Var) && ((Var *)(target_entry->expr))->varlevelsup > 0 &&
                         !is_orderby_col && !is_grouping_col;
      CDXLNode *project_elem_dxlnode =
          TranslateExprToDXLProject(target_entry->expr, target_entry->resname, insist_proj /* insist_new_colids */);
      uint32_t colid = CDXLScalarProjElem::Cast(project_elem_dxlnode->GetOperator())->Id();
      ...
      vars_list = gpdb::ListConcat(
          vars_list, gpdb::ExtractNodesExpression((Node *)target_entry->expr, T_Var, false /*descendIntoSubqueries*/));
    } else if (!IsA(target_entry->expr, Aggref)) {
  ...
  foreach (lc, omitted_te_list) {
    TargetEntry *target_entry = (TargetEntry *)lfirst(lc);
    int32_t sort_group_ref = (int32_t)target_entry->ressortgroupref;

    TargetEntry *te_grouping_col =
        CTranslatorUtils::GetGroupingColumnTargetEntry((Node *)target_entry->expr, plgrpcl, target_list);
    if (nullptr != te_grouping_col) {
      const uint32_t colid =
          CTranslatorUtils::GetColId((int32_t)te_grouping_col->ressortgroupref, sort_grpref_to_colid_mapping);
      StoreAttnoColIdMapping(output_attno_to_colid_mapping, target_entry->resno, colid);
      if (0 < sort_group_ref && 0 < colid && nullptr == sort_grpref_to_colid_mapping->Find(&sort_group_ref)) {
        AddSortingGroupingColumn(target_entry, sort_grpref_to_colid_mapping, colid);
      }
    }
  }
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::CreateDXLProjectNullsForGroupingSets(
  ...
  foreach (lc, target_list) {
    ...
    if (IsA(target_entry->expr, GroupingFunc)) {
      colid = m_context->m_colid_counter->next_id();
      CDXLNode *grouping_func_dxlnode =
          TranslateGroupingFuncToDXL(target_entry->expr, bitset, grpcol_index_to_colid_mapping);
      ...
      CDXLNode *project_elem_dxlnode = GPOS_NEW(m_mp)
          CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarProjElem(m_mp, colid, mdname_alias), grouping_func_dxlnode);
      project_list_dxlnode->AddChild(project_elem_dxlnode);
      StoreAttnoColIdMapping(output_attno_to_colid_mapping, resno, colid);
      ...
      CMDIdGPDB *mdid = GPOS_NEW(m_mp) CMDIdGPDB(IMDId::EmdidGeneral, oid_type);
      CDXLNode *project_elem_dxlnode =
          CTranslatorUtils::CreateDXLProjElemConstNULL(m_mp, m_md_accessor, mdid, colid, target_entry->resname);
      mdid->Release();
    ...
    GPOS_ASSERT_IMP(nullptr == sort_grouping_col_mapping,
                    nullptr != sort_grouping_col_mapping->Find(&sort_group_ref) && "Grouping column with no mapping");

    if (0 < sort_group_ref && 0 < colid && nullptr == sort_grouping_col_mapping->Find(&sort_group_ref)) {
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::CreateDXLProjectGroupingFuncs(List *target_list, CDXLNode *child_dxlnode,
                                                               CBitSet *bitset,
                                                               IntToUlongMap *output_attno_to_colid_mapping,
  ...
  foreach (lc, target_list) {
    ...
    if (IsA(target_entry->expr, GroupingFunc)) {
      uint32_t colid = m_context->m_colid_counter->next_id();
      CDXLNode *grouping_func_dxlnode =
          TranslateGroupingFuncToDXL(target_entry->expr, bitset, grpcol_index_to_colid_mapping);
...
//
//---------------------------------------------------------------------------
void CTranslatorQueryToDXL::StoreAttnoColIdMapping(IntToUlongMap *attno_to_colid_mapping, int32_t attno,
                                                   uint32_t colid) const {
  GPOS_ASSERT(nullptr != attno_to_colid_mapping);

  int32_t *key = GPOS_NEW(m_mp) int32_t(attno);
  uint32_t *value = GPOS_NEW(m_mp) uint32_t(colid);
...
//
//---------------------------------------------------------------------------
CDXLNodeArray *CTranslatorQueryToDXL::CreateDXLOutputCols(List *target_list,
  ...
  foreach (lc, target_list) {
    ...
    const uint32_t colid = CTranslatorUtils::GetColId(resno, attno_to_colid_mapping);

    // create a column reference
    IMDId *mdid_type = GPOS_NEW(m_mp) CMDIdGPDB(IMDId::EmdidGeneral, gpdb::ExprType((Node *)target_entry->expr));
    int32_t type_modifier = gpdb::ExprTypeMod((Node *)target_entry->expr);
    CDXLColRef *dxl_colref = GPOS_NEW(m_mp) CDXLColRef(mdname, colid, mdid_type, type_modifier);
    CDXLScalarIdent *dxl_ident = GPOS_NEW(m_mp) CDXLScalarIdent(m_mp, dxl_colref);
...
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateExprToDXLProject(Expr *expr, const char *alias_name, bool insist_new_colids) {
  ...
  CDXLNode *project_elem_dxlnode =
      GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarProjElem(m_mp, project_elem_id, mdname_alias));
...
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::CreateDXLConstValueTrue() {
  Const *const_expr = (Const *)gpdb::MakeBoolConst(true /*value*/, false /*isnull*/);
...
//
//---------------------------------------------------------------------------
CDXLNode *CTranslatorQueryToDXL::TranslateGroupingFuncToDXL(const Expr *expr, CBitSet *bitset,
  ...
  uint32_t sort_group_ref = gpdb::ListNthInt(grouping_func->refs, 0);
  ...
  CDXLDatum *datum_dxl = GPOS_NEW(m_mp) CDXLDatumInt4(m_mp, mdid, false /* is_null */, l_value);
...
//
//---------------------------------------------------------------------------
void CTranslatorQueryToDXL::ConstructCTEProducerList(List *cte_list, uint32_t cte_query_level) {
  GPOS_ASSERT(nullptr != m_dxl_cte_producers && "CTE Producer list not initialized");
  ...
  foreach (lc, cte_list) {
    ...
    CTranslatorQueryToDXL query_to_dxl_translator(m_context, m_md_accessor, m_var_to_colid_map, cte_query,
    ...
    GPOS_ASSERT(nullptr != cte_child_dxlnode && nullptr != cte_query_output_colds_dxlnode_array &&
    ...
    const uint32_t output_columns = cte_query_output_colds_dxlnode_array->Size();
    for (uint32_t ul = 0; ul < output_columns; ul++) {
      ...
      colid_array->Append(GPOS_NEW(m_mp) uint32_t(dxl_scalar_ident->GetDXLColRef()->Id()));
    }

    CDXLLogicalCTEProducer *lg_cte_prod_dxlop =
        GPOS_NEW(m_mp) CDXLLogicalCTEProducer(m_mp, m_context->m_cte_id_counter->next_id(), colid_array);
    CDXLNode *cte_producer_dxlnode = GPOS_NEW(m_mp) CDXLNode(m_mp, lg_cte_prod_dxlop, cte_child_dxlnode);

    m_dxl_cte_producers->Append(cte_producer_dxlnode);
    bool result GPOS_ASSERTS_ONLY = m_cteid_at_current_query_level_map->Insert(
        GPOS_NEW(m_mp) uint32_t(lg_cte_prod_dxlop->Id()), GPOS_NEW(m_mp) bool(true));
    GPOS_ASSERT(result);
...
//
//---------------------------------------------------------------------------
void CTranslatorQueryToDXL::ConstructCTEAnchors(CDXLNodeArray *dxlnodes, CDXLNode **dxl_cte_anchor_top,
  ...
  const uint32_t num_of_ctes = dxlnodes->Size();

  for (uint32_t ul = num_of_ctes; ul > 0; ul--) {
    ...
    uint32_t cte_producer_id = cte_prod_dxlop->Id();

    if (nullptr == m_cteid_at_current_query_level_map->Find(&cte_producer_id)) {
      // cte not defined at this level: CTE anchor was already added
      continue;
    }

    CDXLNode *cte_anchor_new_dxlnode =
        GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLLogicalCTEAnchor(m_mp, cte_producer_id));
...
//
//---------------------------------------------------------------------------
ULongPtrArray *CTranslatorQueryToDXL::GenerateColIds(CMemoryPool *mp, uint32_t size) const {
  ULongPtrArray *colid_array = GPOS_NEW(mp) ULongPtrArray(mp);

  for (uint32_t ul = 0; ul < size; ul++) {
    colid_array->Append(GPOS_NEW(mp) uint32_t(m_context->m_colid_counter->next_id()));
  }
...
//
//---------------------------------------------------------------------------
ULongPtrArray *CTranslatorQueryToDXL::ExtractColIds(CMemoryPool *mp, IntToUlongMap *attno_to_colid_mapping) const {
  ...
  while (att_iter.Advance()) {
    uint32_t colid = *(att_iter.Value());

    // do not insert colid if already inserted
    if (nullptr == old_new_col_mapping->Find(&colid)) {
      colid_array->Append(GPOS_NEW(m_mp) uint32_t(colid));
      old_new_col_mapping->Insert(GPOS_NEW(m_mp) uint32_t(colid), GPOS_NEW(m_mp) uint32_t(colid));
    }
  }
...
//
//---------------------------------------------------------------------------
IntToUlongMap *CTranslatorQueryToDXL::RemapColIds(CMemoryPool *mp, IntToUlongMap *attno_to_colid_mapping,
  ...
  UlongToUlongMap *old_new_col_mapping = GPOS_NEW(mp) UlongToUlongMap(mp);
  const uint32_t size = from_list_colids->Size();
  for (uint32_t ul = 0; ul < size; ul++) {
    bool result GPOS_ASSERTS_ONLY = old_new_col_mapping->Insert(GPOS_NEW(mp) uint32_t(*((*from_list_colids)[ul])),
                                                                GPOS_NEW(mp) uint32_t(*((*to_list_colids)[ul])));
    GPOS_ASSERT(result);
  }
  ...
  while (mi.Advance()) {
    int32_t *key = GPOS_NEW(mp) int32_t(*(mi.Key()));
    const uint32_t *value = mi.Value();
    GPOS_ASSERT(nullptr != value);

    uint32_t *remapped_value = GPOS_NEW(mp) uint32_t(*(old_new_col_mapping->Find(value)));
...
//
//---------------------------------------------------------------------------
bool CTranslatorQueryToDXL::IsDMLQuery() {
