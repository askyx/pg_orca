...
// ctor
CPartPruneStepsBuilder::CPartPruneStepsBuilder(Relation relation, Index rtindex, ULongPtrArray *part_indexes,
...
List *CPartPruneStepsBuilder::CreatePartPruneInfos(CDXLNode *filterNode, Relation relation, Index rtindex,
                                                   ULongPtrArray *part_indexes,
                                                   CMappingColIdVarPlStmt *colid_var_mapping,
                                                   CTranslatorDXLToScalar *translator_dxl_to_scalar) {
  CPartPruneStepsBuilder builder(relation, rtindex, part_indexes, colid_var_mapping, translator_dxl_to_scalar);
  ...
  PartitionedRelPruneInfo *pinfo = builder.CreatePartPruneInfoForOneLevel(filterNode);
...
PartitionedRelPruneInfo *CPartPruneStepsBuilder::CreatePartPruneInfoForOneLevel(CDXLNode *filterNode) {
  ...
  for (ULONG i = 0; (int)i < pinfo->nparts; ++i) {
    ...
    if (part_ptr < m_part_indexes->Size() && i == *(*m_part_indexes)[part_ptr]) {
      ...
      pinfo->relid_map[i] = gpdb::GPDBRelationRetrievePartitionDesc(m_relation)->oids[i];
...
List *CPartPruneStepsBuilder::PartPruneStepFromScalarCmp(CDXLNode *node, int *step_id, List *steps_list) {
  ...
  Oid opfamily = gpdb::GPDBRelationRetrievePartitionKey(m_relation)->partopfamily[0 /* col */];
  ...
  step->cmpfns = ListMake1Oid(gpdb::GPDBRelationRetrievePartitionKey(m_relation)->partsupfunc[0].fn_oid);
  step->exprs = ListMake1(expr);

  return gpdb::LAppend(steps_list, (PartitionPruneStep *)step);
}

List *CPartPruneStepsBuilder::PartPruneStepFromScalarBoolExpr(CDXLNode *node, int *step_id, List *steps_list) {
  GPOS_ASSERT(nullptr != node);
  ...
  for (ULONG ul = 0; ul < node->Arity(); ul++) {
    ...
    PartitionPruneStep *last_step = (PartitionPruneStep *)lfirst(gpdb::ListTail(steps_list));
...
List *CPartPruneStepsBuilder::PartPruneStepsFromFilter(CDXLNode *node, INT *step_id, List *steps_list) {
  ...
  switch (eopid) {
    ...
    case EdxlopScalarBoolExpr: {
      steps_list = PartPruneStepFromScalarBoolExpr(node, step_id, steps_list);
      break;
    }
    default:
      GPOS_RTL_ASSERT(!"Unsupported operator in PartPruneStepsFromFilter");
      break;
  }
  return steps_list;
}
