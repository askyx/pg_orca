...
//---------------------------------------------------------------------------
BOOL CQueryMutators::NeedsProjListNormalization(const Query *query) {
  if (!query->hasAggs && nullptr == query->groupClause && nullptr == query->groupingSets) {
    return false;
  }
  ...
  foreach (lc, query->targetList) {
    ...
    if (!IsA(target_entry->expr, Aggref) && !IsA(target_entry->expr, GroupingFunc) &&
        !CTranslatorUtils::IsGroupingColumn((Node *)target_entry->expr, query->groupClause, query->targetList)) {
      return true;
    }
  }
...
//---------------------------------------------------------------------------
BOOL CQueryMutators::ShouldFallback(Node *node, SContextTLWalker *context) {
  if (nullptr == node) {
    return false;
  }
  ...
  if (nullptr != entry &&
      CTranslatorUtils::IsGroupingColumn((Node *)entry->expr, context->m_group_clause, context->m_target_entries)) {
    return false;
  }
...
//---------------------------------------------------------------------------
Query *CQueryMutators::NormalizeGroupByProjList(CMemoryPool *mp, CMDAccessor *md_accessor, const Query *query) {
  ...
  Query *new_query =
      ConvertToDerivedTable(query_copy, false /*should_fix_target_list*/, true /*should_fix_having_qual*/);
  ...
  SContextGrpbyPlMutator context(mp, md_accessor, derived_table_query, nullptr);
  ...
  foreach (lc, target_list_copy) {
    ...
    if (!is_grouping_col) {
      ...
      GPOS_ASSERT(!IsA(target_entry->expr, Aggref) && "New target list entry should not contain any Aggrefs");
      GPOS_ASSERT(!IsA(target_entry->expr, GroupingFunc) &&
...
//---------------------------------------------------------------------------
Node *CQueryMutators::RunIncrLevelsUpMutator(Node *node, SContextIncLevelsupMutator *context) {
  if (nullptr == node) {
    return nullptr;
  }
...
//---------------------------------------------------------------------------
BOOL CQueryMutators::RunFixCTELevelsUpWalker(Node *node, SContextIncLevelsupMutator *context) {
  if (nullptr == node) {
    return false;
  }
  ...
  if (IsA(node, TargetEntry) && !context->m_should_fix_top_level_target_list && 0 == context->m_current_query_level) {
...
//
//---------------------------------------------------------------------------
Node *CQueryMutators::RunGroupingColMutator(Node *node, SContextGrpbyPlMutator *context) {
  if (nullptr == node) {
    return nullptr;
  }
  ...
  if (IsA(node, Aggref)) {
    ...
    foreach (lc, old_aggref->args) {
      ...
      new_args =
          gpdb::LAppend(new_args, gpdb::MutateQueryOrExpressionTree(
                                      arg, (MutatorWalkerFn)CQueryMutators::RunGroupingColMutator, (void *)context,
  ...
  if (IsA(node, SubLink)) {
    ...
    new_sublink->testexpr = gpdb::MutateQueryOrExpressionTree(
        old_sublink->testexpr, (MutatorWalkerFn)CQueryMutators::RunGroupingColMutator, (void *)context,
        0  // flags -- mutate into cte-lists
    );
    context->m_current_query_level++;
    ...
    new_sublink->subselect = RunGroupingColMutator(old_sublink->subselect, context);
  ...
  if (IsA(node, CommonTableExpr)) {
    ...
    cte->ctequery = gpdb::MutateQueryOrExpressionTree(
        cte->ctequery, (MutatorWalkerFn)CQueryMutators::RunGroupingColMutator, (void *)context,
        0  // flags --- mutate into cte-lists
    );
  ...
  // recurse into query structure
  if (IsA(node, Query)) {
    Query *query = gpdb::MutateQueryTree((Query *)node, (MutatorWalkerFn)CQueryMutators::RunGroupingColMutator, context,
...
// 		Mutate the grouping columns, fix levels up when necessary
//---------------------------------------------------------------------------
Node *CQueryMutators::FixGroupingCols(Node *node, TargetEntry *orginal_target_entry, SContextGrpbyPlMutator *context) {
...
//---------------------------------------------------------------------------
TargetEntry *CQueryMutators::GetTargetEntryForAggExpr(CMemoryPool *mp, CMDAccessor *md_accessor, Node *node,
                                                      ULONG attno) {
  ...
  if (IsA(node, GroupingFunc)) {
    ...
    CMDIdGPDB *agg_mdid = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, aggref->aggfnoid);
...
Node *CQueryMutators::RunExtractAggregatesMutator(Node *node, SContextGrpbyPlMutator *context) {
  if (nullptr == node) {
    return nullptr;
  }
  ...
  if (IsA(node, Aggref)) {
    ...
    if (old_aggref->agglevelsup == context->m_current_query_level) {
      ...
      foreach (lc, old_aggref->args) {
        ...
        GPOS_ASSERT(nullptr != arg);
        // traverse each argument and fix levels up when needed
        new_args = gpdb::LAppend(new_args, gpdb::MutateQueryOrExpressionTree(
                                               arg, (MutatorWalkerFn)RunExtractAggregatesMutator, (void *)context,
  ...
  if (IsA(node, Var)) {
    ...
    if (var->varlevelsup == context->m_current_query_level) {
      ...
      var->varlevelsup = 0;
      TargetEntry *found_tle = gpdb::FindFirstMatchingMemberInTargetList((Node *)var, context->m_lower_table_tlist);
  ...
  if (IsA(node, CommonTableExpr)) {
    ...
    cte->ctequery =
        gpdb::MutateQueryOrExpressionTree(cte->ctequery, (MutatorWalkerFn)RunExtractAggregatesMutator, (void *)context,
                                          0  // mutate into cte-lists
        );
  ...
  if (IsA(node, SubLink)) {
    ...
    new_sublink->testexpr = gpdb::MutateQueryOrExpressionTree(
        old_sublink->testexpr, (MutatorWalkerFn)RunExtractAggregatesMutator, (void *)context,
        0  // mutate into cte-lists
    );
    context->m_current_query_level++;
    ...
    // context->m_current_query_level must be modified properly
    // while diving into such nested SubLink.
    new_sublink->subselect = RunExtractAggregatesMutator(old_sublink->subselect, context);
  ...
  if (IsA(node, Query)) {
    ...
    Query *query = gpdb::MutateQueryTree((Query *)node, (MutatorWalkerFn)RunExtractAggregatesMutator, context,
    ...
    foreach (lc, query->rtable) {
      ...
      if (RTE_SUBQUERY == rte->rtekind) {
        Query *subquery = rte->subquery;
        context->m_current_query_level++;
        rte->subquery = (Query *)RunExtractAggregatesMutator((Node *)subquery, context);
        context->m_current_query_level--;
        gpdb::GPDBFree(subquery);
      }
    }
...
// Create a new entry in the derived table and return its corresponding var
Var *CQueryMutators::MakeVarInDerivedTable(Node *node, SContextGrpbyPlMutator *context) {
  GPOS_ASSERT(nullptr != node);
  ...
  if (IsA(node, Aggref) || IsA(node, GroupingFunc)) {
    ...
    tle = gpdb::MakeTargetEntry((Expr *)node, (AttrNumber)attno, nullptr, false);
  }
  ...
  Var *new_var = gpdb::MakeVar(1 /* varno */, attno, gpdb::ExprType((Node *)node), gpdb::ExprTypeMod((Node *)node),
...
Node *CQueryMutators::FindNodeInGroupByTargetList(Node *node, SContextGrpbyPlMutator *context) {
  GPOS_ASSERT(nullptr != node);
  GPOS_ASSERT(nullptr != context);

  TargetEntry *found_tle = gpdb::FindFirstMatchingMemberInTargetList(node, context->m_lower_table_tlist);
  ...
  if (nullptr != found_tle) {
    ...
    Var *new_var =
        gpdb::MakeVar(1 /* varno */, found_tle->resno, gpdb::ExprType((Node *)found_tle->expr),
                      gpdb::ExprTypeMod((Node *)found_tle->expr), context->m_current_query_level /* varlevelsup */);
...
Var *CQueryMutators::IncrLevelsUpIfOuterRef(Var *var) {
...
//---------------------------------------------------------------------------
Query *CQueryMutators::NormalizeHaving(CMemoryPool *mp, CMDAccessor *md_accessor, const Query *query) {
  Query *query_copy = (Query *)gpdb::CopyObject(const_cast<Query *>(query));
  ...
  Query *new_query =
      ConvertToDerivedTable(query_copy, true /*should_fix_target_list*/, false /*should_fix_having_qual*/);
  gpdb::GPDBFree(query_copy);
  ...
  if (!rte->subquery->hasAggs && NIL == rte->subquery->groupClause && NIL == rte->subquery->groupingSets) {
    ...
    foreach (lc, rte->subquery->targetList) {
      ...
      new_subquery->targetList = gpdb::LAppend(new_subquery->targetList, (TargetEntry *)gpdb::CopyObject(target_entry));
    }
...
//		memory
//---------------------------------------------------------------------------
TargetEntry *CQueryMutators::MakeTopLevelTargetEntry(TargetEntry *old_target_entry, ULONG attno) {
  Var *new_var = gpdb::MakeVar(1, (AttrNumber)attno, gpdb::ExprType((Node *)old_target_entry->expr),
                               gpdb::ExprTypeMod((Node *)old_target_entry->expr),
                               0  // query levelsup
  );

  TargetEntry *new_target_entry =
      gpdb::MakeTargetEntry((Expr *)new_var, (AttrNumber)attno, old_target_entry->resname, old_target_entry->resjunk);
...
//---------------------------------------------------------------------------
CHAR *CQueryMutators::GetTargetEntryColName(TargetEntry *target_entry, Query *query) {
...
//
//---------------------------------------------------------------------------
Query *CQueryMutators::ConvertToDerivedTable(const Query *original_query, BOOL should_fix_target_list,
  ...
  Query *lower_query;
  {
    ...
    lower_query = gpdb::MutateQueryTree(query_copy, (MutatorWalkerFn)RunIncrLevelsUpMutator, &context1,
                                        0  // flags
    );
  }
  ...
  // derived table.
  {
    ...
    (void)gpdb::WalkQueryOrExpressionTree((Node *)lower_query, (ExprWalkerFn)RunFixCTELevelsUpWalker, &context2,
...
//---------------------------------------------------------------------------
Query *CQueryMutators::EliminateDistinctClause(const Query *query) {
  ...
  foreach (lc, target_entries) {
    ...
    if (!target_entry->resjunk) {
      ...
      Var *new_var = gpdb::MakeVar(1, target_entry->resno, gpdb::ExprType((Node *)target_entry->expr),
                                   gpdb::ExprTypeMod((Node *)target_entry->expr),
                                   0  // query levels up
      );
      TargetEntry *new_target_entry =
          gpdb::MakeTargetEntry((Expr *)new_var, (AttrNumber)resno, target_entry->resname, false);
  ...
  if (gpdb::ListLength(new_query->targetList) != gpdb::ListLength(query->distinctClause)) {
    GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
               GPOS_WSZ_LIT("DISTINCT operation on a subset of target list columns"));
  }
...
//---------------------------------------------------------------------------
BOOL CQueryMutators::NeedsProjListWindowNormalization(const Query *query) {
  ...
  foreach (lc, query->targetList) {
    ...
    if (!CTranslatorUtils::IsReferencedInWindowSpec(target_entry, query->windowClause) &&
        !IsA(target_entry->expr, WindowFunc) && !IsA(target_entry->expr, Var)) {
...
//
//  NEW QUERY:
//    SELECT rn+rk from (SELECT row_number() over() as rn, rank() over(partition by a+b order by a-b) as rk FROM foo)
//    foo_new
//---------------------------------------------------------------------------
Query *CQueryMutators::NormalizeWindowProjList(CMemoryPool *mp, CMDAccessor *md_accessor, const Query *original_query) {
  Query *query_copy = (Query *)gpdb::CopyObject(const_cast<Query *>(original_query));
  ...
  foreach (lc, target_entries) {
    ...
    if (CTranslatorUtils::IsReferencedInWindowSpec(target_entry, original_query->windowClause)) {
      ...
      TargetEntry *lower_target_entry = (TargetEntry *)gpdb::MutateExpressionTree(
          (Node *)target_entry, (MutatorWalkerFn)RunIncrLevelsUpMutator, &level_context);
      ...
      if (!target_entry->resjunk || is_sorting_col) {
        ...
        Var *new_var = gpdb::MakeVar(1, lower_target_entry->resno, gpdb::ExprType((Node *)target_entry->expr),
                                     gpdb::ExprTypeMod((Node *)target_entry->expr),
                                     0  // query levels up
        );
        TargetEntry *upper_target_entry =
            gpdb::MakeTargetEntry((Expr *)new_var, ulResNoNew, target_entry->resname, target_entry->resjunk);
      ...
      TargetEntry *upper_target_entry = (TargetEntry *)gpdb::MutateExpressionTree(
          (Node *)target_entry, (MutatorWalkerFn)RunWindowProjListMutator, &projlist_context);
      upper_query->targetList = gpdb::LAppend(upper_query->targetList, upper_target_entry);
    }
  }
...
//---------------------------------------------------------------------------
Node *CQueryMutators::RunWindowProjListMutator(Node *node, SContextGrpbyPlMutator *context) {
  if (nullptr == node) {
    return nullptr;
  }
  ...
  if (IsA(node, WindowFunc) && 0 == context->m_current_query_level) {
    ...
    CMDIdGPDB *mdid_func = GPOS_NEW(context->m_mp) CMDIdGPDB(IMDId::EmdidGeneral, window_func->winfnoid);
    ...
    TargetEntry *target_entry = gpdb::MakeTargetEntry(
        ...
        CTranslatorUtils::CreateMultiByteCharStringFromWCString(str->GetBuffer()), false /* resjunk */
    );
    ...
    Var *new_var = gpdb::MakeVar(1,  // derived query which is now the only table in FROM expression
...
//---------------------------------------------------------------------------
void CQueryMutators::ReassignSortClause(Query *top_level_query, Query *derived_table_query) {
