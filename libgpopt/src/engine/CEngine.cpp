...
void CEngine::InitLogicalExpression(CExpression *pexpr) {
  ...
  CGroup *pgroupRoot = PgroupInsert(nullptr /*pgroupTarget*/, pexpr, CXform::ExfInvalid, nullptr /*pgexprOrigin*/,
                                    false /*fIntermediate*/);
...
void CEngine::Init(CQueryContext *pqc, CSearchStageArray *search_stage_array) {
  ...
  GPOS_ASSERT_IMP(0 == pqc->Pexpr()->DeriveOutputColumns()->Size(),
                  0 == pqc->Prpp()->PcrsRequired()->Size() && "requiring columns from a zero column expression");
  ...
  if (GPOS_FTRACE(EopttracePrintOptimizationStatistics)) {
    ...
    const uint32_t ulStages = m_search_stage_array->Size();
    for (uint32_t ul = 0; ul < ulStages; ul++) {
      uintptr_t *pulpXformCalls = GPOS_NEW_ARRAY(m_mp, uintptr_t, CXform::ExfSentinel);
      uintptr_t *pulpXformTimes = GPOS_NEW_ARRAY(m_mp, uintptr_t, CXform::ExfSentinel);
      uintptr_t *pulpXformBindings = GPOS_NEW_ARRAY(m_mp, uintptr_t, CXform::ExfSentinel);
      uintptr_t *pulpXformResults = GPOS_NEW_ARRAY(m_mp, uintptr_t, CXform::ExfSentinel);
      for (uint32_t ulXform = 0; ulXform < CXform::ExfSentinel; ulXform++) {
...
//
//---------------------------------------------------------------------------
void CEngine::AddEnforcers(CGroupExpression *pgexpr,  // belongs to group where we need to add enforcers
                           CExpressionArray *pdrgpexprEnforcers) {
  GPOS_ASSERT(nullptr != pdrgpexprEnforcers);
  GPOS_ASSERT(nullptr != pgexpr);

  for (uint32_t ul = 0; ul < pdrgpexprEnforcers->Size(); ul++) {
    // assemble an expression rooted by the enforcer operator
    CExpression *pexprEnforcer = (*pdrgpexprEnforcers)[ul];
#ifdef GPOS_DEBUG
    CGroup *pgroup =
#endif  // GPOS_DEBUG
        PgroupInsert(pgexpr->Pgroup(), pexprEnforcer, CXform::ExfInvalid, nullptr /*pgexprOrigin*/,
                     false /*fIntermediate*/);
...
//
//---------------------------------------------------------------------------
void CEngine::InsertExpressionChildren(CExpression *pexpr, CGroupArray *pdrgpgroupChildren,
  ...
  for (uint32_t i = 0; i < arity; i++) {
    ...
    if (popChild->FPattern() && CPattern::PopConvert(popChild)->FLeaf()) {
      ...
      pgroupChild =
          PgroupInsert(nullptr /*pgroupTarget*/, (*pexpr)[i], exfidOrigin, pgexprOrigin, true /*fIntermediate*/);
    }
...
//
//---------------------------------------------------------------------------
CGroup *CEngine::PgroupInsert(CGroup *pgroupTarget, CExpression *pexpr, CXform::EXformId exfidOrigin,
  ...
  if (nullptr != pexpr->Pgexpr()) {
    ...
    GPOS_ASSERT(nullptr != pgroupOrigin && nullptr == pgroupTarget && "A valid group is expected");
  ...
  CGroupExpression *pgexpr =
      GPOS_NEW(m_mp) CGroupExpression(m_mp, pop, pdrgpgroupChildren, exfidOrigin, pgexprOrigin, fIntermediate);
...
//
//---------------------------------------------------------------------------
void CEngine::InsertXformResult(CGroup *pgroupOrigin, CXformResult *pxfres, CXform::EXformId exfidOrigin,
                                CGroupExpression *pgexprOrigin,
                                uint32_t ulXformTime,  // time consumed by transformation in msec
                                uint32_t ulNumberOfBindings) {
...
//
//---------------------------------------------------------------------------
void CEngine::DeriveStats(CMemoryPool *pmpLocal, CMemoryPool *pmpGlobal, CGroup *pgroup, CReqdPropRelational *prprel) {
...
//---------------------------------------------------------------------------
bool CEngine::FOptimizeChild(CGroupExpression *pgexprParent, CGroupExpression *pgexprChild,
  ...
  GPOS_ASSERT_IMP(nullptr == pgexprParent, pgexprChild->Pgroup() == PgroupRoot());
...
//
//---------------------------------------------------------------------------
bool CEngine::FSafeToPruneWithDPEStats(CGroupExpression *pgexpr,
                                       CReqdPropPlan *,  // prpp
                                       CCostContext *pccChild, uint32_t child_index) {
  ...
  uint32_t ulNextChild = exprhdl.UlNextOptimizedChildIndex(child_index);
...
//
//---------------------------------------------------------------------------
bool CEngine::FSafeToPrune(CGroupExpression *pgexpr, CReqdPropPlan *prpp, CCostContext *pccChild, uint32_t child_index,
  ...
  *pcostLowerBound = GPOPT_INVALID_COST;
  ...
  if (GPOS_FTRACE(EopttraceDeriveStatsForDPE) && !FSafeToPruneWithDPEStats(pgexpr, prpp, pccChild, child_index)) {
...
MemoTreeMap *CEngine::Pmemotmap() {
  ...
  if (nullptr == m_pmemo->Pmemotmap()) {
    ...
    COptimizationContext *poc = GPOS_NEW(m_mp) COptimizationContext(
        ...
        GPOS_NEW(m_mp) CReqdPropRelational(
            GPOS_NEW(m_mp) CColRefSet(m_mp)),   // pass empty required relational properties initially
...
//
//---------------------------------------------------------------------------
void CEngine::ApplyTransformations(CMemoryPool *pmpLocal, CXformSet *xform_set, CGroupExpression *pgexpr) {
  // iterate over xforms
...
//
//---------------------------------------------------------------------------
void CEngine::TransitionGroupExpression(CMemoryPool *pmpLocal, CGroupExpression *pgexpr,
                                        CGroupExpression::EState estTarget) {
  GPOS_ASSERT(CGroupExpression::estExplored == estTarget || CGroupExpression::estImplemented == estTarget);
...
//
//---------------------------------------------------------------------------
void CEngine::TransitionGroup(CMemoryPool *pmpLocal, CGroup *pgroup, CGroup::EState estTarget) {
  // check stack size
  GPOS_CHECK_STACK_SIZE;
  ...
  GPOS_ASSERT(CGroup::estExplored == estTarget || CGroup::estImplemented == estTarget);
  ...
  if (!fTransitioned) {
    ...
    while (nullptr != pgexprCurrent) {
      if (!pgexprCurrent->FTransitioned(estGExprTargetState)) {
        TransitionGroupExpression(pmpLocal, pgexprCurrent, estGExprTargetState);
      }
...
//
//---------------------------------------------------------------------------
COptimizationContext *CEngine::PocChild(
  ...
  COptimizationContext *pocChild = GPOS_NEW(m_mp)
      COptimizationContext(m_mp, pgroupChild, exprhdlPlan.Prpp(child_index), prprel, stats_ctxt, m_ulCurrSearchStage);
...
//
//---------------------------------------------------------------------------
CCostContext *CEngine::PccOptimizeChild(CExpressionHandle &exprhdl,  // initialized with required properties
                                        CExpressionHandle &exprhdlRel,
                                        COptimizationContext *pocOrigin,  // optimization context of parent operator
                                        CDrvdPropArray *pdrgpdp, IStatisticsArray *pdrgpstatCurrentCtxt,
                                        uint32_t child_index, uint32_t ulOptReq) {
  ...
  COptimizationContext *pocFound =
      pgroupChild->PocLookupBest(m_mp, m_search_stage_array->Size(), exprhdl.Prpp(child_index));
  ...
  if (exprhdl.UlFirstOptimizedChildIndex() == child_index &&
    ...
    (void)pgexpr->PccComputeCost(m_mp, pocOrigin, ulOptReq, nullptr /*pdrgpoc*/, true /*fPruned*/, costLowerBound);
    return nullptr;
  }
...
//
//---------------------------------------------------------------------------
COptimizationContextArray *CEngine::PdrgpocOptimizeChildren(
  ...
  bool fSuccess = true;
  uint32_t child_index = exprhdl.UlFirstOptimizedChildIndex();
  do {
    CGroup *pgroupChild = (*exprhdl.Pgexpr())[child_index];
    ...
    CCostContext *pccChildBest =
        PccOptimizeChild(exprhdl, exprhdlRel, pocOrigin, pdrgpdp, pdrgpstatCurrentCtxt, child_index, ulOptReq);
...
//
//---------------------------------------------------------------------------
void CEngine::OptimizeGroupExpression(CGroupExpression *pgexpr, COptimizationContext *poc) {
  CGroup *pgroup = pgexpr->Pgroup();
  ...
  for (uint32_t ul = 0; ul < ulOptRequests; ul++) {
    ...
    CCost costLowerBound(GPOPT_INVALID_COST);
    if (FSafeToPrune(pgexpr, poc->Prpp(), nullptr /*pccChild*/, UINT32_MAX /*child_index*/, &costLowerBound)) {
      (void)pgexpr->PccComputeCost(m_mp, poc, ul, nullptr /*pdrgpoc*/, true /*fPruned*/, costLowerBound);
      continue;
    }
    ...
    if (FCheckReqdProps(exprhdl, poc->Prpp(), ul)) {
      ...
      if (nullptr != pdrgpoc && FCheckEnfdProps(m_mp, pgexpr, poc, ul, pdrgpoc)) {
...
//
//---------------------------------------------------------------------------
CGroupExpression *CEngine::PgexprOptimize(CGroup *pgroup, COptimizationContext *poc, CGroupExpression *pgexprOrigin) {
  // recursive function - check stack
  GPOS_CHECK_STACK_SIZE;
...
//
//---------------------------------------------------------------------------
void CEngine::RecursiveOptimize() {
  ...
  for (uint32_t ul = 0; !FSearchTerminated() && ul < ulSearchStages; ul++) {
    ...
    CExpression *pexprPlan =
        m_pmemo->PexprExtractPlan(m_mp, m_pmemo->PgroupRoot(), m_pqc->Prpp(), m_search_stage_array->Size());
...
void CEngine::DbgPrintExpr(int group_no, int context_no) {
  CAutoTrace at(m_mp);
  ...
  GPOS_TRY {
    ...
    if (nullptr != top_group) {
      ...
      if (nullptr != poc) {
        CExpression *extracted_expr =
            m_pmemo->PexprExtractPlan(m_mp, top_group, poc->Prpp(), m_search_stage_array->Size());
...
//
//---------------------------------------------------------------------------
COptimizationContextArray *CEngine::PdrgpocChildren(CMemoryPool *mp, CExpressionHandle &exprhdl) {
  GPOS_ASSERT(nullptr != exprhdl.Pgexpr());
  ...
  for (uint32_t ul = 0; ul < arity; ul++) {
    ...
    if (!pgroupChild->FScalar()) {
      COptimizationContext *poc = pgroupChild->PocLookupBest(mp, m_search_stage_array->Size(), exprhdl.Prpp(ul));
...
//
//---------------------------------------------------------------------------
void CEngine::ScheduleMainJob(CSchedulerContext *psc, COptimizationContext *poc) const {
  GPOS_ASSERT(nullptr != PgroupRoot());

  CJobGroupOptimization::ScheduleJob(psc, PgroupRoot(), nullptr /*pgexprOrigin*/, poc, nullptr /*pjParent*/);
}
...
//
//---------------------------------------------------------------------------
void CEngine::PrintActivatedXforms(IOstream &os) const {
  if (GPOS_FTRACE(EopttracePrintOptimizationStatistics)) {
    os << std::endl << "[OPT]: <Begin Xforms - stage " << m_ulCurrSearchStage << ">" << std::endl;
    CXformSetIter xsi(*m_xforms);
    while (xsi.Advance()) {
      CXform *pxform = CXformFactory::Pxff()->Pxf(xsi.TBit());
      uint32_t ulCalls = (uint32_t)(*m_pdrgpulpXformCalls)[m_ulCurrSearchStage][pxform->Exfid()];
      uint32_t ulTime = (uint32_t)(*m_pdrgpulpXformTimes)[m_ulCurrSearchStage][pxform->Exfid()];
      uint32_t ulBindings = (uint32_t)(*m_pdrgpulpXformBindings)[m_ulCurrSearchStage][pxform->Exfid()];
      uint32_t ulResults = (uint32_t)(*m_pdrgpulpXformResults)[m_ulCurrSearchStage][pxform->Exfid()];
      os << pxform->SzId() << ": " << ulCalls << " calls, " << ulBindings << " total bindings, " << ulResults
...
//
//---------------------------------------------------------------------------
IOstream &CEngine::OsPrintMemoryConsumption(IOstream &os, const char *szHeader) const {
  CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
  CMDAccessor::MDCache *pcache = md_accessor->Pcache();

  os << std::endl
     << szHeader << "Engine: [" << (double)m_mp->TotalAllocatedSize() / GPOPT_MEM_UNIT << "] " << GPOPT_MEM_UNIT_NAME
     << ", MD Cache: [" << (double)(pcache->TotalAllocatedSize()) / GPOPT_MEM_UNIT << "] " << GPOPT_MEM_UNIT_NAME
     << ", Total: [" << (double)(CMemoryPoolManager::GetMemoryPoolMgr()->TotalAllocatedSize()) / GPOPT_MEM_UNIT << "] "
     << GPOPT_MEM_UNIT_NAME;
...
//
//---------------------------------------------------------------------------
void CEngine::ProcessTraceFlags() {
  ...
  if (GPOS_FTRACE(EopttracePrintOptimizationStatistics)) {
    ...
    at.Os() << std::endl
            << "[OPT]: Memo (stage " << m_ulCurrSearchStage << "): [" << (uint32_t)(m_pmemo->UlpGroups()) << " groups"
            << ", " << m_pmemo->UlDuplicateGroups() << " duplicate groups" << ", " << m_pmemo->UlGrpExprs()
            << " group expressions" << ", " << m_xforms->Size() << " activated xforms]";

    at.Os() << std::endl
            << "[OPT]: stage " << m_ulCurrSearchStage << " completed in " << PssCurrent()->UlElapsedTime() << "ms, ";
...
void CEngine::Optimize() {
  ...
  const uint32_t ulJobs = std::min((uint32_t)GPOPT_JOBS_CAP, (uint32_t)(m_pmemo->UlpGroups() * GPOPT_JOBS_PER_GROUP));
...
//
//---------------------------------------------------------------------------
CExpression *CEngine::PexprUnrank(uint64_t plan_id) {
...
//
//---------------------------------------------------------------------------
CExpression *CEngine::PexprExtractPlan() {
  GPOS_ASSERT(nullptr != m_pqc);
  ...
  if (gpopt::CEnumeratorConfig::FEnumerate()) {
    CAutoTrace at(m_mp);
    uint64_t ullCount = Pmemotmap()->UllCount();
  ...
  if (fGenerateAlt) {
    ...
    pexpr = m_pmemo->PexprExtractPlan(m_mp, m_pmemo->PgroupRoot(), m_pqc->Prpp(), m_search_stage_array->Size());
  }
...
//
//---------------------------------------------------------------------------
uint64_t CEngine::UllRandomPlanId(uint32_t *seed) {
  uint64_t ullCount = Pmemotmap()->UllCount();
  uint64_t plan_id = 0;
  do {
...
//
//---------------------------------------------------------------------------
bool CEngine::FValidPlanSample(CEnumeratorConfig *pec, uint64_t plan_id,
...
//
//---------------------------------------------------------------------------
void CEngine::SamplePlans() {
  COptimizerConfig *optimizer_config = COptCtxt::PoctxtFromTLS()->GetOptimizerConfig();
  GPOS_ASSERT(nullptr != optimizer_config);

  CEnumeratorConfig *pec = optimizer_config->GetEnumeratorCfg();

  uint64_t ullSamples = pec->UllInputSamples();
  ...
  uint64_t ullTargetSamples = ullSamples;
  if (fGenerateAll) {
...
//
//---------------------------------------------------------------------------
bool CEngine::FCheckEnfdProps(CMemoryPool *mp, CGroupExpression *pgexpr, COptimizationContext *poc, uint32_t ulOptReq,
                              COptimizationContextArray *pdrgpoc) {
  GPOS_CHECK_ABORT;

  if (GPOS_FTRACE(EopttracePrintMemoEnforcement)) {
    CAutoTrace at(m_mp);
    at.Os() << "CEngine::FCheckEnfdProps (Group ID: " << pgexpr->Pgroup()->Id() << " Expression ID: " << pgexpr->Id()
            << ")" << std::endl;
  ...
  CBinding binding;
  CExpression *pexpr = binding.PexprExtract(m_mp, exprhdl.Pgexpr(), m_pexprEnforcerPattern, nullptr /* pexprLast */);
  GPOS_ASSERT(nullptr != pexpr);
...
//
//---------------------------------------------------------------------------
bool CEngine::FChildrenOptimized(COptimizationContextArray *pdrgpoc) {
  GPOS_ASSERT(nullptr != pdrgpoc);

  const uint32_t length = pdrgpoc->Size();
...
//		decision.
//---------------------------------------------------------------------------
bool CEngine::FCheckReqdProps(CExpressionHandle &exprhdl, CReqdPropPlan *prpp, uint32_t ulOptReq) {
  GPOS_CHECK_ABORT;
  ...
  if (COperator::EopPhysicalPartitionSelector == popPhysical->Eopid()) {
    CPhysicalPartitionSelector *part_selector = CPhysicalPartitionSelector::PopConvert(popPhysical);
...
UlongPtrArray *CEngine::GetNumberOfBindings() {
...
//
//---------------------------------------------------------------------------
void CEngine::PrintOptCtxts() {
  CAutoTrace at(m_mp);
  COptimizationContext *poc = m_pmemo->PgroupRoot()->PocLookupBest(m_mp, m_search_stage_array->Size(), m_pqc->Prpp());
  GPOS_ASSERT(nullptr != poc);
...
//
//---------------------------------------------------------------------------
IOstream &CEngine::OsPrint(IOstream &os) const {
