...
CExpression *CLeftJoinPruningPreprocessor::PexprCheckLeftOuterJoinPruningConditions(CMemoryPool *mp,
...
BOOL CLeftJoinPruningPreprocessor::CheckJoinPruningCondOnInnerRel(const CExpression *pexprNew,
  ...
  const CColRefSet *derive_output_columns_inner_rel = inner_rel->DeriveOutputColumns();
  ...
  if (output_columns->FIntersects(derive_output_columns_inner_rel) || nullptr == inner_rel->DeriveKeyCollection()) {
    return false;
  }
  return true;
}
...
BOOL CLeftJoinPruningPreprocessor::CheckJoinPruningCondOnJoinCond(CMemoryPool *mp, const CExpression *pexprNew,
  ...
  if ((!CPredicateUtils::IsEqualityOp(join_cond) && !CPredicateUtils::FAnd(join_cond)) ||
  ...
  if (CPredicateUtils::IsEqualityOp(join_cond)) {
    ...
    if (CPredicateUtils::FPlainEquality(join_cond) && 1 == join_cond->DeriveUsedColumns()->Size()) {
      inner_unique_keys->Release();
      return false;
    }
    ...
    CheckUniqueKeyInJoinCond(inner_columns, usedColumns, result, inner_unique_keys, outer_rel_columns);
    inner_columns->Release();
  }

  // Condition 2: If AND operator is present in join condition
  if (CPredicateUtils::FAnd(join_cond) &&
      !CheckAndCondInJoinCond(mp, join_cond, inner_unique_keys, result, outer_rel_columns)) {
    inner_unique_keys->Release();
    return false;
  }
...
BOOL CLeftJoinPruningPreprocessor::CheckAndCondInJoinCond(CMemoryPool *mp, const CExpression *join_cond,
  ...
  for (ULONG ul = 0; ul < arity_join; ul++) {
    ...
    if (!CPredicateUtils::FPlainEqualityIdentConstWithoutCast(join_cond_child)) {
      continue;
    }

    // If the join condition is like table1.col1=table1.col1 then continue
    if (CPredicateUtils::FPlainEquality(join_cond_child) && 1 == join_cond_child->DeriveUsedColumns()->Size()) {
      continue;
    }
...
void CLeftJoinPruningPreprocessor::CheckUniqueKeyInJoinCond(CColRefSet *inner_columns, const CColRefSet *usedColumns,
  ...
  if (usedColumns->Size() == 1 && inner_unique_keys->FMember(usedColumns->PcrFirst())) {
  ...
  if (usedColumns->Size() == 2) {
    ...
    // key or not.
    if (inner_columns->Size() == 1 && inner_unique_keys->FMember(inner_columns->PcrFirst())) {
...
CExpression *CLeftJoinPruningPreprocessor::PexprJoinPruningScalarSubquery(CMemoryPool *mp, CExpression *pexprScalar) {
  ...
  if (COperator::EopScalarSubquery == pexprScalar->Pop()->Eopid()) {
    CScalarSubquery *subquery_pop = CScalarSubquery::PopConvert(pexprScalar->Pop());
    subquery_colref = subquery_pop->Pcr();
  } else if (CUtils::FQuantifiedSubquery(pexprScalar->Pop())) {
    CScalarSubqueryQuantified *subquery_pop = CScalarSubqueryQuantified::PopConvert(pexprScalar->Pop());
    subquery_colref = subquery_pop->Pcr();
  }
  ...
  pexprScalar = JoinPruningTreeTraversal(mp, pexprScalar, pdrgpexpr, subquery_colrefset);
...
// relation.
void CLeftJoinPruningPreprocessor::ComputeOutputColumns(const CExpression *pexpr,
                                                        const CColRefSet *derived_output_columns,
                                                        CColRefSet *output_columns,
                                                        CColRefSet *combined_output_pred_columns,
...
CExpression *CLeftJoinPruningPreprocessor::JoinPruningTreeTraversal(CMemoryPool *mp, const CExpression *pexpr,
  ...
  for (ULONG ul = 0; ul < number_of_childs; ul++) {
    ...
    if (pexpr_child->Pop()->FLogical()) {
      ...
      CExpression *pexprScalarJoinPrunedChild = PexprJoinPruningScalarSubquery(mp, pexpr_child);
...
// +--CLogicalSelect
//    |--CLogicalLeftOuterJoin
//    |  |--CLogicalGet "foo" ("foo"), Columns: ["a" (0), "b" (1), "c" (2), "d" (3), "e" (4)] Key sets: {[0,1], [0,2,3],
//    [5,11]} |  |--CLogicalGet "bar" ("bar"), Columns: ["p" (12), "q" (13), "r" (14), "s" (15), "t" (16)] Key sets:
//    {[0,1], [0,2,3], [5,11]} |  +--CScalarBoolOp (EboolopAnd) |     |--CScalarCmp (=) |     |  |--CScalarIdent "a" (0)
//    |     |  +--CScalarIdent "p" (12)
//    |     |--CScalarCmp (=)
//    |     |  |--CScalarIdent "c" (2)
//    |     |  +--CScalarIdent "r" (14)
...
CExpression *CLeftJoinPruningPreprocessor::PexprPreprocess(CMemoryPool *mp, CExpression *pexpr,
                                                           const CColRefSet *pcrsOutput) {
  GPOS_ASSERT(nullptr != pexpr);
  ...
  ComputeOutputColumns(pexpr, derived_output_columns, output_columns, combined_output_pred_columns, pcrsOutput);

  // Array to hold the child expressions
  CExpressionArray *pdrgpexpr = GPOS_NEW(mp) CExpressionArray(mp);

  // Traversing the tree
  CExpression *pexprNew = JoinPruningTreeTraversal(mp, pexpr, pdrgpexpr, combined_output_pred_columns);
  ...
  if (CPredicateUtils::FLeftOuterJoin(pexprNew)) {
    ...
    pexprNew = PexprCheckLeftOuterJoinPruningConditions(mp, pexprNew, output_columns);
  }
