...
//
//---------------------------------------------------------------------------
CEnfdDistribution *CPhysicalFullHashJoin::Ped(CMemoryPool *mp, CExpressionHandle &exprhdl, CReqdPropPlan *prppInput,
                                              ULONG child_index, CDrvdPropArray *pdrgpdpCtxt, ULONG ulOptReq) {
  return PedRightOrFullJoin(mp, exprhdl, prppInput, child_index, pdrgpdpCtxt, ulOptReq);
}
...
//
//---------------------------------------------------------------------------
CDistributionSpec *CPhysicalFullHashJoin::PdsDerive(CMemoryPool *mp, CExpressionHandle &exprhdl) const {
  ...
  if (CDistributionSpec::EdtHashed == pdsRight->Edt()) {
    ...
    CDistributionSpecHashed *pdshashedLeft = CDistributionSpecHashed::PdsConvert(pdsLeft);
    CDistributionSpecHashed *pdshashedRight = CDistributionSpecHashed::PdsConvert(pdsRight);
    ...
    CDistributionSpecHashed *pdshashedLeftCopy = pdshashedLeft->Copy(mp, false);
    if (pdshashedLeft->IsCoveredBy(PdrgpexprOuterKeys()) && pdshashedRight->IsCoveredBy(PdrgpexprInnerKeys())) {
      ...
      CDistributionSpecHashed *pdshashedRightCopy = pdshashedRight->Copy(mp, false);
      combined_hashed_spec = pdshashedLeftCopy->Combine(mp, pdshashedRightCopy);
  ...
  if (CDistributionSpec::EdtStrictReplicated == pdsLeft->Edt() || CDistributionSpec::EdtUniversal == pdsLeft->Edt()) {
    pdsRight->AddRef();
    return pdsRight;
  } else {
    pdsLeft->AddRef();
