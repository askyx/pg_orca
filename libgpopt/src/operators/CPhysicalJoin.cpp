...
//
//---------------------------------------------------------------------------
CDistributionSpec *CPhysicalJoin::PdsRequired(CMemoryPool *mp GPOS_UNUSED, CExpressionHandle &exprhdl GPOS_UNUSED,
...
CEnfdDistribution *CPhysicalJoin::Ped(CMemoryPool *mp, CExpressionHandle &exprhdl, CReqdPropPlan *prppInput,
  ...
  if (exprhdl.NeedsSingletonExecution()) {
    return GPOS_NEW(mp) CEnfdDistribution(PdsRequireSingleton(mp, exprhdl, pdsRequired, child_index), dmatch);
  }

  if (exprhdl.HasOuterRefs()) {
    if (CDistributionSpec::EdtSingleton == pdsRequired->Edt() ||
        CDistributionSpec::EdtStrictReplicated == pdsRequired->Edt()) {
      return GPOS_NEW(mp) CEnfdDistribution(PdsPassThru(mp, exprhdl, pdsRequired, child_index), dmatch);
    }
    return GPOS_NEW(mp)
        CEnfdDistribution(GPOS_NEW(mp) CDistributionSpecReplicated(CDistributionSpec::EdtReplicated), dmatch);
  }
  ...
  if (1 == child_index) {
    ...
    if (CDistributionSpec::EdtUniversal == pdsOuter->Edt()) {
      ...
      return GPOS_NEW(mp) CEnfdDistribution(GPOS_NEW(mp) CDistributionSpecSingleton(), dmatch);
    }
    ...
    if (CDistributionSpec::EdtSingleton == pdsOuter->Edt() ||
      ...
      return GPOS_NEW(mp)
          CEnfdDistribution(CPhysical::PdssMatching(mp, CDistributionSpecSingleton::PdssConvert(pdsOuter)), dmatch);
    }
    ...
    if ((GPOS_FTRACE(EopttraceEnableRedistributeNLLOJInnerChild) &&
      ...
      CEnfdDistribution *pEnfdHashedDistribution =
          CPhysicalJoin::PedInnerHashedFromOuterHashed(mp, exprhdl, dmatch, (*pdrgpdpCtxt)[0]);
    ...
    return GPOS_NEW(mp) CEnfdDistribution(GPOS_NEW(mp) CDistributionSpecReplicated(CDistributionSpec::EdtReplicated),
                                          CEnfdDistribution::EdmSatisfy);
  }

  // no distribution requirement on the outer side
  return GPOS_NEW(mp) CEnfdDistribution(GPOS_NEW(mp) CDistributionSpecAny(this->Eopid()), dmatch);
}

CEnfdDistribution *CPhysicalJoin::PedInnerHashedFromOuterHashed(CMemoryPool *mp, CExpressionHandle &exprhdl,
  ...
  if (CDistributionSpec::EdtHashed == pdsOuter->Edt()) {
    ...
    CExpression *pexprScPredicate = exprhdl.PexprScalarExactChild(2, true /*error_on_null_return*/);
    ...
    for (ULONG ul = 0; fSuccess && ul < ulSize; ul++) {
      ...
      fSuccess = (nullptr != pexprMatching && CUtils::FScalarIdent(pexprMatching));
      if (fSuccess) {
        IMDId *pmdidTypeInner = CScalar::PopConvert(pexprMatching->Pop())->MdidType();

        IMDId *pmdidTypeOuter = CScalar::PopConvert(pexpr->Pop())->MdidType();

        CMDAccessor *mdAccessor = COptCtxt::PoctxtFromTLS()->Pmda();

        IMDId *mdidOpfamilyInner = mdAccessor->RetrieveType(pmdidTypeInner)->GetDistrOpfamilyMdid();

        IMDId *mdidOpfamilyOuter = mdAccessor->RetrieveType(pmdidTypeOuter)->GetDistrOpfamilyMdid();

        if (mdidOpfamilyOuter->Equals(mdidOpfamilyInner) && mdAccessor->RetrieveType(pmdidTypeInner)->IsHashable()) {
...
//
//---------------------------------------------------------------------------
CDistributionSpec *CPhysicalJoin::PdsDerive(CMemoryPool *mp, CExpressionHandle &exprhdl) const {
...
CEnfdDistribution *CPhysicalJoin::PedCorrelatedJoin(CMemoryPool *mp, CExpressionHandle &exprhdl,
  ...
  if (0 == ulOptReq && pdsRequired->FSingletonOrStrictSingleton()) {
    ...
    return GPOS_NEW(mp)
        CEnfdDistribution(PdsPassThru(mp, exprhdl, pdsRequired, child_index), CEnfdDistribution::EdmSatisfy);
  }
  ...
  return CPhysicalJoin::Ped(mp, exprhdl, prppInput, child_index, pdrgpdpCtxt, ulOptReq);
}
...
//
//---------------------------------------------------------------------------
CEnfdDistribution::EDistributionMatching CPhysicalJoin::Edm(CReqdPropPlan *,  // prppInput
  ...
  if (CDistributionSpec::EdtStrictReplicated == edtPrevChild || CDistributionSpec::EdtUniversal == edtPrevChild) {
