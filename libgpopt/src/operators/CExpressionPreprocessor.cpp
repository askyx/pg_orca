...
static void UpdateExprToConstantPredicateMapping(CMemoryPool *mp, CExpression *pexprFilter,
                                                 ExprToConstantMap *phmExprToConst, bool doInsert);

static CExpression *SubstituteConstantIdentifier(CMemoryPool *mp, CExpression *pexpr,
...
CExpression *CExpressionPreprocessor::PexprEliminateSelfComparison(CMemoryPool *mp, CExpression *pexpr,
  ...
  for (uint32_t ul = 0; ul < arity; ul++) {
    CExpression *pexprChild = PexprEliminateSelfComparison(mp, (*pexpr)[ul], pcrsNotNull);
...
CExpression *CExpressionPreprocessor::PexprPruneSuperfluousEquality(CMemoryPool *mp, CExpression *pexpr) {
  // protect against stack overflow during recursion
  GPOS_CHECK_STACK_SIZE;
  GPOS_ASSERT(nullptr != mp);
  GPOS_ASSERT(nullptr != pexpr);
...
CExpression *CExpressionPreprocessor::PexprTrimExistentialSubqueries(CMemoryPool *mp, CExpression *pexpr) {
  // protect against stack overflow during recursion
  GPOS_CHECK_STACK_SIZE;
  GPOS_ASSERT(nullptr != mp);
  GPOS_ASSERT(nullptr != pexpr);
  ...
  if (CUtils::FExistentialSubquery(pop)) {
    ...
    if (COperator::EopLogicalGbAgg == pexprInner->Pop()->Eopid() &&
      ...
      GPOS_ASSERT(0 < (*pexprInner)[1]->Arity() && "Project list of GbAgg is expected to be non-empty");
      bool fValue = true;
...
CExpression *CExpressionPreprocessor::PexprSimplifyQuantifiedSubqueries(CMemoryPool *mp, CExpression *pexpr) {
  ...
  COperator *pop = pexpr->Pop();
  if (CUtils::FQuantifiedSubquery(pop) && 1 == (*pexpr)[0]->DeriveMaxCard().Ull()) {
    CExpression *pexprInner = (*pexpr)[0];
    ...
    bool fGbAggWithoutGrpCols =
        COperator::EopLogicalGbAgg == popChild->Eopid() && 0 == CLogicalGbAgg::PopConvert(popChild)->Pdrgpcr()->Size();
    ...
    if (fGbAggWithoutGrpCols || fOneRowConstTable) {
      ...
      CExpression *pexprSubquery = GPOS_NEW(mp) CExpression(
          mp, GPOS_NEW(mp) CScalarSubquery(mp, colref, false /*fGeneratedByExist*/, true /*fGeneratedByQuantified*/),
          pexprInner);
...
CExpression *CExpressionPreprocessor::PexprUnnestScalarSubqueries(CMemoryPool *mp, CExpression *pexpr) {
  // protect against stack overflow during recursion
  GPOS_CHECK_STACK_SIZE;
  GPOS_ASSERT(nullptr != mp);
  GPOS_ASSERT(nullptr != pexpr);
...
CExpression *CExpressionPreprocessor::PexprRemoveSuperfluousLimit(CMemoryPool *mp, CExpression *pexpr) {
  // protect against stack overflow during recursion
  GPOS_CHECK_STACK_SIZE;
  GPOS_ASSERT(nullptr != mp);
  GPOS_ASSERT(nullptr != pexpr);
  ...
  if (COperator::EopLogicalLimit == pop->Eopid() && CUtils::FHasZeroOffset(pexpr) &&
...
CExpression *CExpressionPreprocessor::PexprRemoveSuperfluousDistinctInDQA(CMemoryPool *mp, CExpression *pexpr) {
  // protect against stack overflow during recursion
  GPOS_CHECK_STACK_SIZE;
  GPOS_ASSERT(nullptr != mp);
  GPOS_ASSERT(nullptr != pexpr);
  ...
  for (uint32_t ul = 0; ul < arity; ul++) {
    CExpression *pexprChild = PexprRemoveSuperfluousDistinctInDQA(mp, (*pexpr)[ul]);
...
CExpression *CExpressionPreprocessor::PexprRemoveSuperfluousOuterRefs(CMemoryPool *mp, CExpression *pexpr) {
  // protect against stack overflow during recursion
  GPOS_CHECK_STACK_SIZE;
  GPOS_ASSERT(nullptr != mp);
  GPOS_ASSERT(nullptr != pexpr);
  ...
  if (fHasOuterRefs) {
    ...
    if (COperator::EopLogicalLimit == op_id) {
      ...
      pop->Release();
      pop = GPOS_NEW(mp)
          CLogicalLimit(mp, posNew, popLimit->FGlobal(), popLimit->FHasCount(), popLimit->IsTopLimitUnderDMLorCTAS());
      ...
      if (0 < pExprProjList->Arity() || 0 < colref_array->Size()) {
        ...
        pop->Release();
        pop = GPOS_NEW(mp) CLogicalGbAgg(mp, colref_array, pdrgpcrMinimal, popAgg->Egbaggtype(),
        ...
        CExpression *projectExpr = CUtils::PexprAddProjection(mp, child, grouping_cols_arr,
        ...
        for (uint32_t ul = 1; ul < pexpr->PdrgPexpr()->Size(); ul++) {
        ...
        for (uint32_t ul = 0; ul < new_projected_cols->Arity(); ul++) {
          new_grouping_cols->Append(CUtils::PcrFromProjElem((*new_projected_cols)[ul]));
        }
        GPOS_ASSERT(nullptr == popAgg->PdrgpcrArgDQA());
        pop = GPOS_NEW(mp)
            CLogicalGbAgg(mp, new_grouping_cols, nullptr, popAgg->Egbaggtype(), popAgg->FGeneratesDuplicates(),
                          nullptr  // no DQA cols
            );
  ...
  // recursively process children
  const uint32_t arity = newExpr->Arity();
  CExpressionArray *pdrgpexprChildren = GPOS_NEW(mp) CExpressionArray(mp);
  for (uint32_t ul = 0; ul < arity; ul++) {
...
CExpression *CExpressionPreprocessor::PexprScalarBoolOpConvert2In(CMemoryPool *mp, CScalarBoolOp::EBoolOperator eboolop,
...
bool CExpressionPreprocessor::FConvert2InIsConvertable(CExpression *pexpr, CScalarBoolOp::EBoolOperator eboolopParent) {
  bool fConvertableExpression = false;
  if (CPredicateUtils::FCompareIdentToConst(pexpr)) {
    fConvertableExpression |= IMDType::EcmptEq == CScalarCmp::PopConvert(pexpr->Pop())->ParseCmpType() &&
                              CScalarBoolOp::EboolopOr == eboolopParent;
    fConvertableExpression |= IMDType::EcmptNEq == CScalarCmp::PopConvert(pexpr->Pop())->ParseCmpType() &&
...
CExpression *CExpressionPreprocessor::PexprConvert2In(CMemoryPool *mp,
...
CExpression *CExpressionPreprocessor::PexprCollapseJoins(CMemoryPool *mp, CExpression *pexpr) {
  // protect against stack overflow during recursion
  GPOS_CHECK_STACK_SIZE;
  GPOS_ASSERT(nullptr != mp);
  GPOS_ASSERT(nullptr != pexpr);
  ...
  if (CPredicateUtils::FInnerOrNAryJoin(pexpr) ||
    ...
    if (lojPredicates->Size() > 0) {
      ...
      for (uint32_t ul = 0; ul < lojPredicates->Size(); ul++) {
        CExpression *predicate = (*lojPredicates)[ul];
        predicate->AddRef();
        naryJoinPredicates->Append(predicate);
      }

      CExpression *nAryJoinPredicateList =
          GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarNAryJoinPredList(mp), naryJoinPredicates);
      ...
      //    lojPredicates:       [ p23, p35 ]  (p23 corresponds to child pred index 1, p35 corresponds to child pred
      //    index 2)
      ...
      uint32_t highestNumberSeen = 0;

      for (uint32_t ix = 1; ix < lojChildPredIndexes->Size(); ix++) {
        uint32_t nextNumber = *((*lojChildPredIndexes)[ix]);
    ...
    CExpression *pexprNAryJoin =
        GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalNAryJoin(mp, lojChildPredIndexes), newChildNodes);

    COptimizerConfig *optimizer_config = COptCtxt::PoctxtFromTLS()->GetOptimizerConfig();
    uint32_t ulJoinArityLimit = optimizer_config->GetHint()->UlJoinArityForAssociativityCommutativity();
...
void CExpressionPreprocessor::CollectJoinChildrenRecursively(CMemoryPool *mp, CExpression *pexpr,
  ...
  if (CPredicateUtils::FInnerOrNAryJoin(pexpr)) {
    ...
    if (COperator::EopScalarNAryJoinPredList != pexprScalar->Pop()->Eopid()) {
      for (uint32_t ul = 0; ul < arity - 1; ul++) {
        ...
        CollectJoinChildrenRecursively(mp, child, logicalLeafNodes, lojChildPredIndexes, innerJoinPredicates,
                                       lojPredicates);
      }
      ...
      for (uint32_t ul = 0; ul < arity - 1; ul++) {
        if (GPOPT_ZERO_INNER_JOIN_PRED_INDEX == *(*naryJoinPredIndexes)[ul]) {
          ...
          CollectJoinChildrenRecursively(mp, (*pexpr)[ul], logicalLeafNodes, lojChildPredIndexes, innerJoinPredicates,
                                         lojPredicates);
        } else {
          // this is the right child of a non-inner join
          uint32_t oldPredIndex = *(*naryJoinPredIndexes)[ul];
          ...
          uint32_t newPredIndex = lojPredicates->Size();

          lojChildPredIndexes->Append(GPOS_NEW(mp) uint32_t(newPredIndex));
        }
      }
    }
    ...
    CollectJoinChildrenRecursively(mp, leftChild, logicalLeafNodes, lojChildPredIndexes, innerJoinPredicates,
                                   lojPredicates);
    ...
    uint32_t *indexOfThisLOJInTheArray = GPOS_NEW(mp) uint32_t(lojPredicates->Size());
    ...
    uint32_t *innerJoinPredIndex = GPOS_NEW(mp) uint32_t(GPOPT_ZERO_INNER_JOIN_PRED_INDEX);
    lojChildPredIndexes->Append(innerJoinPredIndex);
  }
}

// collapse cascaded logical project operators
CExpression *CExpressionPreprocessor::PexprCollapseProjects(CMemoryPool *mp, CExpression *pexpr) {
  // protect against stack overflow during recursion
  GPOS_CHECK_STACK_SIZE;
  GPOS_ASSERT(nullptr != mp);
  GPOS_ASSERT(nullptr != pexpr);
...
CExpression *CExpressionPreprocessor::PexprProjBelowSubquery(CMemoryPool *mp, CExpression *pexpr, bool fUnderPrList) {
  // protect against stack overflow during recursion
  GPOS_CHECK_STACK_SIZE;
  GPOS_ASSERT(nullptr != mp);
  GPOS_ASSERT(nullptr != pexpr);
  ...
  if (pop->FLogical()) {
    ...
    if (nullptr != prcsOutput && !prcsOutput->FMember(pcrSubquery)) {
      ...
      CExpression *pexprPrEl =
          CUtils::PexprScalarProjectElement(mp, pcrNewSubquery, CUtils::PexprScalarIdent(mp, pcrSubquery));
      CExpression *pexprProjList = GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pexprPrEl);
      CExpression *pexprProj =
          GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalProject(mp), pexprRelNew, pexprProjList);

      CScalarSubquery *popSubq = GPOS_NEW(mp)
          CScalarSubquery(mp, pcrNewSubquery, false /*fGeneratedByExist*/, false /*fGeneratedByQuantified*/);
...
CExpression *CExpressionPreprocessor::PexprCollapseUnionUnionAll(CMemoryPool *mp, CExpression *pexpr) {
  // protect against stack overflow during recursion
  GPOS_CHECK_STACK_SIZE;
  GPOS_ASSERT(nullptr != mp);
  GPOS_ASSERT(nullptr != pexpr);
  ...
  for (uint32_t ul = 0; ul < arity; ul++) {
    if (CPredicateUtils::FCollapsibleChildUnionUnionAll(pexprNew, ul)) {
      fCollapsed = true;
      CPredicateUtils::CollectGrandChildrenUnionUnionAll(mp, pexprNew, ul, pdrgpexprNew, pdrgdrgpcrNew);
    } else {
...
CExpression *CExpressionPreprocessor::PexprOuterJoinToInnerJoin(CMemoryPool *mp, CExpression *pexpr) {
  // protect against stack overflow during recursion
  GPOS_CHECK_STACK_SIZE;
  GPOS_ASSERT(nullptr != mp);
  GPOS_ASSERT(nullptr != pexpr);
  ...
  if (CPredicateUtils::FInnerOrNAryJoin(pexpr)) {
    // the predicates of an inner join on top of outer join can be used to turn the child outer join into another inner
    // join
    CExpression *pexprScalar = (*pexpr)[arity - 1];
    ...
    for (uint32_t ul = 0; ul < arity; ul++) {
      ...
      if (COperator::EopLogicalLeftOuterJoin == pexprChild->Pop()->Eopid()) {
        ...
        if (!GPOS_FTRACE(EopttraceDisableOuterJoin2InnerJoinRewrite) &&
          ...
          pexprChild = CUtils::PexprLogicalJoin<CLogicalInnerJoin>(mp, pexprNewOuter, pexprNewInner, pexprNewScalar);
          fNewChild = true;
        }
      }
...
CExpression *CExpressionPreprocessor::PexprConjEqualityPredicates(CMemoryPool *mp, CColRefSet *pcrs) {
...
bool CExpressionPreprocessor::FEquivClassFromChild(CColRefSet *pcrs, CExpression *pexpr) {
  GPOS_ASSERT(nullptr != pcrs);
  GPOS_ASSERT(nullptr != pexpr);
...
CExpression *CExpressionPreprocessor::PexprAddEqualityPreds(CMemoryPool *mp, CExpression *pexpr,
  ...
  if (CUtils::FLogicalDML(pop)) {
    ...
    const uint32_t ulEquivClasses = pdrgpcrs->Size();
    for (uint32_t ul = 0; ul < ulEquivClasses; ul++) {
  ...
  return CUtils::PexprSafeSelect(mp, GPOS_NEW(mp) CExpression(mp, pop, pdrgpexprChildren), pexprPred);
}
...
CExpression *CExpressionPreprocessor::PexprScalarPredicates(CMemoryPool *mp, CPropConstraint *ppc,
                                                            CPropConstraint *constraintsForOuterRefs,
                                                            CPropConstraint *ppcFromFilterSubquery,
                                                            CColRefSet *pcrsNotNull, CColRefSet *pcrs,
                                                            CColRefSet *pcrsProcessed) {
  CExpressionArray *pdrgpexpr = GPOS_NEW(mp) CExpressionArray(mp);
  ...
  while (crsi.Advance()) {
    ...
    if (ppcFromFilterSubquery != nullptr && (crs = ppcFromFilterSubquery->PcrsEquivClass(colref)) != nullptr) {
      // add predicates that are inferred from subquery
      CConstraint *pcnstr = ppcFromFilterSubquery->Pcnstr()->Pcnstr(mp, crs);
      ...
      if (!(CUtils::FScalarNotNull(pexprScalar) &&
            (pcrsNotNull->FMember(colref) || (ppc->Pcnstr() != nullptr && ppc->Pcnstr()->FConstraint(colref))))) {
        pexprScalar->AddRef();
    ...
    CExpression *pexprScalar = ppc->PexprScalarMappedFromEquivCols(mp, colref, constraintsForOuterRefs);
    ...
    // do not add a NOT NULL predicate if column is not nullable or if it
    // already has another predicate on it
    if (CUtils::FScalarNotNull(pexprScalar) && (pcrsNotNull->FMember(colref) || ppc->Pcnstr()->FConstraint(colref))) {
      pexprScalar->Release();
      continue;
    }
    pdrgpexpr->Append(pexprScalar);
...
CExpression *CExpressionPreprocessor::PexprFromConstraintsScalar(CMemoryPool *mp, CExpression *pexpr,
                                                                 CPropConstraint *constraintsForOuterRefs) {
  GPOS_ASSERT(nullptr != pexpr);
  ...
  for (uint32_t ul = 0; ul < ulChildren; ul++) {
    ...
    if (pexprChild->Pop()->FScalar()) {
      pexprChild = PexprFromConstraintsScalar(mp, pexprChild, constraintsForOuterRefs);
    } else {
      GPOS_ASSERT(pexprChild->Pop()->FLogical());
      CColRefSet *pcrsProcessed = GPOS_NEW(mp) CColRefSet(mp);
      pexprChild = PexprFromConstraints(mp, pexprChild, pcrsProcessed, constraintsForOuterRefs);
...
CExpression *CExpressionPreprocessor::PexprWithImpliedPredsOnLOJInnerChild(
  ...
  CExpression *pexprPred =
      PexprScalarPredicates(mp, ppc, nullptr /*no outer refs*/, nullptr /*no constraints from subquery*/,
...
CExpression *CExpressionPreprocessor::PexprOuterJoinInferPredsFromOuterChildToInnerChild(
...
CExpression *CExpressionPreprocessor::PexprFromConstraints(CMemoryPool *mp, CExpression *pexpr,
                                                           CColRefSet *pcrsProcessed,
                                                           CPropConstraint *constraintsForOuterRefs) {
  GPOS_ASSERT(nullptr != pcrsProcessed);
  GPOS_ASSERT(nullptr != pexpr);
  ...
  CPropConstraint *ppcFromFilterSubquery = CExpressionUtils::GetPropConstraintFromSubquery(mp, pexpr);
  CExpressionArray *pdrgpexprChildren = GPOS_NEW(mp) CExpressionArray(mp);
  ...
  for (uint32_t ul = 0; ul < ulChildren; ul++) {
    ...
    // process child
    CExpression *pexprChildNew = PexprFromConstraints(mp, pexprChild, pcrsProcessed, constraintsForOuterRefs);
    ...
    CExpression *pexprPred = PexprScalarPredicates(mp, ppc, constraintsForOuterRefs, ppcFromFilterSubquery, pcrsNotNull,
  ...
  if (ppcFromFilterSubquery != nullptr) {
    CExpression *pexprNormalized = CNormalizer::PexprNormalize(mp, pexprPred);
    pexprPred->Release();
...
CExpression *CExpressionPreprocessor::PexprPruneEmptySubtrees(CMemoryPool *mp, CExpression *pexpr) {
  GPOS_ASSERT(nullptr != pexpr);
...
CExpression *CExpressionPreprocessor::PexprRemoveUnusedCTEs(CMemoryPool *mp, CExpression *pexpr) {
  GPOS_ASSERT(nullptr != pexpr);

  COperator *pop = pexpr->Pop();
  if (COperator::EopLogicalCTEAnchor == pop->Eopid()) {
    uint32_t id = CLogicalCTEAnchor::PopConvert(pop)->Id();
...
static void UpdateExprToConstantPredicateMapping(CMemoryPool *mp, CExpression *pexprFilter,
  ...
  if (CPredicateUtils::IsEqualityOp(pexprFilter)) {
    ...
    if (!CUtils::FScalarConstOrBinaryCoercible((*pexprFilter)[0]) &&
      ...
      if (doInsert) {
        bool inserted = phmExprToConst->Insert((*pexprFilter)[0], (*pexprFilter)[1]);
        if (inserted) {
    ...
    // the following section handles predicates of the kind (10 = a)
             ...
             !CUtils::FScalarConstOrBinaryCoercible((*pexprFilter)[1])) {
      if (doInsert) {
        bool inserted = phmExprToConst->Insert((*pexprFilter)[1], (*pexprFilter)[0]);
        if (inserted) {
          (*pexprFilter)[0]->AddRef();
          (*pexprFilter)[1]->AddRef();
        }
    ...
    const uint32_t ulChildren = pexprFilter->Arity();
    for (uint32_t ul = 0; ul < ulChildren; ul++) {
      UpdateExprToConstantPredicateMapping(mp, (*pexprFilter)[ul], phmExprToConst, doInsert);
    }
  }
}
...
static CExpression *SubstituteConstantIdentifier(CMemoryPool *mp, CExpression *pexpr,
  ...
  for (uint32_t ul = 0; ul < ulChildren; ul++) {
    ...
    if (COperator::EopScalarConst != (*pexpr)[ul]->Pop()->Eopid() && phmExprToConst->Find((*pexpr)[ul]) != nullptr &&
      ...
      pexprChild = SubstituteConstantIdentifier(mp, (*pexpr)[ul], phmExprToConst);
    }
    pdrgpexpr->Append(pexprChild);
  }
...
//       |--CScalarIdent "c" (0)
//       +--CScalarConst (2)
CExpression *CExpressionPreprocessor::PexprReplaceColWithConst(CMemoryPool *mp, CExpression *pexpr,
                                                               ExprToConstantMap *phmExprToConst,
                                                               bool checkFilterForConstants) {
  GPOS_ASSERT(nullptr != pexpr);

  CExpression *pexprFilter = nullptr;
  if (COperator::EopLogicalNAryJoin == pexpr->Pop()->Eopid() || COperator::EopLogicalSelect == pexpr->Pop()->Eopid()) {
  ...
  if (checkFilterForConstants && COperator::EopLogicalSelect == pexpr->Pop()->Eopid() &&
      (COperator::EopLogicalLeftOuterJoin == ((*pexpr)[0])->Pop()->Eopid() ||
       COperator::EopLogicalNAryJoin == ((*pexpr)[0])->Pop()->Eopid())) {
    UpdateExprToConstantPredicateMapping(mp, pexprFilter, phmExprToConst, true);

    CExpression *pexprNew = PexprReplaceColWithConst(mp, pexpr, phmExprToConst, false);

    // erase values from map...
    UpdateExprToConstantPredicateMapping(mp, pexprFilter, phmExprToConst, false);
  ...
  const uint32_t ulChildren = pexpr->Arity();
  for (uint32_t ul = 0; ul < ulChildren; ul++) {
    CExpression *pexprChild = PexprReplaceColWithConst(mp, (*pexpr)[ul], phmExprToConst, true);
    pdrgpexpr->Append(pexprChild);
  }
  ...
  if (COperator::EopLogicalNAryJoin == pexpr->Pop()->Eopid() && phmExprToConst->Size() > 0 &&
      !CUtils::FHasCTEAnchor(pexprFilter)) {
    CExpression *pexprFilterWithConsts = SubstituteConstantIdentifier(mp, pexprFilter, phmExprToConst);

    return GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalSelect(mp), GPOS_NEW(mp) CExpression(mp, pop, pdrgpexpr),
...
void CExpressionPreprocessor::CollectCTEPredicates(CMemoryPool *mp, CExpression *pexpr, CTEPredsMap *phm) {
  GPOS_CHECK_STACK_SIZE;
  ...
  if (COperator::EopLogicalSelect == pexpr->Pop()->Eopid() &&
    ...
    if (!pexprScalar->DeriveHasSubquery()) {
      ...
      uint32_t ulCTEId = popConsumer->UlCTEId();
      ...
      if (nullptr == pdrgpexpr) {
        ...
        bool fInserted GPOS_ASSERTS_ONLY = phm->Insert(GPOS_NEW(mp) uint32_t(ulCTEId), pdrgpexpr);
...
void CExpressionPreprocessor::AddPredsToCTEProducers(CMemoryPool *mp, CExpression *pexpr) {
  ...
  while (mi.Advance()) {
    uint32_t ulCTEId = *(mi.Key());
    CExpression *pexprProducer = pcteinfo->PexprCTEProducer(ulCTEId);
    GPOS_ASSERT(nullptr != pexprProducer);

    uint32_t ulConsumers = pcteinfo->UlConsumers(ulCTEId);
...
CExpression *CExpressionPreprocessor::PexprAddPredicatesFromConstraints(CMemoryPool *mp, CExpression *pexpr) {
  ...
  CColRefSet *pcrsProcessed = GPOS_NEW(mp) CColRefSet(mp);
  CExpression *pexprConstraints = PexprFromConstraints(mp, pexprNormalized, pcrsProcessed, nullptr);
  GPOS_CHECK_ABORT;
...
CExpression *CExpressionPreprocessor::PexprInferPredicates(CMemoryPool *mp, CExpression *pexpr) {
  GPOS_ASSERT(nullptr != pexpr);
...
CExpression *CExpressionPreprocessor::PexprPruneUnusedComputedCols(CMemoryPool *mp, CExpression *pexpr,
                                                                   CColRefSet *pcrsReqd) {
  GPOS_ASSERT(nullptr != pexpr);

  if (nullptr == pcrsReqd || GPOS_FTRACE(EopttraceDisablePruneUnusedComputedColumns)) {
    pexpr->AddRef();
    return pexpr;
  }
...
// Workhorse for pruning unused computed columns
CExpression *CExpressionPreprocessor::PexprPruneUnusedComputedColsRecursive(CMemoryPool *mp, CExpression *pexpr,
                                                                            CColRefSet *pcrsReqd) {
  GPOS_ASSERT(nullptr != pexpr);
...
CExpression *CExpressionPreprocessor::PexprPruneProjListProjectOrGbAgg(CMemoryPool *mp, CExpression *pexpr,
                                                                       CColRefSet *pcrsUnused, CColRefSet *pcrsDefined,
  ...
  if (pcrsUnused->Size() == pcrsDefined->Size()) {
    ...
    if (COperator::EopLogicalProject == pop->Eopid()) {
      ...
      if (0 < pdrgpcrGroupingCols->Size()) {
        ...
        // TODO:  10/15/2015: if there is no grouping cols, we could remove the entire GbAgg and plug in a ConstTableGet
        // instead
    ...
    const uint32_t ulPrjEls = pexprProjList->Arity();
    CExpressionHandle exprhdl(mp);

    for (uint32_t ul = 0; ul < ulPrjEls; ul++) {
    ...
    CExpression *pexprNewProjectList =
        GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pdrgpexprPrElRemain);
    pop->AddRef();
    pexprRelationalNew = PexprPruneUnusedComputedColsRecursive(mp, pexprRelational, pcrsReqdNew);
...
CExpression *CExpressionPreprocessor::PexprReorderScalarCmpChildren(CMemoryPool *mp, CExpression *pexpr) {
  GPOS_ASSERT(nullptr != pexpr);
  ...
  if (CUtils::FScalarCmp(pexpr) || COperator::EopScalarIsDistinctFrom == pexpr->Pop()->Eopid()) {
    ...
    if (CUtils::FScalarConst(pexprLeft) &&
        (CUtils::FScalarIdent(pexprRight) || CCastUtils::FScalarCastIdent(pexprRight))) {
      CScalarCmp *popScalarCmpCommuted = (dynamic_cast<CScalarCmp *>(pop))->PopCommutedOp(mp);
...
//		+--CScalarSubqueryExists
//		   +--CLogicalGet "foo" ("foo"), Columns: ["c1" (8) ...] Key sets: {[1,7]}
CExpression *CExpressionPreprocessor::ConvertInToSimpleExists(CMemoryPool *mp, CExpression *pexpr) {
  ...
  CExpression *pexprScalarSubqExists =
      GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarSubqueryExists(mp), pexprSubqOfExists);
...
CExpression *CExpressionPreprocessor::PexprExistWithPredFromINSubq(CMemoryPool *mp, CExpression *pexpr) {
  ...
  if (CUtils::FAnySubquery(pop)) {
    ...
    if (COperator::EopLogicalProject == pexprLogicalProject->Pop()->Eopid()) {
      ...
      if (!CUtils::HasOuterRefs(pexprLogicalProject) || CUtils::FInnerRefInProjectList(pexprLogicalProject)) {
        return pexprNew;
      }
      // also bail out if the project list returns set
      CExpression *pexprProjectList = (*pexprLogicalProject)[1];
      if (pexprProjectList->DeriveSetReturningFunctionColumns()->Size() > 0) {
        return pexprNew;
      }
    } else {
...
// partition.
CConstraint *CExpressionPreprocessor::PcnstrFromChildPartition(const IMDRelation *partrel, CColRefArray *pdrgpcrOutput,
  ...
  for (uint32_t ul = 0; ul < pdrgpcrOutput->Size(); ++ul) {
    CColRef *colref = (*pdrgpcrOutput)[ul];
    uint32_t *colid = root_col_mapping->Find(colref);
    GPOS_ASSERT(nullptr != colid);
    mapped_colids->Append(GPOS_NEW(mp) uint32_t(*colid));
  }

  CTranslatorDXLToExpr dxltr(mp, md_accessor);
  part_constraint_expr = dxltr.PexprTranslateScalar(dxlnode, pdrgpcrOutput, mapped_colids);
  ...
  cnstr = CConstraint::PcnstrFromScalarExpr(mp, part_constraint_expr, &pdrgpcrsChild, true /* infer_nulls_as */,
...
//             |--CScalarConst (104)
//             +--CScalarConst (1)
CExpression *CExpressionPreprocessor::PexprTransposeSelectAndProject(CMemoryPool *mp, CExpression *pexpr) {
  // protect against stack overflow during recursion
  GPOS_CHECK_STACK_SIZE;
  GPOS_ASSERT(nullptr != mp);
  GPOS_ASSERT(nullptr != pexpr);
  ...
  if (pexpr->Pop()->Eopid() == COperator::EopLogicalSelect &&
    ...
    for (uint32_t ul = 0; ul < pprojectList->Arity(); ul++) {
      CExpression *pprojexpr = CUtils::PNthProjectElementExpr(pproject, ul);

      if (pprojexpr->DeriveHasNonScalarFunction() || pprojexpr->DeriveHasSubquery()) {
    ...
    for (uint32_t ul = 0; ul < pprojectList->Arity(); ul++) {
      CExpression *pprojexpr = CUtils::PNthProjectElementExpr(pproject, ul);
      gpopt::CColRef *pcolref = CUtils::PNthProjectElement(pproject, ul)->Pcr();
      CExpression *pexprSelectPredTemp = CUtils::ReplaceColrefWithProjectExpr(mp, pexprSelectPred, pcolref, pprojexpr);
    ...
    CExpression *pexprSelectRelNew = PexprTransposeSelectAndProject(mp, (*(*pexpr)[0])[0]);

    // Transpose Select's Predicate expr so it can be used to create collpased select
    CExpression *pexprSelectPredNew = PexprTransposeSelectAndProject(mp, pexprSelectPred);
    pexprSelectPred->Release();

    // Create collapsed select from updated Select children
    CExpression *pexprCollapsedSelect =
        GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalSelect(mp), pexprSelectRelNew, pexprSelectPredNew);
    ...
    pdrgpexpr->Append(GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pdrgpprojelems));

    return GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalProject(mp), pdrgpexpr);
  } else {
    CExpressionArray *pdrgpexprChildren = GPOS_NEW(mp) CExpressionArray(mp);
    for (uint32_t ul = 0; ul < pexpr->Arity(); ul++) {
      pdrgpexprChildren->Append(PexprTransposeSelectAndProject(mp, (*pexpr)[ul]));
    }
...
// Input:
// +--CLogicalUpdate ("foo"), Split Update, Delete Columns: ["a" (0), "b" (1)], Insert Columns: ["a" (0), "b" (9)],
// "ctid" (2), "gp_segment_id" (8)
//   +--CLogicalProject
//      |--CLogicalGet
//      +--CScalarProjectList
//
// Output:
// +--CLogicalUpdate ("foo"), In-place Update, Delete Columns: ["a" (0), "b" (1)], Insert Columns: ["a" (0), "b" (9)],
// "ctid" (2), "gp_segment_id" (8)
//	 +--CLogicalProject
//		|--CLogicalGet
//		+--CScalarProjectList

CExpression *CExpressionPreprocessor::ConvertSplitUpdateToInPlaceUpdate(CMemoryPool *mp, CExpression *pexpr) {
  GPOS_ASSERT(nullptr != mp);
  GPOS_ASSERT(nullptr != pexpr);
  COperator *pop = pexpr->Pop();
  ...
  const uint32_t num_cols = pdrgpcrInsert->Size();
  ...
  for (uint32_t ul = 0; ul < num_cols; ul++) {
    ...
    if (pcrInsert != pcrDelete && (pcrDelete->IsDistCol() || ppartColRefs->Find(pcrInsert) != nullptr)) {
      split_update = true;
      break;
    }
  }
  ...
  if (!split_update) {
    ...
    CExpression *pexprNew =
        GPOS_NEW(mp) CExpression(mp,
                                 GPOS_NEW(mp) CLogicalUpdate(mp, tabdesc, pdrgpcrDelete, pdrgpcrInsert,
                                                             popUpdate->PcrCtid(), popUpdate->PcrSegmentId(), false),
                                 pexprChild);
    return pexprNew;
  }
  pexpr->AddRef();
...
CExpression *CExpressionPreprocessor::PexprPreprocess(
    CMemoryPool *mp, CExpression *pexpr,
  ...
  CExpression *pexprPredWithConstReplaced = PexprReplaceColWithConst(mp, pexprTrimmed2, phmExprToConst, true);
  GPOS_CHECK_ABORT;
  ...
  CExpression *pexprReorderedScalarCmpChildren = PexprReorderScalarCmpChildren(mp, pexprPredWithConstReplaced);
  GPOS_CHECK_ABORT;
  pexprPredWithConstReplaced->Release();

  // simplify quantified subqueries
  CExpression *pexprSubqSimplified = PexprSimplifyQuantifiedSubqueries(mp, pexprReorderedScalarCmpChildren);
  GPOS_CHECK_ABORT;
  ...
  CExpression *pexprJoinPruned =
      CLeftJoinPruningPreprocessor::PexprPreprocess(mp, pexprConvert2In, pcrsOutputAndOrderCols);
  GPOS_CHECK_ABORT;
  ...
  CExpression *pexprSelfCompEliminated =
      PexprEliminateSelfComparison(mp, pexprInferredPreds, pexprInferredPreds->DeriveNotNullColumns());
  GPOS_CHECK_ABORT;
  pexprInferredPreds->Release();
  ...
  CExpression *pexprOrderedAggPreprocessed = COrderedAggPreprocessor::PexprPreprocess(mp, pexprPrefiltersExtracted);
  GPOS_CHECK_ABORT;
  pexprPrefiltersExtracted->Release();

  // eliminate unused computed columns
  CExpression *pexprNoUnusedPrEl =
      PexprPruneUnusedComputedCols(mp, pexprOrderedAggPreprocessed, pcrsOutputAndOrderCols);
  GPOS_CHECK_ABORT;
  ...
  // rewrite IN subquery to EXIST subquery with a predicate
  CExpression *pexprExistWithPredFromINSubq = PexprExistWithPredFromINSubq(mp, pexprSubquery);
  GPOS_CHECK_ABORT;
