...
//
//---------------------------------------------------------------------------
CLogicalDynamicForeignGet::CLogicalDynamicForeignGet(CMemoryPool *mp) : CLogicalDynamicGetBase(mp) {}
...
//
//---------------------------------------------------------------------------
CLogicalDynamicForeignGet::CLogicalDynamicForeignGet(CMemoryPool *mp, const CName *pnameAlias,
                                                     CTableDescriptor *ptabdesc, ULONG ulPartIndex,
                                                     CColRefArray *pdrgpcrOutput, CColRef2dArray *pdrgpdrgpcrPart,
                                                     IMdIdArray *partition_mdids, OID foreign_server_oid,
...
//
//---------------------------------------------------------------------------
BOOL CLogicalDynamicForeignGet::Matches(COperator *pop) const {
  if (pop->Eopid() != Eopid()) {
    return false;
  }
  CLogicalDynamicForeignGet *popGet = CLogicalDynamicForeignGet::PopConvert(pop);

  return Ptabdesc() == popGet->Ptabdesc() && PdrgpcrOutput()->Equals(popGet->PdrgpcrOutput()) &&
...
//
//---------------------------------------------------------------------------
COperator *CLogicalDynamicForeignGet::PopCopyWithRemappedColumns(CMemoryPool *mp, UlongToColRefMap *colref_mapping,
                                                                 BOOL must_exist) {
  ...
  return GPOS_NEW(mp) CLogicalDynamicForeignGet(mp, pnameAlias, Ptabdesc(), m_scan_id, pdrgpcrOutput, pdrgpdrgpcrPart,
...
//
//---------------------------------------------------------------------------
CXformSet *CLogicalDynamicForeignGet::PxfsCandidates(CMemoryPool *mp) const {
  CXformSet *xform_set = GPOS_NEW(mp) CXformSet(mp);
  (void)xform_set->ExchangeSet(CXform::ExfDynamicForeignGet2DynamicForeignScan);

  return xform_set;
}

IStatistics *CLogicalDynamicForeignGet::PstatsDerive(CMemoryPool *mp, CExpressionHandle &exprhdl,
