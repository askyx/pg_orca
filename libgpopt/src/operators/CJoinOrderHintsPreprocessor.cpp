...
//---------------------------------------------------------------------------
static CExpression *GetChildMatchingAlias(CMemoryPool *mp, CExpression *pexpr, StringPtrArray *aliases,
                                          ULONG *joinChildPredIndex, gpos::set<ULONG> &usedChildrenIndexes) {
  GPOS_ASSERT(COperator::EopLogicalNAryJoin == pexpr->Pop()->Eopid());

  for (ULONG ul = 0; ul < pexpr->Arity() - 1; ul++) {
    StringPtrArray *pexpr_aliases =
        CHintUtils::GetAliasesFromTableDescriptors(mp, (*pexpr)[ul]->DeriveTableDescriptor());
    ...
    if (is_contained) {
      if (CLogicalNAryJoin::PopConvert(pexpr->Pop())->HasOuterJoinChildren()) {
        CLogicalNAryJoin *naryop = CLogicalNAryJoin::PopConvert(pexpr->Pop());

        *joinChildPredIndex = *(*naryop->GetLojChildPredIndexes())[ul];

        if (!naryop->IsInnerJoinChild(ul) &&
            CUtils::FContainsScalarIdentNullTest(naryop->GetOnPredicateForLOJChild(pexpr, ul))) {
...
//---------------------------------------------------------------------------
static bool IsChildMatchingAlias(CMemoryPool *mp, CExpression *pexpr, StringPtrArray *aliases,
                                 ULONG *joinChildPredIndex, gpos::set<ULONG> &usedChildrenIndexes) {
  return nullptr != GetChildMatchingAlias(mp, pexpr, aliases, joinChildPredIndex, usedChildrenIndexes);
}
...
//		aliases.
//---------------------------------------------------------------------------
GPOS_ASSERTS_ONLY static bool IsAliasSubsetOrDisjoint(CTableDescriptorHashSet *tabdescs, StringPtrArray *aliases) {
  int includedCount = 0;
  int excludedCount = 0;
...
//---------------------------------------------------------------------------
CExpressionArray *CJoinOrderHintsPreprocessor::ConstructNAryJoinChildren(CMemoryPool *mp, CExpression *naryJoinPexpr,
  ...
  StringPtrArray *usedNames = CHintUtils::GetAliasesFromTableDescriptors(mp, binaryJoinExpr->DeriveTableDescriptor());
  ...
  for (ULONG ul = 0; ul < naryJoinPexpr->Arity() - 1; ul++) {
    ...
    GPOS_ASSERT(IsAliasSubsetOrDisjoint((*naryJoinPexpr)[ul]->DeriveTableDescriptor(), usedNames));

    const CWStringConst *alias = (*naryJoinPexpr)[ul]->DeriveTableDescriptor()->First()->Name().Pstr();
    if (nullptr == usedNames->Find(alias)) {
      ...
      naryChildren->Append(
          CJoinOrderHintsPreprocessor::PexprPreprocess(mp, (*naryJoinPexpr)[ul], nullptr /* joinnode */));
...
//		allOnPreds.
//---------------------------------------------------------------------------
static CExpression *GetOnPreds(CMemoryPool *mp, CExpression *outer, CExpression *inner, CExpressionArray *allOnPreds) {
  ...
  for (ULONG ul = 0; ul < allOnPreds->Size(); ul++) {
    ...
    if (innerAndOuter->ContainsAll(predCols) && predCols->FIntersects(outer->DeriveOutputColumns()) &&
...
//---------------------------------------------------------------------------
CExpression *CJoinOrderHintsPreprocessor::RecursiveApplyJoinOrderHintsOnNAryJoin(CMemoryPool *mp, CExpression *pexpr,
                                                                                 const CJoinHint::JoinNode *joinnode,
                                                                                 gpos::set<ULONG> &usedChildrenIndexes,
                                                                                 gpos::set<ULONG> &usedPredicateIndexes,
                                                                                 ULONG *joinChildPredIndex) {
  GPOS_ASSERT(COperator::EopLogicalNAryJoin == pexpr->Pop()->Eopid());
  ...
  if (joinnode->GetName()) {
    ...
    aliases->Append(GPOS_NEW(mp) CWStringConst(mp, joinnode->GetName()->GetBuffer()));
    pexprAppliedHints = GetChildMatchingAlias(mp, pexpr, aliases, joinChildPredIndex, usedChildrenIndexes);
  ...
  if (IsChildMatchingAlias(mp, pexpr, hint_aliases, joinChildPredIndex, usedChildrenIndexes)) {
    // Base case 2:
    //
    ...
    pexprAppliedHints = CJoinOrderHintsPreprocessor::PexprPreprocess(
        mp, GetChildMatchingAlias(mp, pexpr, hint_aliases, joinChildPredIndex, usedChildrenIndexes), joinnode);
    hint_aliases->Release();
    return pexprAppliedHints;
  }
  ...
  CExpression *outer = RecursiveApplyJoinOrderHintsOnNAryJoin(mp, pexpr, joinnode->GetOuter(), usedChildrenIndexes,
                                                              usedPredicateIndexes, &outer_join_child_pred_index);
  CExpression *inner = RecursiveApplyJoinOrderHintsOnNAryJoin(mp, pexpr, joinnode->GetInner(), usedChildrenIndexes,
  ...
  if (outer == nullptr || inner == nullptr ||
      ((outer_join_child_pred_index == MAX_INDEX || inner_join_child_pred_index == MAX_INDEX) &&
  ...
  CExpressionArray *all_on_preds =
      CPredicateUtils::PdrgpexprConjuncts(mp, (*pexpr->PdrgPexpr())[pexpr->PdrgPexpr()->Size() - 1]);
  ...
  if (CLogicalNAryJoin::PopConvert(pexpr->Pop())->HasOuterJoinChildren()) {
    ...
    if (outer_join_child_pred_index != inner_join_child_pred_index) {
      ...
      // expression tree.
      *joinChildPredIndex = std::min(outer_join_child_pred_index, inner_join_child_pred_index);

      // LEFT/RIGHT JOIN use rightmost predicate index. Mark used
      ULONG current_index = std::max(outer_join_child_pred_index, inner_join_child_pred_index);
      ...
      if (innerAndOuterCols->ContainsAll(pred->DeriveUsedColumns())) {
        ...
        pred->AddRef();
        if (outer_join_child_pred_index < inner_join_child_pred_index) {
          pexprAppliedHints = GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalLeftOuterJoin(mp), outer, inner, pred);
        } else {
          GPOS_ASSERT(outer_join_child_pred_index > inner_join_child_pred_index);

          pexprAppliedHints = GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalRightOuterJoin(mp), outer, inner, pred);
        }
      } else {
        CRefCount::SafeRelease(outer);
        CRefCount::SafeRelease(inner);
      ...
      *joinChildPredIndex = outer_join_child_pred_index;
      ...
      pred->AddRef();
      pexprAppliedHints = GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalInnerJoin(mp), outer, inner, pred);
    }
  } else {
    pexprAppliedHints = GPOS_NEW(mp)
        CExpression(mp, GPOS_NEW(mp) CLogicalInnerJoin(mp), outer, inner, GetOnPreds(mp, outer, inner, all_on_preds));
  }
...
//---------------------------------------------------------------------------
CExpression *CJoinOrderHintsPreprocessor::PexprPreprocess(CMemoryPool *mp, CExpression *pexpr,
                                                          const CJoinHint::JoinNode *joinnode) {
  ...
  if (COperator::EopLogicalNAryJoin == pop->Eopid() && nullptr != joinnode) {
    ...
    CExpression *pexprAppliedHints = RecursiveApplyJoinOrderHintsOnNAryJoin(mp, pexpr, joinnode, usedChildrenIndexes,
                                                                            usedPredicateIndexes, &joinChildPredIndex);
    ...
    CExpressionArray *naryChildren = ConstructNAryJoinChildren(mp, pexpr, pexprAppliedHints, usedChildrenIndexes);
    if (naryChildren->Size() == 1) {
      ...
      return pexprResult;
    } else if (CLogicalNAryJoin::PopConvert(pexpr->Pop())->HasOuterJoinChildren()) {
      ...
      for (ULONG ul = 0; ul < CLogicalNAryJoin::PopConvert(pexpr->Pop())->GetLojChildPredIndexes()->Size(); ul++) {
        ULONG predicateIndex = *(*CLogicalNAryJoin::PopConvert(pexpr->Pop())->GetLojChildPredIndexes())[ul];
        if (usedPredicateIndexes.count(predicateIndex) == 0) {
          ...
          lojChildPredIndexes->Append(GPOS_NEW(mp) ULONG(adjustedPredicateIndex));
        }
      }
      naryChildren->Append(GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarNAryJoinPredList(mp), naryJoinPredicates));

      return GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalNAryJoin(mp, lojChildPredIndexes), naryChildren);
    } else {
      (*pexpr->PdrgPexpr())[pexpr->PdrgPexpr()->Size() - 1]->AddRef();
      naryChildren->Append((*pexpr->PdrgPexpr())[pexpr->PdrgPexpr()->Size() - 1]);

      return GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalNAryJoin(mp), naryChildren);
    }
  }
  ...
  for (ULONG ul = 0; ul < pexpr->Arity(); ul++) {
    pdrgpexpr->Append(CJoinOrderHintsPreprocessor::PexprPreprocess(mp, (*pdrgexprChildren)[ul], joinnode));
  }
