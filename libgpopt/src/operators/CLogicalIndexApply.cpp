...
CLogicalIndexApply::CLogicalIndexApply(CMemoryPool *mp)
    : CLogicalApply(mp), m_pdrgpcrOuterRefs(nullptr), m_fOuterJoin(false), m_origJoinPred(nullptr) {
  m_fPattern = true;
}
...
CMaxCard CLogicalIndexApply::DeriveMaxCard(CMemoryPool *,  // mp
                                           CExpressionHandle &exprhdl) const {
  return CLogical::Maxcard(exprhdl, 2 /*ulScalarIndex*/, MaxcardDef(exprhdl));
}

CXformSet *CLogicalIndexApply::PxfsCandidates(CMemoryPool *mp) const {
...
IStatistics *CLogicalIndexApply::PstatsDerive(CMemoryPool *mp, CExpressionHandle &exprhdl,
  ...
  IStatistics *stats =
      CJoinStatsProcessor::CalcAllJoinStats(mp, statistics_array, pexprScalar, const_cast<CLogicalIndexApply *>(this));
...
COperator *CLogicalIndexApply::PopCopyWithRemappedColumns(CMemoryPool *mp, UlongToColRefMap *colref_mapping,
  ...
  if (nullptr != m_origJoinPred) {
    remapped_orig_join_pred = m_origJoinPred->PexprCopyWithRemappedColumns(mp, colref_mapping, must_exist);
  }

  result = GPOS_NEW(mp) CLogicalIndexApply(mp, colref_array, m_fOuterJoin, remapped_orig_join_pred);
