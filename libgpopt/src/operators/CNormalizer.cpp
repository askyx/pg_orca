...
//
//---------------------------------------------------------------------------
bool CNormalizer::FPushThruOuterChild(CExpression *pexprLogical) {
  GPOS_ASSERT(nullptr != pexprLogical);

  COperator::EOperatorId op_id = pexprLogical->Pop()->Eopid();

  return COperator::EopLogicalLeftOuterJoin == op_id || COperator::EopLogicalLeftOuterApply == op_id ||
         COperator::EopLogicalLeftOuterCorrelatedApply == op_id || CUtils::FLeftAntiSemiApply(pexprLogical->Pop()) ||
...
//
//---------------------------------------------------------------------------
bool CNormalizer::FPushableThruSeqPrjChild(CExpression *pexprSeqPrj, CExpression *pexprPred) {
...
//
//---------------------------------------------------------------------------
bool CNormalizer::FPushable(CExpression *pexprLogical, CExpression *pexprPred) {
  GPOS_ASSERT(nullptr != pexprLogical);
  GPOS_ASSERT(nullptr != pexprPred);
...
//
//---------------------------------------------------------------------------
CExpression *CNormalizer::PexprRecursiveNormalize(CMemoryPool *mp, CExpression *pexpr) {
...
//
//---------------------------------------------------------------------------
void CNormalizer::SplitConjunct(CMemoryPool *mp, CExpression *pexpr, CExpression *pexprConj,
                                CExpressionArray **ppdrgpexprPushable, CExpressionArray **ppdrgpexprUnpushable) {
...
//
//---------------------------------------------------------------------------
void CNormalizer::PushThruOuterChild(CMemoryPool *mp, CExpression *pexpr, CExpression *pexprConj,
...
//
//---------------------------------------------------------------------------
bool CNormalizer::FSimplifySelectOnOuterJoin(CMemoryPool *mp, CExpression *pexprOuterJoin,
  ...
  if (!GPOS_FTRACE(EopttraceDisableOuterJoin2InnerJoinRewrite) &&
    ...
    *ppexprResult = GPOS_NEW(mp)
        CExpression(mp, GPOS_NEW(mp) CLogicalInnerJoin(mp), pexprOuterJoinOuterChild, pexprOuterJoinInnerChild,
...
bool CNormalizer::FSimplifySelectOnFullJoin(CMemoryPool *mp, CExpression *pexprFullJoin,
                                            CExpression *pexprPred,  // selection predicate
                                            CExpression **ppexprResult) {
  GPOS_ASSERT(nullptr != mp);
  ...
  if (CPredicateUtils::FNullRejecting(mp, pexprPred, pcrsOutputLeftChild)) {
    ...
    *ppexprResult = GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalSelect(mp),
...
//
//---------------------------------------------------------------------------
void CNormalizer::PushThruSelect(CMemoryPool *mp, CExpression *pexprSelect, CExpression *pexprConj,
                                 CExpression **ppexprResult) {
  GPOS_ASSERT(nullptr != pexprConj);
  GPOS_ASSERT(nullptr != ppexprResult);
...
//
//---------------------------------------------------------------------------
CExpression *CNormalizer::PexprSelect(CMemoryPool *mp, CExpression *pexpr, CExpressionArray *pdrgpexpr) {
  GPOS_ASSERT(nullptr != pexpr);
...
//
//---------------------------------------------------------------------------
void CNormalizer::PushThruUnaryWithoutScalarChild(CMemoryPool *mp, CExpression *pexprLogical, CExpression *pexprConj,
                                                  CExpression **ppexprResult) {
  GPOS_ASSERT(nullptr != pexprLogical);
...
//
//---------------------------------------------------------------------------
void CNormalizer::PushThruUnaryWithScalarChild(CMemoryPool *mp, CExpression *pexprLogical, CExpression *pexprConj,
                                               CExpression **ppexprResult) {
  GPOS_ASSERT(nullptr != pexprLogical);
...
//
//---------------------------------------------------------------------------
void CNormalizer::SplitConjunctForSeqPrj(CMemoryPool *mp, CExpression *pexprSeqPrj, CExpression *pexprConj,
...
//
//---------------------------------------------------------------------------
void CNormalizer::PushThruSeqPrj(CMemoryPool *mp, CExpression *pexprSeqPrj, CExpression *pexprConj,
                                 CExpression **ppexprResult) {
  GPOS_ASSERT(nullptr != pexprSeqPrj);
  GPOS_ASSERT(CLogical::EopLogicalSequenceProject == pexprSeqPrj->Pop()->Eopid());
  GPOS_ASSERT(nullptr != pexprConj);
...
//
//---------------------------------------------------------------------------
void CNormalizer::PushThruSetOp(CMemoryPool *mp, CExpression *pexprSetOp, CExpression *pexprConj,
                                CExpression **ppexprResult) {
...
//
//---------------------------------------------------------------------------
void CNormalizer::PushThruJoin(CMemoryPool *mp, CExpression *pexprJoin, CExpression *pexprConj,
                               CExpression **ppexprResult) {
  GPOS_ASSERT(nullptr != pexprConj);
  GPOS_ASSERT(nullptr != ppexprResult);
  ...
  const uint32_t arity = pexprJoin->Arity();
  bool fLASApply = CUtils::FLeftAntiSemiApply(pop);
  COperator::EOperatorId op_id = pop->Eopid();
  bool fOuterJoin = COperator::EopLogicalLeftOuterJoin == op_id || COperator::EopLogicalLeftOuterApply == op_id ||
  ...
  if (popNAryJoin && !fMixedInnerOuterJoin && CUtils::FScalarConstFalse(pexprConj)) {
    COperator *popCTG = GPOS_NEW(mp)
        CLogicalConstTableGet(mp, pexprJoin->DeriveOutputColumns()->Pdrgpcr(mp), GPOS_NEW(mp) IDatum2dArray(mp));
  ...
  if (fMixedInnerOuterJoin) {
    ...
    CExpression *pexprScalar = (*pexprJoin)[arity - 1];
    uint32_t scalar_arity = pexprScalar->Arity();

    for (uint32_t count = 1; count < scalar_arity; count++) {
...
//
//---------------------------------------------------------------------------
bool CNormalizer::FChild(CExpression *pexpr, CExpression *pexprChild) {
  GPOS_ASSERT(nullptr != pexpr);
  GPOS_ASSERT(nullptr != pexprChild);

  bool fFound = false;
  const uint32_t arity = pexpr->Arity();
  for (uint32_t ul = 0; !fFound && ul < arity; ul++) {
...
//
//---------------------------------------------------------------------------
void CNormalizer::PushThru(CMemoryPool *mp, CExpression *pexprLogical, CExpression *pexprConj,
                           CExpression **ppexprResult) {
  ...
  switch (pexprLogical->Pop()->Eopid()) {
    ...
    case COperator::EopLogicalGbAgg:
      PushThruUnaryWithScalarChild(mp, pexprLogical, pexprConj, ppexprResult);
      break;
    ...
    case COperator::EopLogicalCTEAnchor:
      PushThruUnaryWithoutScalarChild(mp, pexprLogical, pexprConj, ppexprResult);
      break;
    ...
    default: {
      if (COperator::EopLogicalLeftOuterCorrelatedApply == pexprLogical->Pop()->Eopid() &&
          CLogicalLeftOuterCorrelatedApply::PopConvert(pexprLogical->Pop())->IsPredicatePushDownAllowed()) {
        PushThruJoin(mp, pexprLogical, pexprConj, ppexprResult);
        break;
      }
...
//
//---------------------------------------------------------------------------
void CNormalizer::PushThru(CMemoryPool *mp, CExpression *pexprLogical, CExpressionArray *pdrgpexprConjuncts,
                           CExpression **ppexprResult, CExpressionArray **ppdrgpexprRemaining) {
  GPOS_ASSERT(nullptr != pexprLogical);
...
//
//---------------------------------------------------------------------------
CExpression *CNormalizer::PexprNormalize(CMemoryPool *mp, CExpression *pexpr) {
  GPOS_CHECK_STACK_SIZE;
  GPOS_ASSERT(nullptr != pexpr);
  ...
  for (uint32_t ul = 0; ul < arity; ul++) {
    ...
    if (pop->FLogical() && CLogical::PopConvert(pop)->FCanPullProjectionsUp(ul) &&
      ...
      CExpression *pexprNewChild =
          PexprPullUpProjectElements(mp, pexprChild, pcrsUsed, pcrsOutput, pdrgpexprPrElPullUp);
...
//
//---------------------------------------------------------------------------
CExpression *CNormalizer::PexprPullUpProjectElements(
    CMemoryPool *mp, CExpression *pexpr, CColRefSet *pcrsUsed, CColRefSet *pcrsOutput,
    CExpressionArray *pdrgpexprPrElPullUp  // output: the pulled-up project elements
) {
  GPOS_ASSERT(nullptr != pexpr);
  ...
  const uint32_t ulProjElements = pexprPrL->Arity();
  for (uint32_t ul = 0; ul < ulProjElements; ul++) {
    ...
    if (!pcrsUsed->FMember(pcrDefined) && pcrsOutput->ContainsAll(pcrsUsedByProjElem) &&
  ...
  if (0 == pdrgpexprPrElNoPullUp->Size()) {
    ...
    CExpression *pexprPrjList =
        GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pdrgpexprPrElNoPullUp);
...
//
//---------------------------------------------------------------------------
CExpression *CNormalizer::PexprPullUpProjections(CMemoryPool *mp, CExpression *pexpr) {
  GPOS_ASSERT(nullptr != pexpr);
...
//
//---------------------------------------------------------------------------
bool CNormalizer::FLocalColsSubsetOfInputCols(CMemoryPool *mp, CExpression *pexpr) {
  GPOS_ASSERT(nullptr != pexpr);
  GPOS_CHECK_STACK_SIZE;
  ...
  const uint32_t ulExprArity = pexpr->Arity();
  for (uint32_t ulChildIdx = 0; ulChildIdx < ulExprArity && fValid; ulChildIdx++) {
