...
BOOL CScalarSortGroupClause::Matches(COperator *other) const {
  ...
  return Index() == op->Index() && EqOp() == op->EqOp() && SortOp() == op->SortOp() &&
         NullsFirst() == op->NullsFirst() && IsHashable() == op->IsHashable();
}
...
//
//---------------------------------------------------------------------------
IOstream &CScalarSortGroupClause::OsPrint(IOstream &os) const {
  os << SzId();
...
//
//---------------------------------------------------------------------------
CScalar::EBoolEvalResult CScalarSortGroupClause::Eber(ULongPtrArray *  // pdrgpulChildren
) const {
  return EberAny;
}
