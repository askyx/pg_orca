...
CLogicalDynamicGet::CLogicalDynamicGet(CMemoryPool *mp, const CName *pnameAlias, CTableDescriptor *ptabdesc,
                                       ...
                                       IMdIdArray *partition_mdids, CConstraint *partition_cnstrs_disj,
                                       BOOL static_pruned, IMdIdArray *foreign_server_mdids, BOOL hasSecurityQuals)
    : CLogicalDynamicGetBase(mp, pnameAlias, ptabdesc, ulPartIndex, pdrgpcrOutput, pdrgpdrgpcrPart, partition_mdids),
...
//---------------------------------------------------------------------------
CLogicalDynamicGet::CLogicalDynamicGet(CMemoryPool *mp, const CName *pnameAlias, CTableDescriptor *ptabdesc,
                                       ULONG ulPartIndex, IMdIdArray *partition_mdids, IMdIdArray *foreign_server_mdids,
                                       BOOL hasSecurityQuals)
    : CLogicalDynamicGetBase(mp, pnameAlias, ptabdesc, ulPartIndex, partition_mdids),
      m_foreign_server_mdids(foreign_server_mdids),
      m_has_security_quals(hasSecurityQuals) {
  GPOS_ASSERT(nullptr != foreign_server_mdids);
}
...
//
//---------------------------------------------------------------------------
BOOL CLogicalDynamicGet::Matches(COperator *pop) const {
  ...
  return CUtils::FMatchDynamicScan(this, pop) && this->HasSecurityQuals() == popGet->HasSecurityQuals();
}
...
//
//---------------------------------------------------------------------------
COperator *CLogicalDynamicGet::PopCopyWithRemappedColumns(CMemoryPool *mp, UlongToColRefMap *colref_mapping,
                                                          BOOL must_exist) {
  ...
  CColRef2dArray *pdrgpdrgpcrPart = PdrgpdrgpcrCreatePartCols(mp, pdrgpcrOutput, Ptabdesc()->PdrgpulPart());
  ...
  if (m_partition_cnstrs_disj) {
    partition_cnstrs_disj = m_partition_cnstrs_disj->PcnstrCopyWithRemappedColumns(mp, colref_mapping, must_exist);
  }
  ...
  return GPOS_NEW(mp)
      CLogicalDynamicGet(mp, pnameAlias, Ptabdesc(), m_scan_id, pdrgpcrOutput, pdrgpdrgpcrPart, m_partition_mdids,
                         partition_cnstrs_disj, m_static_pruned, m_foreign_server_mdids, m_has_security_quals);
}
...
//
//---------------------------------------------------------------------------
BOOL CLogicalDynamicGet::FInputOrderSensitive() const {
  GPOS_ASSERT(!"Unexpected function call of FInputOrderSensitive");
  return false;
}

CMaxCard CLogicalDynamicGet::DeriveMaxCard(CMemoryPool *mp, CExpressionHandle &exprhdl) const {
...
//
//---------------------------------------------------------------------------
CXformSet *CLogicalDynamicGet::PxfsCandidates(CMemoryPool *mp) const {
  CXformSet *xform_set = GPOS_NEW(mp) CXformSet(mp);
  (void)xform_set->ExchangeSet(CXform::ExfDynamicGet2DynamicTableScan);
  (void)xform_set->ExchangeSet(CXform::ExfExpandDynamicGetWithForeignPartitions);
  return xform_set;
}
...
//
//---------------------------------------------------------------------------
IOstream &CLogicalDynamicGet::OsPrint(IOstream &os) const {
  if (m_fPattern) {
...
//
//---------------------------------------------------------------------------
IStatistics *CLogicalDynamicGet::PstatsDerive(CMemoryPool *mp, CExpressionHandle &exprhdl,
...
//
//---------------------------------------------------------------------------
IStatistics *CLogicalDynamicGet::PstatsDeriveFilter(CMemoryPool *mp, CExpressionHandle &exprhdl,
  ...
  CStatistics *pstatsFullTable = dynamic_cast<CStatistics *>(PstatsBaseTable(mp, exprhdl, Ptabdesc(), pcrsStat));
  ...
  if (dyn_get->FStaticPruned()) {
    ...
    for (ULONG ul = 0; ul < partition_mdids->Size(); ++ul) {
      ...
      CDouble part_rows = md_accessor->RetrieveRel(partition_mdid)->Rows();
      ...
      if (-1 == part_rows) {
        ...
        CMDIdRelStats *rel_stats_mdid = GPOS_NEW(mp) CMDIdRelStats(CMDIdGPDB::CastMdid(partition_mdid));
        part_rows = md_accessor->Pmdrelstats(rel_stats_mdid)->Rows();
        rel_stats_mdid->Release();
      }
      unpruned_partitions_rows += part_rows.Get();
    }
    selected_partitions_rows = std::max(selected_partitions_rows.Get(), unpruned_partitions_rows);
  }

  CStatsPred *pred_stats = CStatsPredUtils::ExtractPredStats(mp, pexprFilterNew, nullptr /*outer_refs*/
  );
  pexprFilterNew->Release();

  IStatistics *result_stats = CFilterStatsProcessor::MakeStatsFilter(mp, pstatsFullTable, pred_stats,
                                                                     true /* do_cap_NDVs */, selected_partitions_rows);
  pred_stats->Release();
...
BOOL CLogicalDynamicGet::ContainsForeignParts() const {
