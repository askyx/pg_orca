...
//
//---------------------------------------------------------------------------
CDistributionSpec *CPhysicalLeftAntiSemiHashJoinNotIn::PdsRequired(
  ...
  GPOS_RAISE(CException::ExmaInvalid, CException::ExmiInvalid,
             GPOS_WSZ_LIT("PdsRequired should not be called for CPhysicalLeftAntiSemiHashJoinNotIn"));
  return nullptr;
}

CEnfdDistribution *CPhysicalLeftAntiSemiHashJoinNotIn::Ped(CMemoryPool *mp, CExpressionHandle &exprhdl,
  ...
  if (0 == ulOptReq && 1 == child_index &&
    ...
    enfd_dist = GPOS_NEW(mp) CEnfdDistribution(
        GPOS_NEW(mp) CDistributionSpecReplicated(CDistributionSpec::EdtReplicated), CEnfdDistribution::EdmSatisfy);
  } else {
    enfd_dist = CPhysicalHashJoin::Ped(mp, exprhdl, prppInput, child_index, pdrgpdpCtxt, ulOptReq);
  }
  ...
  if (enfd_dist->PdsRequired()->Edt() == CDistributionSpec::EdtReplicated) {
    CDistributionSpecReplicated *pds_rep = GPOS_NEW(mp)
        CDistributionSpecReplicated(CDistributionSpec::EdtReplicated, true /* ignore_broadcast_threshold */);
