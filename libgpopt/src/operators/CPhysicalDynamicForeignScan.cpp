...
//
//---------------------------------------------------------------------------
CPhysicalDynamicForeignScan::CPhysicalDynamicForeignScan(CMemoryPool *mp, const CName *pnameAlias,
  ...
  if (m_exec_location == IMDRelation::EreldistrCoordinatorOnly) {
    m_pds = GPOS_NEW(mp) CDistributionSpecStrictSingleton(CDistributionSpecSingleton::EstCoordinator);
  }
...
//
//---------------------------------------------------------------------------
BOOL CPhysicalDynamicForeignScan::Matches(COperator *pop) const {
  if (Eopid() != pop->Eopid()) {
    return false;
  }

  CPhysicalDynamicForeignScan *popForeignScan = CPhysicalDynamicForeignScan::PopConvert(pop);
  return CUtils::FMatchDynamicScan(this, pop) && m_foreign_server_oid == popForeignScan->GetForeignServerOid();
}
...
//
//---------------------------------------------------------------------------
CEnfdProp::EPropEnforcingType CPhysicalDynamicForeignScan::EpetRewindability(CExpressionHandle &exprhdl,
...
//
//---------------------------------------------------------------------------
IStatistics *CPhysicalDynamicForeignScan::PstatsDerive(CMemoryPool *mp, CExpressionHandle &exprhdl,
...
CPartitionPropagationSpec *CPhysicalDynamicForeignScan::PppsDerive(CMemoryPool *mp, CExpressionHandle &) const {
