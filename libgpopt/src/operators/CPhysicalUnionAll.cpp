...
static void AssertValidChildDistributions(
    ...
    const CDistributionSpec::EDistributionType *pedt,  // array of distribution types to check
...
static void CheckChildDistributions(CMemoryPool *mp, CExpressionHandle &exprhdl, BOOL fSingletonChild,
                                    BOOL fReplicatedChild, BOOL fUniversalOuterChild);

#endif  // GPOS_DEBUG

// helper to do value equality check of arrays of ULONG pointers
BOOL Equals(ULongPtrArray *pdrgpulFst, ULongPtrArray *pdrgpulSnd) {
...
void CPhysicalUnionAll::PopulateDistrSpecs(CMemoryPool *mp, CColRefArray *pdrgpcrOutput,
  ...
  for (ULONG ulChild = 0; ulChild < arity; ulChild++) {
    ...
    CDistributionSpec *pdshashed =
        CDistributionSpecHashed::MakeHashedDistrSpec(mp, pdrgpexpr, fNullsColocated, nullptr, nullptr);
...
//
//---------------------------------------------------------------------------
CDistributionSpec *CPhysicalUnionAll::PdsDerive(CMemoryPool *mp, CExpressionHandle &exprhdl) const {
...
CDistributionSpecRandom *CPhysicalUnionAll::PdsStrictRandomParallelUnionAllChildren(CMemoryPool *mp,
...
//
//---------------------------------------------------------------------------
CDistributionSpecHashed *CPhysicalUnionAll::PdshashedDerive(CMemoryPool *mp, CExpressionHandle &exprhdl) const {
  ...
  for (ULONG ulChild = 0; fSuccess && ulChild < arity; ulChild++) {
    ...
    fSuccess = (CDistributionSpec::EdtHashed == edtChild || CDistributionSpec::EdtHashedNoOp == edtChild ||
  ...
  for (ULONG ulChild = 1; fSuccess && ulChild < arity; ulChild++) {
    ...
    while (pdsChildHashed && !equi_hash_spec_matches) {
      ...
      equi_hash_spec_matches = (nullptr != pdrgpulChild) && Equals(pdrgpulOuter, pdrgpulChild);
...
//		hashed distribution
//---------------------------------------------------------------------------
CDistributionSpecHashed *CPhysicalUnionAll::PdsMatching(CMemoryPool *mp, const ULongPtrArray *pdrgpulOuter) const {
...
//
//---------------------------------------------------------------------------
CDistributionSpecHashed *CPhysicalUnionAll::PdshashedPassThru(CMemoryPool *mp,
...
//
//---------------------------------------------------------------------------
CDistributionSpec *CPhysicalUnionAll::PdsDeriveFromChildren(CMemoryPool *
#ifdef GPOS_DEBUG
                                                                mp
#endif  // GPOS_DEBUG
                                                            ,
  ...
  for (ULONG ul = 0; ul < arity; ul++) {
    ...
    if (CDistributionSpec::EdtSingleton == edtChild || CDistributionSpec::EdtStrictSingleton == edtChild) {
...
void CheckChildDistributions(CMemoryPool *mp, CExpressionHandle &exprhdl, BOOL fSingletonChild, BOOL fReplicatedChild,
  ...
  if (fReplicatedChild) {
    ...
    AssertValidChildDistributions(
        mp, exprhdl, rgedt + 2 /*start from Universal in rgedt*/, 3 /*ulDistrs*/,
