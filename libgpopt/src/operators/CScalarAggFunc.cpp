...
//
//---------------------------------------------------------------------------
const CWStringConst *CScalarAggFunc::PstrAggFunc() const {
...
//
//---------------------------------------------------------------------------
IMDId *CScalarAggFunc::MDId() const {
...
bool CScalarAggFunc::Matches(COperator *pop) const {
  if (pop->Eopid() == Eopid()) {
    ...
    return ((popScAggFunc->IsDistinct() == m_is_distinct) && (popScAggFunc->Eaggfuncstage() == Eaggfuncstage()) &&
            (popScAggFunc->FSplit() == m_fSplit) && m_pmdidAggFunc->Equals(popScAggFunc->MDId()));
  }
...
//
//---------------------------------------------------------------------------
IOstream &CScalarAggFunc::OsPrint(IOstream &os) const {
  os << SzId() << " (";
