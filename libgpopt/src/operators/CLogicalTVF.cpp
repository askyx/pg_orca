...
//
//---------------------------------------------------------------------------
CLogicalTVF::CLogicalTVF(CMemoryPool *mp, IMDId *mdid_func, IMDId *mdid_return_type, CWStringConst *str,
...
//---------------------------------------------------------------------------
CLogicalTVF::CLogicalTVF(CMemoryPool *mp, IMDId *mdid_func, IMDId *mdid_return_type, CWStringConst *str,
                         CColumnDescriptorArray *pdrgpcoldesc, CColRefArray *pdrgpcrOutput)
    : CLogical(mp),
      m_func_mdid(mdid_func),
      m_return_type_mdid(mdid_return_type),
      m_pstr(str),
...
CLogicalTVF::HashValue() const {
  ULONG ulHash = gpos::CombineHashes(
      ...
      gpos::CombineHashes(
          ...
          gpos::CombineHashes(m_return_type_mdid->HashValue(), gpos::HashPtr<CColumnDescriptorArray>(m_pdrgpcoldesc))));
...
BOOL CLogicalTVF::Matches(COperator *pop) const {
  ...
  return m_func_mdid->Equals(popTVF->FuncMdId()) && m_return_type_mdid->Equals(popTVF->ReturnTypeMdId()) &&
         m_pdrgpcoldesc->Equals(popTVF->Pdrgpcoldesc()) && m_pdrgpcrOutput->Equals(popTVF->PdrgpcrOutput());
}
...
//
//---------------------------------------------------------------------------
CColRefSet *CLogicalTVF::DeriveOutputColumns(CMemoryPool *mp,
...
//
//---------------------------------------------------------------------------
CXformSet *CLogicalTVF::PxfsCandidates(CMemoryPool *mp) const {
...
//
//---------------------------------------------------------------------------
CMaxCard CLogicalTVF::DeriveMaxCard(CMemoryPool *,       // mp
                                    CExpressionHandle &  // exprhdl
) const {
  if (m_returns_set) {
...
IStatistics *CLogicalTVF::PstatsDerive(CMemoryPool *mp, CExpressionHandle &exprhdl,
...
//
//---------------------------------------------------------------------------
IOstream &CLogicalTVF::OsPrint(IOstream &os) const {
  if (m_fPattern) {
