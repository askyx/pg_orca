...
//
//---------------------------------------------------------------------------
CLogicalUnionAll::CLogicalUnionAll(CMemoryPool *mp, CColRefArray *pdrgpcrOutput, CColRef2dArray *pdrgpdrgpcrInput)
...
//
//---------------------------------------------------------------------------
CKeyCollection *CLogicalUnionAll::DeriveKeyCollection(CMemoryPool *,       // mp,
...
//
//---------------------------------------------------------------------------
CXformSet *CLogicalUnionAll::PxfsCandidates(CMemoryPool *mp) const {
...
//
//---------------------------------------------------------------------------
IStatistics *CLogicalUnionAll::PstatsDeriveUnionAll(CMemoryPool *mp, CExpressionHandle &exprhdl,
                                                    CColRefArray *pdrgpcrOutput, CColRef2dArray *pdrgpdrgpcrInput) {
  ...
  for (ULONG ul = 1; ul < arity; ul++) {
    ...
    CStatistics *stats = CUnionAllStatsProcessor::CreateStatsForUnionAll(
        mp, dynamic_cast<CStatistics *>(result_stats), dynamic_cast<CStatistics *>(child_stats),
        CColRef::Pdrgpul(mp, pdrgpcrOutput), CColRef::Pdrgpul(mp, (*pdrgpdrgpcrInput)[0]),
...
//
//---------------------------------------------------------------------------
IStatistics *CLogicalUnionAll::PstatsDerive(CMemoryPool *mp, CExpressionHandle &exprhdl,
  ...
  IStatistics *result = PstatsDeriveUnionAll(mp, exprhdl, pdrgpcrOutputStats, pdrgpdrgpcrInputStats);
...
CColRefSet *CLogicalUnionAll::PcrsStat(CMemoryPool *mp,
  ...
  if (pcrsChildInput->ContainsAll(pcrsInput)) {
    ...
    UlongToColRefMap *mapping = CUtils::PhmulcrMapping(mp, m_pdrgpcrOutput, (*m_pdrgpdrgpcrInput)[child_index]);
