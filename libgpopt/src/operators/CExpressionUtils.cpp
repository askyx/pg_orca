...
void CExpressionUtils::UnnestChild(CMemoryPool *mp,
  ...
  if (fHasNegatedChild && CPredicateUtils::FNot(pexprChild) && CPredicateUtils::FNot((*pexprChild)[0])) {
...
//
//---------------------------------------------------------------------------
CExpressionArray *CExpressionUtils::PdrgpexprUnnestChildren(CMemoryPool *mp, CExpression *pexpr) {
...
//
//---------------------------------------------------------------------------
CExpression *CExpressionUtils::PexprUnnest(CMemoryPool *mp, CExpression *pexpr) {
  // protect against stack overflow during recursion
  GPOS_CHECK_STACK_SIZE;
  GPOS_ASSERT(nullptr != mp);
  GPOS_ASSERT(nullptr != pexpr);
...
//
//---------------------------------------------------------------------------
CExpression *CExpressionUtils::PexprDedupChildren(CMemoryPool *mp, CExpression *pexpr) {
  // protect against stack overflow during recursion
  GPOS_CHECK_STACK_SIZE;
  GPOS_ASSERT(nullptr != mp);
  GPOS_ASSERT(nullptr != pexpr);
...
CPropConstraint *CExpressionUtils::GetPropConstraintFromSubquery(CMemoryPool *mp, CExpression *pexpr) {
  GPOS_ASSERT(nullptr != pexpr);
  ...
  CConstraint *pcnstr = CConstraint::PcnstrFromScalarExpr(mp, exprSubquery, &colRefSetArray);
