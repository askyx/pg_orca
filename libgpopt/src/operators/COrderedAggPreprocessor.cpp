...
//
//---------------------------------------------------------------------------
void COrderedAggPreprocessor::SplitPrjList(
    CMemoryPool *mp, CExpression *pexprInputAggPrj,
    CExpressionArray **ppdrgpexprOrderedAggsPrEl,  // output: list of project elements with Ordered Aggs
    CExpressionArray **ppdrgpexprOtherPrEl         // output: list of project elements with Other aggregate functions
) {
  ...
  CExpressionArray *pdrgpexprOrderedAggsPrEl = GPOS_NEW(mp) CExpressionArray(mp);
  ...
  for (uint32_t ul = 0; ul < arity; ul++) {
    ...
    if (CUtils::FHasOrderedAggToSplit(pexprAggFunc)) {
      CExpression *pexprSortCol = (*(*pexprAggFunc)[EAggfuncChildIndices::EaggfuncIndexArgs])[0];
      CExpression *pexprSortGroupClause = (*(*pexprAggFunc)[EAggfuncChildIndices::EaggfuncIndexOrder])[0];
      ...
      if (nullptr == pdrgpexprSortClauseArray) {
        ...
        for (uint32_t uidx = 0; uidx < pdrgpexprSortClauseArray->Size(); uidx++) {
          ...
          // Eg. SELECT percentile_cont(0.25) WITHIN GROUP(ORDER BY a), percentile_cont(0.5) WITHIN GROUP(ORDER BY a)
          // from tab; Currently, to check if colref's SORT ORDER Spec(Asc/Desc) is same, we match only the SortOp and
          // NullFirst as that is what is used for creating the OrderSpec TO-DO: (enhancement) Instead of this loop, we
          // can probably use a map (colref->SortGroupClause)
          CScalarSortGroupClause *visited_sort_clause =
              CScalarSortGroupClause::PopConvert((*pdrgpexprSortClauseArray)[uidx]->Pop());
          if (colref == (*colref_array)[uidx] && curr_sort_clause->SortOp() == visited_sort_clause->SortOp() &&
              curr_sort_clause->NullsFirst() == visited_sort_clause->NullsFirst()) {
            ...
            //       ...
            CExpression *pCurrExprProjList = (*(*pdrgpexprOrderedAggsPrEl)[uidx])[1];
            CExpression *pCurrExprAggFunc = (*(*pCurrExprProjList)[0])[0];
            const CColRef *total_count_colref = CCastUtils::PcrExtractFromScIdOrCastScId(
                (*(*pCurrExprAggFunc)[EAggfuncChildIndices::EaggfuncIndexArgs])[2]);
            const CColRef *peer_count_colref = CCastUtils::PcrExtractFromScIdOrCastScId(
                (*(*pCurrExprAggFunc)[EAggfuncChildIndices::EaggfuncIndexArgs])[3]);

            CExpression *pexprNewAggFunc =
                PexprFinalAgg(mp, pexprAggFunc, pcrPrjElem, const_cast<CColRef *>(total_count_colref),
                              const_cast<CColRef *>(peer_count_colref));
            CExpression *pexprAddPrjElem = CUtils::PexprScalarProjectElement(mp, pcrPrjElem, pexprNewAggFunc);
            // Updated ProjList:
            // +--CScalarProjectList
            //    |--CScalarProjectElement \"percentile_cont\" (12)
            //    |  +--CScalarAggFunc
            ...
            //          |  +--CScalarIdent \"ColRef_0041\" (41)
            //          ...
            CExpressionArray *pCurrProjList = pCurrExprProjList->PdrgPexpr();
      ...
      CExpression *pexprAgg = CUtils::PexprAgg(mp, md_accessor, IMDType::EaggCount, colref, false);
      CColRef *peer_count_colref =
          col_factory->PcrCreate(md_accessor->RetrieveType(CScalarAggFunc::PopConvert(pexprAgg->Pop())->MdidType()),
                                 CScalarAggFunc::PopConvert(pexprAgg->Pop())->TypeModifier());
      CExpression *pexprNewPrjElem = CUtils::PexprScalarProjectElement(mp, peer_count_colref, pexprAgg);
      CExpression *pexprCountAggPrjList =
          GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pexprNewPrjElem);
      CColRefArray *pc = GPOS_NEW(mp) CColRefArray(mp);
      pc->Append(const_cast<CColRef *>(colref));
      (*pexprInputAggPrj)[0]->AddRef();
      CExpression *pexprPeerCountAgg =
          CUtils::PexprLogicalGbAggGlobal(mp, pc, (*pexprInputAggPrj)[0], pexprCountAggPrjList);

      // STEP 1(a): CREATE CTE for the peer_count
      CExpression *pexprRemappedPCConsumer = nullptr;
      CExpression *pexprPCConsumer = nullptr;
      CreateCTE(mp, pexprPeerCountAgg, &pexprRemappedPCConsumer, &pexprPCConsumer);

      // STEP 2: Create Aggregate for calculating total_count SUM(peer_count)
      CExpression *pexprSumAgg = CUtils::PexprAgg(mp, md_accessor, IMDType::EaggSum, peer_count_colref, false);
      CColRef *total_count_colref =
          col_factory->PcrCreate(md_accessor->RetrieveType(CScalarAggFunc::PopConvert(pexprSumAgg->Pop())->MdidType()),
                                 CScalarAggFunc::PopConvert(pexprSumAgg->Pop())->TypeModifier());
      CExpression *pexprSumPrjElem = CUtils::PexprScalarProjectElement(mp, total_count_colref, pexprSumAgg);
      CExpression *pexprSumAggPrjList =
          GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pexprSumPrjElem);
      CExpression *pexprTotalCountAgg =
          CUtils::PexprLogicalGbAggGlobal(mp, GPOS_NEW(mp) CColRefArray(mp), pexprPeerCountAgg, pexprSumAggPrjList);
      ...
      CColRefArray *pdrgpcrChildOutput = pexprPCConsumer->DeriveOutputColumns()->Pdrgpcr(mp);
      CColRefArray *pdrgpcrConsumerOutput = CLogicalCTEConsumer::PopConvert(pexprRemappedPCConsumer->Pop())->Pdrgpcr();
      ...
      CExpression *pexprTotalCountAggRemapped =
          pexprTotalCountAgg->PexprCopyWithRemappedColumns(mp, colref_mapping, false /*must_exist*/);
      ...
      CExpression *pexprFinalGbAgg = GPOS_NEW(mp)
          CExpression(mp, pexprTotalCountAggRemapped->Pop(), pexprRemappedPCConsumer, (*pexprTotalCountAggRemapped)[1]);
      ...
      const CColRef *pcrSum = CScalarProjectElement::PopConvert((*(*pexprFinalGbAgg)[1])[0]->Pop())->Pcr();
      ...
      IMDId *mdid_func = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT8_CAST);
      ...
      CScalarFunc *popCastScalarFunc = GPOS_NEW(mp)
          CScalarFunc(mp, mdid_func, cast_func->GetResultTypeMdid(), -1, pstrFunc, false /* funcvariadic */);
      CExpression *pexprCastScalarIdent = GPOS_NEW(mp) CExpression(mp, popCastScalarFunc, pexprScalarIdentSum);
      CExpressionArray *colref_array1 = GPOS_NEW(mp) CExpressionArray(mp);
      colref_array1->Append(pexprCastScalarIdent);
      CExpression *pexprProjected = CUtils::PexprAddProjection(mp, pexprFinalGbAgg, colref_array1, false);
      CColRef *new_total_count_ref = CUtils::PcrFromProjElem((*(*pexprProjected)[1])[0]);
      CExpression *pexprJoin =
          CUtils::PexprLogicalJoin<CLogicalInnerJoin>(mp, pexprPCConsumer, pexprProjected, pexprJoinCondition);
      ...
      IMDId *mdid_curr_sortop = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, curr_sort_clause->SortOp());
      IMDType::ECmpType curr_cmp_type = CUtils::ParseCmpType(mdid_curr_sortop);
      ...
      IMDId *mdid_pos = colref->RetrieveType()->GetMdidForCmpType(curr_cmp_type);
      ...
      CExpression *pexprLimitCount = CUtils::PexprScalarConstInt8(mp, 1, true);

      CExpression *pexprJoinLimit =
          GPOS_NEW(mp) CExpression(mp, popLimit, pexprJoin, pexprLimitOffset, pexprLimitCount);

      // STEP 5: CREATE the final gp_percentile agg on top
      CExpression *pexprNewAggFunc =
          PexprFinalAgg(mp, pexprAggFunc, pcrPrjElem, new_total_count_ref, peer_count_colref);
      CExpression *pexprExistingPrjElem = CUtils::PexprScalarProjectElement(mp, pcrPrjElem, pexprNewAggFunc);
      CExpression *pexprNewAggPrjList =
          GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pexprExistingPrjElem);
      CColRefArray *colref_array = GPOS_NEW(mp) CColRefArray(mp);
      CExpression *pexprNewAgg = CUtils::PexprLogicalGbAggGlobal(mp, colref_array, pexprJoinLimit, pexprNewAggPrjList);
      pdrgpexprOrderedAggsPrEl->Append(pexprNewAgg);
    } else {
      pexprAggFunc->AddRef();
      CExpression *pexprNewPrjElem = CUtils::PexprScalarProjectElement(mp, pcrPrjElem, pexprAggFunc);
...
//
//---------------------------------------------------------------------------
void COrderedAggPreprocessor::SplitOrderedAggsPrj(
    CMemoryPool *mp, CExpression *pexprInputAggPrj,
    CExpression **ppexprOrderedGbAgg,  // output: GbAgg expression containing ordered aggs split to normal agg
    CExpression **ppexprRemainingAgg   // output: GbAgg expression containing all remaining agg functions
) {
  GPOS_ASSERT(nullptr != pexprInputAggPrj);
  ...
  SplitPrjList(mp, pexprInputAggPrj, &ppdrgpexprOrderedAggsPrEl, &pdrgpexprOtherPrEl);
  ...
  for (uint32_t ul = 1; ul < ppdrgpexprOrderedAggsPrEl->Size(); ul++) {
    ...
    CExpression *pexprJoin =
        CUtils::PexprLogicalJoin<CLogicalInnerJoin>(mp, pexpr, pexprWindowConsumer, pexprJoinCondition);
    pexpr = pexprJoin;
  }
  ...
  *ppexprRemainingAgg = GPOS_NEW(mp) CExpression(
      mp, GPOS_NEW(mp) CLogicalGbAgg(mp, pdrgpcrGrpCols, COperator::EgbaggtypeGlobal), pexprInputAggPrjChild,
...
//
//---------------------------------------------------------------------------
void COrderedAggPreprocessor::CreateCTE(CMemoryPool *mp, CExpression *pexprChild, CExpression **ppexprFirstConsumer,
  ...
  *ppexprFirstConsumer = GPOS_NEW(mp)
      CExpression(mp, GPOS_NEW(mp) CLogicalCTEConsumer(mp, ulCTEId, CUtils::PdrgpcrCopy(mp, pdrgpcrProducerOutput)));
...
//
//---------------------------------------------------------------------------
CExpression *COrderedAggPreprocessor::PexprFinalAgg(CMemoryPool *mp, CExpression *pexprAggFunc, CColRef *arg_col_ref,
                                                    CColRef *total_count_colref, CColRef *peer_count_colref) {
  ...
  CExpression *pexprAggFuncArg = (*(*pexprAggFunc)[EAggfuncChildIndices::EaggfuncIndexArgs])[0];
  ...
  CScalarAggFunc *popNewAggFunc = CUtils::PopAggFunc(mp, mdid, arg_col_ref->Name().Pstr(), false /*is_distinct*/,
...
//
//---------------------------------------------------------------------------
CExpression *COrderedAggPreprocessor::PexprInputAggPrj2Join(CMemoryPool *mp, CExpression *pexprInputAggPrj) {
  GPOS_ASSERT(nullptr != pexprInputAggPrj);
  ...
  if (COperator::EopLogicalGbAgg == pexprOtherAgg->Pop()->Eopid()) {
    CExpression *pexprJoinCondition = CUtils::PexprScalarConstBool(mp, true /*value*/);
    pexprFinalJoin =
        CUtils::PexprLogicalJoin<CLogicalInnerJoin>(mp, pexprOtherAgg, pexprOrderedAgg, pexprJoinCondition);
  } else {
  ...
  pexpResult = GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalCTEAnchor(mp, ulCTEIdStart), pexpResult);
  return pexpResult;
}
...
//
//---------------------------------------------------------------------------
CExpression *COrderedAggPreprocessor::PexprPreprocess(CMemoryPool *mp, CExpression *pexpr) {
  ...
  if (COperator::EopLogicalGbAgg == pop->Eopid() && 0 == CLogicalGbAgg::PopConvert(pop)->Pdrgpcr()->Size() &&
