...
BOOL CScalarWindowFunc::Matches(COperator *pop) const {
  if (pop->Eopid() == Eopid()) {
    ...
    return ((popFunc->IsDistinct() == m_is_distinct) && (popFunc->IsStarArg() == m_is_star_arg) &&
            (popFunc->IsSimpleAgg() == m_is_simple_agg) && (popFunc->FAgg() == m_fAgg) &&
            m_func_mdid->Equals(popFunc->FuncMdId()) && m_return_type_mdid->Equals(popFunc->MdidType()) &&
