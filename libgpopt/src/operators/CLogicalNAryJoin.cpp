...
//
//---------------------------------------------------------------------------
CLogicalNAryJoin::CLogicalNAryJoin(CMemoryPool *mp) : CLogicalJoin(mp), m_lojChildPred(nullptr) {
...
CMaxCard CLogicalNAryJoin::DeriveMaxCard(CMemoryPool *mp, CExpressionHandle &exprhdl) const {
  ...
  if (nullptr != pexprScalar) {
    if (COperator::EopScalarNAryJoinPredList == pexprScalar->Pop()->Eopid()) {
      ...
      if (nullptr != pexprScalarChild && CUtils::FScalarConstFalse(pexprScalarChild)) {
...
CColRefSet *CLogicalNAryJoin::DeriveNotNullColumns(CMemoryPool *mp, CExpressionHandle &exprhdl) const {
...
CPropConstraint *CLogicalNAryJoin::DerivePropertyConstraint(CMemoryPool *mp, CExpressionHandle &exprhdl) const {
...
CExpression *CLogicalNAryJoin::GetTrueInnerJoinPreds(CMemoryPool *mp, CExpressionHandle &exprhdl) const {
...
//---------------------------------------------------------------------------
CExpression *CLogicalNAryJoin::ReplaceInnerJoinPredicates(CMemoryPool *mp, CExpression *old_nary_join_scalar_expr,
...
//
//---------------------------------------------------------------------------
IOstream &CLogicalNAryJoin::OsPrint(IOstream &os) const {
  os << SzId();
