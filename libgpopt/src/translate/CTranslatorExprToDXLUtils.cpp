...
//
//---------------------------------------------------------------------------
void CTranslatorExprToDXLUtils::SetDirectDispatchInfo(CMemoryPool *mp, CMDAccessor *md_accessor, CDXLNode *dxlnode,
  ...
  if (CDistributionSpec::EdtHashed == pds->Edt() || CDistributionSpec::EdtRandom == pds->Edt()) {
    ...
    for (ULONG i = 0; i < size; i++) {
      ...
      if (nullptr != ppc->Pcnstr()) {
        ...
        if (CDistributionSpec::EdtHashed == pds->Edt()) {
          ...
          dxl_direct_dispatch_info = GetDXLDirectDispatchInfo(mp, md_accessor, pdrgpexprHashed, ppc->Pcnstr());
          ...
          dxl_direct_dispatch_info = GetDXLDirectDispatchInfoRandDist(mp, md_accessor, pcrDistrCol, pcnstrDistrCol);
        }
