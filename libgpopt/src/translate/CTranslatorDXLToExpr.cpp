...
//
//---------------------------------------------------------------------------
CTranslatorDXLToExpr::CTranslatorDXLToExpr(CMemoryPool *mp, CMDAccessor *md_accessor, bool fInitColumnFactory)
    : m_mp(mp),
...
//
//---------------------------------------------------------------------------
ULongPtrArray *CTranslatorDXLToExpr::PdrgpulOutputColRefs() {
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::Pexpr(const CDXLNode *dxlnode, const CDXLNodeArray *query_output_dxlnode_array,
  ...
  const uint32_t ulCTEs = cte_producers->Size();
  for (uint32_t ul = 0; ul < ulCTEs; ul++) {
    ...
    bool fres GPOS_ASSERTS_ONLY =
        m_phmulpdxlnCTEProducer->Insert(GPOS_NEW(m_mp) uint32_t(pdxlopCTEProducer->Id()), pdxlnCTE);
    GPOS_ASSERT(fres);
  }
  ...
  const uint32_t length = query_output_dxlnode_array->Size();
  for (uint32_t ul = 0; ul < length; ul++) {
    ...
    if (fGenerateRequiredColumns) {
      const uint32_t ulColRefId = colref->Id();
      uint32_t *pulCopy = GPOS_NEW(m_mp) uint32_t(ulColRefId);
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::Pexpr(const CDXLNode *dxlnode) {
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprTranslateQuery(const CDXLNode *dxlnode,
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprTranslateScalar(const CDXLNode *dxlnode, CColRefArray *colref_array,
                                                        ULongPtrArray *pdrgpul) {
  GPOS_ASSERT_IMP(nullptr != pdrgpul, nullptr != colref_array);
  GPOS_ASSERT_IMP(nullptr != pdrgpul, pdrgpul->Size() == colref_array->Size());
  ...
  if (nullptr != colref_array) {
    ...
    for (uint32_t ul = 0; ul < length; ul++) {
      ...
      // copy key
      uint32_t *pulKey = nullptr;
      if (nullptr == pdrgpul) {
        pulKey = GPOS_NEW(m_mp) uint32_t(ul + 1);
      } else {
        pulKey = GPOS_NEW(m_mp) uint32_t(*((*pdrgpul)[ul]) + 1);
      }
#ifdef GPOS_DEBUG
      bool fres =
#endif  // GPOS_DEBUG
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprLogical(const CDXLNode *dxlnode) {
  // recursive function - check stack
  GPOS_CHECK_STACK_SIZE;
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprLogicalTVF(const CDXLNode *dxlnode) {
  ...
  const uint32_t ulColumns = dxl_op->Arity();
  ...
  for (uint32_t ul = 0; ul < ulColumns; ul++) {
    ...
    int32_t attrnum = pdxlcoldesc->AttrNum();
    CColumnDescriptor *pcoldesc =
        GPOS_NEW(m_mp) CColumnDescriptor(m_mp, pmdtype, pdxlcoldesc->TypeModifier(), CName(m_mp, &strColName), attrnum,
                                         true,  // is_nullable
                                         pdxlcoldesc->Width());
  ...
  CLogicalTVF *popTVF = GPOS_NEW(m_mp)
      CLogicalTVF(m_mp, mdid_func, mdid_return_type,
                  GPOS_NEW(m_mp) CWStringConst(m_mp, dxl_op->MdName()->GetMDName()->GetBuffer()), pdrgpcoldesc);
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprLogicalGet(const CDXLNode *dxlnode) {
  CDXLOperator *dxl_op = dxlnode->GetOperator();
  ...
  const uint32_t ulColumns = colref_array->Size();
  // construct the mapping between the DXL ColId and CColRef
  for (uint32_t ul = 0; ul < ulColumns; ul++) {
    CColRef *colref = (*colref_array)[ul];
    ...
    // copy key
    uint32_t *pulKey = GPOS_NEW(m_mp) uint32_t(pdxlcd->Id());
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprLogicalSetOp(const CDXLNode *dxlnode) {
  GPOS_ASSERT(nullptr != dxlnode);
  ...
  CColRefArray *pdrgpcrOutput =
      CTranslatorDXLToExprUtils::Pdrgpcr(m_mp, m_phmulcr, pdrgpulOutput /*array of colids of the first child*/);
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprCastPrjElem(IMDId *pmdidSource, IMDId *mdid_dest, const CColRef *pcrToCast,
                                                    CColRef *pcrToReturn) {
  ...
  if (pmdcast->GetMDPathType() == IMDCast::EmdtArrayCoerce) {
    ...
    CExpression *pexprCastScalarFunc = CUtils::PexprFuncElemExpr(
        m_mp, m_pmda, mdid_func, parrayCoerceCast->GetSrcElemTypeMdId(), parrayCoerceCast->TypeModifier());

    pexprCast = GPOS_NEW(m_mp) CExpression(
        m_mp,
        GPOS_NEW(m_mp) CScalarArrayCoerceExpr(m_mp, mdid_dest, parrayCoerceCast->TypeModifier(),
                                              (COperator::ECoercionForm)parrayCoerceCast->GetCoercionForm(),
                                              parrayCoerceCast->Location()),
        GPOS_NEW(m_mp) CExpression(m_mp, GPOS_NEW(m_mp) CScalarIdent(m_mp, pcrToCast)), pexprCastScalarFunc);
  } else {
    pexprCast = GPOS_NEW(m_mp) CExpression(
        m_mp, GPOS_NEW(m_mp) CScalarCast(m_mp, mdid_dest, pmdcast->GetCastFuncMdId(), pmdcast->IsBinaryCoercible()),
        GPOS_NEW(m_mp) CExpression(m_mp, GPOS_NEW(m_mp) CScalarIdent(m_mp, pcrToCast)));
  }

  return GPOS_NEW(m_mp) CExpression(m_mp, GPOS_NEW(m_mp) CScalarProjectElement(m_mp, pcrToReturn), pexprCast);
}
...
//
//---------------------------------------------------------------------------
void CTranslatorDXLToExpr::BuildSetOpChild(
  ...
  *ppexprChild = PexprLogical(child_dxlnode);
  ...
  const uint32_t ulInputCols = pdrgpulInput->Size();
  CColRefSet *pcrsChildOutput = (*ppexprChild)->DeriveOutputColumns();
  for (uint32_t ulColPos = 0; ulColPos < ulInputCols; ulColPos++) {
    // column identifier of the input column
    uint32_t colid = *(*pdrgpulInput)[ulColPos];
    const CColRef *colref = LookupColRef(m_phmulcr, colid);
    ...
    int32_t type_modifier = pdxlcdOutput->TypeModifier();
    ...
    if (!pcrsChildOutput->FMember(colref)) {
      ...
      if (fEqualTypes) {
        ...
        pexprChildProjElem =
            GPOS_NEW(m_mp) CExpression(m_mp, GPOS_NEW(m_mp) CScalarProjectElement(m_mp, new_colref),
...
//
//---------------------------------------------------------------------------
CExpressionArray *CTranslatorDXLToExpr::PdrgpexprPreprocessSetOpInputs(const CDXLNode *dxlnode,
  ...
  const uint32_t ulOutputCols = dxl_op->Arity();
  ...
  for (uint32_t ulOutputColPos = 0; ulOutputColPos < ulOutputCols; ulOutputColPos++) {
    const CDXLColDescr *pdxlcdOutput = dxl_op->GetColumnDescrAt(ulOutputColPos);
    pdrgpulOutput->Append(GPOS_NEW(m_mp) uint32_t(pdxlcdOutput->Id()));
  }
...
//---------------------------------------------------------------------------
bool CTranslatorDXLToExpr::FCastingUnknownType(IMDId *pmdidSource, IMDId *mdid_dest) {
...
//---------------------------------------------------------------------------
CColRef *CTranslatorDXLToExpr::LookupColRef(UlongToColRefMap *colref_mapping, uint32_t colid) {
...
//
//---------------------------------------------------------------------------
CColRef *CTranslatorDXLToExpr::PcrCreate(const CColRef *colref, const IMDType *pmdtype, int32_t type_modifier,
                                         bool fStoreMapping, uint32_t colid) {
  // generate a new column reference
  CName name(colref->Name().Pstr());
  CColRef *new_colref = m_pcf->PcrCreate(pmdtype, type_modifier, name);

  if (fStoreMapping) {
    bool result GPOS_ASSERTS_ONLY = m_phmulcr->Insert(GPOS_NEW(m_mp) uint32_t(colid), new_colref);
...
//
//---------------------------------------------------------------------------
CColRefArray *CTranslatorDXLToExpr::Pdrgpcr(const CDXLColDescrArray *dxl_col_descr_array) {
  GPOS_ASSERT(nullptr != dxl_col_descr_array);
  CColRefArray *pdrgpcrOutput = GPOS_NEW(m_mp) CColRefArray(m_mp);
  uint32_t ulOutputCols = dxl_col_descr_array->Size();
  for (uint32_t ul = 0; ul < ulOutputCols; ul++) {
...
//
//---------------------------------------------------------------------------
void CTranslatorDXLToExpr::ConstructDXLColId2ColRefMapping(const CDXLColDescrArray *dxl_col_descr_array,
                                                           const CColRefArray *colref_array) {
  GPOS_ASSERT(nullptr != dxl_col_descr_array);
  GPOS_ASSERT(nullptr != colref_array);

  const uint32_t ulColumns = dxl_col_descr_array->Size();
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprLogicalSelect(const CDXLNode *dxlnode) {
  GPOS_ASSERT(nullptr != dxlnode);
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprLogicalProject(const CDXLNode *dxlnode) {
  GPOS_ASSERT(nullptr != dxlnode);
...
CExpression *CTranslatorDXLToExpr::PexprLogicalCTEAnchor(const CDXLNode *dxlnode) {
  ...
  if (0 < pcrsProducerOuter->Size()) {
    GPOS_RAISE(gpopt::ExmaGPOPT, gpopt::ExmiUnsupportedOp, GPOS_WSZ_LIT("CTE with outer references"));
  }
  ...
  return GPOS_NEW(m_mp) CExpression(m_mp, GPOS_NEW(m_mp) CLogicalCTEAnchor(m_mp, id), pexprChild);
}
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprLogicalCTEProducer(const CDXLNode *dxlnode) {
  GPOS_ASSERT(nullptr != dxlnode);
  CDXLLogicalCTEProducer *pdxlopCTEProducer = CDXLLogicalCTEProducer::Cast(dxlnode->GetOperator());
  uint32_t id = UlMapCTEId(pdxlopCTEProducer->Id());
  ...
  const uint32_t length = pdrgpulCols->Size();
  for (uint32_t ul = 0; ul < length; ul++) {
    uint32_t *pulColId = (*pdrgpulCols)[ul];
    ...
    if (pcrsProducer->FMember(colref)) {
      ...
      CExpression *pexprPrEl = CUtils::PexprScalarProjectElement(
          m_mp, new_colref, GPOS_NEW(m_mp) CExpression(m_mp, GPOS_NEW(m_mp) CScalarIdent(m_mp, colref)));
  ...
  if (0 < pdrgpexprPrEl->Size()) {
    ...
    CExpression *pexprPr = GPOS_NEW(m_mp)
        CExpression(m_mp, GPOS_NEW(m_mp) CLogicalProject(m_mp), pexprChild,
  ...
  return GPOS_NEW(m_mp) CExpression(m_mp, GPOS_NEW(m_mp) CLogicalCTEProducer(m_mp, id, colref_array), pexprChild);
}
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprLogicalCTEConsumer(const CDXLNode *dxlnode) {
  GPOS_ASSERT(nullptr != dxlnode);

  CDXLLogicalCTEConsumer *pdxlopCTEConsumer = CDXLLogicalCTEConsumer::Cast(dxlnode->GetOperator());
  uint32_t id = UlMapCTEId(pdxlopCTEConsumer->Id());
  ...
  const uint32_t num_cols = pdrgpcrConsumer->Size();
  GPOS_ASSERT(pdrgpulCols->Size() == num_cols);
  for (uint32_t ul = 0; ul < num_cols; ul++) {
    uint32_t *pulColId = GPOS_NEW(m_mp) uint32_t(*(*pdrgpulCols)[ul]);
...
//
//---------------------------------------------------------------------------
uint32_t CTranslatorDXLToExpr::UlMapCTEId(const uint32_t ulIdOld) {
  uint32_t *pulNewId = m_phmululCTE->Find(&ulIdOld);
  if (nullptr == pulNewId) {
    pulNewId = GPOS_NEW(m_mp) uint32_t(COptCtxt::PoctxtFromTLS()->Pcteinfo()->next_id());

    bool fInserted GPOS_ASSERTS_ONLY = m_phmululCTE->Insert(GPOS_NEW(m_mp) uint32_t(ulIdOld), pulNewId);
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprLogicalInsert(const CDXLNode *dxlnode) {
  GPOS_ASSERT(nullptr != dxlnode);
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprLogicalDelete(const CDXLNode *dxlnode) {
  GPOS_ASSERT(nullptr != dxlnode);
  ...
  if (COptCtxt::PoctxtFromTLS()->HasReplicatedTables()) {
    GPOS_RAISE(gpopt::ExmaGPOPT, gpopt::ExmiUnsupportedOp, GPOS_WSZ_LIT("Delete on replicated tables"));
  }

  uint32_t ctid_colid = pdxlopDelete->GetCtIdColId();
  uint32_t segid_colid = pdxlopDelete->GetSegmentIdColId();
  ...
  return GPOS_NEW(m_mp)
      CExpression(m_mp, GPOS_NEW(m_mp) CLogicalDelete(m_mp, ptabdesc, colref_array, pcrCtid, pcrSegmentId), pexprChild);
}
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprLogicalUpdate(const CDXLNode *dxlnode) {
  GPOS_ASSERT(nullptr != dxlnode);
  ...
  if (COptCtxt::PoctxtFromTLS()->HasReplicatedTables()) {
    GPOS_RAISE(gpopt::ExmaGPOPT, gpopt::ExmiUnsupportedOp, GPOS_WSZ_LIT("Update on replicated tables"));
  }
  ...
  if (pmdrel->IsPartitioned()) {
    ...
    for (uint32_t ul = 0; ul < partition_mdids->Size(); ++ul) {
  ...
  uint32_t ctid_colid = pdxlopUpdate->GetCtIdColId();
  uint32_t segid_colid = pdxlopUpdate->GetSegmentIdColId();
  ...
  return GPOS_NEW(m_mp) CExpression(
      m_mp, GPOS_NEW(m_mp) CLogicalUpdate(m_mp, ptabdesc, pdrgpcrDelete, pdrgpcrInsert, pcrCtid, pcrSegmentId, true),
      pexprChild);
}
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprLogicalGroupBy(const CDXLNode *dxlnode) {
  // get children
  ...
  CColRefArray *pdrgpcrGroupingCols =
      CTranslatorDXLToExprUtils::Pdrgpcr(m_mp, m_phmulcr, pdxlopGrpby->GetGroupingColidArray());
  ...
  return GPOS_NEW(m_mp)
      CExpression(m_mp,
                  GPOS_NEW(m_mp) CLogicalGbAgg(m_mp, pdrgpcrGroupingCols, COperator::EgbaggtypeGlobal /*egbaggtype*/
                                               ),
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprLogicalLimit(const CDXLNode *dxlnode) {
  GPOS_ASSERT(nullptr != dxlnode && EdxlopLogicalLimit == dxlnode->GetOperator()->GetDXLOperator());
  ...
  if (1 == pdxlnCount->Arity()) {
    ...
    if (!fConst || (fConst && !CScalarConst::PopConvert(popCount)->GetDatum()->IsNull())) {
      fHasCount = true;
    }
  } else {
  ...
  CLogicalLimit *popLimit = GPOS_NEW(m_mp) CLogicalLimit(m_mp, pos, true /*fGlobal*/, fHasCount, fNonRemovable);
...
CExpression *CTranslatorDXLToExpr::PexprLogicalSeqPr(const CDXLNode *dxlnode) {
  ...
  CDXLNode *pdxlnPrL = (*dxlnode)[0];
  GPOS_ASSERT(EdxlopScalarProjectList == pdxlnPrL->GetOperator()->GetDXLOperator());
  const uint32_t arity = pdxlnPrL->Arity();
  GPOS_ASSERT(0 < arity);
  ...
  for (uint32_t ul = 0; ul < arity; ul++) {
    ...
    GPOS_ASSERT(EdxlopScalarProjectElem == pdxlnProjElem->GetOperator()->GetDXLOperator() &&
    ...
    GPOS_ASSERT(EdxlopScalarIdent == pdxlnPrElChild->GetOperator()->GetDXLOperator() ||
                EdxlopScalarWindowRef == pdxlnPrElChild->GetOperator()->GetDXLOperator());
    ...
    if (EdxlopScalarWindowRef == pdxlnPrElChild->GetOperator()->GetDXLOperator()) {
      ...
      bool fInserted GPOS_ASSERTS_ONLY = m_phmulcr->Insert(GPOS_NEW(m_mp) uint32_t(pdxlopPrEl->Id()), colref);
      GPOS_ASSERT(fInserted);
      ...
      uint32_t ulSpecPos = pdxlopWindowRef->GetWindSpecPos();
      ...
      if (nullptr == pdrgpexpr) {
        ...
        bool fInsert GPOS_ASSERTS_ONLY = phmulpdrgpexpr->Insert(GPOS_NEW(m_mp) uint32_t(ulSpecPos), pdrgpexprNew);
        GPOS_ASSERT(fInsert);
      } else {
  ...
  while (hmiterulpdrgexpr.Advance()) {
    uint32_t ulPos = *(hmiterulpdrgexpr.Key());
...
//
//---------------------------------------------------------------------------
CColRefArray *CTranslatorDXLToExpr::PdrgpcrPartitionByCol(const ULongPtrArray *partition_by_colid_array) {
  const uint32_t size = partition_by_colid_array->Size();
  CColRefArray *colref_array = GPOS_NEW(m_mp) CColRefArray(m_mp);
  for (uint32_t ul = 0; ul < size; ul++) {
    const uint32_t *pulColId = (*partition_by_colid_array)[ul];
...
//
//---------------------------------------------------------------------------
CWindowFrame *CTranslatorDXLToExpr::Pwf(const CDXLWindowFrame *window_frame) {
  ...
  CWindowFrame::EFrameBoundary efbLead =
      Efb(CDXLScalarWindowFrameEdge::Cast(pdxlnLead->GetOperator())->ParseDXLFrameBoundary());
  CWindowFrame::EFrameBoundary efbTrail =
      Efb(CDXLScalarWindowFrameEdge::Cast(pdxlnTrail->GetOperator())->ParseDXLFrameBoundary());
  ...
  CWindowFrame *pwf = GPOS_NEW(m_mp)
      CWindowFrame(m_mp, efs, efbLead, efbTrail, pexprLead, pexprTrail, efes, window_frame->PdxlnStartInRangeFunc(),
                   window_frame->PdxlnEndInRangeFunc(), window_frame->PdxlnInRangeColl(),
                   window_frame->PdxlnInRangeAsc(), window_frame->PdxlnInRangeNullsFirst());
...
//
//---------------------------------------------------------------------------
CWindowFrame::EFrameBoundary CTranslatorDXLToExpr::Efb(EdxlFrameBoundary frame_boundary) {
  ...
  GPOS_ASSERT(arity > (uint32_t)frame_boundary && "Invalid window frame boundary");
#endif
  CWindowFrame::EFrameBoundary efb =
      (CWindowFrame::EFrameBoundary)window_frame_boundary_to_frame_boundary_mapping[(uint32_t)frame_boundary][1];
...
//
//---------------------------------------------------------------------------
CWindowFrame::EFrameExclusionStrategy CTranslatorDXLToExpr::Efes(EdxlFrameExclusionStrategy edxlfeb) {
  uint32_t window_frame_boundary_to_frame_boundary_mapping[][2] = {{EdxlfesNone, CWindowFrame::EfesNone},
...
#ifdef GPOS_DEBUG
  const uint32_t arity = GPOS_ARRAY_SIZE(window_frame_boundary_to_frame_boundary_mapping);
  GPOS_ASSERT(arity > (uint32_t)edxlfeb && "Invalid window frame exclusion strategy");
#endif
  CWindowFrame::EFrameExclusionStrategy efeb =
      (CWindowFrame::EFrameExclusionStrategy)window_frame_boundary_to_frame_boundary_mapping[(uint32_t)edxlfeb][1];
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprLogicalJoin(const CDXLNode *dxlnode) {
  GPOS_ASSERT(nullptr != dxlnode);
  ...
  const uint32_t ulChildCount = dxlnode->Arity();
  for (uint32_t ul = 0; ul < ulChildCount - 1; ++ul) {
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprRightOuterJoin(const CDXLNode *dxlnode) {
#ifdef GPOS_DEBUG
  CDXLLogicalJoin *pdxlopJoin = CDXLLogicalJoin::Cast(dxlnode->GetOperator());
  const uint32_t ulChildCount = dxlnode->Arity();
#endif  // GPOS_DEBUG
...
//
//---------------------------------------------------------------------------
CTableDescriptor *CTranslatorDXLToExpr::Ptabdesc(CDXLTableDescr *table_descr) {
  ...
  const uint32_t ulAllColumns = pmdrel->ColumnCount();
  uint32_t ulPosNonDropped = 0;
  for (uint32_t ulPos = 0; ulPos < ulAllColumns; ulPos++) {
    const IMDColumn *pmdcol = pmdrel->GetMdCol(ulPos);
    if (pmdcol->IsDropped()) {
      continue;
    }
    (void)phmiulAttnoColMapping->Insert(GPOS_NEW(m_mp) int32_t(pmdcol->AttrNum()),
                                        GPOS_NEW(m_mp) uint32_t(ulPosNonDropped));
    (void)phmiulAttnoPosMapping->Insert(GPOS_NEW(m_mp) int32_t(pmdcol->AttrNum()), GPOS_NEW(m_mp) uint32_t(ulPos));
    (void)phmululColMapping->Insert(GPOS_NEW(m_mp) uint32_t(ulPos), GPOS_NEW(m_mp) uint32_t(ulPosNonDropped));
  ...
  const uint32_t ulColumns = table_descr->Arity();
  for (uint32_t ul = 0; ul < ulColumns; ul++) {
    const CDXLColDescr *pdxlcoldesc = table_descr->GetColumnDescrAt(ul);
    int32_t attno = pdxlcoldesc->AttrNum();

    uint32_t *pulPos = phmiulAttnoPosMapping->Find(&attno);
    ...
    const uint32_t ulWidth = pdxlcoldesc->Width();
  ...
  if (pmdrel->IsPartitioned()) {
    const uint32_t ulPartCols = pmdrel->PartColumnCount();
    // compute partition columns for table descriptor
    for (uint32_t ul = 0; ul < ulPartCols; ul++) {
      const IMDColumn *pmdcol = pmdrel->PartColAt(ul);
      int32_t attrnum = pmdcol->AttrNum();
      uint32_t *pulPos = phmiulAttnoColMapping->Find(&attrnum);
      GPOS_ASSERT(nullptr != pulPos);
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarSubqueryExistential(Edxlopid edxlopid, CDXLNode *pdxlnLogicalChild) {
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprLogicalConstTableGet(const CDXLNode *pdxlnConstTable) {
  ...
  for (uint32_t ulColIdx = 0; ulColIdx < ulColumns; ulColIdx++) {
    ...
    const uint32_t ulWidth = pdxlcd->Width();
    CColumnDescriptor *pcoldesc = GPOS_NEW(m_mp) CColumnDescriptor(m_mp, pmdtype, pdxlcd->TypeModifier(), name,
                                                                   ulColIdx + 1,  // attno
                                                                   true,          // IsNullable
                                                                   ulWidth);
    pdrgpcoldesc->Append(pcoldesc);
  ...
  const uint32_t ulValues = pdxlopConstTable->GetConstTupleCount();
  for (uint32_t ul = 0; ul < ulValues; ul++) {
  ...
  CLogicalConstTableGet *popConstTableGet = GPOS_NEW(m_mp) CLogicalConstTableGet(m_mp, pdrgpcoldesc, pdrgpdrgpdatum);
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarSubqueryQuantified(Edxlopid edxlopid, IMDId *scalar_op_mdid,
                                                                 const CWStringConst *str, uint32_t colid,
  ...
  if (EdxlopScalarSubqueryAny == edxlopid) {
    popScalarSubquery = GPOS_NEW(m_mp)
        CScalarSubqueryAny(m_mp, scalar_op_mdid, GPOS_NEW(m_mp) CWStringConst(m_mp, str->GetBuffer()), colref);
  } else {
    popScalarSubquery = GPOS_NEW(m_mp)
        CScalarSubqueryAll(m_mp, scalar_op_mdid, GPOS_NEW(m_mp) CWStringConst(m_mp, str->GetBuffer()), colref);
  }
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarSubqueryQuantified(const CDXLNode *pdxlnSubquery) {
  ...
  mdid->AddRef();
  return PexprScalarSubqueryQuantified(dxl_op->GetDXLOperator(), mdid,
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalar(const CDXLNode *dxlnode) {
  GPOS_ASSERT(nullptr != dxlnode);
  GPOS_ASSERT(EdxloptypeScalar == dxlnode->GetOperator()->GetDXLOperatorType());
  CDXLOperator *dxl_op = dxlnode->GetOperator();
  uint32_t ulOpId = (uint32_t)dxl_op->GetDXLOperator();

  if (EdxlopScalarSubqueryExists == ulOpId || EdxlopScalarSubqueryNotExists == ulOpId) {
    return PexprScalarSubqueryExistential(dxlnode->GetOperator()->GetDXLOperator(), (*dxlnode)[0]);
  }
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprCollapseNot(const CDXLNode *pdxlnNotExpr) {
  ...
  if (EdxlopScalarSubqueryExists == edxlopid || EdxlopScalarSubqueryNotExists == edxlopid) {
    ...
    Edxlopid edxlopidNew =
        (EdxlopScalarSubqueryExists == edxlopid) ? EdxlopScalarSubqueryNotExists : EdxlopScalarSubqueryExists;
  ...
  if (EdxlopScalarSubqueryAny == edxlopid || EdxlopScalarSubqueryAll == edxlopid) {
    ...
    return PexprScalarSubqueryQuantified(
        edxlopidNew, pmdidInverseOp, pstrInverseOp, pdxlopSubqueryQuantified->GetColId(),
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarBoolOp(const CDXLNode *pdxlnBoolExpr) {
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarOp(const CDXLNode *pdxlnOpExpr) {
  // TODO: Aug 22 2011; In GPDB the opexpr can only have two children. However, in other
  // databases this may not be the case
  GPOS_ASSERT(nullptr != pdxlnOpExpr && (1 == pdxlnOpExpr->Arity() || (2 == pdxlnOpExpr->Arity())));
  ...
  CScalarOp *pscop = GPOS_NEW(m_mp) CScalarOp(
      m_mp, mdid, return_type_mdid, GPOS_NEW(m_mp) CWStringConst(m_mp, dxl_op->GetScalarOpNameStr()->GetBuffer()));
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarIsDistinctFrom(const CDXLNode *pdxlnDistCmp) {
  ...
  CScalarIsDistinctFrom *popScIDF = GPOS_NEW(m_mp) CScalarIsDistinctFrom(
      m_mp, mdid_op, GPOS_NEW(m_mp) CWStringConst(m_mp, (md_scalar_op->Mdname().GetMDName())->GetBuffer()));
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarNullIf(const CDXLNode *pdxlnNullIf) {
  ...
  return GPOS_NEW(m_mp)
      CExpression(m_mp, GPOS_NEW(m_mp) CScalarNullIf(m_mp, mdid_op, mdid_type), pexprLeft, pexprRight);
}
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarCmp(const CDXLNode *pdxlnCmp) {
  ...
  CScalarCmp *popScCmp = GPOS_NEW(m_mp)
      CScalarCmp(m_mp, mdid, GPOS_NEW(m_mp) CWStringConst(m_mp, pdxlopComp->GetComparisonOpName()->GetBuffer()),
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarFunc(const CDXLNode *pdxlnFunc) {
  GPOS_ASSERT(nullptr != pdxlnFunc);

  const uint32_t length = pdxlnFunc->Arity();
  ...
  if (CTranslatorDXLToExprUtils::FCastFunc(m_pmda, pdxlnFunc, pmdidInput)) {
    ...
    if (pmdcast->GetMDPathType() == IMDCast::EmdtArrayCoerce) {
      CMDArrayCoerceCastGPDB *parrayCoerceCast = (CMDArrayCoerceCastGPDB *)pmdcast;
      pop = GPOS_NEW(m_mp) CScalarArrayCoerceExpr(m_mp, mdid_return_type, parrayCoerceCast->TypeModifier(),
                                                  (COperator::ECoercionForm)parrayCoerceCast->GetCoercionForm(),
                                                  parrayCoerceCast->Location());
    } else {
      pop = GPOS_NEW(m_mp) CScalarCast(m_mp, mdid_return_type, mdid_func, pmdcast->IsBinaryCoercible());
    }
  } else {
    pop = GPOS_NEW(m_mp) CScalarFunc(m_mp, mdid_func, mdid_return_type, pdxlopFuncExpr->TypeModifier(),
...
CExpression *CTranslatorDXLToExpr::PexprWindowFunc(const CDXLNode *pdxlnWindowRef) {
  ...
  CWStringConst *str_name =
      GPOS_NEW(m_mp) CWStringConst(m_mp, CMDAccessorUtils::PstrWindowFuncName(m_pmda, mdid_func)->GetBuffer());
  ...
  CScalarWindowFunc *popWindowFunc =
      GPOS_NEW(m_mp) CScalarWindowFunc(m_mp, mdid_func, mdid_return_type, str_name, ews, pdxlopWinref->IsDistinct(),
                                       pdxlopWinref->IsStarArg(), pdxlopWinref->IsSimpleAgg());
...
//
//---------------------------------------------------------------------------
CScalarWindowFunc::EWinStage CTranslatorDXLToExpr::Ews(EdxlWinStage edxlws) {
  uint32_t window_frame_boundary_to_frame_boundary_mapping[][2] = {
...
#ifdef GPOS_DEBUG
  const uint32_t arity = GPOS_ARRAY_SIZE(window_frame_boundary_to_frame_boundary_mapping);
  GPOS_ASSERT(arity > (uint32_t)edxlws && "Invalid window stage");
#endif
  CScalarWindowFunc::EWinStage ews =
      (CScalarWindowFunc::EWinStage)window_frame_boundary_to_frame_boundary_mapping[(uint32_t)edxlws][1];
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarCoalesce(const CDXLNode *pdxlnCoalesce) {
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarMinMax(const CDXLNode *pdxlnMinMax) {
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprAggFunc(const CDXLNode *pdxlnAggref) {
  ...
  CScalarAggFunc *popScAggFunc = CUtils::PopAggFunc(
      m_mp, agg_func_mdid, GPOS_NEW(m_mp) CWStringConst(m_mp, (pmdagg->Mdname().GetMDName())->GetBuffer()),
      dxl_op->IsDistinct(), agg_func_stage, fSplit, resolved_return_type_mdid, agg_func_kind, dxl_op->GetArgTypes(),
      pmdagg->IsAggRepSafe());
  ...
  if (0 < pdxlnAggref->Arity()) {
    ...
    for (uint32_t ul = 0; ul < (*pdrgpexprArgs)[0]->Arity(); ul++) {
      CExpression *pexprAggrefChild = (*(*pdrgpexprArgs)[0])[ul];

      if (pexprAggrefChild->DeriveHasNonScalarFunction()) {
        GPOS_RAISE(gpopt::ExmaGPOPT, gpopt::ExmiUnsupportedOp,
                   GPOS_WSZ_LIT("Aggregate function with set returning attributes"));
      }
    }
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprArray(const CDXLNode *dxlnode) {
  ...
  CScalarArray *popArray =
      GPOS_NEW(m_mp) CScalarArray(m_mp, elem_type_mdid, array_type_mdid, dxl_op->IsMultiDimensional());
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprArrayRef(const CDXLNode *dxlnode) {
  ...
  CScalarArrayRef *popArrayref =
      GPOS_NEW(m_mp) CScalarArrayRef(m_mp, elem_type_mdid, dxl_op->TypeModifier(), array_type_mdid, return_type_mdid);
...
CExpression *CTranslatorDXLToExpr::PexprValuesList(const CDXLNode *dxlnode) {
  CScalarValuesList *popScalarValuesList = GPOS_NEW(m_mp) CScalarValuesList(m_mp);

  CExpressionArray *pdrgpexprChildren = PdrgpexprChildren(dxlnode);

  return GPOS_NEW(m_mp) CExpression(m_mp, popScalarValuesList, pdrgpexprChildren);
}
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprFieldSelect(const CDXLNode *dxlnode) {
  CDXLScalarFieldSelect *dxl_op = CDXLScalarFieldSelect::Cast(dxlnode->GetOperator());

  IMDId *field_type = dxl_op->GetDXLFieldType();
  field_type->AddRef();
  IMDId *field_collation = dxl_op->GetDXLFieldCollation();
  field_collation->AddRef();
  int32_t type_modifier = dxl_op->GetDXLTypeModifier();
  int16_t field_number = dxl_op->GetDXLFieldNumber();

  CScalarFieldSelect *popFieldSelect =
      GPOS_NEW(m_mp) CScalarFieldSelect(m_mp, field_type, field_collation, type_modifier, field_number);
  CExpressionArray *pdrgpexprChildren = PdrgpexprChildren(dxlnode);
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprArrayRefIndexList(const CDXLNode *dxlnode) {
  ...
  CScalarArrayRefIndexList *popIndexlist =
      GPOS_NEW(m_mp) CScalarArrayRefIndexList(m_mp, Eilt(dxl_op->GetDXLIndexListBound()));
...
//
//---------------------------------------------------------------------------
CScalarArrayRefIndexList::EIndexListType CTranslatorDXLToExpr::Eilt(
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprArrayCmp(const CDXLNode *dxlnode) {
  ...
  CScalarArrayCmp *popArrayCmp = GPOS_NEW(m_mp)
      CScalarArrayCmp(m_mp, mdid_op, GPOS_NEW(m_mp) CWStringConst(m_mp, str_opname->GetBuffer()), earrcmpt);
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarIdent(const CDXLNode *pdxlnIdent) {
  // get dxl scalar identifier
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarParam(const CDXLNode *pdxlnParam) {
  // get dxl scalar param
  CDXLScalarParam *dxl_op = CDXLScalarParam::Cast(pdxlnParam->GetOperator());

  dxl_op->GetMDIdType()->AddRef();

  CScalarParam *scalar_param =
      GPOS_NEW(m_mp) CScalarParam(m_mp, dxl_op->GetId(), dxl_op->GetMDIdType(), dxl_op->GetTypeModifier());
...
//
//---------------------------------------------------------------------------
CExpressionArray *CTranslatorDXLToExpr::PdrgpexprChildren(const CDXLNode *dxlnode) {
  CExpressionArray *pdrgpexpr = GPOS_NEW(m_mp) CExpressionArray(m_mp);
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarIf(const CDXLNode *pdxlnIfStmt) {
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarSwitch(const CDXLNode *pdxlnSwitch) {
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarSwitchCase(const CDXLNode *pdxlnSwitchCase) {
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarCaseTest(const CDXLNode *pdxlnCaseTest) {
  GPOS_ASSERT(nullptr != pdxlnCaseTest);

  CDXLScalarCaseTest *dxl_op = CDXLScalarCaseTest::Cast(pdxlnCaseTest->GetOperator());
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarNullTest(const CDXLNode *pdxlnNullTest) {
  // get dxl scalar null test
  CDXLScalarNullTest *dxl_op = CDXLScalarNullTest::Cast(pdxlnNullTest->GetOperator());
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarBooleanTest(const CDXLNode *pdxlnScBoolTest) {
  ...
  // get dxl scalar null test
  CDXLScalarBooleanTest *dxl_op = CDXLScalarBooleanTest::Cast(pdxlnScBoolTest->GetOperator());
  ...
  CScalarBooleanTest::EBoolTest ebt =
      (CScalarBooleanTest::EBoolTest)(rgulBoolTestMapping[dxl_op->GetDxlBoolTypeStr()][1]);
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarCast(const CDXLNode *pdxlnCast) {
  ...
  if (pmdcast->GetMDPathType() == IMDCast::EmdtArrayCoerce) {
    ...
    pexpr = GPOS_NEW(m_mp)
        CExpression(m_mp,
                    GPOS_NEW(m_mp) CScalarArrayCoerceExpr(m_mp, mdid_type, parrayCoerceCast->TypeModifier(),
                                                          (COperator::ECoercionForm)parrayCoerceCast->GetCoercionForm(),
                                                          parrayCoerceCast->Location()),
                    pexprChild, pexprCastScalarFunc);
  } else {
    pexpr =
        GPOS_NEW(m_mp) CExpression(m_mp, GPOS_NEW(m_mp) CScalarCast(m_mp, mdid_type, mdid_func, fRelabel), pexprChild);
  }
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarCoerceToDomain(const CDXLNode *pdxlnCoerce) {
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarCoerceViaIO(const CDXLNode *pdxlnCoerce) {
  // get dxl scalar coerce operator
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarArrayCoerceExpr(const CDXLNode *pdxlnArrayCoerceExpr) {
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarConst(const CDXLNode *pdxlnConstVal) {
  GPOS_ASSERT(nullptr != pdxlnConstVal);

  // translate the dxl scalar const value
  CDXLScalarConstValue *dxl_op = CDXLScalarConstValue::Cast(pdxlnConstVal->GetOperator());
...
CExpression *CTranslatorDXLToExpr::PexprSortGroupClause(const CDXLNode *pdxlnSortGroupClause) {
  GPOS_ASSERT(nullptr != pdxlnSortGroupClause);

  // translate the dxl scalar const value
  CDXLScalarSortGroupClause *dxl_op = CDXLScalarSortGroupClause::Cast(pdxlnSortGroupClause->GetOperator());
  CScalarSortGroupClause *sgc = GPOS_NEW(m_mp) CScalarSortGroupClause(
      m_mp, dxl_op->Index(), dxl_op->EqOp(), dxl_op->SortOp(), dxl_op->NullsFirst(), dxl_op->IsHashable());
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarSubquery(const CDXLNode *pdxlnSubquery) {
  GPOS_ASSERT(nullptr != pdxlnSubquery);
  CDXLScalarSubquery *pdxlopSubquery = CDXLScalarSubquery::Cast(pdxlnSubquery->GetOperator());
  ...
  uint32_t colid = pdxlopSubquery->GetColId();
  const CColRef *colref = LookupColRef(m_phmulcr, colid);

  CScalarSubquery *popScalarSubquery =
      GPOS_NEW(m_mp) CScalarSubquery(m_mp, colref, false /*fGeneratedByExist*/, false /*fGeneratedByQuantified*/);
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarProjElem(const CDXLNode *pdxlnPrEl) {
  GPOS_ASSERT(nullptr != pdxlnPrEl && EdxlopScalarProjectElem == pdxlnPrEl->GetOperator()->GetDXLOperator());
  ...
  CExpression *pexprProjElem =
      GPOS_NEW(m_mp) CExpression(m_mp, GPOS_NEW(m_mp) CScalarProjectElement(m_mp, colref), pexprChild);
...
//
//---------------------------------------------------------------------------
CExpression *CTranslatorDXLToExpr::PexprScalarProjList(const CDXLNode *pdxlnPrL) {
  GPOS_ASSERT(nullptr != pdxlnPrL && EdxlopScalarProjectList == pdxlnPrL->GetOperator()->GetDXLOperator());
  ...
  if (0 == pdxlnPrL->Arity()) {
    ...
    const uint32_t length = pdxlnPrL->Arity();
    for (uint32_t ul = 0; ul < length; ul++) {
      CDXLNode *pdxlnProjElem = (*pdxlnPrL)[ul];
