...
// name.
template <typename T>
static CScanHint *GetScanHint(T *pop, CPlanHint *plan_hint) {
...
BOOL CHintUtils::SatisfiesPlanHints(CLogicalGet *pop, CPlanHint *plan_hint) {
  GPOS_ASSERT(nullptr != pop);
...
BOOL CHintUtils::SatisfiesPlanHints(CLogicalIndexGet *pop, CPlanHint *plan_hint) {
  GPOS_ASSERT(nullptr != pop);
...
BOOL CHintUtils::SatisfiesJoinTypeHints(CMemoryPool *mp, CExpression *pexpr, CPlanHint *plan_hint) {
  ...
  while (hsiter.Advance()) {
    ...
    aliases->Append(GPOS_NEW(mp) CWStringConst(mp, tabdesc->Name().Pstr()->GetBuffer()));
  }
...
const WCHAR *CHintUtils::ScanHintEnumToString(CScanHint::EType type) {
  switch (type) {
...
CScanHint::EType CHintUtils::ScanHintStringToEnum(const WCHAR *type) {
  if (0 == clib::Wcsncmp(type, GPOS_WSZ_LIT("SeqScan"), gpos::clib::Wcslen(type))) {
    return CScanHint::SeqScan;
  }
  if (0 == clib::Wcsncmp(type, GPOS_WSZ_LIT("NoSeqScan"), gpos::clib::Wcslen(type))) {
    return CScanHint::NoSeqScan;
  }
  if (0 == clib::Wcsncmp(type, GPOS_WSZ_LIT("IndexScan"), gpos::clib::Wcslen(type))) {
    return CScanHint::IndexScan;
  }
  if (0 == clib::Wcsncmp(type, GPOS_WSZ_LIT("NoIndexScan"), gpos::clib::Wcslen(type))) {
    return CScanHint::NoIndexScan;
  }
  if (0 == clib::Wcsncmp(type, GPOS_WSZ_LIT("IndexOnlyScan"), gpos::clib::Wcslen(type))) {
    return CScanHint::IndexOnlyScan;
  }
  if (0 == clib::Wcsncmp(type, GPOS_WSZ_LIT("NoIndexOnlyScan"), gpos::clib::Wcslen(type))) {
    return CScanHint::NoIndexOnlyScan;
  }
  if (0 == clib::Wcsncmp(type, GPOS_WSZ_LIT("BitmapScan"), gpos::clib::Wcslen(type))) {
    return CScanHint::BitmapScan;
  }
  if (0 == clib::Wcsncmp(type, GPOS_WSZ_LIT("NoBitmapScan"), gpos::clib::Wcslen(type))) {
    return CScanHint::NoBitmapScan;
  }
  return CScanHint::Sentinal;
}

const WCHAR *CHintUtils::JoinTypeHintEnumToString(CJoinTypeHint::JoinType type) {
  switch (type) {
...
CJoinTypeHint::JoinType CHintUtils::JoinTypeHintStringToEnum(const WCHAR *type) {
  if (0 == clib::Wcsncmp(type, GPOS_WSZ_LIT("NestedLoopJoin"), gpos::clib::Wcslen(type))) {
    return CJoinTypeHint::HINT_KEYWORD_NESTLOOP;
  }
  if (0 == clib::Wcsncmp(type, GPOS_WSZ_LIT("NoNestedLoopJoin"), gpos::clib::Wcslen(type))) {
    return CJoinTypeHint::HINT_KEYWORD_NONESTLOOP;
  }
  if (0 == clib::Wcsncmp(type, GPOS_WSZ_LIT("MergeJoin"), gpos::clib::Wcslen(type))) {
    return CJoinTypeHint::HINT_KEYWORD_MERGEJOIN;
  }
  if (0 == clib::Wcsncmp(type, GPOS_WSZ_LIT("NoMergeJoin"), gpos::clib::Wcslen(type))) {
    return CJoinTypeHint::HINT_KEYWORD_NOMERGEJOIN;
  }
  if (0 == clib::Wcsncmp(type, GPOS_WSZ_LIT("HashJoin"), gpos::clib::Wcslen(type))) {
    return CJoinTypeHint::HINT_KEYWORD_HASHJOIN;
  }
  if (0 == clib::Wcsncmp(type, GPOS_WSZ_LIT("NoHashJoin"), gpos::clib::Wcslen(type))) {
...
//		descriptor hash set.
//---------------------------------------------------------------------------
StringPtrArray *CHintUtils::GetAliasesFromTableDescriptors(CMemoryPool *mp, CTableDescriptorHashSet *ptabs) {
  ...
  while (tabiter.Advance()) {
    ...
    pexpr_aliases->Append(GPOS_NEW(mp) CWStringConst(mp, tabdesc->Name().Pstr()->GetBuffer()));
  }
...
//---------------------------------------------------------------------------
StringPtrArray *CHintUtils::GetAliasesFromHint(CMemoryPool *mp, const CJoinHint::JoinNode *joinnode) {
  StringPtrArray *aliases = GPOS_NEW(mp) StringPtrArray(mp);
  ...
  while (q.size() > 0) {
    ...
    if (nullptr != pair->GetName()) {
      aliases->Append(GPOS_NEW(mp) CWStringConst(mp, pair->GetName()->GetBuffer()));
    } else {
