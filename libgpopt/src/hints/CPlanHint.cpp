...
void CPlanHint::AddHint(CScanHint *hint) {
  m_scan_hints->Append(hint);
}

void CPlanHint::AddHint(CRowHint *hint) {
  m_row_hints->Append(hint);
}

void CPlanHint::AddHint(CJoinHint *hint) {
  m_join_hints->Append(hint);
}

void CPlanHint::AddHint(CJoinTypeHint *hint) {
  m_join_type_hints->Append(hint);
}

CScanHint *CPlanHint::GetScanHint(const char *relname) {
...
CScanHint *CPlanHint::GetScanHint(const CWStringBase *name) {
...
//
//---------------------------------------------------------------------------
CRowHint *CPlanHint::GetRowHint(CTableDescriptorHashSet *ptabdescs) {
  if (ptabdescs->Size() == 0) {
    return nullptr;
  }
  ...
  while (hsiter.Advance()) {
    ...
    aliases->Append(GPOS_NEW(m_mp) CWStringConst(m_mp, ptabdesc->Name().Pstr()->GetBuffer()));
  }
...
//
//---------------------------------------------------------------------------
static StringPtrArray *GetHintSubtrees(CMemoryPool *mp, const CJoinHint::JoinNode *node, StringPtr2dArray *subtrees) {
  if (node->GetName()) {
    StringPtrArray *subtree = GPOS_NEW(mp) StringPtrArray(mp);
    subtree->Append(GPOS_NEW(mp) CWStringConst(mp, node->GetName()->GetBuffer()));
  ...
  for (ULONG ul = 0; ul < outer->Size(); ul++) {
    subtree->Append(GPOS_NEW(mp) CWStringConst(mp, (*outer)[ul]->GetBuffer()));
  }
  StringPtrArray *inner = GetHintSubtrees(mp, node->GetInner(), subtrees);
  for (ULONG ul = 0; ul < inner->Size(); ul++) {
    subtree->Append(GPOS_NEW(mp) CWStringConst(mp, (*inner)[ul]->GetBuffer()));
  }
  subtrees->Append(subtree);
  return subtree;
}
...
//
//---------------------------------------------------------------------------
static bool IsAliasHintSubtreeOrDisjoint(StringPtr2dArray *subtrees, StringPtrArray *childaliases) {
...
//
//---------------------------------------------------------------------------
CJoinHint *CPlanHint::GetJoinHint(CExpression *pexpr) {
  ...
  StringPtrArray *pexprAliases = CHintUtils::GetAliasesFromTableDescriptors(m_mp, ptabdesc);
  ...
  for (ULONG ul = 0; ul < m_join_hints->Size(); ul++) {
    ...
    StringPtrArray *hintAliases = CHintUtils::GetAliasesFromHint(m_mp, hint->GetJoinNode());
    ...
    for (ULONG i = 0; i < pexpr->Arity(); i++) {
      CTableDescriptorHashSet *childtabs = (*pexpr)[i]->DeriveTableDescriptor();

      // is a direct child and a hint
      if (childtabs->Size() == 1 && nullptr != hintAliases->Find(childtabs->First()->Name().Pstr())) {
      ...
      StringPtrArray *childaliases = CHintUtils::GetAliasesFromTableDescriptors(m_mp, childtabs);
      if (childtabs->Size() > 1 && !IsAliasHintSubtreeOrDisjoint(hint_subtrees, childaliases)) {
...
//
//---------------------------------------------------------------------------
CJoinTypeHint *CPlanHint::GetJoinTypeHint(StringPtrArray *aliases) {
  aliases->Sort(CWStringBase::Compare);
...
//
//---------------------------------------------------------------------------
bool CPlanHint::WasCreatedViaDirectedHint(CExpression *pexpr) {
...
IOstream &CPlanHint::OsPrint(IOstream &os) const {
  os << "PlanHint: [";
