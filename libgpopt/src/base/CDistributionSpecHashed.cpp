...
//
//---------------------------------------------------------------------------
CDistributionSpecHashed::CDistributionSpecHashed(CExpressionArray *pdrgpexpr, BOOL fNullsColocated,
  ...
  if (GPOS_FTRACE(EopttraceConsiderOpfamiliesForDistribution) && nullptr == opfamilies) {
    PopulateDefaultOpfamilies();
  }
  GPOS_ASSERT(m_opfamilies == nullptr || m_opfamilies->Size() == m_pdrgpexpr->Size());
}
...
void CDistributionSpecHashed::PopulateDefaultOpfamilies() {
...
//
//---------------------------------------------------------------------------
CDistributionSpec *CDistributionSpecHashed::PdsCopyWithRemappedColumns(CMemoryPool *mp,
...
CDistributionSpec *CDistributionSpecHashed::StripEquivColumns(CMemoryPool *mp) {
  m_pdrgpexpr->AddRef();
  if (nullptr != m_opfamilies) {
    m_opfamilies->AddRef();
  }
  return GPOS_NEW(mp) CDistributionSpecHashed(m_pdrgpexpr, m_fNullsColocated, m_opfamilies);
}
...
//
//---------------------------------------------------------------------------
BOOL CDistributionSpecHashed::FSatisfies(const CDistributionSpec *pds) const {
  ...
  if (EdtAny == pds->Edt() || EdtNonSingleton == pds->Edt() || EdtNonReplicated == pds->Edt()) {
...
//
//---------------------------------------------------------------------------
BOOL CDistributionSpecHashed::FMatchSubset(const CDistributionSpecHashed *pdsHashed) const {
  const ULONG ulOwnExprs = m_pdrgpexpr->Size();
  const ULONG ulOtherExprs = pdsHashed->m_pdrgpexpr->Size();

  if (ulOtherExprs < ulOwnExprs || !FNullsColocatedCompatible(pdsHashed) || !FDuplicateSensitiveCompatible(pdsHashed)) {
    return false;
  }
  ...
  for (ULONG ulOuter = 0; ulOuter < ulOwnExprs; ulOuter++) {
    ...
    for (ULONG ulInner = 0; ulInner < ulOtherExprs; ulInner++) {
      ...
      if (GPOS_FTRACE(EopttraceConsiderOpfamiliesForDistribution)) {
        ...
        if (expr_type->GetDistrOpfamilyMdid() != opfamily_own || expr_type->GetDistrOpfamilyMdid() != opfamily_other) {
...
//
//---------------------------------------------------------------------------
CDistributionSpecHashed *CDistributionSpecHashed::PdshashedExcludeColumns(CMemoryPool *mp, CColRefSet *pcrs) {
...
//
//---------------------------------------------------------------------------
void CDistributionSpecHashed::AppendEnforcers(CMemoryPool *mp,
  ...
  CExpression *pexprMotion =
      GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CPhysicalMotionHashDistribute(mp, this_copy), pexpr);
...
//
//---------------------------------------------------------------------------
CColRefSet *CDistributionSpecHashed::PcrsUsed(CMemoryPool *mp) const {
...
//
//---------------------------------------------------------------------------
BOOL CDistributionSpecHashed::FMatchHashedDistribution(const CDistributionSpecHashed *pdshashed) const {
  GPOS_ASSERT(nullptr != pdshashed);

  if (m_pdrgpexpr->Size() != pdshashed->m_pdrgpexpr->Size() || !FNullsColocatedCompatible(pdshashed) ||
...
//
//---------------------------------------------------------------------------
BOOL CDistributionSpecHashed::Matches(const CDistributionSpec *pds) const {
  ...
  if (nullptr != pdshashed->PdshashedEquiv() && pdshashed->PdshashedEquiv()->Matches(this)) {
    return true;
  }
...
BOOL CDistributionSpecHashed::Equals(const CDistributionSpec *input_spec) const {
  GPOS_CHECK_STACK_SIZE;
...
//
//---------------------------------------------------------------------------
CDistributionSpecHashed *CDistributionSpecHashed::PdshashedMaximal(CMemoryPool *mp, CColRefArray *colref_array,
...
// expression array
BOOL CDistributionSpecHashed::IsCoveredBy(const CExpressionArray *dist_cols_expr_array) const {
  BOOL covers = false;
...
void CDistributionSpecHashed::ComputeEquivHashExprs(CMemoryPool *mp, CExpressionHandle &expression_handle) {
      ...
      if (nullptr != equiv_cols) {
        ...
        while (equiv_cols_iter.Advance()) {
          ...
          CExpression *predicate_expr_with_inferred_quals =
              CUtils::PexprScalarEqCmp(mp, distribution_colref, equiv_col);
          ...
          CExpression *left_distribution_expr =
              CCastUtils::PexprWithoutBinaryCoercibleCasts((*original_predicate_expr)[0]);
          CExpression *right_distribution_expr =
              CCastUtils::PexprWithoutBinaryCoercibleCasts((*original_predicate_expr)[1]);
...
CDistributionSpecHashed *CDistributionSpecHashed::Copy(CMemoryPool *mp) {
  ...
  CDistributionSpecHashed *result = GPOS_NEW(mp) CDistributionSpecHashed(m_pdrgpexpr, m_fNullsColocated, m_opfamilies);
...
CDistributionSpecHashed *CDistributionSpecHashed::Copy(CMemoryPool *mp, BOOL fNullsColocated) {
  if (nullptr != m_pdrgpexpr) {
    m_pdrgpexpr->AddRef();
  }
  ...
  CDistributionSpecHashed *result = GPOS_NEW(mp) CDistributionSpecHashed(m_pdrgpexpr, fNullsColocated, m_opfamilies);
...
//
//---------------------------------------------------------------------------
IOstream &CDistributionSpecHashed::OsPrint(IOstream &os) const {
  return OsPrintWithPrefix(os, "");
}

IOstream &CDistributionSpecHashed::OsPrintWithPrefix(IOstream &os, const char *prefix) const {
  os << this->SzId() << ": [ ";
  const ULONG length = m_pdrgpexpr->Size();
  for (ULONG ul = 0; ul < length; ul++) {
  ...
  if (nullptr != m_pdshashedEquiv && GPOS_FTRACE(EopttracePrintEquivDistrSpecs)) {
    os << ", equiv. dist: ";
    m_pdshashedEquiv->OsPrint(os);
  }

  if (nullptr != m_equiv_hash_exprs && m_equiv_hash_exprs->Size() > 0 && GPOS_FTRACE(EopttracePrintEquivDistrSpecs)) {
    os << "," << std::endl;
    for (ULONG ul = 0; ul < m_equiv_hash_exprs->Size(); ul++) {
      CExpressionArray *equiv_distribution_exprs = (*m_equiv_hash_exprs)[ul];
      os << "equiv exprs: " << ul << ":";
...
CExpressionArrays *CDistributionSpecHashed::GetAllDistributionExprs(CMemoryPool *mp) {
...
// matter.
CDistributionSpecHashed *CDistributionSpecHashed::Combine(CMemoryPool *mp, CDistributionSpecHashed *other_spec) {
...
BOOL CDistributionSpecHashed::HasCompleteEquivSpec(CMemoryPool *mp) const {
...
CDistributionSpecHashed *CDistributionSpecHashed::TryToCompleteEquivSpec(CMemoryPool *mp,
  ...
  for (ULONG ul = 0; ul < size; ul++) {
    ...
    if (nullptr != pexprMatching && outerRefs->FIntersects(pexprMatching->DeriveUsedColumns())) {
  ...
  CDistributionSpecHashed *pdshashedEquiv = GPOS_NEW(mp) CDistributionSpecHashed(pdrgpexprResult, fNullsColocated);

  return pdshashedEquiv;
}

CDistributionSpecHashed *CDistributionSpecHashed::MakeHashedDistrSpec(CMemoryPool *mp, CExpressionArray *pdrgpexpr,
                                                                      BOOL fNullsColocated,
