...
BOOL CDefaultComparator::FUseInternalEvaluator(const IDatum *datum1, const IDatum *datum2,
  ...
  *can_use_external_evaluator = GPOS_FTRACE(EopttraceEnableConstantExpressionEvaluation) &&
  ...
  if (mdid1->Equals(datum2->MDId()) && datum1->StatsAreComparable(datum2) &&
      (CMDIdGPDB::m_mdid_date.Equals(mdid1) || CMDIdGPDB::m_mdid_bool.Equals(mdid1) ||
       CMDIdGPDB::m_mdid_time.Equals(mdid1) || CMDIdGPDB::m_mdid_timestamp.Equals(mdid1) ||
       CMDIdGPDB::m_mdid_float4.Equals(mdid1) || CMDIdGPDB::m_mdid_float8.Equals(mdid1) ||
       CMDIdGPDB::m_mdid_numeric.Equals(mdid1))) {
    return true;
  }

  if (!*can_use_external_evaluator) {
    GPOS_RAISE(gpopt::ExmaGPOPT, gpopt::ExmiUnsupportedOp, GPOS_WSZ_LIT("Unsupported comparator evaluator for types"));
  }
