...
//
//---------------------------------------------------------------------------
void CDrvdPropRelational::Derive(CMemoryPool *,  // mp,
  ...
  for (uint32_t ul = 0; ul < size; ul++) {
    ...
    if (pcrsOutput->ContainsAll(pfd->PcrsKey())) {
      ...
      if (0 < pcrsDetermined->Size()) {
        ...
        CFunctionalDependency *pfdNew = GPOS_NEW(mp) CFunctionalDependency(pfd->PcrsKey(), pcrsDetermined);
...
//
//---------------------------------------------------------------------------
CFunctionalDependencyArray *CDrvdPropRelational::DeriveLocalFunctionalDependencies(CMemoryPool *mp,
                                                                                   CExpressionHandle &exprhdl) {
  CFunctionalDependencyArray *pdrgpfd = GPOS_NEW(mp) CFunctionalDependencyArray(mp);
...
IOstream &CDrvdPropRelational::OsPrint(IOstream &os) const {
  os << "Output Cols: [" << *GetOutputColumns() << "]" << ", Outer Refs: [" << *GetOuterReferences() << "]"
     << ", Not Null Cols: [" << *GetNotNullColumns() << "]" << ", Corr. Apply Cols: [" << *GetCorrelatedApplyColumns()
     << "]";
...
// table descriptor
CTableDescriptorHashSet *CDrvdPropRelational::GetTableDescriptor() const {
  GPOS_RTL_ASSERT(IsComplete());
  return m_table_descriptor;
}

CTableDescriptorHashSet *CDrvdPropRelational::DeriveTableDescriptor(CExpressionHandle &exprhdl) {
