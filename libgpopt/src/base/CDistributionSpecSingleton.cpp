...
const CHAR *CDistributionSpecSingleton::m_szSegmentType[EstSentinel] = {"coordinator", "segment"};
...
//
//---------------------------------------------------------------------------
CDistributionSpecSingleton::CDistributionSpecSingleton(ESegmentType est) : m_est(est) {
...
//
//---------------------------------------------------------------------------
BOOL CDistributionSpecSingleton::FSatisfies(const CDistributionSpec *pds) const {
  if (Matches(pds)) {
  ...
  if (EdtHashed == pds->Edt() && CDistributionSpecHashed::PdsConvert(pds)->FSatisfiedBySingleton()) {
...
//
//---------------------------------------------------------------------------
void CDistributionSpecSingleton::AppendEnforcers(CMemoryPool *mp,
  ...
  pexpr->AddRef();
  CExpression *pexprMotion = GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CPhysicalMotionGather(mp, m_est), pexpr);
  ...
  if (!prpp->Peo()->PosRequired()->IsEmpty()) {
    ...
    CExpression *pexprGatherMerge =
        GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CPhysicalMotionGather(mp, m_est, pos), pexpr);
...
//
//---------------------------------------------------------------------------
IOstream &CDistributionSpecSingleton::OsPrint(IOstream &os) const {
