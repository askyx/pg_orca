...
CColumnFactory::CColumnFactory() {
  ...
  m_sht.Init(m_mp, GPOPT_COLFACTORY_HT_BUCKETS, GPOS_OFFSET(CColRef, m_link), GPOS_OFFSET(CColRef, m_id),
             &(CColRef::m_ulInvalid), CColRef::HashValue, CColRef::Equals);
...
//
//---------------------------------------------------------------------------
void CColumnFactory::Initialize() {
...
CColRef *CColumnFactory::PcrCreate(const IMDType *pmdtype, INT type_modifier, BOOL mark_as_used, IMDId *mdid_table,
  ...
  CColRef *colref = GPOS_NEW(m_mp)
      CColRefTable(pmdtype, type_modifier, attno, is_nullable, id, pnameCopy, ulOpSource, isDistCol, ulWidth);
  (void)a_pnameCopy.Reset();
  CAutoP<CColRef> a_pcr(colref);
...
//
//---------------------------------------------------------------------------
CColRef *CColumnFactory::PcrCopy(const CColRef *colref) {
  ...
  return PcrCreate(colref->RetrieveType(), colref->TypeModifier(), colref->GetUsage(true, true) == CColRef::EUsed,
                   colref->GetMdidTable(), pcrTable->AttrNum(), pcrTable->IsNullable(), id, name,
                   pcrTable->UlSourceOpId(), colref->IsDistCol(), pcrTable->Width());
}
