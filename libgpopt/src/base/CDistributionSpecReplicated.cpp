...
//	| Non-Replicated          | F                | F                 |
//	+-------------------------+------------------+-------------------+
BOOL CDistributionSpecReplicated::FSatisfies(const CDistributionSpec *pdss) const {
  ...
  if (Edt() == CDistributionSpec::EdtTaintedReplicated) {
    ...
    switch (pdss->Edt()) {
      ...
      case CDistributionSpec::EdtNonSingleton:
        ...
        return CDistributionSpecNonSingleton::PdsConvert(pdss)->FAllowReplicated();
      case CDistributionSpec::EdtSingleton:
        ...
        return CDistributionSpecSingleton::PdssConvert(pdss)->Est() == CDistributionSpecSingleton::EstSegment;
    }
    ...
    return !(EdtSingleton == pdss->Edt() && (dynamic_cast<const CDistributionSpecSingleton *>(pdss))->FOnCoordinator());
  }

  return false;
}

void CDistributionSpecReplicated::AppendEnforcers(CMemoryPool *mp,
  ...
  CExpression *pexprMotion =
      GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CPhysicalMotionBroadcast(mp, m_ignore_broadcast_threshold), pexpr);
