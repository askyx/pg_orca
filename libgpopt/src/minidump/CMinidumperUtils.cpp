...
//
//---------------------------------------------------------------------------
CDXLMinidump *CMinidumperUtils::PdxlmdLoad(CMemoryPool *mp, const CHAR *file_name) {
  {
    CAutoTrace at(mp);
    at.Os() << "parsing DXL File " << file_name;
  }

  CParseHandlerDXL *parse_handler_dxl = CDXLUtils::GetParseHandlerForDXLFile(mp, file_name, nullptr /*xsd_file_path*/);
  ...
  return GPOS_NEW(mp) CDXLMinidump(pbs, optimizer_config, query, query_output_dxlnode_array, cte_producers, pdxlnPlan,
                                   mdcache_obj_array, pdrgpsysid, plan_id, plan_space_size);
}
...
//
//---------------------------------------------------------------------------
void CMinidumperUtils::GenerateMinidumpFileName(
  ...
  if (nullptr == szMinidumpFileName) {
    ...
    clib::Strncpy(buf + ulPrefixLength, szMinidumpFileName + ulNameStart, ulNameLength + 1);
  }
}
...
//
//---------------------------------------------------------------------------
void CMinidumperUtils::Finalize(CMiniDumperDXL *pmdmp, BOOL fSerializeErrCtx) {
  if (fSerializeErrCtx) {
...
//
//---------------------------------------------------------------------------
CDXLNode *CMinidumperUtils::PdxlnExecuteMinidump(CMemoryPool *mp, const CHAR *file_name, ULONG ulSegments,
  ...
  CDXLNode *pdxlnPlan =
      PdxlnExecuteMinidump(mp, pdxlmd, file_name, ulSegments, ulSessionId, ulCmdId, optimizer_config, pceeval);
...
//
//---------------------------------------------------------------------------
CDXLNode *CMinidumperUtils::PdxlnExecuteMinidump(CMemoryPool *mp, CDXLMinidump *pdxlmd, const CHAR *file_name,
  ...
  CDXLNode *result = CMinidumperUtils::PdxlnExecuteMinidump(mp, factory.Pmda(), pdxlmd, file_name, ulSegments,
...
//
//---------------------------------------------------------------------------
CDXLNode *CMinidumperUtils::PdxlnExecuteMinidump(CMemoryPool *mp, CMDAccessor *md_accessor, CDXLMinidump *pdxlmd,
  ...
  GPOS_ASSERT(nullptr != pdxlmd->GetQueryDXLRoot() && nullptr != pdxlmd->PdrgpdxlnQueryOutput() &&
              nullptr != pdxlmd->GetCTEProducerDXLArray() && "No query found in Minidump");
  GPOS_ASSERT(nullptr != pdxlmd->GetMdIdCachedObjArray() && nullptr != pdxlmd->GetSysidPtrArray() &&
  ...
  GPOS_TRY_HDL(&errhdl) {
    pdxlnPlan = COptimizer::PdxlnOptimize(mp, md_accessor, pdxlmd->GetQueryDXLRoot(), pdxlmd->PdrgpdxlnQueryOutput(),
                                          pdxlmd->GetCTEProducerDXLArray(), pceeval, ulSegments, ulSessionId, ulCmdId,
