...
//
//---------------------------------------------------------------------------
CXformExpandDynamicGetWithForeignPartitions::CXformExpandDynamicGetWithForeignPartitions(CMemoryPool *mp)
    : CXformExploration(
          // pattern
          GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalDynamicGet(mp))) {}

CXform::EXformPromise CXformExpandDynamicGetWithForeignPartitions::Exfp(CExpressionHandle &exprhdl) const {
...
// The physical plan created after this xform and the corresponding logical->physical xforms will create the following
// from Dynamic get containing both foreign and non-foreign tables.
...
void CXformExpandDynamicGetWithForeignPartitions::Transform(CXformContext *pxfctxt, CXformResult *pxfres,
  ...
  SForeignServerToIMdIdArrayMap *foreign_server_to_part_oid_array_map = GPOS_NEW(mp) SForeignServerToIMdIdArrayMap(mp);
  ...
  for (ULONG ul = 0; ul < all_part_mdids->Size(); ++ul) {
    ...
    if (!foreign_server_mdid->IsValid()) {
      ...
      OID foreign_server_oid = CMDIdGPDB::CastMdid(foreign_server_mdid)->Oid();
      SForeignServer foreign_server_lookup = {foreign_server_oid, pmdrel->GetRelDistribution()};
      const IMdIdArray *foreign_server = foreign_server_to_part_oid_array_map->Find(&foreign_server_lookup);
      if (nullptr == foreign_server) {
        ...
        BOOL fres GPOS_ASSERTS_ONLY = foreign_server_to_part_oid_array_map->Insert(
            GPOS_NEW(mp) SForeignServer{foreign_server_oid, pmdrel->GetRelDistribution()}, part_oid_array);
        GPOS_ASSERT(fres);
      } else {
  ...
  if (non_foreign_parts_size > 0) {
    ...
    CLogicalDynamicGet *nonForeignDynamicGet = GPOS_NEW(mp)
        CLogicalDynamicGet(mp, new_alias, popGet->Ptabdesc(), popGet->ScanId(), pdrgpcrNew, popGet->PdrgpdrgpcrPart(),
                           non_foreign_parts, popGet->GetPartitionConstraintsDisj(), popGet->FStaticPruned(),
                           GPOS_NEW(mp) IMdIdArray(mp) /* foreign_server_mdids */, hasSecurityQuals);
    CExpression *pexprNonForeignDynamicGet = GPOS_NEW(mp) CExpression(mp, nonForeignDynamicGet);
  ...
  SForeignServerToIMdIdArrayMapIter map_iter(foreign_server_to_part_oid_array_map);
  BOOL no_union_all = foreign_server_to_part_oid_array_map->GetKeys()->Size() == 1 && non_foreign_parts_size == 0;
  while (map_iter.Advance()) {
    ...
    CLogicalDynamicForeignGet *dynamicForeignGet = GPOS_NEW(mp) CLogicalDynamicForeignGet(
        mp, new_alias, popGet->Ptabdesc(), popGet->ScanId(), pdrgpcrNew, popGet->PdrgpdrgpcrPart(), part_oid_array,
        foreign_server.m_foreign_server_oid, foreign_server.m_exec_location);
    CExpression *pexprDynamicForeignGet = GPOS_NEW(mp) CExpression(mp, dynamicForeignGet);
  ...
  CExpression *pexprResult = GPOS_NEW(mp) CExpression(
      mp, GPOS_NEW(mp) CLogicalUnionAll(mp, popGet->PdrgpcrOutput(), inputColArrayForUnion), expressionsForUnion);
