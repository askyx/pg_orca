...
CXform::EXformPromise CXformCTEAnchor2TrivialSelect::Exfp(CExpressionHandle &exprhdl) const {
  ...
  if ((pcteinfo->FEnableInlining() || 1 == ulConsumers || exprhdl.DeriveOuterReferences()->Size() > 0) &&
...
//
//---------------------------------------------------------------------------
void CXformCTEAnchor2TrivialSelect::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const {
  ...
  CExpression *pexprSelect = GPOS_NEW(mp)
      CExpression(mp, GPOS_NEW(mp) CLogicalSelect(mp), pexprChild, CUtils::PexprScalarConstBool(mp, true /*fValue*/));
