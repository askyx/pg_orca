...
//
//---------------------------------------------------------------------------
CXform::EXformPromise CXformUtils::ExfpLogicalJoin2PhysicalJoin(CExpressionHandle &exprhdl) {
...
//
//---------------------------------------------------------------------------
CXform::EXformPromise CXformUtils::ExfpSemiJoin2CrossProduct(CExpressionHandle &exprhdl) {
#ifdef GPOS_DEBUG
  COperator::EOperatorId op_id = exprhdl.Pop()->Eopid();
#endif  // GPOS_DEBUG
  GPOS_ASSERT(COperator::EopLogicalLeftSemiJoin == op_id || COperator::EopLogicalLeftAntiSemiJoin == op_id ||
...
//
//---------------------------------------------------------------------------
CXform::EXformPromise CXformUtils::ExfpExpandJoinOrder(CExpressionHandle &exprhdl, const CXform *xform) {
...
//
//---------------------------------------------------------------------------
bool CXformUtils::FInlinableCTE(uint32_t ulCTEId) {
  ...
  return IMDFunction::EfsVolatile > pfp->Efs() && !pfp->NeedsSingletonExecution() &&
...
//
//---------------------------------------------------------------------------
CColRefSet *CXformUtils::PcrsFKey(CMemoryPool *mp,
  ...
  const uint32_t ulConjuncts = pdrgpexpr->Size();
  for (uint32_t ul = 0; ul < ulConjuncts; ul++) {
...
//
//---------------------------------------------------------------------------
CColRefSet *CXformUtils::PcrsFKey(CMemoryPool *mp, CExpression *pexprOuter, CExpression *pexprInner,
...
//
//---------------------------------------------------------------------------
bool CXformUtils::FSwapableJoinType(COperator::EOperatorId op_id) {
  return (COperator::EopLogicalLeftSemiJoin == op_id || COperator::EopLogicalLeftAntiSemiJoin == op_id ||
          COperator::EopLogicalLeftAntiSemiJoinNotIn == op_id || COperator::EopLogicalInnerJoin == op_id);
}
#endif  // GPOS_DEBUG
...
//
//---------------------------------------------------------------------------
CExpression *CXformUtils::PexprSwapJoins(CMemoryPool *mp, CExpression *pexprTopJoin, CExpression *pexprBottomJoin) {
#ifdef GPOS_DEBUG
  ...
  pexprRight->AddRef();
  pexprScalar->AddRef();
  CExpression *pexprNewBottomJoin = GPOS_NEW(mp) CExpression(mp, pop, pexprChild, pexprRight, pexprScalar);
  ...
  return GPOS_NEW(mp) CExpression(mp, pop, pexprNewBottomJoin, pexprChildOther, pexprScalar);
}
...
//
//---------------------------------------------------------------------------
CExpression *CXformUtils::PexprPushGbBelowJoin(CMemoryPool *mp, CExpression *pexpr) {
  COperator::EOperatorId op_id = pexpr->Pop()->Eopid();

  GPOS_ASSERT(COperator::EopLogicalGbAgg == op_id || COperator::EopLogicalGbAggDeduplicate == op_id ||
  ...
  bool fCanPush =
      FCanPushGbAggBelowJoin(pcrsGrpCols, pcrsOuterOutput, pcrsScalarFromOuter, pcrsAggOutput, pcrsUsed, pcrsFKey);
...
//
//---------------------------------------------------------------------------
CLogicalGbAgg *CXformUtils::PopGbAggPushableBelowJoin(CMemoryPool *mp, CLogicalGbAgg *popGbAggOld,
                                                      CColRefSet *pcrsOutputOuter, CColRefSet *pcrsGrpCols) {
  ...
  if (!pcrsOutputOuter->ContainsAll(pcrsGrpCols)) {
    ...
    if (COperator::EopLogicalGbAggDeduplicate == popGbAggOld->Eopid()) {
      ...
      popGbAggNew = GPOS_NEW(mp)
          CLogicalGbAggDeduplicate(mp, pcrsGrpColsNew->Pdrgpcr(mp), popGbAggOld->Egbaggtype(), pdrgpcrKeys);
    } else {
...
//		satisfied
//---------------------------------------------------------------------------
bool CXformUtils::FCanPushGbAggBelowJoin(CColRefSet *pcrsGrpCols, CColRefSet *pcrsJoinOuterChildOutput,
                                         CColRefSet *pcrsJoinScalarUsedFromOuter, CColRefSet *pcrsGrpByOutput,
                                         CColRefSet *pcrsGrpByUsed, CColRefSet *pcrsFKey) {
...
//
//---------------------------------------------------------------------------
bool CXformUtils::FSameDatatype(CColRef2dArray *pdrgpdrgpcrInput) {
  GPOS_ASSERT(1 < pdrgpdrgpcrInput->Size());

  CColRefArray *pdrgpcrOuter = (*pdrgpdrgpcrInput)[0];

  uint32_t ulColIndex = pdrgpcrOuter->Size();
  uint32_t child_index = pdrgpdrgpcrInput->Size();
  for (uint32_t ulColCounter = 0; ulColCounter < ulColIndex; ulColCounter++) {
    CColRef *pcrOuter = (*pdrgpcrOuter)[ulColCounter];

    for (uint32_t ulChildCounter = 1; ulChildCounter < child_index; ulChildCounter++) {
...
void CXformUtils::ExistentialToAgg(CMemoryPool *mp, CExpression *pexprSubquery,
  ...
  *ppexprNewSubquery = GPOS_NEW(mp) CExpression(
      mp, GPOS_NEW(mp) CScalarSubquery(mp, pcrCount, true /*fGeneratedByExist*/, false /*fGeneratedByQuantified*/),
      pexprInnerNew);
  *ppexprNewScalar =
      CUtils::PexprCmpWithZero(mp, CUtils::PexprScalarIdent(mp, pcrCount), pcrCount->RetrieveType()->MDId(), ecmptype);
}
...
//		Examples:
//
//			- For 'b1 in (select b2 from R)', with b2 not-nullable and is a well-known operator, we produce
// the following:
...
//
//---------------------------------------------------------------------------
void CXformUtils::SubqueryAnyToAgg(CMemoryPool *mp, CExpression *pexprSubquery,
                                   CExpression **ppexprNewSubquery,  // output argument for new scalar subquery
                                   CExpression **ppexprNewScalar     // output argument for new scalar expression
) {
  GPOS_ASSERT(CUtils::FQuantifiedSubquery(pexprSubquery->Pop()));
  ...
  CExpression *pexprSubqPred =
      sh.PexprSubqueryPred(pexprInner, pexprSubquery, &pexprResult, CSubqueryHandler::EsqctxtFilter);
  ...
  if (fCanEvaluateToNull) {
    ...
    CExpression *pexprCountEqZero =
        CUtils::PexprCmpWithZero(mp, pexprScalarIdentCount,
                                 CScalarIdent::PopConvert(pexprScalarIdentCount->Pop())->MdidType(), IMDType::EcmptEq);
    ...
    CExpression *pexprProjected = GPOS_NEW(mp) CExpression(
        mp, GPOS_NEW(mp) CScalarIf(mp, pmdidInt8), pexprCountEqZero, CUtils::PexprScalarConstInt8(mp, 0 /*val*/),
        GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarIf(mp, pmdidInt8), pexprCountEqSum,
                                 CUtils::PexprScalarConstInt8(mp, -1 /*val*/), CUtils::PexprScalarIdent(mp, pcrCount)));
    ...
    *ppexprNewSubquery = GPOS_NEW(mp) CExpression(
        mp, GPOS_NEW(mp) CScalarSubquery(mp, pcrSubquery, false /*fGeneratedByExist*/, true /*fGeneratedByQuantified*/),
        pexprPrj);
    *ppexprNewScalar = CUtils::PexprCmpWithZero(mp, CUtils::PexprScalarIdent(mp, pcrSubquery),
    ...
    *ppexprNewSubquery = GPOS_NEW(mp) CExpression(
        mp, GPOS_NEW(mp) CScalarSubquery(mp, pcrCount, false /*fGeneratedByExist*/, true /*fGeneratedByQuantified*/),
        pexprInnerNew);
...
void CXformUtils::SubqueryAllToAgg(CMemoryPool *mp, CExpression *pexprSubquery,
  ...
  CExpression *pexprSubqTest = GPOS_NEW(mp)
      CExpression(mp, GPOS_NEW(mp) CScalarIf(mp, pmdidInt4), pexprSubqPred, CUtils::PexprScalarConstInt4(mp, 1 /*val*/),
  ...
  CColRef *pcrSubqTest = CScalarProjectElement::PopConvert((*(*pexprPrj)[1])[0]->Pop())->Pcr();
  CColRef *pcrInnerNullTest = CScalarProjectElement::PopConvert((*(*pexprPrj)[1])[1]->Pop())->Pcr();
  ...
  CExpression *pexprSumTest = CUtils::PexprCmpWithZero(
      mp, pexprScalarIdentSum, CScalarIdent::PopConvert(pexprScalarIdentSum->Pop())->MdidType(), IMDType::EcmptEq);
  pexprScalarIdentSum->AddRef();
  CExpression *pexprIsInnerEmpty = CUtils::PexprIsNull(mp, pexprScalarIdentSum);
  CExpression *pexprInnerHasNulls =
      CUtils::PexprCmpWithZero(mp, pexprScalarIdentSumNulls,
                               CScalarIdent::PopConvert(pexprScalarIdentSumNulls->Pop())->MdidType(), IMDType::EcmptG);
  ...
  pexprPrj = GPOS_NEW(mp) CExpression(
      ...
      GPOS_NEW(mp) CExpression(
          ...
          GPOS_NEW(mp) CExpression(
              ...
              GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarIf(mp, pmdidBool),
  ...
  *ppexprNewSubquery = GPOS_NEW(mp) CExpression(
      mp, GPOS_NEW(mp) CScalarSubquery(mp, pcrSubquery, false /*fGeneratedByExist*/, true /*fGeneratedByQuantified*/),
      pexprProjected);
  *ppexprNewScalar = CUtils::PexprScalarCmp(mp, CUtils::PexprScalarIdent(mp, pcrSubquery),
...
//---------------------------------------------------------------------------
CExpression *CXformUtils::PexprSeparateSubqueryPreds(CMemoryPool *mp, CExpression *pexpr) {
  COperator::EOperatorId op_id = pexpr->Pop()->Eopid();
  GPOS_ASSERT(COperator::EopLogicalInnerJoin == op_id || COperator::EopLogicalNAryJoin == op_id);
  ...
  const uint32_t ulConjuncts = pdrgpexprConjuncts->Size();
  for (uint32_t ul = 0; ul < ulConjuncts; ul++) {
  ...
  if (nullptr == naryLOJOp) {
    ...
    pdrgpexpr->Append(
        naryLOJOp->ReplaceInnerJoinPredicates(mp, pexprScalar, CPredicateUtils::PexprConjunction(mp, pdrgpexprNonSQ)));
  }
...
//
//---------------------------------------------------------------------------
CExpression *CXformUtils::PexprInversePred(CMemoryPool *mp, CExpression *pexprSubquery) {
...
//
//---------------------------------------------------------------------------
CExpression *CXformUtils::PexprNullIndicator(CMemoryPool *mp, CExpression *pexpr) {
  CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
  ...
  mdid->AddRef();
  return GPOS_NEW(mp)
      CExpression(mp, GPOS_NEW(mp) CScalarIf(mp, mdid), pexprIsNull, CUtils::PexprScalarConstInt4(mp, 1 /*val*/),
                  CUtils::PexprScalarConstInt4(mp, 0 /*val*/));
}
...
//
//---------------------------------------------------------------------------
CExpression *CXformUtils::PexprLogicalDMLOverProject(CMemoryPool *mp, CExpression *pexprChild,
  ...
  int32_t val = CScalarDMLAction::EdmlactionInsert;
...
//
//---------------------------------------------------------------------------
CExpressionArray *CXformUtils::PdrgpexprPartEqFilters(CMemoryPool *mp, CTableDescriptor *ptabdesc,
  ...
  const uint32_t ulPartKeys = pdrgpulPart->Size();
  GPOS_ASSERT(0 < ulPartKeys);
  GPOS_ASSERT(pdrgpcrSource->Size() >= ulPartKeys);

  for (uint32_t ul = 0; ul < ulPartKeys; ul++) {
    uint32_t *pulPartKey = (*pdrgpulPart)[ul];
...
//
//---------------------------------------------------------------------------
bool CXformUtils::FSplitAggXform(CXform::EXformId exfid) {
  return CXform::ExfSplitGbAgg == exfid || CXform::ExfSplitDQA == exfid || CXform::ExfSplitGbAggDedup == exfid ||
         CXform::ExfEagerAgg == exfid;
}

bool CXformUtils::FAggGenBySplitDQAXform(CExpression *pexprAgg) {
...
// or origin xform
bool CXformUtils::FMultiStageAgg(CExpression *pexprAgg) {
...
bool CXformUtils::FLocalAggCreatedByEagerAggXform(CExpression *pexprAgg) {
  GPOS_ASSERT(nullptr != pexprAgg);
  GPOS_ASSERT(COperator::EopLogicalGbAgg == pexprAgg->Pop()->Eopid() ||
              COperator::EopLogicalGbAggDeduplicate == pexprAgg->Pop()->Eopid());
...
//
//---------------------------------------------------------------------------
bool CXformUtils::FXformInArray(CXform::EXformId exfid, const CXform::EXformId rgXforms[], uint32_t ulXforms) {
  for (uint32_t ul = 0; ul < ulXforms; ul++) {
...
//
//---------------------------------------------------------------------------
bool CXformUtils::FApplyToNextBinding(CXform *pxform,
...
//
//---------------------------------------------------------------------------
CWStringConst *CXformUtils::PstrErrorMessage(CMemoryPool *mp, uint32_t major, uint32_t minor, ...) {
  wchar_t wsz[1024];
...
//
//---------------------------------------------------------------------------
CColRefArray *CXformUtils::PdrgpcrIndexKeys(CMemoryPool *mp, CColRefArray *colref_array, const IMDIndex *pmdindex,
...
//
//---------------------------------------------------------------------------
CColRefSet *CXformUtils::PcrsIndexKeys(CMemoryPool *mp, CColRefArray *colref_array, const IMDIndex *pmdindex,
                                       const IMDRelation *pmdrel) {
...
//
//---------------------------------------------------------------------------
CColRefSet *CXformUtils::PcrsIndexColumns(CMemoryPool *mp, CColRefArray *colref_array, const IMDIndex *pmdindex,
                                          const IMDRelation *pmdrel) {
  CColRefArray *pdrgpcrIndexColumns = PdrgpcrIndexColumns(mp, colref_array, pmdindex, pmdrel);
...
//
//---------------------------------------------------------------------------
CColRefSet *CXformUtils::PcrsIndexReturnableColumns(CMemoryPool *mp, CColRefArray *colref_array,
                                                    const IMDIndex *pmdindex, const IMDRelation *pmdrel) {
  CColRefSet *pcrsCols = GPOS_NEW(mp) CColRefSet(mp);

  // returnable columns
  for (uint32_t ul = 0; ul < pmdindex->ReturnableCols(); ul++) {
    uint32_t ulPos = pmdindex->ReturnableColAt(ul);

    uint32_t ulPosNonDropped = pmdrel->NonDroppedColAt(ulPos);
    GPOS_ASSERT(UINT32_MAX != ulPosNonDropped);
...
//
//---------------------------------------------------------------------------
CColRefArray *CXformUtils::PdrgpcrIndexColumns(CMemoryPool *mp, CColRefArray *colref_array, const IMDIndex *pmdindex,
                                               const IMDRelation *pmdrel) {
  CColRefArray *pdrgpcrIndex = GPOS_NEW(mp) CColRefArray(mp);

  // key columns
  for (uint32_t ul = 0; ul < pmdindex->Keys(); ul++) {
    uint32_t ulPos = pmdindex->KeyAt(ul);
  ...
  for (uint32_t ul = 0; ul < pmdindex->IncludedCols(); ul++) {
    uint32_t ulPos = pmdindex->IncludedColAt(ul);
...
//
//---------------------------------------------------------------------------
bool CXformUtils::FIndexApplicable(CMemoryPool *mp, const IMDIndex *pmdindex, const IMDRelation *pmdrel,
                                   CColRefArray *pdrgpcrOutput, CColRefSet *pcrsScalar,
                                   IMDIndex::EmdindexType emdindtype, IMDIndex::EmdindexType altindtype) {
  bool possible_ao_table =
      pmdrel->IsAORowOrColTable() || pmdrel->RetrieveRelStorageType() == IMDRelation::ErelstorageMixedPartitioned;
  // GiST and Hash can match with either Btree or Bitmap indexes
  if (pmdindex->IndexType() == IMDIndex::EmdindGist || pmdindex->IndexType() == IMDIndex::EmdindHash ||
      // GIN and BRIN can only match with Bitmap Indexes
      (emdindtype == IMDIndex::EmdindBitmap &&
       (IMDIndex::EmdindGin == pmdindex->IndexType() || IMDIndex::EmdindBrin == pmdindex->IndexType()))) {
    // continue
  } else if (emdindtype == IMDIndex::EmdindBitmap && pmdindex->IndexType() == IMDIndex::EmdindBtree &&
             possible_ao_table) {
...
//
//---------------------------------------------------------------------------
CExpression *CXformUtils::PexprRowNumber(CMemoryPool *mp) {
  OID row_number_oid = COptCtxt::PoctxtFromTLS()->GetOptimizerConfig()->GetWindowOids()->OidRowNumber();

  CScalarWindowFunc *popRowNumber = GPOS_NEW(mp)
      CScalarWindowFunc(mp, GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, row_number_oid),
                        GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT8_OID),
                        GPOS_NEW(mp) CWStringConst(mp, GPOS_WSZ_LIT("row_number")), CScalarWindowFunc::EwsImmediate,
                        false /* is_distinct */, false /* is_star_arg */, false /* is_simple_agg */
      );
...
//---------------------------------------------------------------------------
CColRef *CXformUtils::PcrProjectElement(CExpression *pexpr, uint32_t ulIdxProjElement) {
...
void CXformUtils::LookupJoinKeys(CMemoryPool *mp, CExpression *pexpr, CExpressionArray **ppdrgpexprOuter,
                                 CExpressionArray **ppdrgpexprInner, IMdIdArray **join_opfamilies) {
...
void CXformUtils::CacheJoinKeys(CExpression *pexpr, CExpressionArray *pdrgpexprOuter, CExpressionArray *pdrgpexprInner,
...
//
//---------------------------------------------------------------------------
CExpression *CXformUtils::PexprAddCTEProducer(CMemoryPool *mp, uint32_t ulCTEId, CColRefArray *colref_array,
                                              CExpression *pexpr) {
  ...
  CExpression *pexprProducer =
      GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalCTEProducer(mp, ulCTEId, pdrgpcrProd), pexprRemapped);
...
//
//---------------------------------------------------------------------------
bool CXformUtils::FExtractEquality(
    CExpression *pexpr,
...
//
//---------------------------------------------------------------------------
bool CXformUtils::FProcessGPDBAntiSemiHashJoin(
  ...
  *ppexprResult = nullptr;
  ...
  for (uint32_t ul = 0; ul < ulPreds; ul++) {
    ...
    if (CPredicateUtils::FIDFFalse(pexprPred)) {
      ...
      if (FExtractEquality(pexprPred, &pexprEquality,
                           &pexprFalse) &&  // extracted equality expression
          IMDId::EmdidGeneral == CScalarConst::PopConvert(pexprFalse->Pop())
                                     ->GetDatum()
                                     ->MDId()
                                     ->MdidType() &&  // underlying system is GPDB
          CPhysicalJoin::FHashJoinCompatible(pexprEquality, pexprOuter,
  ...
  *ppexprResult = GPOS_NEW(mp)
      CExpression(mp, pexpr->Pop(), pexprOuter, pexprInner, CPredicateUtils::PexprConjunction(mp, pdrgpexprNew));
...
//
//---------------------------------------------------------------------------
CExpression *CXformUtils::PexprBuildBtreeIndexPlan(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprGet,
                                                   uint32_t ulOriginOpId, CExpressionArray *pdrgpexprConds,
  ...
  if (!FIndexApplicable(mp, pmdindex, pmdrel, pdrgpcrOutput, pcrsScalarExpr, IMDIndex::EmdindBtree)) {
    GPOS_DELETE(alias);
  ...
  uint32_t ulUnindexedPredColCount = 0;

  CPredicateUtils::ExtractIndexPredicates(mp, md_accessor, pdrgpexprConds, pmdindex, pdrgppcrIndexCols, pdrgpexprIndex,
                                          pdrgpexprResidual, ulUnindexedPredColCount, outer_refs);
  ...
  if ((0 == pdrgpexprIndex->Size() || outer_refs_in_index_get->Size() == 0) && !indexForOrderBy) {
    // clean up
    GPOS_DELETE(alias);
  ...
  // most GiST indexes are lossy, so conservatively re-add all the index quals to the residual so that they can be
  // rechecked
  if (pmdindex->IndexType() == IMDIndex::EmdindGist) {
    for (uint32_t ul = 0; ul < pdrgpexprIndex->Size(); ul++) {
  ...
  return PexprRewrittenBtreeIndexPath(mp, pexprIndexCond, pexprResidualCond, pmdindex, ptabdesc, popLogicalGet);
}
...
//
//---------------------------------------------------------------------------
CExpression *CXformUtils::PexprScalarBitmapBoolOp(CMemoryPool *mp, CMDAccessor *md_accessor,
  ...
  const uint32_t ulPredicates = pdrgpexpr->Size();
  ...
  CreateBitmapIndexProbeOps(mp, md_accessor, pexprOriginalPred, pdrgpexpr, ptabdesc, pmdrel, pdrgpcrOutput, outer_refs,
                            pcrsReqd, fConjunction, pdrgpexprBitmap, pdrgpexprRecheckNew, pdrgpexprResidualNew,
                            isAPartialPredicate);

  GPOS_ASSERT(pdrgpexprRecheckNew->Size() == pdrgpexprBitmap->Size());

  const uint32_t ulBitmapExpr = pdrgpexprBitmap->Size();
  ...
  if (nullptr != *ppexprRecheck) {
    CExpression *pexprRecheckNewCombined =
        CPredicateUtils::PexprConjDisj(mp, *ppexprRecheck, pexprRecheckNew, fConjunction);
...
//
//---------------------------------------------------------------------------
void CXformUtils::ComputeBitmapTableScanResidualPredicate(
  ...
  if (nullptr != *ppexprResidual) {
    CExpression *pexprResidualNewCombined =
        CPredicateUtils::PexprConjDisj(mp, *ppexprResidual, pexprResidualNew, fConjunction);
...
//
//---------------------------------------------------------------------------
CExpression *CXformUtils::PexprBitmapBoolOp(CMemoryPool *mp, IMDId *pmdidBitmapType, CExpression *pexprLeft,
                                            CExpression *pexprRight, bool fConjunction) {
  ...
  return GPOS_NEW(mp)
      CExpression(mp, GPOS_NEW(mp) CScalarBitmapBoolOp(mp, ebitmapboolop, pmdidBitmapType), pexprLeft, pexprRight);
}
...
//
//---------------------------------------------------------------------------
CExpression *CXformUtils::PexprBitmapLookupWithPredicateBreakDown(
    CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprOriginalPred, CExpression *pexprPred,
    CTableDescriptor *ptabdesc, const IMDRelation *pmdrel, CColRefArray *pdrgpcrOutput, CColRefSet *outer_refs,
    CColRefSet *pcrsReqd, CExpression **ppexprRecheck, CExpression **ppexprResidual) {
  ...
  if (fConjunction) {
    ...
    const uint32_t size = temp_conjuncts->Size();
  ...
  CExpression *pexprResult = PexprScalarBitmapBoolOp(
      mp, md_accessor, pexprOriginalPred, pdrgpexpr, ptabdesc, pmdrel, pdrgpcrOutput, outer_refs, pcrsReqd,
      fConjunction, ppexprRecheck, ppexprResidual, !fConjunction /* we are now breaking up something other
                                                                    than an AND predicate and want to consider
                                                                    BTree indexes as well */
  );
  pdrgpexpr->Release();
...
//
//---------------------------------------------------------------------------
CExpression *CXformUtils::PexprBitmapSelectBestIndex(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprPred,
                                                     CTableDescriptor *ptabdesc, const IMDRelation *pmdrel,
  ...
  const uint32_t ulIndexes = pmdrel->IndexCount();
  uint32_t ulUnsupportedPredCount = 0;

  for (uint32_t ul = 0; ul < ulIndexes; ul++) {
    const IMDIndex *pmdindex = md_accessor->RetrieveIndex(pmdrel->IndexMDidAt(ul));

    if (CXformUtils::FIndexApplicable(mp, pmdindex, pmdrel, pdrgpcrOutput, pcrsScalar, IMDIndex::EmdindBitmap,
                                      altIndexType)) {
      ...
      CPredicateUtils::ExtractIndexPredicates(mp, md_accessor, pdrgpexprScalar, pmdindex, pdrgpcrIndexCols,
                                              pdrgpexprIndex, pdrgpexprResidual, ulUnsupportedPredCount, pcrsOuterRefs,
      ...
      if (selectivity > AO_TABLE_BTREE_INDEX_SELECTIVITY_THRESHOLD && possible_ao_table &&
      ...
      if (!pcrsScalar->FMember(pcrFirstIndexKey) && pmdindex->IndexType() != IMDIndex::EmdindBrin) {
      ...
      uint32_t numResiduals = pdrgpexprResidual->Size();
      uint32_t numIndexCols = indexColumns->Size();
      ...
      if (bestSelectivity > selectivity ||
          (bestSelectivity == selectivity && (bestNumResiduals > numResiduals ||
                                              (bestNumResiduals == numResiduals && bestNumIndexCols > numIndexCols)))) {
        CRefCount::SafeRelease((*ppexprResidual));
...
//
//---------------------------------------------------------------------------
void CXformUtils::CreateBitmapIndexProbeOps(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprOriginalPred,
  ...
  uint32_t ulPredicates = pdrgpexprPreds->Size();

  for (uint32_t ul = 0; ul < ulPredicates; ul++) {
    CExpression *pexprPred = (*pdrgpexprPreds)[ul];
    CExpression *pexprBitmap = nullptr;
    CExpression *pexprRecheck = nullptr;

    CreateBitmapIndexProbesWithOrWithoutPredBreakdown(mp, md_accessor, pexprOriginalPred, pexprPred, ptabdesc, pmdrel,
...
//
//---------------------------------------------------------------------------
void CXformUtils::CreateBitmapIndexProbesWithOrWithoutPredBreakdown(
    CMemoryPool *pmp, CMDAccessor *pmda, CExpression *pexprOriginalPred, CExpression *pexprPred,
    CTableDescriptor *ptabdesc, const IMDRelation *pmdrel, CColRefArray *pdrgpcrOutput, CColRefSet *pcrsOuterRefs,
    CColRefSet *pcrsReqd, CExpression **pexprBitmapResult, CExpression **pexprRecheckResult,
  ...
  while (nullptr != pexprPred) {
    ...
    if (CPredicateUtils::FBitmapLookupSupportedPredicateOrConjunct(pexprPred, pcrsOuterRefs)) {
      ...
      if (!isAPartialPredicateOrArrayCmp) {
        ...
        uint32_t size = conjuncts->Size();

        for (uint32_t i = 0; i < size && !isAPartialPredicateOrArrayCmp; i++) {
      ...
      pexprBitmapLocal = PexprBitmapSelectBestIndex(pmp, pmda, pexprPred, ptabdesc, pmdrel, pdrgpcrOutput,
      ...
      pexprBitmapLocal = PexprBitmapLookupWithPredicateBreakDown(pmp, pmda, pexprOriginalPred, pexprPred, ptabdesc,
                                                                 pmdrel, pdrgpcrOutput, pcrsOuterRefs, pcrsReqd,
                                                                 &pexprRecheckLocal, &pexprResidualLocal);
  ...
  // for simple conjuncts, there may be multiple index paths generated by
  // CreateBitmapIndexProbesWithOrWithoutPredBreakdown() due to the retry, in that case we combine them with BitmapAnd
  // expression. for example, if you have index foo_a and foo_b with a predicate (a = 1 and b = 2 and c = 3)
  ...
  const uint32_t ulBitmapExpr = pdrgpexprBitmapTemp->Size();
  if (0 < ulBitmapExpr) {
    JoinBitmapIndexProbes(pmp, pdrgpexprBitmapTemp, pdrgpexprRecheckTemp, true /*fConjunction*/, pexprBitmapResult,
                          pexprRecheckResult);
  }
...
void CXformUtils::JoinBitmapIndexProbes(CMemoryPool *pmp, CExpressionArray *pdrgpexprBitmap,
                                        CExpressionArray *pdrgpexprRecheck, bool fConjunction,
                                        CExpression **ppexprBitmap, CExpression **ppexprRecheck) {
  const uint32_t ulBitmapExpr = pdrgpexprBitmap->Size();
  ...
  for (uint32_t ul = 1; ul < ulBitmapExpr; ul++) {
...
//
//---------------------------------------------------------------------------
bool CXformUtils::FHasAmbiguousType(CExpression *pexpr, CMDAccessor *md_accessor) {
  ...
  if (!fAmbiguous) {
    ...
    const uint32_t arity = pexpr->Arity();
    for (uint32_t ul = 0; !fAmbiguous && ul < arity; ul++) {
      CExpression *pexprChild = (*pexpr)[ul];
...
//
//---------------------------------------------------------------------------
CExpression *CXformUtils::PexprSelect2BitmapBoolOp(CMemoryPool *mp, CExpression *pexpr) {
  GPOS_ASSERT(nullptr != pexpr);
  ...
  CTableDescriptorHashSet *ptabdescset = pexprRelational->DeriveTableDescriptor();
  GPOS_ASSERT(1 == ptabdescset->Size());
  CTableDescriptor *ptabdesc = ptabdescset->First();

  GPOS_ASSERT(nullptr != ptabdesc);
  const uint32_t ulIndices = ptabdesc->IndexCount();
  if (0 == ulIndices) {
    return nullptr;
  }
  ...
  CExpression *pexprResult = PexprBitmapTableGet(mp, popGet, pexpr->Pop()->UlOpId(), ptabdesc, pexprScalar,
                                                 nullptr,  // outer_refs
                                                 pcrsReqd);
  pcrsReqd->Release();
...
//
//---------------------------------------------------------------------------
CExpression *CXformUtils::PexprBitmapTableGet(CMemoryPool *mp, CLogical *popGet, uint32_t ulOriginOpId,
  ...
  CExpression *pexprBitmap =
      PexprScalarBitmapBoolOp(mp, md_accessor, pexprScalar, pdrgpexpr, ptabdesc, pmdrel, pdrgpcrOutput, outer_refs,
                              pcrsReqd, fConjunction, &pexprRecheck, &pexprResidual, false /*isAPartialPredicate*/
      );
  CExpression *pexprResult = nullptr;
  ...
  if (nullptr != pexprBitmap) {
    ...
    if (nullptr != pexprResidual) {
      ...
      pexprResult = GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalSelect(mp), pexprResult, pexprResidual);
    }
  }
...
//
//---------------------------------------------------------------------------
CExpression *CXformUtils::PexprRemapColumns(CMemoryPool *mp, CExpression *pexpr, CColRefArray *pdrgpcrA,
                                            CColRefArray *pdrgpcrRemappedA, CColRefArray *pdrgpcrB,
...
//
//---------------------------------------------------------------------------
bool CXformUtils::FJoinPredOnSingleChild(CMemoryPool *mp, CExpressionHandle &exprhdl) {
  ...
  CExpressionArray *pdrgpexprPreds =
      CPredicateUtils::PdrgpexprConjuncts(mp, exprhdl.PexprScalarExactChild(arity - 1, true /*error_on_null_return*/));
  const uint32_t ulPreds = pdrgpexprPreds->Size();
  bool fPredUsesSingleChild = false;
  for (uint32_t ulPred = 0; !fPredUsesSingleChild && ulPred < ulPreds; ulPred++) {
    CExpression *pexpr = (*pdrgpexprPreds)[ulPred];
    CColRefSet *pcrsUsed = pexpr->DeriveUsedColumns();
    for (uint32_t ulChild = 0; !fPredUsesSingleChild && ulChild < arity - 1; ulChild++) {
...
//
//---------------------------------------------------------------------------
CExpression *CXformUtils::PexprCTEConsumer(CMemoryPool *mp, uint32_t ulCTEId, CColRefArray *colref_array) {
...
//		at the positions indicated by the given uint32_t array 'pdrgpulIndexesOfRefs'
//
//---------------------------------------------------------------------------
CColRefArray *CXformUtils::PdrgpcrReorderedSubsequence(CMemoryPool *mp, CColRefArray *colref_array,
                                                       ULongPtrArray *pdrgpulIndexesOfRefs) {
  GPOS_ASSERT(nullptr != colref_array);
  GPOS_ASSERT(nullptr != pdrgpulIndexesOfRefs);

  const uint32_t length = pdrgpulIndexesOfRefs->Size();
  ...
  for (uint32_t ul = 0; ul < length; ul++) {
    uint32_t ulPos = *(*pdrgpulIndexesOfRefs)[ul];
...
CExpression *CXformUtils::PexprWinFuncAgg2ScalarAgg(CMemoryPool *mp, CExpression *pexprWinFunc) {
  ...
  CExpressionArray *pdrgpexprFullWinFuncArgs = GPOS_NEW(mp) CExpressionArray(mp);

  CExpressionArray *pdrgpexprWinFuncArgs = GPOS_NEW(mp) CExpressionArray(mp);
  const uint32_t ulArgs = pexprWinFunc->Arity();
  for (uint32_t ul = 0; ul < ulArgs; ul++) {
    CExpression *pexprArg = (*pexprWinFunc)[ul];
    pexprArg->AddRef();
    pdrgpexprWinFuncArgs->Append(pexprArg);
  }

  pdrgpexprFullWinFuncArgs->Append(GPOS_NEW(mp)
                                       CExpression(mp, GPOS_NEW(mp) CScalarValuesList(mp), pdrgpexprWinFuncArgs));

  pdrgpexprFullWinFuncArgs->Append(
      GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarValuesList(mp), GPOS_NEW(mp) CExpressionArray(mp)));
  pdrgpexprFullWinFuncArgs->Append(
      GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarValuesList(mp), GPOS_NEW(mp) CExpressionArray(mp)));
  pdrgpexprFullWinFuncArgs->Append(
      GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarValuesList(mp), GPOS_NEW(mp) CExpressionArray(mp)));
  ...
  return GPOS_NEW(mp) CExpression(
      mp,
      CUtils::PopAggFunc(mp, mdid_func, GPOS_NEW(mp) CWStringConst(mp, popScWinFunc->PstrFunc()->GetBuffer()),
...
//
//---------------------------------------------------------------------------
void CXformUtils::MapPrjElemsWithDistinctAggs(
    CMemoryPool *mp, CExpression *pexprPrjList,
    ExprToExprArrayMap **pphmexprdrgpexpr,  // output: created map
    uint32_t *pulDifferentDQAs              // output: number of DQAs with different arguments
) {
  ...
  for (uint32_t ul = 0; ul < arity; ul++) {
    ...
    if (COperator::EopScalarAggFunc != eopidChild && COperator::EopScalarWindowFunc != eopidChild) {
      continue;
    }
...
//
//---------------------------------------------------------------------------
int32_t CXformUtils::ICmpPrjElemsArr(const void *pvFst, const void *pvSnd) {
  ...
  uint32_t ulIdFst = CScalarProjectElement::PopConvert(pexprPrjElemFst->Pop())->Pcr()->Id();
  uint32_t ulIdSnd = CScalarProjectElement::PopConvert(pexprPrjElemSnd->Pop())->Pcr()->Id();
...
CExpression *CXformUtils::PexprGbAggOnCTEConsumer2Join(CMemoryPool *mp, CExpression *pexprGbAgg) {
  ...
  uint32_t ulDistinctAggs = pexprPrjList->DeriveTotalDistinctAggs();
  ...
  uint32_t ulConsumers = 0;

  const uint32_t size = pdrgpdrgpexprPrjElems->Size();
  for (uint32_t ulPrjElemsArr = 0; ulPrjElemsArr < size; ulPrjElemsArr++) {
    ...
    if (0 == ulConsumers) {
      ...
      pdrgpexprPrjElems->AddRef();
      pexprNewGbAgg = GPOS_NEW(mp)
          CExpression(mp, GPOS_NEW(mp) CLogicalGbAgg(mp, pdrgpcrGrpCols, COperator::EgbaggtypeGlobal), pexprCTEConsumer,
      ...
      const uint32_t ulPrjElems = pdrgpexprPrjElems->Size();
      ...
      pexprNewGbAgg = GPOS_NEW(mp) CExpression(
          mp, GPOS_NEW(mp) CLogicalGbAgg(mp, pdrgpcrNewGrpCols, COperator::EgbaggtypeGlobal), pexprNewConsumer,
...
//
//---------------------------------------------------------------------------
CExpression *CXformUtils::AddALinearStackOfUnaryExpressions(CMemoryPool *mp, CExpression *lowerPartOfExpr,
  ...
  uint32_t arity = topOfStack->Arity();
  ...
  CExpression *processedRestOfStack =
      AddALinearStackOfUnaryExpressions(mp, lowerPartOfExpr, (*topOfStack)[0], exclusiveBottomOfStack);
...
//
//---------------------------------------------------------------------------
bool CXformUtils::FCoverIndex(CMemoryPool *mp, CIndexDescriptor *pindexdesc, CTableDescriptor *ptabdesc,
  ...
  CColRefSet *returnable_cols = CXformUtils::PcrsIndexReturnableColumns(mp, pdrgpcrOutput, pmdindex, pmdrel);
  ...
  for (uint32_t i = 0; i < pdrgpcrOutput->Size(); i++) {
...
//---------------------------------------------------------------------------
void CXformUtils::ComputeOrderSpecForIndexKey(CMemoryPool *mp, COrderSpec **order_spec, const IMDIndex *pmdindex,
  ...
  if (scan_direction == EForwardScan) {
    ...
    // vice-versa
    ent = (pmdindex->KeyNullsDirectionAt(key_position) == COrderSpec::EntLast) ? COrderSpec::EntLast
                                                                               : COrderSpec::EntFirst;
  }
  ...
  // commutative to index's sort and nulls order.
    ...
    // vice-versa
    ent = (pmdindex->KeyNullsDirectionAt(key_position) == COrderSpec::EntLast) ? COrderSpec::EntFirst
                                                                               : COrderSpec::EntLast;
  }
  mdid->AddRef();
