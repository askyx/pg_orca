...
//
//---------------------------------------------------------------------------
CXform::EXformPromise CXformSimplifySubquery::Exfp(CExpressionHandle &exprhdl) const {
...
//
//---------------------------------------------------------------------------
void CXformSimplifySubquery::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const {
  ...
  if (nullptr != pexprResult) {
    ...
    pexprResult = FSimplifySubquery(mp, pexprResult, FSimplifyExistential, CUtils::FExistentialSubquery);
    if (nullptr != pexprResult) {
      pxfres->Add(pexprResult);
    }
  }
...
CExpression *CXformSimplifySubquery::FSimplifySubquery(CMemoryPool *mp, CExpression *pexprInput,
  ...
  if (!FSimplifySubqueryRecursive(mp, pexprScalar, &pexprNewScalar, pfnsimplify, pfnmatch)) {
