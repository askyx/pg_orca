...
CXform::EXformPromise CXformPushJoinBelowUnionAll::Exfp(CExpressionHandle &exprhdl) const {
...
//
//---------------------------------------------------------------------------
void CXformPushJoinBelowUnionAll::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const {
  ...
  CColRefArray *other_colref_array = pexprOther->DeriveOutputColumns()->Pdrgpcr(mp);
  ...
  for (ULONG ul = 0; ul < arity; ul++) {
    ...
    CExpression *pexprLeftChild, *pexprRightChild, *pexprRemappedScalar, *pexprRemappedOther, *join_expr;
    ...
    if (0 == ul) {
      ...
      CColRefArray *other_colref_array_copy = CUtils::PdrgpcrCopy(mp, other_colref_array);
      ...
      UlongToColRefMap *colref_mapping = CUtils::PhmulcrMapping(mp, colref_array_from, colref_array_to);

      // Create a copy of the join condition with remapped columns,
      // and a copy of the other expression with remapped columns
      pexprRemappedScalar = pexprScalar->PexprCopyWithRemappedColumns(mp, colref_mapping, true /*must_exist*/);
      pexprRemappedOther = pexprOther->PexprCopyWithRemappedColumns(mp, colref_mapping, true);
    ...
    if (isOuterJoin) {
      ...
      join_expr = CUtils::PexprLogicalJoin<CLogicalInnerJoin>(mp, pexprLeftChild, pexprRightChild, pexprRemappedScalar);
    }
  ...
  CExpression *pexprAlt =
      GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalUnionAll(mp, output_columns, input_columns), join_array);
