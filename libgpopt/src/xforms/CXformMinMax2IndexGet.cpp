      ...
      CXformExploration(
          ...
          GPOS_NEW(mp) CExpression(
              ...
              GPOS_NEW(mp) CExpression(
                  ...
                  GPOS_NEW(mp) CExpression(mp,
                                           GPOS_NEW(mp) CScalarProjectElement(mp),  // scalar project element
...
//
//---------------------------------------------------------------------------
CXform::EXformPromise CXformMinMax2IndexGet::Exfp(CExpressionHandle &exprhdl) const {
...
//---------------------------------------------------------------------------
void CXformMinMax2IndexGet::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const {
  ...
  IMdIdArray *btree_indices =
      GetApplicableIndices(mp, agg_colref, popGet->PdrgpcrOutput(), md_accessor, pmdrel, ulIndices);
  ...
  CExpression *notNullExpr = CUtils::PexprIsNotNull(mp, CUtils::PexprScalarIdent(mp, agg_colref));
  ...
  popGet->AddRef();
  CExpression *pexprGetNotNull = GPOS_NEW(mp) CExpression(mp, popGet, notNullExpr);
  ...
  for (ULONG ul = 0; ul < btree_indices->Size(); ul++) {
    ...
    CExpression *pexprIndexGet = CXformUtils::PexprBuildBtreeIndexPlan(
        ...
        pmdrel, scan_direction, false, indexonly);

    if (pexprIndexGet != nullptr) {
      COrderSpec *pos = nullptr;
      // Compute and update the required OrderSpec for first index key
      CXformUtils::ComputeOrderSpecForIndexKey(mp, &pos, pmdindex, scan_direction, agg_colref, 0 /*key position*/);

      // build Limit expression
      CExpression *pexprLimit =
          CUtils::BuildLimitExprWithOrderSpec(mp, pexprIndexGet, pos, 0 /*limitOffset*/, 1 /*limitCount*/);

      popAgg->AddRef();
      pexprScalarPrjList->AddRef();

      // build Aggregate expression
      CExpression *finalpexpr = GPOS_NEW(mp) CExpression(mp, popAgg, pexprLimit, pexprScalarPrjList);
...
//---------------------------------------------------------------------------
EIndexScanDirection CXformMinMax2IndexGet::GetScanDirection(const IMDIndex *pmdindex, CScalarAggFunc *popScAggFunc,
                                                            const IMDType *agg_col_type) {
  // If Aggregate function is min()
  if (popScAggFunc->MDId()->Equals(agg_col_type->GetMdidForAggType(IMDType::EaggMin))) {
    ...
    return pmdindex->KeySortDirectionAt(0) == SORT_DESC ? EBackwardScan : EForwardScan;
  }
  // If Aggregate function is max()
    ...
    return pmdindex->KeySortDirectionAt(0) == SORT_DESC ? EForwardScan : EBackwardScan;
  }
}
...
BOOL CXformMinMax2IndexGet::IsMinMaxAggOnColumn(const IMDType *agg_func_type, CExpression *pexprAggFunc,
  ...
  //      | +--CScalarIdent "a" (0)
  *agg_colref =
      CCastUtils::PcrExtractFromScIdOrCastScId((*(*pexprAggFunc)[EAggfuncChildIndices::EaggfuncIndexArgs])[0]);
...
IMdIdArray *CXformMinMax2IndexGet::GetApplicableIndices(CMemoryPool *mp, const CColRef *agg_colref,
  ...
  for (ULONG ul = 0; ul < ulIndices; ul++) {
    ...
    pdrgpcrIndexColumns = CXformUtils::PdrgpcrIndexKeys(mp, output_col_array, pmdindex, pmdrel);
    ...
    if (pmdindex->IndexType() == IMDIndex::EmdindBtree && CColRef::Equals(agg_colref, (*pdrgpcrIndexColumns)[0])) {
      pmdidIndex->AddRef();
