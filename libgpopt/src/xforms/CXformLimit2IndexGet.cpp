...
//
//---------------------------------------------------------------------------
CXform::EXformPromise CXformLimit2IndexGet::Exfp(CExpressionHandle &exprhdl) const {
...
//
//---------------------------------------------------------------------------
void CXformLimit2IndexGet::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const {
  ...
  popGet->AddRef();
  CExpression *pexprUpdtdRltn = GPOS_NEW(mp) CExpression(mp, popGet, boolConstExpr);
  ...
  for (ULONG ul = 0; ul < ulIndices; ul++) {
    ...
    pdrgpcrIndexColumns = CXformUtils::PdrgpcrIndexKeys(mp, popGet->PdrgpcrOutput(), pmdindex, pmdrel);
    ...
    if (scan_direction != EisdSentinel) {
      ...
      CExpression *pexprIndexGet = CXformUtils::PexprBuildBtreeIndexPlan(
          ...
          pmdindex, pmdrel, scan_direction, true, indexonly);
      ...
      if (pexprIndexGet != nullptr) {
        ...
        CExpression *pexprLimit = GPOS_NEW(mp)
            CExpression(mp,
                        GPOS_NEW(mp) CLogicalLimit(mp, pos, popLimit->FGlobal(),  // fGlobal
...
//---------------------------------------------------------------------------
EIndexScanDirection CXformLimit2IndexGet::GetScanDirection(COrderSpec *pos, CColRefArray *pdrgpcrIndexColumns,
