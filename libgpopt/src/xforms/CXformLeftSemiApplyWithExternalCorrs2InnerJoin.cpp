...
bool CXformLeftSemiApplyWithExternalCorrs2InnerJoin::FDecorrelate(CMemoryPool *mp, CExpression *pexpr,
  ...
  CExpressionArray *pdrgpexprOriginal = CPredicateUtils::PdrgpexprConjuncts(mp, pexprScalar);
...
//
//---------------------------------------------------------------------------
CExpression *CXformLeftSemiApplyWithExternalCorrs2InnerJoin::PexprDecorrelate(CMemoryPool *mp, CExpression *pexpr) {
  GPOS_ASSERT(nullptr != pexpr);
  GPOS_ASSERT(COperator::EopLogicalLeftSemiApply == pexpr->Pop()->Eopid() ||
              COperator::EopLogicalLeftSemiApplyIn == pexpr->Pop()->Eopid());
  ...
  if (nullptr == exprhdl.DeriveKeyCollection(0 /*child_index*/) || !CUtils::FInnerUsesExternalCols(exprhdl)) {
  ...
  if (!FSplitCorrelations(mp, (*pexpr)[0], pexprInnerNew, pdrgpexprAllCorr, &pdrgpexprExternal, &pdrgpexprResidual,
                          &pcrsInnerUsed)) {
  ...
  CExpression *pexprInnerJoin = CUtils::PexprLogicalJoin<CLogicalInnerJoin>(
      mp, pexprOuter, pexprInnerNew, CPredicateUtils::PexprConjunction(mp, pdrgpexprResidual));
  ...
  CColRefArray *pdrgpcrGrpCols = CUtils::PdrgpcrGroupingKey(mp, pexprOuter, &pdrgpcrKey);
  ...
  CExpression *pexprGb = CUtils::PexprLogicalGbAggGlobal(
      mp, pdrgpcrGrpCols, pexprInnerJoin, GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp)));
...
//
//---------------------------------------------------------------------------
CXform::EXformPromise CXformLeftSemiApplyWithExternalCorrs2InnerJoin::Exfp(CExpressionHandle &exprhdl) const {
...
//
//---------------------------------------------------------------------------
void CXformLeftSemiApplyWithExternalCorrs2InnerJoin::Transform(CXformContext *pxfctxt, CXformResult *pxfres,
