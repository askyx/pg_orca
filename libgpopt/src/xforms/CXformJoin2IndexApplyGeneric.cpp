...
bool CXformJoin2IndexApplyGeneric::FCanLeftOuterIndexApply(CMemoryPool *mp, CExpression *pexprInner,
  ...
  if (fCanOuterIndexApply) {
    ...
    for (uint32_t ul = 0; ul < pdrgpexpr->Size(); ul++) {
      ...
      if (!pcrsPred->IsDisjoint(pcrsDist) && CPredicateUtils::IsEqualityOp(pexprPred)) {
...
CXform::EXformPromise CXformJoin2IndexApplyGeneric::Exfp(CExpressionHandle &exprhdl) const {
...
// actual transform
void CXformJoin2IndexApplyGeneric::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const {
  ...
  for (CExpression *pexprCurrInnerChild = pexprInner; nullptr == pexprGet;
       pexprCurrInnerChild = (nullptr == pexprGet ? (*pexprCurrInnerChild)[0] : nullptr)) {
    switch (pexprCurrInnerChild->Pop()->Eopid()) {
      ...
      case COperator::EopLogicalProject:
        ...
        // be used for index predicates.
        {
          ...
          if (COperator::EopLogicalGbAgg == pexprCurrInnerChild->Pop()->Eopid()) {
            CLogicalGbAgg *grbyAggOp = CLogicalGbAgg::PopConvert(pexprCurrInnerChild->Pop());

            GPOS_ASSERT(nullptr != grbyAggOp);
            if (nullptr != grbyAggOp->Pdrgpcr() && 0 < grbyAggOp->Pdrgpcr()->Size()) {
              ...
              CColRefSet *groupingCols = GPOS_NEW(mp) CColRefSet(mp, grbyAggOp->Pdrgpcr());
      ...
      case COperator::EopLogicalGet: {
        CLogicalGet *popGet = CLogicalGet::PopConvert(pexprCurrInnerChild->Pop());
  ...
  CreateHomogeneousIndexApplyAlternatives(mp, pexpr->Pop(), pexprOuter, pexprGet, pexprAllPredicates, pexprScalar,
                                          nodesToInsertAboveIndexGet, endOfNodesToInsertAboveIndexGet, ptabdescInner,
                                          pxfres,
