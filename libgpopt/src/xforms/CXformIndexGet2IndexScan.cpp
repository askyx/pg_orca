...
CXform::EXformPromise CXformIndexGet2IndexScan::Exfp(CExpressionHandle &exprhdl) const {
...
//
//---------------------------------------------------------------------------
void CXformIndexGet2IndexScan::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const {
  ...
  CExpression *pexprAlt =
      GPOS_NEW(mp) CExpression(mp,
                               GPOS_NEW(mp) CPhysicalIndexScan(mp, pindexdesc, ptabdesc, pexpr->Pop()->UlOpId(),
                                                               GPOS_NEW(mp) CName(mp, pop->NameAlias()), pdrgpcrOutput,
                                                               pos, pop->ResidualPredicateSize(), pop->ScanDirection()),
                               pexprIndexCond);
  pxfres->Add(pexprAlt);
}
