...
//
//---------------------------------------------------------------------------
CXformDynamicForeignGet2DynamicForeignScan::CXformDynamicForeignGet2DynamicForeignScan(CMemoryPool *mp)
    : CXformImplementation(
          // pattern
          GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalDynamicForeignGet(mp))) {}
...
//
//---------------------------------------------------------------------------
CXform::EXformPromise CXformDynamicForeignGet2DynamicForeignScan::Exfp(CExpressionHandle &  // exprhdl
) const {
...
//
//---------------------------------------------------------------------------
void CXformDynamicForeignGet2DynamicForeignScan::Transform(CXformContext *pxfctxt, CXformResult *pxfres,
                                                           CExpression *pexpr) const {
  GPOS_ASSERT(nullptr != pxfctxt);
  GPOS_ASSERT(FPromising(pxfctxt->Pmp(), this, pexpr));
  GPOS_ASSERT(FCheckPattern(pexpr));

  CLogicalDynamicForeignGet *popGet = CLogicalDynamicForeignGet::PopConvert(pexpr->Pop());
  ...
  CExpression *pexprAlt =
      GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CPhysicalDynamicForeignScan(
                                       mp, pname, ptabdesc, popGet->UlOpId(), popGet->ScanId(), pdrgpcrOutput,
                                       pdrgpdrgpcrPart, popGet->GetPartitionMdids(), popGet->GetRootColMappingPerPart(),
