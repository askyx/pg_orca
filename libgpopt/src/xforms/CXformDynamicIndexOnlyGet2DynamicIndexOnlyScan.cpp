...
//
//---------------------------------------------------------------------------
CXformDynamicIndexOnlyGet2DynamicIndexOnlyScan::CXformDynamicIndexOnlyGet2DynamicIndexOnlyScan(CMemoryPool *mp)
    : CXformImplementation(
          // pattern
          GPOS_NEW(mp)
              CExpression(mp, GPOS_NEW(mp) CLogicalDynamicIndexOnlyGet(mp),
                          GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CPatternLeaf(mp))  // index lookup predicate
                          )) {}

CXform::EXformPromise CXformDynamicIndexOnlyGet2DynamicIndexOnlyScan::Exfp(CExpressionHandle &exprhdl) const {
  CLogicalDynamicIndexOnlyGet *popGet = CLogicalDynamicIndexOnlyGet::PopConvert(exprhdl.Pop());
...
//
//---------------------------------------------------------------------------
void CXformDynamicIndexOnlyGet2DynamicIndexOnlyScan::Transform(CXformContext *pxfctxt GPOS_ASSERTS_ONLY,
  ...
  CLogicalDynamicIndexOnlyGet *popIndexGet = CLogicalDynamicIndexOnlyGet::PopConvert(pexpr->Pop());
  ...
  pxfres->Add(GPOS_NEW(mp) CExpression(
      ...
      GPOS_NEW(mp) CPhysicalDynamicIndexOnlyScan(
          mp, pindexdesc, ptabdesc, pexpr->Pop()->UlOpId(), GPOS_NEW(mp) CName(mp, popIndexGet->Name()), pdrgpcrOutput,
