...
// ctor
CXformEagerAgg::CXformEagerAgg(CExpression *exprPattern) : CXformExploration(exprPattern) {}

// compute xform promise for a given expression handle;
CXform::EXformPromise CXformEagerAgg::Exfp(CExpressionHandle &exprhdl) const {
  if (GPOS_FTRACE(EopttraceEnableEagerAgg) && CLogicalGbAgg::PopConvert(exprhdl.Pop())->FGlobal()) {
...
void CXformEagerAgg::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *agg_expr) const {
  ...
  //		WHERE t1.j1 = t2.j2
  //		GROUP BY t1.g1;
  CColRefSet *push_down_gb_cols = GPOS_NEW(mp) CColRefSet(mp, *(join_condition_expr->DeriveUsedColumns()));
  ...
  (void)PopulateLowerUpperProjectList(mp, agg_proj_list_expr, &lower_expr_proj_list, &upper_expr_proj_list);
  ...
  CExpression *lower_agg_expr =
      GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalGbAgg(mp, push_down_gb_col_array, COperator::EgbaggtypeLocal),
  ...
  CExpression *new_join_expr =
      GPOS_NEW(mp) CExpression(mp, join_op, lower_agg_expr, join_inner_child_expr, join_condition_expr);

  // upper expression as a global aggregate
  CColRefArray *grouping_col_array = grouping_cols->Pdrgpcr(mp);
  CExpression *upper_agg_expr =
      GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalGbAgg(mp, grouping_col_array, COperator::EgbaggtypeGlobal),
...
void CXformEagerAgg::PopulateLowerUpperProjectList(
  ...
  for (uint32_t ul = 0; ul < num_proj_elements; ul++) {
    ...
    PopulateLowerProjectElement(
        mp, orig_agg_mdid, GPOS_NEW(mp) CWStringConst(mp, orig_agg_func->PstrAggFunc()->GetBuffer()),
        orig_agg_expr->PdrgPexpr(), orig_agg_func->IsDistinct(), orig_agg_func->GetArgTypes(), &lower_proj_elem_expr);
    lower_proj_elem_array->Append(lower_proj_elem_expr);

    CExpression *upper_proj_elem_expr = nullptr;
    PopulateUpperProjectElement(
        mp, orig_agg_mdid, GPOS_NEW(mp) CWStringConst(mp, orig_agg_func->PstrAggFunc()->GetBuffer()),
        CScalarProjectElement::PopConvert(lower_proj_elem_expr->Pop())->Pcr(), orig_proj_elem->Pcr(),
        orig_agg_func->IsDistinct(), orig_agg_func->GetArgTypes(), &upper_proj_elem_expr);
    upper_proj_elem_array->Append(upper_proj_elem_expr);
  }  // end of loop over each project element

  /* 3. create new project lists */
  *lower_proj_list = GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), lower_proj_elem_array);

  *upper_proj_list = GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), upper_proj_elem_array);
}
...
void CXformEagerAgg::PopulateLowerProjectElement(
  ...
  agg_mdid->AddRef();
  arg_types->AddRef();
  CScalarAggFunc *lower_agg_func = CUtils::PopAggFunc(mp, agg_mdid, agg_name, is_distinct, EaggfuncstageLocal, true,
                                                      nullptr, EaggfunckindNormal, arg_types, false);
  ...
  CExpression *lower_agg_expr = GPOS_NEW(mp) CExpression(mp, lower_agg_func, agg_arg_array);
  ...
  IMDId *lower_agg_ret_mdid = md_accessor->RetrieveAgg(agg_mdid)->GetIntermediateResultTypeMdid();
  ...
  *lower_proj_elem_expr = CUtils::PexprScalarProjectElement(mp, lower_colref, lower_agg_expr);
}
...
void CXformEagerAgg::PopulateUpperProjectElement(
  ...
  agg_mdid->AddRef();
  arg_types->AddRef();
  CScalarAggFunc *upper_agg_func = CUtils::PopAggFunc(mp, agg_mdid, agg_name, is_distinct, EaggfuncstageGlobal, true,
                                                      nullptr, EaggfunckindNormal, arg_types, false);
  ...
  upper_agg_arg_array->Append(GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarValuesList(mp), args));
  ...
  CExpression *upper_agg_expr = GPOS_NEW(mp) CExpression(mp, upper_agg_func, upper_agg_arg_array);

  // determine column reference for the new project element
  *upper_proj_elem_expr = CUtils::PexprScalarProjectElement(mp, output_colref, upper_agg_expr);
}
// EOF
