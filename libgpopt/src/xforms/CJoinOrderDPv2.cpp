...
//
//---------------------------------------------------------------------------
CJoinOrderDPv2::CJoinOrderDPv2(CMemoryPool *mp, CExpressionArray *pdrgpexprAtoms, CExpressionArray *innerJoinConjuncts,
                               CExpressionArray *onPredConjuncts, ULongPtrArray *childPredIndexes,
  ...
  for (uint32_t l = 0; l <= m_ulComps; l++) {
  ...
  m_top_k_expressions = GPOS_NEW(mp) CKHeap<SExpressionInfoArray, SExpressionInfo>(mp, GPOPT_DPV2_JOIN_ORDERING_TOPK);
  ...
  m_top_k_part_expressions =
      GPOS_NEW(mp) CKHeap<SExpressionInfoArray, SExpressionInfo>(mp, 1 /* keep top 1 expression */
      );

  m_mp = mp;
  if (0 < m_on_pred_conjuncts->Size()) {
    // we have non-inner joins, add dependency info
    uint32_t numNonInnerJoins = m_on_pred_conjuncts->Size();

    m_non_inner_join_dependencies = GPOS_NEW(mp) CBitSetArray(mp, numNonInnerJoins);
    for (uint32_t ul = 0; ul < numNonInnerJoins; ul++) {
    ...
    for (uint32_t en = 0; en < m_ulEdges; en++) {
      SEdge *pedge = m_rgpedge[en];

      if (0 < pedge->m_loj_num) {
        // edge represents a non-inner join pred
        uint32_t logicalChildNum = FindLogicalChildByNijId(pedge->m_loj_num);
...
//
//---------------------------------------------------------------------------
void CJoinOrderDPv2::ComputeCost(SExpressionInfo *expr_info, CDouble join_cardinality) {
...
//
//---------------------------------------------------------------------------
CExpression *CJoinOrderDPv2::PexprBuildInnerJoinPred(CBitSet *pbsFst, CBitSet *pbsSnd) {
  ...
  for (uint32_t ul = 0; ul < m_ulEdges; ul++) {
    ...
    if (
        ...
        !pbsFst->IsDisjoint(pedge->m_pbs) && !pbsSnd->IsDisjoint(pedge->m_pbs)) {
...
//
//---------------------------------------------------------------------------
CJoinOrderDPv2::SExpressionInfo *CJoinOrderDPv2::GetJoinExprForProperties(SGroupInfo *left_child,
...
//
//---------------------------------------------------------------------------
CJoinOrderDPv2::SExpressionInfo *CJoinOrderDPv2::GetJoinExpr(const SGroupAndExpression &left_child_expr,
...
//
//---------------------------------------------------------------------------
CJoinOrderDPv2::SGroupAndExpression CJoinOrderDPv2::GetBestExprForProperties(SGroupInfo *group_info,
                                                                             SExpressionProperties &props) {
  uint32_t best_ix = UINT32_MAX;
  CDouble best_cost(0.0);

  for (uint32_t ul = 0; ul < group_info->m_best_expr_info_array->Size(); ul++) {
    SExpressionInfo *expr_info = (*group_info->m_best_expr_info_array)[ul];

    if (IsASupersetOfProperties(expr_info->m_properties, props)) {
      if (UINT32_MAX == best_ix || expr_info->GetCost() < best_cost) {
...
//
//---------------------------------------------------------------------------
void CJoinOrderDPv2::PopulateExpressionToEdgeMapIfNeeded() {
  bool populate = false;
  ...
  if (!populate && nullptr != m_child_pred_indexes) {
    ...
    for (uint32_t c = 0; c < m_child_pred_indexes->Size(); c++) {
      if (0 < *((*m_child_pred_indexes)[c])) {
        loj_right_children->ExchangeSet(c);
      }
    }

    for (uint32_t en1 = 0; en1 < m_ulEdges; en1++) {
  ...
  if (populate) {
    ...
    for (uint32_t en2 = 0; en2 < m_ulEdges; en2++) {
...
//
//---------------------------------------------------------------------------
CExpression *CJoinOrderDPv2::AddSelectNodeForRemainingEdges(CExpression *join_expr) {
...
//   1. The benefits of a PS are from the selectivity of a single table, rather than the join result between two tables.
//   We find this table by looking for logical selects.
...
void CJoinOrderDPv2::PopulateDPEInfo(SExpressionInfo *join_expr_info, SGroupInfo *part_table_group_info,
  ...
  while (iter_pt.Advance()) {
    ...
    if (partition_keys == nullptr || partition_keys->Size() == 0 || join_expr_info->m_contain_PS->Get(iter_pt.Bit())) {
      continue;
    }
    ...
    if (!part_table_group_info->IsAnAtom()) {
      CExpression *left_child_join_expr = join_expr_info->m_left_child_expr.GetExprInfo()->m_expr;
      left_child_join_condition_cols = (*left_child_join_expr)[left_child_join_expr->Arity() - 1]->DeriveUsedColumns();
      ...
      CTableDescriptor *atom_table_descriptor = CLogical::PtabdescFromTableGet(popLogical);
      if (nullptr != atom_table_descriptor && nullptr != left_child_join_condition_cols) {
        CColRefArray *atomOutputColArray = pt_atom_expr->DeriveOutputColumns()->Pdrgpcr(m_mp);
        CColRefSet *pt_atom_distribution_cols = CLogical::PcrsDist(m_mp, atom_table_descriptor, atomOutputColArray);
        ...
        if (!left_child_join_condition_cols->ContainsAll(pt_atom_distribution_cols)) {
    ...
    for (uint32_t i = 0; i < partition_keys->Size(); i++) {
      ...
      if ((*(*atom_groups)[iter_ps.Bit()]->m_best_expr_info_array)[0]->m_expr->Pop()->Eopid() ==
        ...
        join_expr_info->m_cost_adj_PS =
            CostJoinWithPartitionSelection(join_expr_info, atom_ps, pt_atom, part_selector_group_info);
        return;
      }
    }
  }
...
CDouble CJoinOrderDPv2::CostJoinWithPartitionSelection(SExpressionInfo *join_expr_info, SExpressionInfo *atom_ps,
  ...
  uint32_t broadcast_threshold = optimizer_config->GetHint()->UlBroadcastThreshold();
  ...
  return join_expr_info->m_cost_adj_PS - (percent_reduction * pt_atom->m_cardinality) + broadcast_cost;
}
//---------------------------------------------------------------------------
//	@function:
...
//
//---------------------------------------------------------------------------
void CJoinOrderDPv2::SearchJoinOrders(uint32_t left_level, uint32_t right_level) {
  GPOS_ASSERT(left_level > 0 && right_level > 0 && left_level + right_level <= m_ulComps);
  ...
  uint32_t left_size = left_group_info_array->Size();
  uint32_t right_size = right_group_info_array->Size();
  for (uint32_t left_ix = 0; left_ix < left_size; left_ix++) {
...
//
//---------------------------------------------------------------------------
void CJoinOrderDPv2::GreedySearchJoinOrders(uint32_t left_level, JoinOrderPropType algo) {
  uint32_t right_level = 1;
  ...
  for (; right_ix < right_size; right_ix++) {
    ...
    if (nullptr != join_expr_info) {
      ...
      if (nullptr == best_expr_info_in_level || join_cost < best_cost_in_level) {
...
//
//---------------------------------------------------------------------------
CJoinOrderDPv2::SGroupInfo *CJoinOrderDPv2::LookupOrCreateGroupInfo(SLevelInfo *levelInfo, CBitSet *atoms,
  ...
  if (nullptr == group_info) {
    ...
    if (!stats_expr_info->m_properties.Satisfies(EJoinOrderStats)) {
      ...
      real_expr_info_for_stats =
          GetJoinExprForProperties(stats_expr_info->m_left_child_expr.m_group_info,
...
//
//---------------------------------------------------------------------------
void CJoinOrderDPv2::FinalizeDPLevel(uint32_t level) {
  GPOS_ASSERT(level >= 2);
  ...
  if (nullptr != level_info->m_top_k_groups) {
    ...
    while (nullptr != (winner = level_info->m_top_k_groups->RemoveBestElement())) {
    ...
    while (nullptr != (loser = level_info->m_top_k_groups->RemoveNextElement())) {
...
//
//---------------------------------------------------------------------------
void CJoinOrderDPv2::SearchBushyJoinOrders(uint32_t current_level) {
  ...
  for (uint32_t right_level = 2; right_level <= current_level / 2; right_level++) {
...
//
//---------------------------------------------------------------------------
void CJoinOrderDPv2::PexprExpand() {
  ...
  for (uint32_t atom_id = 0; atom_id < m_ulComps; atom_id++) {
    ...
    // populate partition keys for atoms. This is an array of partition keys for each atom, NULL for non-partition
    // tables
    ...
    CTableDescriptorHashSet *table_desc_set = pexpr_atom->DeriveTableDescriptor();
...
//
//---------------------------------------------------------------------------
void CJoinOrderDPv2::EnumerateDP() {
  ...
  uint32_t join_order_exhaustive_limit = phint->UlJoinOrderDPLimit();
  ...
  if (join_order_exhaustive_limit < m_ulComps) {
    for (uint32_t l = 2; l <= m_ulComps; l++) {
      uint32_t number_of_allowed_groups = 0;
      ...
      Level(l)->m_top_k_groups = GPOS_NEW(m_mp) CKHeap<SGroupInfoArray, SGroupInfo>(m_mp, number_of_allowed_groups);
    }
  }

  // build n-ary joins from the bottom up, starting with 2-way, 3-way up to m_ulComps-way
  for (uint32_t current_join_level = 2; current_join_level <= m_ulComps; current_join_level++) {
...
//
//---------------------------------------------------------------------------
void CJoinOrderDPv2::EnumerateQuery() {
  if (GPOS_FTRACE(EopttraceGreedyOnlyInDPv2) || GPOS_FTRACE(EopttraceMinCardOnlyInDPv2)) {
    return;
  }
...
//
//---------------------------------------------------------------------------
void CJoinOrderDPv2::FindLowestCardTwoWayJoin(JoinOrderPropType prop_type) {
  ...
  for (uint32_t ul = 0; ul < level_2->m_groups->Size(); ul++) {
    ...
    if (EJoinOrderGreedyAvoidXProd == prop_type && !CUtils::IsHashJoinPossible(m_mp, first_expr)) {
...
//
//---------------------------------------------------------------------------
void CJoinOrderDPv2::EnumerateGreedyAvoidXProd() {
...
//
//---------------------------------------------------------------------------
CExpression *CJoinOrderDPv2::GetNextOfTopK() {
...
//
//---------------------------------------------------------------------------
bool CJoinOrderDPv2::IsRightChildOfNIJ(SGroupInfo *groupInfo, CExpression **onPredToUse, CBitSet **requiredBitsOnLeft) {
  GPOS_ASSERT(nullptr == onPredToUse || nullptr == *onPredToUse);
  GPOS_ASSERT(nullptr == requiredBitsOnLeft || nullptr == *requiredBitsOnLeft);
  ...
  uint32_t childPredIndex = *(*m_child_pred_indexes)[iter.Bit()];
...
//
//---------------------------------------------------------------------------
uint32_t CJoinOrderDPv2::FindLogicalChildByNijId(uint32_t nij_num) {
...
//
//---------------------------------------------------------------------------
uint32_t CJoinOrderDPv2::NChooseK(uint32_t n, uint32_t k) {
...
//
//---------------------------------------------------------------------------
bool CJoinOrderDPv2::LevelIsFull(uint32_t level) {
...
IOstream &CJoinOrderDPv2::OsPrint(IOstream &os) const {
  ...
  for (uint32_t lev = 1; lev < num_levels; lev++) {
    ...
    for (uint32_t c = 0; c < num_bitsets_this_level; c++) {
      ...
      for (uint32_t x = 0; x < num_exprs; x++) {
...
IOstream &CJoinOrderDPv2::OsPrintProperty(IOstream &os, SExpressionProperties &props) {
  os << "{ ";
