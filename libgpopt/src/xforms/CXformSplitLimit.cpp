...
//
//---------------------------------------------------------------------------
CXform::EXformPromise CXformSplitLimit::Exfp(CExpressionHandle &exprhdl) const {
...
//
//---------------------------------------------------------------------------
void CXformSplitLimit::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const {
  ...
  CExpression *pexprLimitLocal = PexprLimit(mp, pexprRelational, pexprScalarStart, pexprScalarRows, pos,
                                            false,  // fGlobal
                                            popLimit->FHasCount(), popLimit->IsTopLimitUnderDMLorCTAS());

  // assemble global limit operator
  CExpression *pexprLimitGlobal = PexprLimit(mp, pexprLimitLocal, pexprScalarStart, pexprScalarRows, pos,
                                             true,  // fGlobal
                                             popLimit->FHasCount(), popLimit->IsTopLimitUnderDMLorCTAS());
...
CExpression *CXformSplitLimit::PexprLimit(CMemoryPool *mp, CExpression *pexprRelational, CExpression *pexprScalarStart,
  ...
  CExpression *pexprLimit =
      GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalLimit(mp, pos, fGlobal, fHasCount, fTopLimitUnderDML),
