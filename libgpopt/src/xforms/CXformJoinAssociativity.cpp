...
//
//---------------------------------------------------------------------------
void CXformJoinAssociativity::CreatePredicates(CMemoryPool *mp, CExpression *pexpr, CExpressionArray *pdrgpexprLower,
...
//
//---------------------------------------------------------------------------
CXform::EXformPromise CXformJoinAssociativity::Exfp(CExpressionHandle &exprhdl) const {
  if (GPOPT_MAX_JOIN_DEPTH_FOR_ASSOCIATIVITY < exprhdl.DeriveJoinDepth() ||  // disallow xform beyond max join depth
      GPOPT_MAX_JOIN_RIGHT_CHILD_DEPTH_FOR_ASSOCIATIVITY <
          exprhdl.DeriveJoinDepth(1)  // disallow xform if input is not a left deep tree
  ) {
...
void CXformJoinAssociativity::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const {
  ...
  if (!fOutputLeftIsCrossJoin || fInputLeftIsCrossJoin) {
    ...
    CExpression *pexprBottomJoin = CUtils::PexprLogicalJoin<CLogicalInnerJoin>(
        mp, pexprLeftLeft, pexprRight, CPredicateUtils::PexprConjunction(mp, pdrgpexprLower));

    CExpression *pexprResult = CUtils::PexprLogicalJoin<CLogicalInnerJoin>(
        mp, pexprBottomJoin, pexprLeftRight, CPredicateUtils::PexprConjunction(mp, pdrgpexprUpper));
