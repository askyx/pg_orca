...
//
//---------------------------------------------------------------------------
CGroupExpression *CBinding::PgexprNext(CGroup *pgroup, CGroupExpression *pgexpr) {
...
CExpression *CBinding::PexprExpandPattern(CExpression *pexprPattern, ULONG ulPos, ULONG arity) {
  ...
  if (0 < pexprPattern->Arity() && CPattern::FMultiNode((*pexprPattern)[0]->Pop())) {
...
//
//---------------------------------------------------------------------------
CExpression *CBinding::PexprFinalize(CMemoryPool *mp, CGroupExpression *pgexpr, CExpressionArray *pdrgpexpr) {
  COperator *pop = pgexpr->Pop();

  pop->AddRef();
  CExpression *pexpr =
      GPOS_NEW(mp) CExpression(mp, pop, pgexpr, pdrgpexpr, nullptr /* prpp */, nullptr /*input_stats*/);
...
//
//---------------------------------------------------------------------------
CExpression *CBinding::PexprExtract(CMemoryPool *mp, CGroupExpression *pgexpr, CExpression *pexprPattern,
  ...
  for (ULONG ul = 0; ul < arity; ul++) {
    ...
    if (fCursorAdvanced) {
      ...
      if (nullptr == pexprNewChild) {
        ...
        pexprNewChild = PexprExtract(mp, pgroup, pexprPatternChild, nullptr /*pexprLastChild*/);
        ulExhaustedCursors++;
      } else {
...
//
//---------------------------------------------------------------------------
CExpression *CBinding::PexprExtract(CMemoryPool *mp, CGroup *pgroup, CExpression *pexprPattern,
                                    CExpression *pexprLast) {
  GPOS_CHECK_STACK_SIZE;
  GPOS_CHECK_ABORT;
  ...
  do {
    if (pexprPattern->FMatchPattern(pgexpr)) {
      CExpression *pexprResult = PexprExtract(mp, pgexpr, pexprPattern, pexprStart);
