...
//
//---------------------------------------------------------------------------
CMemo::CMemo(CMemoryPool *mp) : m_mp(mp), m_aul(0), m_pgroupRoot(nullptr), m_ulpGrps(0), m_pmemotmap(nullptr) {
  GPOS_ASSERT(nullptr != mp);

  m_sht.Init(mp, GPOPT_MEMO_HT_BUCKETS, GPOS_OFFSET(CGroupExpression, m_linkMemo),
             0, /*cKeyOffset (0 because we use CGroupExpression class as key)*/
             &(CGroupExpression::m_gexprInvalid), CGroupExpression::HashValue, CGroupExpression::Equals);
...
//
//---------------------------------------------------------------------------
CGroup *CMemo::PgroupInsert(CGroup *pgroupTarget, CExpression *pexprOrigin, CGroupExpression *pgexpr) {
  GPOS_ASSERT(nullptr != pgexpr);
  GPOS_CHECK_ABORT;
  GPOS_ASSERT(nullptr != pexprOrigin);
...
CExpression *CMemo::PexprExtractPlan(CMemoryPool *mp, CGroup *pgroupRoot, CReqdPropPlan *prppInput,
  ...
  for (ULONG i = 0; i < arity; i++) {
    ...
    CExpression *pexprChild = PexprExtractPlan(mp, pgroupChild, prppChild, ulSearchStages);
  ...
  CExpression *pexpr = GPOS_NEW(mp) CExpression(mp, pgexprBest->Pop(), pgexprBest, pdrgpexpr, prpp, stats, cost);
...
//
//---------------------------------------------------------------------------
IOstream &CMemo::OsPrint(IOstream &os) const {
  CGroup *pgroup = m_listGroups.PtFirst();
...
//
//---------------------------------------------------------------------------
void CMemo::DeriveStatsIfAbsent(CMemoryPool *pmpLocal) {
  CGroup *pgroup = m_listGroups.PtFirst();
