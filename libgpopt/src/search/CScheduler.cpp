...
#ifdef GPOS_DEBUG
  if (FTrackingJobs()) {
    GPOS_ASSERT(CJob::EjsInit == pj->Ejs() || CJob::EjsRunning == pj->Ejs() || CJob::EjsSuspended == pj->Ejs());
...
void CScheduler::PreExecute(CJob *pj) {
  GPOS_ASSERT(nullptr != pj);
  GPOS_ASSERT(0 == pj->UlpRefs() && "Runnable job cannot have pending children");
...
//
//---------------------------------------------------------------------------
void CScheduler::PrintStats() const {
  GPOS_TRACE_FORMAT(
      "Job statistics: Queued=%d Dequeued=%d Suspended=%d "
      "Resumed=%d CompletedQueued=%d Completed=%d",
      m_ulpStatsQueued, m_ulpStatsDequeued, m_ulpStatsSuspended, m_ulpStatsResumed, m_ulpStatsCompletedQueued,
      m_ulpStatsCompleted);
}
...
//
//---------------------------------------------------------------------------
IOstream &CScheduler::OsPrintActiveJobs(IOstream &os) {
