...
class CLogicalLeftSemiCorrelatedApply : public CLogicalLeftSemiApply {
 private:
 public:
  CLogicalLeftSemiCorrelatedApply(const CLogicalLeftSemiCorrelatedApply &) = delete;

  // ctor for patterns
  explicit CLogicalLeftSemiCorrelatedApply(CMemoryPool *mp);

  // ctor
  CLogicalLeftSemiCorrelatedApply(CMemoryPool *mp, CColRefArray *pdrgpcrInner, EOperatorId eopidOriginSubq);

  // dtor
  ~CLogicalLeftSemiCorrelatedApply() override = default;

  // ident accessors
  EOperatorId Eopid() const override { return EopLogicalLeftSemiCorrelatedApply; }

  // return a string for operator name
  const CHAR *SzId() const override { return "CLogicalLeftSemiCorrelatedApply"; }
  ...
  // conversion function
  static CLogicalLeftSemiCorrelatedApply *PopConvert(COperator *pop) {
