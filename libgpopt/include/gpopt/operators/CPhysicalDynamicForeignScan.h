...
class CPhysicalDynamicForeignScan : public CPhysicalDynamicScan {
 ...
 public:
  ...
  // ctor
  CPhysicalDynamicForeignScan(CMemoryPool *mp, const CName *pnameAlias, CTableDescriptor *ptabdesc, ULONG ulOriginOpId,
                              ULONG scan_id, CColRefArray *pdrgpcrOutput, CColRef2dArray *pdrgpdrgpcrParts,
                              IMdIdArray *partition_mdids, ColRefToUlongMapArray *root_col_mapping_per_part,
                              OID foreign_server_oid, IMDRelation::Ereldistrpolicy exec_location);

  EOperatorId Eopid() const override { return EopPhysicalDynamicForeignScan; }

  // return a string for operator name
  const CHAR *SzId() const override { return "CPhysicalDynamicForeignScan"; }
  ...
  OID GetForeignServerOid() const { return m_foreign_server_oid; }

  // This is the execution location of the foreign table. See
  IMDRelation::Ereldistrpolicy DistributionType() const { return m_exec_location; }
  //-------------------------------------------------------------------------------------
  ...
  // conversion function
  static CPhysicalDynamicForeignScan *PopConvert(COperator *pop) {
