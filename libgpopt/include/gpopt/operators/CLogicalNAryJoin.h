...
class CLogicalNAryJoin : public CLogicalJoin {
 ...
 public:
  ...
  // dtor
  ~CLogicalNAryJoin() override { CRefCount::SafeRelease(m_lojChildPred); }
  ...
  // conversion function
  static CLogicalNAryJoin *PopConvert(COperator *pop) {
  ...
  ULongPtrArray *GetLojChildPredIndexes() const { return m_lojChildPred; }

  CExpression *GetInnerJoinPreds(CExpression *nary_join_expr) const {
  ...
  CExpression *GetOnPredicateForLOJChild(CExpression *nary_join_expr, ULONG child_num) const {
    ...
    return (*((*nary_join_expr)[nary_join_expr->Arity() - 1]))[*(*m_lojChildPred)[child_num]];
  }

  // get the true inner join predicates, excluding predicates that use ColRefs
  // coming from non-inner joins
  CExpression *GetTrueInnerJoinPreds(CMemoryPool *mp, CExpressionHandle &exprhdl) const;
  ...
  CExpression *ReplaceInnerJoinPredicates(CMemoryPool *mp, CExpression *old_nary_join_scalar_expr,
