...
class CLogicalIntersectAll : public CLogicalSetOp {
 ...
 public:
  ...
  CLogicalIntersectAll(CMemoryPool *mp, CColRefArray *pdrgpcrOutput, CColRef2dArray *pdrgpdrgpcrInput);

  // dtor
  ~CLogicalIntersectAll() override;

  // ident accessors
  EOperatorId Eopid() const override { return EopLogicalIntersectAll; }

  // return a string for operator name
  const char *SzId() const override { return "CLogicalIntersectAll"; }
  ...
  // conversion function
  static CLogicalIntersectAll *PopConvert(COperator *pop) {
  ...
  // derive statistics
  static IStatistics *PstatsDerive(CMemoryPool *mp, CExpressionHandle &exprhdl, CColRef2dArray *pdrgpdrgpcrInput,
