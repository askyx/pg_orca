...
class CLogicalRightOuterJoin : public CLogicalJoin {
 ...
 public:
  ...
  // ctor
  explicit CLogicalRightOuterJoin(CMemoryPool *mp, CXform::EXformId origin_xform = CXform::ExfSentinel);

  // dtor
  ~CLogicalRightOuterJoin() override = default;

  // ident accessors
  EOperatorId Eopid() const override { return EopLogicalRightOuterJoin; }

  // return a string for operator name
  const char *SzId() const override { return "CLogicalRightOuterJoin"; }

  // return true if we can pull projections up past this operator from its given child
  bool FCanPullProjectionsUp(uint32_t child_index) const override { return (1 == child_index); }
  ...
  // conversion function
  static CLogicalRightOuterJoin *PopConvert(COperator *pop) {
