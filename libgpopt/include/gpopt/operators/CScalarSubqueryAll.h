...
class CScalarSubqueryAll : public CScalarSubqueryQuantified {
 ...
 public:
  ...
  // ctor
  CScalarSubqueryAll(CMemoryPool *mp, IMDId *scalar_op_mdid, const CWStringConst *pstrScalarOp, const CColRef *colref);

  // dtor
  ~CScalarSubqueryAll() override = default;

  // ident accessors
  EOperatorId Eopid() const override { return EopScalarSubqueryAll; }

  // return a string for scalar subquery
  const CHAR *SzId() const override { return "CScalarSubqueryAll"; }

  // return a copy of the operator with remapped columns
  COperator *PopCopyWithRemappedColumns(CMemoryPool *mp, UlongToColRefMap *colref_mapping, BOOL must_exist) override;

  // conversion function
  static CScalarSubqueryAll *PopConvert(COperator *pop) {
