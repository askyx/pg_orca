...
class CLogicalDynamicIndexOnlyGet : public CLogicalDynamicIndexGet {
 public:
  ...
  // ctors
  explicit CLogicalDynamicIndexOnlyGet(CMemoryPool *mp) : CLogicalDynamicIndexGet(mp) {}

  CLogicalDynamicIndexOnlyGet(CMemoryPool *mp, const IMDIndex *pmdindex, CTableDescriptor *ptabdesc, ULONG ulOriginOpId,
                              const CName *pnameAlias, ULONG ulPartIndex, CColRefArray *pdrgpcrOutput,
                              CColRef2dArray *pdrgpdrgpcrPart, IMdIdArray *partition_mdids,
                              ULONG ulUnindexedPredColCount)
      : CLogicalDynamicIndexGet(mp, pmdindex, ptabdesc, ulOriginOpId, pnameAlias, ulPartIndex, pdrgpcrOutput,
                                pdrgpdrgpcrPart, partition_mdids, ulUnindexedPredColCount) {}

  // ident accessors
  EOperatorId Eopid() const override { return EopLogicalDynamicIndexOnlyGet; }

  const CHAR *SzId() const override { return "CLogicalDynamicIndexOnlyGet"; }

  // match function
  BOOL Matches(COperator *pop) const override { return CUtils::FMatchDynamicIndex(this, pop); }
  ...
  CXformSet *PxfsCandidates(CMemoryPool *mp) const override {
    ...
    (void)xform_set->ExchangeSet(CXform::ExfDynamicIndexOnlyGet2DynamicIndexOnlyScan);
  ...
  static CLogicalDynamicIndexOnlyGet *PopConvert(COperator *pop) {
