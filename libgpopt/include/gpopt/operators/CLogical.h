...
enum EIndexScanDirection { EBackwardScan = 0, EForwardScan, EisdSentinel };
// forward declaration
class CColRefSet;
...
class CLogical : public COperator {
 ...
 protected:
  ...
  IStatistics *PstatsDeriveDummy(CMemoryPool *mp, CExpressionHandle &exprhdl, CDouble rows) const;
  ...
  static CColRefSet *PcrsDeriveNotNullPassThruOuter(CExpressionHandle &exprhdl);

  // helper for common case of output derivation from all logical children
  static CColRefSet *PcrsDeriveOutputCombineLogical(CMemoryPool *mp, CExpressionHandle &exprhdl);

  // helper for common case of combining not nullable columns from all logical children
  static CColRefSet *PcrsDeriveNotNullCombineLogical(CMemoryPool *mp, CExpressionHandle &exprhdl);
  ...
  static CKeyCollection *PkcKeysBaseTable(CMemoryPool *mp, const CBitSetArray *pdrgpbsKeys,
  ...
  static CPartInfo *PpartinfoDeriveCombine(CMemoryPool *mp, CExpressionHandle &exprhdl);

  // derive constraint property from a table/index get
  static CPropConstraint *PpcDeriveConstraintFromTable(CMemoryPool *mp, const CTableDescriptor *ptabdesc,
                                                       const CColRefArray *pdrgpcrOutput);

  // derive constraint property from a table/index get with predicates
  static CPropConstraint *PpcDeriveConstraintFromTableWithPredicates(CMemoryPool *mp, CExpressionHandle &exprhdl,
  ...
  static CFunctionProp *PfpDeriveFromScalar(CMemoryPool *mp, CExpressionHandle &exprhdl);

  // derive outer references
  static CColRefSet *DeriveOuterReferences(CMemoryPool *mp, CExpressionHandle &exprhdl, CColRefSet *pcrsUsedAdditional);
 ...
 public:
  ...
  CColRefSet *PcrsLocalUsed() const { return m_pcrsLocalUsed; }
  ...
  virtual CColRefSet *DeriveOuterReferences(CMemoryPool *mp, CExpressionHandle &exprhdl) {
    return DeriveOuterReferences(mp, exprhdl, nullptr /*pcrsUsedAdditional*/);
  }
  ...
  // derive statistics
  virtual IStatistics *PstatsDerive(CMemoryPool *mp, CExpressionHandle &exprhdl,
  ...
  static IStatistics *PstatsBaseTable(CMemoryPool *mp, CExpressionHandle &exprhdl, CTableDescriptor *ptabdesc,
                                      CColRefSet *pcrsStatExtra = nullptr);

  // conversion function
  static CLogical *PopConvert(COperator *pop) {
  ...
  static CColRefSet *PcrsDist(CMemoryPool *mp, const CTableDescriptor *ptabdesc, const CColRefArray *colref_array);

  // derive constraint property when expression has relational children and predicates
  static CPropConstraint *PpcDeriveConstraintFromPredicates(CMemoryPool *mp, CExpressionHandle &exprhdl);
