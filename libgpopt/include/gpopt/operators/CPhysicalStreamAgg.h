...
class CPhysicalStreamAgg : public CPhysicalAgg {
 private:
  ...
  static COrderSpec *PosCovering(CMemoryPool *mp, COrderSpec *posRequired, CColRefArray *pdrgpcrGrp);

 protected:
  // compute required sort columns of the n-th child
  COrderSpec *PosRequiredStreamAgg(CMemoryPool *mp, CExpressionHandle &exprhdl, COrderSpec *posRequired,
 ...
 public:
  ...
  // ctor
  CPhysicalStreamAgg(CMemoryPool *mp, CColRefArray *colref_array,
  ...
  // ident accessors
  EOperatorId Eopid() const override { return EopPhysicalStreamAgg; }

  // return a string for operator name
  const CHAR *SzId() const override { return "CPhysicalStreamAgg"; }
  ...
  // conversion function
  static CPhysicalStreamAgg *PopConvert(COperator *pop) {
    GPOS_ASSERT(nullptr != pop);
    GPOS_ASSERT(EopPhysicalStreamAgg == pop->Eopid() || EopPhysicalStreamAggDeduplicate == pop->Eopid());
