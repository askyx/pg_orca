...
class CPropConstraint : public CRefCount {
 ...
 public:
  ...
  // ctor
  CPropConstraint(CMemoryPool *mp, CColRefSetArray *pdrgpcrs, CConstraint *pcnstr);

  // dtor
  ~CPropConstraint() override;

  // equivalence classes
  CColRefSetArray *PdrgpcrsEquivClasses() const { return m_pdrgpcrs; }

  // mapping
  CColRefSet *PcrsEquivClass(const CColRef *colref) const {
  ...
  CExpression *PexprScalarMappedFromEquivCols(CMemoryPool *mp, CColRef *colref,
...
inline IOstream &operator<<(IOstream &os, CPropConstraint &pc) {
