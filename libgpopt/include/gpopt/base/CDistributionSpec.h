...
class CDistributionSpec : public CPropSpec {
 public:
  ...
  enum EDistributionType {
    ...
    EdtHashedNoOp,    // same as hashed, used to force multiple slices for parallel union all. The motions always mirror
                      // the underlying distributions.
    EdtStrictHashed,  // same as hashed, used to force multiple slices for parallel union all. The motions mirror the
                      // distribution of the output columns.
    ...
    EdtTaintedReplicated,  // data once-replicated, after being processed by an input-order-sensitive operator or
                           // volatile function (derived only)
    EdtAny,                // data can be anywhere on the segments (required only)
    EdtSingleton,          // data is on a single segment or the coordinator
    EdtStrictSingleton,    // data is on a single segment or the coordinator (derived only, only compatible with other
 ...
 public:
  ...
  // property type
  EPropSpecType Epst() const override { return EpstDistribution; }

  // return true if distribution spec can be required
  virtual BOOL FRequirable() const { return true; }

  // return true if distribution spec can be derived
  virtual BOOL FDerivable() const { return true; }

  // default match function for distribution specs
  virtual BOOL Matches(const CDistributionSpec *pds) const { return Edt() == pds->Edt(); }
  ...
  virtual BOOL Equals(const CDistributionSpec *pds) const { return Matches(pds); }

  // return a copy of the distribution spec with remapped columns
  virtual CDistributionSpec *PdsCopyWithRemappedColumns(CMemoryPool *,       // mp,
  ...
  BOOL FSingletonOrStrictSingleton() const {
...
using CDistributionSpecArray = CDynamicPtrArray<CDistributionSpec, CleanupRelease>;
}  // namespace gpopt
