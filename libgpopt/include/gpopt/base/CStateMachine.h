...
#define GRAPHVIZ_SHAPE(shape, x) "node [shape = " << shape << " ]; \"" << x << "\" ; node [shape = circle];"
...
#define GRAPHVIZ_BOX(x) GRAPHVIZ_SHAPE("box", x)
  ...
  using PfWalker = void (*)(const CStateMachine *, TEnumState, TEnumState, TEnumEvent, void *);
  ...
  static void Diagram(const CStateMachine *psm, TEnumState tenumstateSource, TEnumState tenumstateSink,
                      TEnumEvent tenumevent, void *pvContext) {
    IOstream &os = *(IOstream *)pvContext;

    os << "\"" << psm->WszState(tenumstateSource) << "\" -> \"" << psm->WszState(tenumstateSink) << "\" [ label = \""
       << psm->WszEvent(tenumevent) << "\" ];" << std::endl;
  }

  // check for unreachable nodes -- used with walker
  static void Unreachable(const CStateMachine *, TEnumState tenumstateSource, TEnumState tenumstateSink, TEnumEvent,
                          void *pvContext) {
 ...
 public:
  CStateMachine(const CStateMachine<TEnumState, tenumstateSentinel, TEnumEvent, tenumeventSentinel> &) = delete;
  ...
  TEnumState Estate() const { return m_tenumstate; }

  // get initial state
  TEnumState TesInitial() const { return (TEnumState)0; }

  // get final state
  TEnumState TesFinal() const { return (TEnumState)(tenumstateSentinel - 1); }
