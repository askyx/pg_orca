...
class CDistributionSpecAny : public CDistributionSpec {
 ...
 public:
  ...
  // ctor
  CDistributionSpecAny(COperator::EOperatorId eopidRequested, BOOL fAllowOuterRefs)
      : m_eopidRequested(eopidRequested), m_fAllowOuterRefs(fAllowOuterRefs) {}

  // accessor
  EDistributionType Edt() const override { return CDistributionSpec::EdtAny; }

  // does current distribution satisfy the given one
  BOOL FSatisfies(const CDistributionSpec *pds) const override { return EdtAny == pds->Edt(); }
  ...
  // print
  IOstream &OsPrint(IOstream &os) const override { return os << "ANY " << " EOperatorId: " << m_eopidRequested << " "; }

  // return distribution partitioning type
  EDistributionPartitioningType Edpt() const override { return EdptUnknown; }

  // allow outer references in the operator tree where distribution is requested
  BOOL FAllowOuterRefs() const { return m_fAllowOuterRefs; }

  // conversion function
  static CDistributionSpecAny *PdsConvert(CDistributionSpec *pds) {
  ...
  COperator::EOperatorId GetRequestedOperatorId() const { return m_eopidRequested; }
