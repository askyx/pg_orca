...
class CDistributionSpecSingleton : public CDistributionSpec {
 public:
  // type of segment
  enum ESegmentType { EstCoordinator, EstSegment, EstSentinel };
 ...
 public:
  ...
  EDistributionType Edt() const override { return CDistributionSpec::EdtSingleton; }

  // segment type accessor
  ESegmentType Est() const { return m_est; }

  // is this a coordinator-only distribution
  BOOL FOnCoordinator() const { return EstCoordinator == m_est; }
  ...
  HashValue() const override {
    ...
    return gpos::CombineHashes(gpos::HashValue<ULONG>(&ulEdt), gpos::HashValue<BOOL>(&fOnCoordinator));
  }

  // match function for singleton distribution specs
  BOOL Matches(const CDistributionSpec *pds) const override {
    return Edt() == pds->Edt() &&
           FOnCoordinator() == dynamic_cast<const CDistributionSpecSingleton *>(pds)->FOnCoordinator();
  }
  ...
  // conversion function
  static CDistributionSpecSingleton *PdssConvert(CDistributionSpec *pds) {
  ...
  // conversion function
  static const CDistributionSpecSingleton *PdssConvert(const CDistributionSpec *pds) {
    GPOS_ASSERT(nullptr != pds);
    GPOS_ASSERT(EdtSingleton == pds->Edt() || EdtStrictSingleton == pds->Edt());
