...
class CRange : public CRefCount {
 ...
 private:
  ...
  static CExpression *PexprScalarCompEnd(CMemoryPool *mp, IDatum *datum, ERangeInclusion eri,
                                         IMDType::ECmpType ecmptIncl, IMDType::ECmpType ecmptExcl,
                                         const CColRef *colref);

  // inverse of the inclusion option
  static ERangeInclusion EriInverseInclusion(ERangeInclusion eri) {
    if (EriIncluded == eri) {
      return EriExcluded;
    }
    return EriIncluded;
 ...
 public:
  ...
  // ctor
  CRange(IMDId *mdid, const IComparator *pcomp, IDatum *pdatumLeft, ERangeInclusion eriLeft, IDatum *pdatumRight,
  ...
  // range beginning
  IDatum *PdatumLeft() const { return m_pdatumLeft; }

  // range end
  IDatum *PdatumRight() const { return m_pdatumRight; }

  // left end inclusion
  ERangeInclusion EriLeft() const { return m_eriLeft; }
...
inline IOstream &operator<<(IOstream &os, CRange &range) {
