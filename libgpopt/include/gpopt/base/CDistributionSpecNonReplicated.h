...
class CDistributionSpecNonReplicated : public CDistributionSpecSingleton {
 ...
 public:
  ...
  // accessor
  EDistributionType Edt() const override { return CDistributionSpec::EdtNonReplicated; }

  // does current distribution satisfy the given one
  BOOL FSatisfies(const CDistributionSpec *pds GPOS_UNUSED) const override {
  ...
  // print
  IOstream &OsPrint(IOstream &os) const override { return os << "NON-REPLICATED"; }
