...
using ReqdPropPlanToCostMap =
    CHashMap<CReqdPropPlan, CCost, CReqdPropPlan::UlHashForCostBounding, CReqdPropPlan::FEqualForCostBounding,
...
class CGroup : public CRefCount, public DbgPrintMixin<CGroup> {
 ...
 private:
  ...
  using ShtIter = CSyncHashtableIter<COptimizationContext, COptimizationContext>;

  // definition of hash table iter accessor
  using ShtAccIter = CSyncHashtableAccessByIter<COptimizationContext, COptimizationContext>;

  // definition of hash table accessor
  using ShtAcc = CSyncHashtableAccessByKey<COptimizationContext, COptimizationContext>;
  ...
  using OptCtxtToIStatisticsMap =
      CHashMap<COptimizationContext, IStatistics, COptimizationContext::UlHashForStats,
               COptimizationContext::FEqualForStats, CleanupRelease<COptimizationContext>, CleanupRelease<IStatistics>>;
  ...
  // set hash join keys
  void SetJoinKeys(CExpressionArray *pdrgpexprOuter, CExpressionArray *pdrgpexprInner, IMdIdArray *join_opfamilies);
  ...
  CGroupExpression *PgexprBestPromise(CMemoryPool *pmpLocal, CMemoryPool *pmpGlobal, CReqdPropRelational *prprelInput,
 ...
 public:
  ...
  CExpressionArray *PdrgpexprJoinKeysInner() const { return m_pdrgpexprJoinKeysInner; }

  // join op families
  IMdIdArray *JoinOpfamilies() const { return m_join_opfamilies; }
  ...
  CJobQueue *PjqExploration() { return &m_jqExploration; }
  ...
  static INT Compare(const void *left, const void *right) {
    ...
    GPOS_ASSERT((*((CGroup **)left))->Id() == ((*(CGroup **)right))->Id());
    return 0;
  }
