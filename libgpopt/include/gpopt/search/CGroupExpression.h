...
class CGroupExpression : public CRefCount {
 ...
 private:
  ...
  using PartialPlanToCostMap = CHashMap<CPartialPlan, CCost, CPartialPlan::HashValue, CPartialPlan::Equals,
  ...
  void PostprocessTransform(CMemoryPool *pmpLocal, CMemoryPool *pmpGlobal, CXform *pxform) const;
 ...
 public:
  ...
  CGroupExpression *PgexprDuplicate() const { return m_pgexprDuplicate; }
  ...
  // origin xform
  CXform::EXformId ExfidOrigin() const { return m_exfidOrigin; }

  // origin group expression
  CGroupExpression *PgexprOrigin() const { return m_pgexprOrigin; }
