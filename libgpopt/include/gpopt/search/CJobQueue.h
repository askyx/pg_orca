...
class CJobQueue {
 ...
 public:
  ...
  enum EJobQueueResult { EjqrMain = 0, EjqrQueued, EjqrCompleted };

  // ctor
  CJobQueue() { m_listjQueued.Init(GPOS_OFFSET(CJob, m_linkQueue)); }

  // dtor
  ~CJobQueue() {
    GPOS_ASSERT_IMP(nullptr != ITask::Self() && !ITask::Self()->HasPendingExceptions(), m_listjQueued.IsEmpty());
  }
