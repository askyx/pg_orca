...
class CTranslatorDXLToExpr {
 private:
  ...
  CExpression *PexprCastPrjElem(IMDId *pmdidSource, IMDId *mdid_dest, const CColRef *pcrToCast, CColRef *pcrToReturn);
  ...
  CExpressionArray *PdrgpexprPreprocessSetOpInputs(const CDXLNode *dxlnode, CColRef2dArray *pdrgdrgpcrInput,
  ...
  // translate the DXL representation of window frame exclusion strategy into its respective representation in the
  // optimizer
  static CWindowFrame::EFrameExclusionStrategy Efes(EdxlFrameExclusionStrategy edxlfeb);
  ...
  static CScalarArrayRefIndexList::EIndexListType Eilt(const CDXLScalarArrayRefIndexList::EIndexListBound eilb);
 ...
 public:
  ...
  CExpression *PexprTranslateQuery(const CDXLNode *dxlnode, const CDXLNodeArray *query_output_dxlnode_array,
                                   const CDXLNodeArray *cte_producers);

  // translate a dxl scalar expression
  CExpression *PexprTranslateScalar(const CDXLNode *dxlnode, CColRefArray *colref_array,
