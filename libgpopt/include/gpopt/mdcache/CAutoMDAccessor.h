...
class CAutoMDAccessor : public CStackObject {
 ...
 public:
  ...
  CAutoMDAccessor(CMemoryPool *mp, IMDProvider *pmdp, CSystemId sysid)
    ...
    m_pcache = CCacheFactory::CreateCache<gpmd::IMDCacheObject *, gpopt::CMDKey *>(
        true /*fUnique*/, 0 /* unlimited cache quota */, gpopt::CMDKey::UlHashMDKey, gpopt::CMDKey::FEqualMDKey);
    m_pmda = GPOS_NEW(mp) CMDAccessor(mp, m_pcache, sysid, pmdp);
  }

  // ctor
  CAutoMDAccessor(CMemoryPool *mp, IMDProvider *pmdp, CSystemId sysid, CMDAccessor::MDCache *pcache)
  ...
  // accessor of cache
  CMDAccessor::MDCache *Pcache() const { return m_pcache; }

  // accessor of metadata accessor
  CMDAccessor *Pmda() const { return m_pmda; }

  // accessor of metadata provider
  IMDProvider *Pimdp() const { return m_pimdp; }

  // accessor of system id
  CSystemId Sysid() const { return m_sysid; }
};
}  // namespace gpopt
