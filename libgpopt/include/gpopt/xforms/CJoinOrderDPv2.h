...
//---------------------------------------------------------------------------
class CJoinOrderDPv2 : public CJoinOrder, public gpos::DbgPrintMixin<CJoinOrderDPv2> {
 private:
  ...
  struct SExpressionInfo : public CRefCount {
    ...
    SExpressionInfo(CMemoryPool *mp, CExpression *expr, const SGroupAndExpression &left_child_expr_info,
                    const SGroupAndExpression &right_child_expr_info, SExpressionProperties &properties)
        : m_expr(expr),
    ...
    SExpressionInfo(CMemoryPool *mp, CExpression *expr, SExpressionProperties &properties)
        : m_expr(expr),
          m_properties(properties),
          m_atom_part_keys_array(nullptr),
          m_cost(0.0),
    ...
    CDouble GetCostForHeap() const { return m_properties.IsGreedy() ? -1.0 : GetCost(); }
  ...
  using SExpressionInfoArray = CDynamicPtrArray<SExpressionInfo, CleanupRelease<SExpressionInfo>>;
  ...
  struct SGroupInfo : public CRefCount {
    ...
    SGroupInfo(CMemoryPool *mp, CBitSet *atoms) : m_atoms(atoms), m_cardinality(-1.0), m_lowest_expr_cost(-1.0) {
  ...
  using SGroupInfoArray = CDynamicPtrArray<SGroupInfo, CleanupRelease<SGroupInfo>>;
  ...
  using ExpressionToEdgeMap = CHashMap<CExpression, SEdge, CExpression::HashValue, CUtils::Equals,
                                       CleanupRelease<CExpression>, CleanupRelease<SEdge>>;

  // dynamic array of SGroupInfos
  using BitSetToGroupInfoMap =
      CHashMap<CBitSet, SGroupInfo, UlHashBitSet, FEqualBitSet, CleanupRelease<CBitSet>, CleanupRelease<SGroupInfo>>;

  // iterator over group infos in a level
  using BitSetToGroupInfoMapIter = CHashMapIter<CBitSet, SGroupInfo, UlHashBitSet, FEqualBitSet,
  ...
  SExpressionInfo *GetJoinExprForProperties(SGroupInfo *left_child, SGroupInfo *right_child,
                                            SExpressionProperties &required_properties);

  // get a join expression from two child groups with specified child expressions
  SExpressionInfo *GetJoinExpr(const SGroupAndExpression &left_child_expr, const SGroupAndExpression &right_child_expr,
 ...
 public:
  // ctor
  CJoinOrderDPv2(CMemoryPool *mp, CExpressionArray *pdrgpexprAtoms, CExpressionArray *innerJoinConjuncts,
                 CExpressionArray *onPredConjuncts, ULongPtrArray *childPredIndexes, CColRefSet *outerRefs);
  ...
  CXform::EXformId EOriginXForm() const override { return CXform::ExfExpandNAryJoinDPv2; }
