...
class CXformPushJoinBelowLeftUnionAll : public CXformPushJoinBelowUnionAll {
 private:
 public:
  CXformPushJoinBelowLeftUnionAll(const CXformPushJoinBelowLeftUnionAll &) = delete;
  ...
  explicit CXformPushJoinBelowLeftUnionAll(CMemoryPool *mp)
      : CXformPushJoinBelowUnionAll(
            ...
            GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CPatternNode(mp, CPatternNode::EmtMatchInnerOrLeftOuterJoin),

                                     GPOS_NEW(mp) CExpression  // outer child is a union all operation
  ...
  const CHAR *SzId() const override { return "CXformPushJoinBelowLeftUnionAll"; }

  // ident accessors
  EXformId Exfid() const override { return ExfPushJoinBelowLeftUnionAll; }
