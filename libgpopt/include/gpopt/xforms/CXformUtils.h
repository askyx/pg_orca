...
using ExprToExprArrayMap = CHashMap<CExpression, CExpressionArray, CExpression::HashValue, CUtils::Equals,
                                    CleanupRelease<CExpression>, CleanupRelease<CExpressionArray>>;

// iterator of map of expression to array of expressions
using ExprToExprArrayMapIter = CHashMapIter<CExpression, CExpressionArray, CExpression::HashValue, CUtils::Equals,
                                            CleanupRelease<CExpression>, CleanupRelease<CExpressionArray>>;
...
class CXformUtils {
 private:
  ...
  static CColRefSet *PcrsIndexColumns(CMemoryPool *mp, CColRefArray *colref_array, const IMDIndex *pmdindex,
  ...
  static CColRefSet *PcrsIndexReturnableColumns(CMemoryPool *mp, CColRefArray *colref_array, const IMDIndex *pmdindex,
                                                const IMDRelation *pmdrel);

  // return the ordered array of columns from the given array of columns which appear
  // in the index included / key columns
  static CColRefArray *PdrgpcrIndexColumns(CMemoryPool *mp, CColRefArray *colref_array, const IMDIndex *pmdindex,
                                           const IMDRelation *pmdrel);

  // lookup join keys in scalar child group
  static void LookupJoinKeys(CMemoryPool *mp, CExpression *pexpr, CExpressionArray **ppdrgpexprOuter,
                             CExpressionArray **ppdrgpexprInner, IMdIdArray **join_opfamilies);

  // cache join keys on scalar child group
  static void CacheJoinKeys(CExpression *pexpr, CExpressionArray *pdrgpexprOuter, CExpressionArray *pdrgpexprInner,
  ...
  static bool FXformInArray(CXform::EXformId exfid, const CXform::EXformId rgXforms[], uint32_t ulXforms);
  ...
  template <class T>
  static void AddHashOrMergeJoinAlternative(CMemoryPool *mp, CExpression *pexprJoin, CExpressionArray *pdrgpexprOuter,
  ...
  static void SubqueryAllToAgg(CMemoryPool *mp, CExpression *pexprSubquery,
  ...
  static void SubqueryAnyToAgg(CMemoryPool *mp, CExpression *pexprSubquery,
                               CExpression **ppexprNewSubquery,  // output argument for new scalar subquery
                               CExpression **ppexprNewScalar     // output argument for new scalar expression
  );

  // create the Gb operator to be pushed below a join
  static CLogicalGbAgg *PopGbAggPushableBelowJoin(CMemoryPool *mp, CLogicalGbAgg *popGbAggOld, CColRefSet *prcsOutput,
                                                  CColRefSet *pcrsGrpCols);

  // check if the preconditions for pushing down Group by through join are satisfied
  static bool FCanPushGbAggBelowJoin(CColRefSet *pcrsGrpCols, CColRefSet *pcrsJoinOuterChildOutput,
  ...
  template <class T>
  static CLogical *PopDynamicBtreeIndexOpConstructor(CMemoryPool *mp, const IMDIndex *pmdindex,
                                                     CTableDescriptor *ptabdesc, uint32_t ulOriginOpId, CName *pname,
                                                     uint32_t ulPartIndex, CColRefArray *pdrgpcrOutput,
                                                     CColRef2dArray *pdrgpdrgpcrPart, IMdIdArray *partition_mdids,
                                                     uint32_t ulUnindexedPredColCount) {
    return GPOS_NEW(mp) T(mp, pmdindex, ptabdesc, ulOriginOpId, pname, ulPartIndex, pdrgpcrOutput, pdrgpdrgpcrPart,
                          partition_mdids, ulUnindexedPredColCount);
  }

  //	create a static operator for a btree index plan
  template <class T>
  static CLogical *PopStaticBtreeIndexOpConstructor(CMemoryPool *mp, const IMDIndex *pmdindex,
                                                    CTableDescriptor *ptabdesc, uint32_t ulOriginOpId, CName *pname,
                                                    CColRefArray *pdrgpcrOutput, uint32_t ulUnindexedPredColCount,
                                                    EIndexScanDirection indexScanDirection) {
    return GPOS_NEW(mp)
        T(mp, pmdindex, ptabdesc, ulOriginOpId, pname, pdrgpcrOutput, ulUnindexedPredColCount, indexScanDirection);
  }

  //	produce an expression representing a new btree index path
  static CExpression *PexprRewrittenBtreeIndexPath(CMemoryPool *mp, CExpression *pexprIndexCond,
  ...
  static CExpression *PexprBitmapLookupWithPredicateBreakDown(CMemoryPool *mp, CMDAccessor *md_accessor,
  ...
  static void ComputeBitmapTableScanResidualPredicate(CMemoryPool *mp, bool fConjunction,
                                                      CExpression *pexprOriginalPred, CExpression **ppexprResidual,
  ...
  static CExpression *PexprBitmapSelectBestIndex(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprPred,
                                                 CTableDescriptor *ptabdesc, const IMDRelation *pmdrel,
  ...
  // iterate over given hash map and return array of arrays of project elements sorted by the column id of the first
  // entries
  ...
  static int32_t ICmpPrjElemsArr(const void *pvFst, const void *pvSnd);
 ...
 public:
  ...
  template <class T>
  static void TransformImplementBinaryOp(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr);

  // helper function for implementation of hash joins
  template <class T>
  static void ImplementHashJoin(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr);

  // helper function for implementation of merge joins
  template <class T>
  static void ImplementMergeJoin(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr);

  // helper function for implementation of nested loops joins
  template <class T>
  static void ImplementNLJoin(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr);

  // helper for removing IsNotFalse join predicate for GPDB anti-semi hash join
  static bool FProcessGPDBAntiSemiHashJoin(CMemoryPool *mp, CExpression *pexpr, CExpression **ppexprResult);

  // check the applicability of logical join to physical join xform
  static CXform::EXformPromise ExfpLogicalJoin2PhysicalJoin(CExpressionHandle &exprhdl);

  // check the applicability of semi join to cross product
  static CXform::EXformPromise ExfpSemiJoin2CrossProduct(CExpressionHandle &exprhdl);

  // check the applicability of N-ary join expansion
  static CXform::EXformPromise ExfpExpandJoinOrder(CExpressionHandle &exprhdl, const CXform *xform);

  // extract foreign key
  static CColRefSet *PcrsFKey(CMemoryPool *mp, CExpression *pexprOuter, CExpression *pexprInner,
                              CExpression *pexprScalar);

  // compute a swap of the two given joins
  static CExpression *PexprSwapJoins(CMemoryPool *mp, CExpression *pexprTopJoin, CExpression *pexprBottomJoin);

  // push a Gb, optionally with a having clause below the child join
  static CExpression *PexprPushGbBelowJoin(CMemoryPool *mp, CExpression *pexpr);
  ...
  static CExpression *PexprSeparateSubqueryPreds(CMemoryPool *mp, CExpression *pexpr);

  // helper for creating inverse predicate for unnesting subquery ALL
  static CExpression *PexprInversePred(CMemoryPool *mp, CExpression *pexprSubquery);
  ...
  static CExpression *PexprLogicalDMLOverProject(CMemoryPool *mp, CExpression *pexprChild,
  ...
  static CExpressionArray *PdrgpexprPartEqFilters(CMemoryPool *mp, CTableDescriptor *ptabdesc,
                                                  CColRefArray *pdrgpcrSource);

  // helper for creating Agg expression equivalent to quantified subquery
  static void QuantifiedToAgg(CMemoryPool *mp, CExpression *pexprSubquery, CExpression **ppexprNewSubquery,
  ...
  static bool FApplyToNextBinding(CXform *pxform, CExpression *pexprLastBinding);

  // return a formatted error message for the given exception
  static CWStringConst *PstrErrorMessage(CMemoryPool *mp, uint32_t major, uint32_t minor, ...);

  // return the array of key columns from the given array of columns which appear
  // in the index key columns
  static CColRefArray *PdrgpcrIndexKeys(CMemoryPool *mp, CColRefArray *colref_array, const IMDIndex *pmdindex,
                                        const IMDRelation *pmdrel);

  // return the set of key columns from the given array of columns which appear
  // in the index key columns
  static CColRefSet *PcrsIndexKeys(CMemoryPool *mp, CColRefArray *colref_array, const IMDIndex *pmdindex,
                                   const IMDRelation *pmdrel);

  // check if an index is applicable given the required, output and scalar
  // expression columns
  static bool FIndexApplicable(CMemoryPool *mp, const IMDIndex *pmdindex, const IMDRelation *pmdrel,
                               CColRefArray *pdrgpcrOutput, CColRefSet *pcrsScalar, IMDIndex::EmdindexType emdindtype,
                               IMDIndex::EmdindexType altindtype = IMDIndex::EmdindSentinel);

  // check whether a CTE should be inlined
  static bool FInlinableCTE(uint32_t ulCTEId);

  // return the column reference of the n-th project element
  static CColRef *PcrProjectElement(CExpression *pexpr, uint32_t ulIdxProjElement);
  ...
  static bool FGenerateApply(CXform::EXformId exfid) {
    return CXform::ExfSelect2Apply == exfid || CXform::ExfProject2Apply == exfid || CXform::ExfGbAgg2Apply == exfid ||
           CXform::ExfSubqJoin2Apply == exfid || CXform::ExfSubqNAryJoin2Apply == exfid ||
           CXform::ExfSequenceProject2Apply == exfid;
  }

  // helper for creating bitmap bool op expressions
  static CExpression *PexprScalarBitmapBoolOp(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprOriginalPred,
  ...
  // path expressions
  static CExpression *PexprBitmapBoolOp(CMemoryPool *mp, IMDId *pmdidBitmapType, CExpression *pexprLeft,
  ...
  static void CreateBitmapIndexProbeOps(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprOriginalPred,
                                        CExpressionArray *pdrgpexprPreds, CTableDescriptor *ptabdesc,
                                        const IMDRelation *pmdrel, CColRefArray *pdrgpcrOutput, CColRefSet *outer_refs,
                                        CColRefSet *pcrsReqd, bool fConjunction, CExpressionArray *pdrgpexprBitmap,
                                        CExpressionArray *pdrgpexprRecheck, CExpressionArray *pdrgpexprResidual,
                                        bool isAPartialPredicate);
  ...
  static CExpression *PexprBitmapTableGet(CMemoryPool *mp, CLogical *popGet, uint32_t ulOriginOpId,
                                          CTableDescriptor *ptabdesc, CExpression *pexprScalar, CColRefSet *outer_refs,
                                          CColRefSet *pcrsReqd);

  // transform a Select over a (dynamic) table get into a bitmap table scan
  // over bitmap bool op
  static CExpression *PexprSelect2BitmapBoolOp(CMemoryPool *mp, CExpression *pexpr);

  // remap the expression from the old columns to the new ones
  static CExpression *PexprRemapColumns(CMemoryPool *mp, CExpression *pexprScalar, CColRefArray *pdrgpcrA,
  ...
  static CExpression *PexprCTEConsumer(CMemoryPool *mp, uint32_t ulCTEId, CColRefArray *pdrgpcrConsumer);
  ...
  static CColRefArray *PdrgpcrReorderedSubsequence(CMemoryPool *mp, CColRefArray *colref_array,
  ...
  // create a map from the argument of each Distinct Agg to the array of project elements that define Distinct Aggs on
  // the same argument
  static void MapPrjElemsWithDistinctAggs(CMemoryPool *mp, CExpression *pexprPrjList,
                                          ExprToExprArrayMap **pphmexprdrgpexpr, uint32_t *pulDifferentDQAs);
  ...
  static void JoinBitmapIndexProbes(CMemoryPool *pmp, CExpressionArray *pdrgpexprBitmapOld,
                                    CExpressionArray *pdrgpexprRecheckOld, bool fConjunction,
                                    CExpression **ppexprBitmap, CExpression **ppexprRecheck);

  static CExpression *AddALinearStackOfUnaryExpressions(CMemoryPool *mp, CExpression *lowerPartOfExpr,
                                                        CExpression *topOfStack, CExpression *exclusiveBottomOfStack);

  static bool FCoverIndex(CMemoryPool *mp, CIndexDescriptor *pindexdesc, CTableDescriptor *ptabdesc,
                          CColRefArray *pdrgpcrOutput);

  static void ComputeOrderSpecForIndexKey(CMemoryPool *mp, COrderSpec **order_spec, const IMDIndex *pmdindex,
  ...
  static CExpression *PexprBuildBtreeIndexPlan(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprGet,
...
//---------------------------------------------------------------------------
template <class T>
void CXformUtils::TransformImplementBinaryOp(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) {
  ...
  CExpression *pexprBinary = GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) T(mp), pexprLeft, pexprRight, pexprScalar);

#ifdef GPOS_DEBUG
  COperator::EOperatorId op_id = pexprBinary->Pop()->Eopid();
#endif  // GPOS_DEBUG
  GPOS_ASSERT(COperator::EopPhysicalInnerNLJoin == op_id || COperator::EopPhysicalLeftOuterNLJoin == op_id ||
              COperator::EopPhysicalLeftSemiNLJoin == op_id || COperator::EopPhysicalLeftAntiSemiNLJoin == op_id ||
...
template <class T>
void CXformUtils::AddHashOrMergeJoinAlternative(CMemoryPool *mp, CExpression *pexprJoin,
                                                CExpressionArray *pdrgpexprOuter, CExpressionArray *pdrgpexprInner,
                                                IMdIdArray *opfamilies, CXformResult *pxfres,
  ...
  T *op = GPOS_NEW(mp)
      T(mp, pdrgpexprOuter, pdrgpexprInner, opfamilies, is_hash_join_null_aware, popLogicalJoin->OriginXform());
  CExpression *pexprResult = GPOS_NEW(mp) CExpression(mp, op, (*pexprJoin)[0], (*pexprJoin)[1], (*pexprJoin)[2]);
...
//---------------------------------------------------------------------------
template <class T>
void CXformUtils::ImplementHashJoin(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) {
  GPOS_ASSERT(nullptr != pxfctxt);
  ...
  uint32_t ulPreds = pdrgpexpr->Size();
  ...
  for (uint32_t ul = 0; ul < ulPreds; ul++) {
    ...
    if (CPhysicalJoin::FHashJoinCompatible(pexprPred, pexprOuter, pexprInner)) {
      ...
      IMDId *mdid_scop;
      CPhysicalJoin::AlignJoinKeyOuterInner(pexprPred, pexprOuter, pexprInner, &pexprPredOuter, &pexprPredInner,
                                            &mdid_scop);
  ...
  CExpression *pexprResult = GPOS_NEW(mp)
      CExpression(mp, pexpr->Pop(), pexprOuter, pexprInner, CPredicateUtils::PexprConjunction(mp, pdrgpexpr));
...
template <class T>
void CXformUtils::ImplementMergeJoin(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) {
  GPOS_ASSERT(nullptr != pxfctxt);
  ...
  if (nullptr != pdrgpexprOuter) {
    ...
    if (0 == pdrgpexprOuter->Size()) {
      ...
      AddHashOrMergeJoinAlternative<T>(mp, pexpr, pdrgpexprOuter, pdrgpexprInner, nullptr /* opfamilies */, pxfres,
  ...
  if (0 != pdrgpexprOuter->Size()) {
    AddHashOrMergeJoinAlternative<T>(mp, pexprResult, pdrgpexprOuter, pdrgpexprInner, nullptr /* opfamilies */, pxfres,
                                     true /*is_hash_join_null_aware*/);
  } else {
    // clean up
    pdrgpexprOuter->Release();
...
//---------------------------------------------------------------------------
template <class T>
void CXformUtils::ImplementNLJoin(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) {
  GPOS_ASSERT(nullptr != pxfctxt);
