...
struct SForeignServer {
  ...
  SForeignServer(ULONG foreign_server_oid, IMDRelation::Ereldistrpolicy exec_location)
      : m_foreign_server_oid(foreign_server_oid), m_exec_location(exec_location) {}
};

static ULONG UlHashSForeignServer(const SForeignServer *fs) {
  return fs->m_foreign_server_oid;
}

static BOOL FEqualSForeignServer(const SForeignServer *fs1, const SForeignServer *fs2) {
  return fs1->m_foreign_server_oid == fs2->m_foreign_server_oid && fs1->m_exec_location == fs2->m_exec_location;
}

// hash maps ULONG -> array of ULONGs
using SForeignServerToIMdIdArrayMap = CHashMap<SForeignServer, IMdIdArray, UlHashSForeignServer, FEqualSForeignServer,
                                               CleanupDelete<SForeignServer>, CleanupRelease<IMdIdArray>>;

// iterator
using SForeignServerToIMdIdArrayMapIter =
    CHashMapIter<SForeignServer, IMdIdArray, UlHashSForeignServer, FEqualSForeignServer, CleanupDelete<SForeignServer>,
...
class CXformExpandDynamicGetWithForeignPartitions : public CXformExploration {
 private:
 public:
  CXformExpandDynamicGetWithForeignPartitions(const CXformExpandDynamicGetWithForeignPartitions &) = delete;
  ...
  // ident accessors
  EXformId Exfid() const override { return ExfExpandDynamicGetWithForeignPartitions; }

  // return a string for xform name
  const CHAR *SzId() const override { return "CXformExpandDynamicGetWithForeignPartitions"; }
