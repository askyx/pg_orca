...
class CXformJoin2IndexApply : public CXformExploration {
 private:
  ...
  // helper to add IndexApply expression to given xform results container
  // for homogeneous b-tree indexes
  static void CreateAlternativesForBtreeIndex(CMemoryPool *mp, CExpression *pexprLogicalIndexGet, COperator *joinOp,
  ...
  static void ComputeColumnSets(CMemoryPool *mp, CExpression *pexprInner, CExpression *pexprScalar,
                                CColRefSet **ppcrsScalarExpr, CColRefSet **ppcrsOuterRefs, CColRefSet **ppcrsReqd);
 ...
 protected:
  ...
  virtual void CreateHomogeneousIndexApplyAlternatives(
      CMemoryPool *mp, COperator *joinOp, CExpression *pexprOuter, CExpression *pexprInner, CExpression *pexprScalar,
      CExpression *origJoinPred, CExpression *nodesToInsertAboveIndexGet, CExpression *endOfNodesToInsertAboveIndexGet,
      CTableDescriptor *PtabdescInner, CXformResult *pxfres, gpmd::IMDIndex::EmdindexType emdtype) const;

 public:
  CXformJoin2IndexApply(const CXformJoin2IndexApply &) = delete;

  // ctor
  explicit CXformJoin2IndexApply(CExpression *pexprPattern) : CXformExploration(pexprPattern) {
