...
class CScanHint : public IHint, public DbgPrintMixin<CScanHint> {
 public:
  ...
  using CEHintTypeSetSetIter = CEnumSetIter<CScanHint::EType, CScanHint::Sentinal>;
 ...
 public:
  CScanHint(CMemoryPool *mp, const CWStringBase *name, StringPtrArray *indexnames)
      : m_mp(mp), m_name(name), m_indexnames(indexnames), m_types(GPOS_NEW(mp) CEHintTypeSet(mp)) {}
  ...
  const CWStringBase *GetName() const { return m_name; }

  const StringPtrArray *GetIndexNames() const { return m_indexnames; }

  void AddType(CScanHint::EType type) { m_types->ExchangeSet(type); }
