...
class CRowHint : public IHint, public DbgPrintMixin<CRowHint> {
 ...
 public:
  CRowHint(CMemoryPool *mp, StringPtrArray *aliases, CDouble rows, RowsValueType type)
      : m_mp(mp), m_aliases(aliases), m_rows(rows), m_type(type) {
    aliases->Sort(CWStringBase::Compare);
  }

  ~CRowHint() override { CRefCount::SafeRelease(m_aliases); }

  const StringPtrArray *GetAliasNames() const { return m_aliases; }

  RowsValueType GetType() const { return m_type; }
  ...
  //
  //---------------------------------------------------------------------------
  CDouble ComputeRows(CDouble input) const {
    CDouble result(-1);
    switch (m_type) {
      case RVT_ABSOLUTE: {
        result = m_rows;
      ...
      default: {
        CWStringDynamic *error_message = GPOS_NEW(m_mp) CWStringDynamic(m_mp, GPOS_WSZ_LIT("Unknown row type"));
