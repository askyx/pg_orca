...
class CParseHandlerMetadata : public CParseHandlerBase {
 private:
  ...
  CSystemIdArray *GetSrcSysIdArray(const Attributes &attr, Edxltoken target_attr, Edxltoken target_elem);

 public:
  CParseHandlerMetadata(const CParseHandlerMetadata &) = delete;

  // ctor
  CParseHandlerMetadata(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
