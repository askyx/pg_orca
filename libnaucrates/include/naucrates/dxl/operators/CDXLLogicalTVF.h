...
class CDXLLogicalTVF : public CDXLLogical {
 ...
 public:
  ...
  // ctor/dtor
  CDXLLogicalTVF(CMemoryPool *mp, IMDId *mdid_func, IMDId *mdid_return_type, CMDName *mdname,
  ...
  const CDXLColDescrArray *GetDXLColumnDescrArray() const { return m_dxl_col_descr_array; }

  // get the column descriptor at the given position
  const CDXLColDescr *GetColumnDescrAt(uint32_t ul) const;
