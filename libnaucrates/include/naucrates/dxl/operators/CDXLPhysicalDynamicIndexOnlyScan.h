...
class CDXLPhysicalDynamicIndexOnlyScan : public CDXLPhysicalDynamicIndexScan {
 public:
  CDXLPhysicalDynamicIndexOnlyScan(CDXLPhysicalDynamicIndexOnlyScan &) = delete;

  // ctor
  CDXLPhysicalDynamicIndexOnlyScan(CMemoryPool *mp, CDXLTableDescr *table_descr, CDXLIndexDescr *dxl_index_descr,
                                   EdxlIndexScanDirection idx_scan_direction, IMdIdArray *part_mdids,
                                   ULongPtrArray *selector_ids)
      : CDXLPhysicalDynamicIndexScan(mp, table_descr, dxl_index_descr, idx_scan_direction, part_mdids, selector_ids) {}
  ...
  // conversion function
  static CDXLPhysicalDynamicIndexOnlyScan *Cast(CDXLOperator *dxl_op) {
    GPOS_ASSERT(nullptr != dxl_op);
    GPOS_ASSERT(EdxlopPhysicalDynamicIndexOnlyScan == dxl_op->GetDXLOperator());
