...
enum Edxlsccmp { EdxlsccmpIndexLeft = 0, EdxlsccmpIndexRight, EdxlsccmpSentinel };
...
class CDXLScalarComp : public CDXLScalar {
 ...
 public:
  ...
  // ctor/dtor
  CDXLScalarComp(CMemoryPool *mp, IMDId *operator_mdid, const CWStringConst *comparison_operator_name);
  ...
  // conversion function
  static CDXLScalarComp *Cast(CDXLOperator *dxl_op) {
    GPOS_ASSERT(nullptr != dxl_op);
    GPOS_ASSERT(EdxlopScalarCmp == dxl_op->GetDXLOperator() || EdxlopScalarDistinct == dxl_op->GetDXLOperator() ||
