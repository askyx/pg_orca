...
class CDXLPhysicalRoutedDistributeMotion : public CDXLPhysicalMotion {
 ...
 public:
  CDXLPhysicalRoutedDistributeMotion(const CDXLPhysicalRoutedDistributeMotion &) = delete;
  ...
  // segment id column
  ULONG
  SegmentIdCol() const { return m_segment_id_col; }

  // index of relational child node in the children array
  ULONG
  GetRelationChildIdx() const override { return EdxlroutedmIndexChild; }

  // serialize operator in DXL format
  void SerializeToDXL(CXMLSerializer *xml_serializer, const CDXLNode *dxlnode) const override;

  // conversion function
  static CDXLPhysicalRoutedDistributeMotion *Cast(CDXLOperator *dxl_op) {
