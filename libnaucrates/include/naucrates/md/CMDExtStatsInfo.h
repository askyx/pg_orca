...
class CMDExtStatsInfo : public CRefCount {
 public:
  enum Estattype { EstatDependencies, EstatNDistinct, EstatMCV, EstatSentinel };
 ...
 public:
  CMDExtStatsInfo(CMemoryPool *mp, OID stat_oid, CMDName *stat_name, Estattype kind, CBitSet *keys)
      : m_mp(mp), m_stat_oid(stat_oid), m_stat_name(stat_name), m_kind(kind), m_keys(keys) {}
  ...
  Estattype GetStatKind() const { return m_kind; }

  CBitSet *GetStatKeys() const { return m_keys; }
};
