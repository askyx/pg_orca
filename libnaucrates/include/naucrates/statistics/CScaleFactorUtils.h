...
class CScaleFactorUtils {
 public:
  struct SJoinCondition {
    ...
    // We hash/compare the pointer rather than the contents of the IMdId, as we want to discern between different
  ...
  using OIDPairToScaleFactorArrayMap =
      CHashMap<IMdIdArray, CDoubleArray, SJoinCondition::HashValue, SJoinCondition::Equals, CleanupRelease<IMdIdArray>,
               CleanupRelease<CDoubleArray>>;

  using OIDPairToScaleFactorArrayMapIter =
      CHashMapIter<IMdIdArray, CDoubleArray, SJoinCondition::HashValue, SJoinCondition::Equals,
                   CleanupRelease<IMdIdArray>, CleanupRelease<CDoubleArray>>;

  // generate the hashmap of scale factors grouped by pred tables, also produces array of complex join preds
  static OIDPairToScaleFactorArrayMap *GenerateScaleFactorMap(CMemoryPool *mp,
  ...
  static CDouble CalcCumulativeScaleFactorSqrtAlg(OIDPairToScaleFactorArrayMap *scale_factor_hashmap,
                                                  CDoubleArray *complex_join_preds);

  // calculate the cumulative join scaling factor
  static CDouble CumulativeJoinScaleFactor(CMemoryPool *mp, const CStatisticsConfig *stats_config,
                                           SJoinConditionArray *join_conds_scale_factors,
  ...
  static CDouble CalcScaleFactorCumulativeDisj(const CStatisticsConfig *stats_config, CDoubleArray *scale_factors,
                                               CDouble tota_rows);
