...
class CStatisticsUtils {
 private:
  ...
  struct SMcvPair {
    ...
    // ctor
    SMcvPair(IDatum *datum_mcv, CDouble mcv_freq) : m_datum_mcv(datum_mcv), m_mcv_freq(mcv_freq) {}

    // dtor
    ~SMcvPair() { m_datum_mcv->Release(); }
  };
  ...
  static CBucketArray *MergeMcvHistBucket(CMemoryPool *mp, const CBucketArray *mcv_buckets,
                                          const CBucketArray *histogram_buckets);

  // split a histogram bucket given an MCV bucket
  static CBucketArray *SplitHistBucketGivenMcvBuckets(CMemoryPool *mp, const CBucket *histogram_bucket,
  ...
  static CBucket *CreateValidBucket(CMemoryPool *mp, CPoint *bucket_lower_bound, CPoint *bucket_upper_bound,
  ...
  static bool IsValidBucket(CPoint *bucket_lower_bound, CPoint *bucket_upper_bound, bool is_lower_closed,
                            bool is_upper_closed);

  // find the MCVs that fall within the same histogram bucket and perform the split
  static void SplitHistDriver(CMemoryPool *mp, const CBucket *histogram_bucket, const CBucketArray *mcv_buckets,
                              CBucketArray *merged_buckets, uint32_t *mcv_index, uint32_t mcv);

  // distribute total distinct and frequency of the histogram bucket into the new buckets
  static CBucketArray *DistributeBucketProperties(CMemoryPool *mp, CDouble total_frequency,
                                                  CDouble total_distinct_values, const CBucketArray *buckets);

  // add the NDVs for all of the grouping columns
  static void AddNdvForAllGrpCols(CMemoryPool *mp, const CStatistics *input_stats,
  ...
  static CDouble MaxNumGroupsForGivenSrcGprCols(CMemoryPool *mp, const CStatisticsConfig *stats_config,
 ...
 public:
  ...
  static CHistogram *TransformMCVToHist(CMemoryPool *mp, const IMDType *mdtype, IDatumArray *mcv_datums,
                                        CDoubleArray *freq_array, uint32_t num_mcv_values);

  // merge MCVs and histogram
  static CHistogram *MergeMCVHist(CMemoryPool *mp, const CHistogram *mcv_histogram, const CHistogram *histogram);

  // comparison function for sorting MCVs
  static inline int32_t GetMcvPairCmpFunc(const void *val1, const void *val2);

  // utility function to print column stats before/after applying filter
  static void PrintColStats(CMemoryPool *mp, CStatsPred *pred_stats, uint32_t cond_colid, CHistogram *histogram,
                            CDouble last_scale_factor, bool is_filter_applied_before);

  // extract all the column identifiers used in the statistics filter
  static void ExtractUsedColIds(CMemoryPool *mp, CBitSet *colids_bitset, CStatsPred *pred_stats, ULongPtrArray *colids);

  // given the previously generated histogram, update the intermediate
  // result of the disjunction
  static void UpdateDisjStatistics(CMemoryPool *mp, CBitSet *non_updatable_cols, CDouble input_disjunct_rows,
                                   CDouble local_rows, CHistogram *previous_histogram,
                                   UlongToHistogramMap *disjunctive_result_histograms, uint32_t last_colid);
  ...
  // disjunction
  static CBitSet *GetColsNonUpdatableHistForDisj(CMemoryPool *mp, CStatsPredDisj *pred_stats);

  // helper method to add a histogram to a map
  static void AddHistogram(CMemoryPool *mp, uint32_t colid, const CHistogram *histogram,
                           UlongToHistogramMap *col_histogram_mapping, bool replace_old = false);

  // create a new hash map of histograms after merging
  // the histograms generated by the child of disjunctive predicate
  static UlongToHistogramMap *MergeHistogramMapsForDisjPreds(CMemoryPool *mp, CBitSet *non_updatable_cols,
                                                             UlongToHistogramMap *hmap1, UlongToHistogramMap *hmap2,
                                                             CDouble rows1, CDouble rows2);

  // helper method to copy the hash map of histograms
  static UlongToHistogramMap *CopyHistHashMap(CMemoryPool *mp, UlongToHistogramMap *col_histogram_mapping);

  // return the column identifier of the filter if the predicate is
  // on a single column else	return UINT32_MAX
  static uint32_t GetColId(const CStatsPredPtrArry *stats_preds_array);

  // add remaining buckets from one array of buckets to the other
  static void AddRemainingBuckets(CMemoryPool *mp, const CBucketArray *src_buckets, CBucketArray *dest_buckets,
                                  uint32_t *start_val);
  ...
  static IStatistics *DeriveStatsForBitmapTableGet(CMemoryPool *mp, CExpressionHandle &exprhdl,
                                                   IStatisticsArray *stats_contexts);
  ...
  static CDouble GetCumulativeNDVs(const CStatisticsConfig *stats_config, CDoubleArray *ndv_array);

  // return the mapping between the table column used for grouping to the logical operator id where it was defined.
  // If the grouping column is not a table column then the logical op id is initialized to UINT32_MAX
  static UlongToUlongPtrArrayMap *GetGrpColIdToUpperBoundNDVIdxMap(CMemoryPool *mp, CStatistics *stats,
                                                                   const CColRefSet *grouping_cols_refset,
                                                                   CBitSet *keys);

  // extract NDVs for the given array of grouping columns
  static CDoubleArray *ExtractNDVForGrpCols(CMemoryPool *mp, const CStatisticsConfig *stats_config,
  ...
  static void AddGrpColStats(CMemoryPool *mp, const CStatistics *input_stats, CColRefSet *grp_cols_refset,
...
// comparison function for sorting MCVs
int32_t CStatisticsUtils::GetMcvPairCmpFunc(const void *val1, const void *val2) {
