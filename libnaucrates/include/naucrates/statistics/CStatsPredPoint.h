...
class CStatsPredPoint : public CStatsPred {
 ...
 public:
  ...
  // dtor
  ~CStatsPredPoint() override { m_pred_point->Release(); }

  // comparison types for stats computation
  virtual CStatsPred::EStatsCmpType GetCmpType() const { return m_stats_cmp_type; }

  // filter point
  virtual CPoint *GetPredPoint() const { return m_pred_point; }

  // filter type id
  EStatsPredType GetPredStatsType() const override { return CStatsPred::EsptPoint; }
