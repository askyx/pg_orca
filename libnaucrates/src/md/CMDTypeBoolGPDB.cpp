...
CMDTypeBoolGPDB::CMDTypeBoolGPDB(CMemoryPool *mp) : m_mp(mp) {
  ...
  m_mdid_op_leq = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_BOOL_LEQ_OP);
  m_mdid_op_gt = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_BOOL_GT_OP);
  m_mdid_op_geq = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_BOOL_GEQ_OP);
  m_mdid_op_cmp = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_BOOL_COMP_OP);
  m_mdid_type_array = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_BOOL_ARRAY_TYPE);
  ...
  m_mdid_count = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_BOOL_AGG_COUNT);
...
//
//---------------------------------------------------------------------------
IMDId *CMDTypeBoolGPDB::GetMdidForCmpType(ECmpType cmp_type) const {
  switch (cmp_type) {
    case EcmptEq:
      return m_mdid_op_eq;
    case EcmptNEq:
...
//
//---------------------------------------------------------------------------
IMDId *CMDTypeBoolGPDB::GetMdidForAggType(EAggType agg_type) const {
  switch (agg_type) {
    case EaggMin:
      return m_mdid_min;
    case EaggMax:
...
//
//---------------------------------------------------------------------------
IDatum *CMDTypeBoolGPDB::GetDatumForDXLConstVal(const CDXLScalarConstValue *dxl_op) const {
...
//
//---------------------------------------------------------------------------
CDXLDatum *CMDTypeBoolGPDB::GetDatumVal(CMemoryPool *mp, IDatum *datum) const {
...
//
//---------------------------------------------------------------------------
CDXLScalarConstValue *CMDTypeBoolGPDB::GetDXLOpScConst(CMemoryPool *mp, IDatum *datum) const {
  CDatumBoolGPDB *datum_bool_gpdb = dynamic_cast<CDatumBoolGPDB *>(datum);

  m_mdid->AddRef();
  CDXLDatumBool *dxl_datum =
      GPOS_NEW(mp) CDXLDatumBool(mp, m_mdid, datum_bool_gpdb->IsNull(), datum_bool_gpdb->GetValue());
...
//
//---------------------------------------------------------------------------
CDXLDatum *CMDTypeBoolGPDB::GetDXLDatumNull(CMemoryPool *mp) const {
  m_mdid->AddRef();
...
//
//---------------------------------------------------------------------------
void CMDTypeBoolGPDB::DebugPrint(IOstream &os) const {
