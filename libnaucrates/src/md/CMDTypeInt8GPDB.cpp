...
CMDTypeInt8GPDB::CMDTypeInt8GPDB(CMemoryPool *mp) : m_mp(mp) {
  ...
  m_mdid_op_leq = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT8_LEQ_OP);
  m_mdid_op_gt = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT8_GT_OP);
  m_mdid_op_geq = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT8_GEQ_OP);
  m_mdid_op_cmp = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT8_COMP_OP);
  m_mdid_type_array = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT8_ARRAY_TYPE);
  ...
  m_mdid_count = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT8_AGG_COUNT);
...
const CWStringDynamic *CMDTypeInt8GPDB::GetStrRepr() {
  if (nullptr == m_dxl_str) {
...
//
//---------------------------------------------------------------------------
CMDName CMDTypeInt8GPDB::Mdname() const {
  return m_mdname;
}
...
//
//---------------------------------------------------------------------------
IMDId *CMDTypeInt8GPDB::GetMdidForCmpType(ECmpType cmp_type) const {
  switch (cmp_type) {
    case EcmptEq:
      return m_mdid_op_eq;
    case EcmptNEq:
...
//
//---------------------------------------------------------------------------
IMDId *CMDTypeInt8GPDB::GetMdidForAggType(EAggType agg_type) const {
  switch (agg_type) {
    case EaggMin:
      return m_mdid_min;
    case EaggMax:
...
//
//---------------------------------------------------------------------------
void CMDTypeInt8GPDB::Serialize(CXMLSerializer *xml_serializer) const {
...
//
//---------------------------------------------------------------------------
IDatum *CMDTypeInt8GPDB::GetDatumForDXLConstVal(const CDXLScalarConstValue *dxl_op) const {
...
//
//---------------------------------------------------------------------------
CDXLDatum *CMDTypeInt8GPDB::GetDatumVal(CMemoryPool *mp, IDatum *datum) const {
...
//
//---------------------------------------------------------------------------
CDXLScalarConstValue *CMDTypeInt8GPDB::GetDXLOpScConst(CMemoryPool *mp, IDatum *datum) const {
...
//
//---------------------------------------------------------------------------
CDXLDatum *CMDTypeInt8GPDB::GetDXLDatumNull(CMemoryPool *mp) const {
  m_mdid->AddRef();
...
//
//---------------------------------------------------------------------------
void CMDTypeInt8GPDB::DebugPrint(IOstream &os) const {
