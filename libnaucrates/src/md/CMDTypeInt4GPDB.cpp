...
CMDTypeInt4GPDB::CMDTypeInt4GPDB(CMemoryPool *mp) : m_mp(mp) {
  ...
  if (GPOS_FTRACE(EopttraceConsiderOpfamiliesForDistribution)) {
    m_distr_opfamily = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_OPFAMILY);
    m_legacy_distr_opfamily = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_LEGACY_OPFAMILY);
  } else {
    m_distr_opfamily = nullptr;
    m_legacy_distr_opfamily = nullptr;
  }
  m_part_opfamily = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_PART_OPFAMILY);
  m_mdid_op_eq = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_EQ_OP);
  m_mdid_op_neq = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_NEQ_OP);
  m_mdid_op_lt = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_LT_OP);
  m_mdid_op_leq = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_LEQ_OP);
  m_mdid_op_gt = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_GT_OP);
  m_mdid_op_geq = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_GEQ_OP);
  m_mdid_op_cmp = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_COMP_OP);
  m_mdid_type_array = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_ARRAY_TYPE);
  ...
  m_mdid_count = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_AGG_COUNT);
...
//
//---------------------------------------------------------------------------
IMDId *CMDTypeInt4GPDB::MDId() const {
  return m_mdid;
}

IMDId *CMDTypeInt4GPDB::GetDistrOpfamilyMdid() const {
...
IMDId *CMDTypeInt4GPDB::GetPartOpfamilyMdid() const {
...
//
//---------------------------------------------------------------------------
CMDName CMDTypeInt4GPDB::Mdname() const {
...
//
//---------------------------------------------------------------------------
IMDId *CMDTypeInt4GPDB::GetMdidForCmpType(ECmpType cmp_type) const {
  switch (cmp_type) {
    case EcmptEq:
      return m_mdid_op_eq;
    case EcmptNEq:
...
//
//---------------------------------------------------------------------------
IMDId *CMDTypeInt4GPDB::GetMdidForAggType(EAggType agg_type) const {
  switch (agg_type) {
    case EaggMin:
      return m_mdid_min;
    case EaggMax:
...
//
//---------------------------------------------------------------------------
IDatum *CMDTypeInt4GPDB::GetDatumForDXLConstVal(const CDXLScalarConstValue *dxl_op) const {
...
//
//---------------------------------------------------------------------------
CDXLDatum *CMDTypeInt4GPDB::GetDatumVal(CMemoryPool *mp, IDatum *datum) const {
  m_mdid->AddRef();
...
//
//---------------------------------------------------------------------------
CDXLScalarConstValue *CMDTypeInt4GPDB::GetDXLOpScConst(CMemoryPool *mp, IDatum *datum) const {
...
//
//---------------------------------------------------------------------------
CDXLDatum *CMDTypeInt4GPDB::GetDXLDatumNull(CMemoryPool *mp) const {
  m_mdid->AddRef();
...
//
//---------------------------------------------------------------------------
void CMDTypeInt4GPDB::DebugPrint(IOstream &os) const {
