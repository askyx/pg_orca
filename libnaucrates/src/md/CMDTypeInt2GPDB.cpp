...
CMDTypeInt2GPDB::CMDTypeInt2GPDB(CMemoryPool *mp) : m_mp(mp) {
  ...
  if (GPOS_FTRACE(EopttraceConsiderOpfamiliesForDistribution)) {
    m_distr_opfamily = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT2_OPFAMILY);
    m_legacy_distr_opfamily = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT2_LEGACY_OPFAMILY);
  } else {
    m_distr_opfamily = nullptr;
    m_legacy_distr_opfamily = nullptr;
  }
  m_part_opfamily = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT2_PART_OPFAMILY);
  m_mdid_op_eq = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT2_EQ_OP);
  m_mdid_op_neq = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT2_NEQ_OP);
  m_mdid_op_lt = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT2_LT_OP);
  m_mdid_op_leq = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT2_LEQ_OP);
  m_mdid_op_gt = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT2_GT_OP);
  m_mdid_op_geq = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT2_GEQ_OP);
  m_mdid_op_cmp = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT2_COMP_OP);
  m_mdid_type_array = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT2_ARRAY_TYPE);
  ...
  m_mdid_count = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT2_AGG_COUNT);

  GPOS_ASSERT(GPDB_INT2_OID == CMDIdGPDB::CastMdid(m_mdid)->Oid());
  m_mdid->AddRef();
  m_datum_null = GPOS_NEW(mp) CDatumInt2GPDB(m_mdid, 1 /* value */, true /* is_null */);
}
...
const CWStringDynamic *CMDTypeInt2GPDB::GetStrRepr() {
  if (nullptr == m_dxl_str) {
...
//
//---------------------------------------------------------------------------
IMDId *CMDTypeInt2GPDB::MDId() const {
  return m_mdid;
}

IMDId *CMDTypeInt2GPDB::GetDistrOpfamilyMdid() const {
...
IMDId *CMDTypeInt2GPDB::GetPartOpfamilyMdid() const {
...
//
//---------------------------------------------------------------------------
CMDName CMDTypeInt2GPDB::Mdname() const {
...
//
//---------------------------------------------------------------------------
IMDId *CMDTypeInt2GPDB::GetMdidForCmpType(ECmpType cmp_type) const {
  switch (cmp_type) {
    case EcmptEq:
      return m_mdid_op_eq;
    case EcmptNEq:
...
//
//---------------------------------------------------------------------------
IMDId *CMDTypeInt2GPDB::GetMdidForAggType(EAggType agg_type) const {
  switch (agg_type) {
    case EaggMin:
      return m_mdid_min;
    case EaggMax:
...
//
//---------------------------------------------------------------------------
void CMDTypeInt2GPDB::Serialize(CXMLSerializer *xml_serializer) const {
...
//
//---------------------------------------------------------------------------
IDatum *CMDTypeInt2GPDB::GetDatumForDXLConstVal(const CDXLScalarConstValue *dxl_op) const {
  CDXLDatumInt2 *dxl_datum = CDXLDatumInt2::Cast(const_cast<CDXLDatum *>(dxl_op->GetDatumVal()));

  return GPOS_NEW(m_mp) CDatumInt2GPDB(m_mdid->Sysid(), dxl_datum->Value(), dxl_datum->IsNull());
}
...
//
//---------------------------------------------------------------------------
CDXLDatum *CMDTypeInt2GPDB::GetDatumVal(CMemoryPool *mp, IDatum *datum) const {
  m_mdid->AddRef();
...
//
//---------------------------------------------------------------------------
CDXLScalarConstValue *CMDTypeInt2GPDB::GetDXLOpScConst(CMemoryPool *mp, IDatum *datum) const {
  CDatumInt2GPDB *int2gpdb_datum = dynamic_cast<CDatumInt2GPDB *>(datum);

  m_mdid->AddRef();
  CDXLDatumInt2 *dxl_datum = GPOS_NEW(mp) CDXLDatumInt2(mp, m_mdid, int2gpdb_datum->IsNull(), int2gpdb_datum->Value());
...
//
//---------------------------------------------------------------------------
CDXLDatum *CMDTypeInt2GPDB::GetDXLDatumNull(CMemoryPool *mp) const {
  m_mdid->AddRef();

  return GPOS_NEW(mp) CDXLDatumInt2(mp, m_mdid, true /*is_null*/, 1 /* a dummy value */);
}
...
//
//---------------------------------------------------------------------------
void CMDTypeInt2GPDB::DebugPrint(IOstream &os) const {
