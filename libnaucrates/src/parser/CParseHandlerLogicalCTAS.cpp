...
//
//---------------------------------------------------------------------------
CParseHandlerLogicalCTAS::CParseHandlerLogicalCTAS(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerLogicalCTAS::StartElement(const XMLCh *const element_uri GPOS_UNUSED,
  ...
  if (nullptr != xml_str_schema_name) {
    m_mdname_schema =
        CDXLUtils::CreateMDNameFromXMLChar(m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_schema_name);
  }
  ...
  if (IMDRelation::EreldistrHash == m_rel_distr_policy) {
    ...
    const XMLCh *rel_distr_cols_xml =
        CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenDistrColumns, EdxltokenLogicalCTAS);
    m_distr_column_pos_array = CDXLOperatorFactory::ExtractIntsToUlongArray(
        m_parse_handler_mgr->GetDXLMemoryManager(), rel_distr_cols_xml, EdxltokenDistrColumns, EdxltokenLogicalCTAS);
  }
  ...
  m_src_colids_array = CDXLOperatorFactory::ExtractIntsToUlongArray(
      m_parse_handler_mgr->GetDXLMemoryManager(), src_colids_xml, EdxltokenInsertCols, EdxltokenLogicalCTAS);
  ...
  m_vartypemod_array = CDXLOperatorFactory::ExtractIntsToIntArray(
      m_parse_handler_mgr->GetDXLMemoryManager(), vartypemod_xml, EdxltokenVarTypeModList, EdxltokenLogicalCTAS);

  m_is_temp_table = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
      m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenRelTemporary, EdxltokenLogicalCTAS);
  ...
  CParseHandlerBase *opclasses_parse_handler = CParseHandlerFactory::GetParseHandler(
  ...
  CParseHandlerBase *opfamilies_parse_handler = CParseHandlerFactory::GetParseHandler(
      m_mp, CDXLTokens::XmlstrToken(EdxltokenMetadataIdList), m_parse_handler_mgr, this);
  ...
  CParseHandlerBase *col_descr_parse_handler =
      CParseHandlerFactory::GetParseHandler(m_mp, CDXLTokens::XmlstrToken(EdxltokenColumns), m_parse_handler_mgr, this);
...
//
//---------------------------------------------------------------------------
void CParseHandlerLogicalCTAS::EndElement(const XMLCh *const,  // element_uri,
                                          const XMLCh *const element_local_name,
  ...
  CParseHandlerMetadataIdList *opfamilies_parse_handler = dynamic_cast<CParseHandlerMetadataIdList *>((*this)[2]);
  CParseHandlerMetadataIdList *opclasses_parse_handler = dynamic_cast<CParseHandlerMetadataIdList *>((*this)[3]);
  CParseHandlerLogicalOp *child_parse_handler = dynamic_cast<CParseHandlerLogicalOp *>((*this)[4]);

  GPOS_ASSERT(nullptr != col_descr_parse_handler->GetDXLColumnDescrArray());
  GPOS_ASSERT(nullptr != ctas_options_parse_handler->GetDxlCtasStorageOption());
  ...
  IMdIdArray *distr_opfamilies = dynamic_cast<CParseHandlerMetadataIdList *>(opfamilies_parse_handler)->GetMdIdArray();
  distr_opfamilies->AddRef();

  IMdIdArray *distr_opclasses = dynamic_cast<CParseHandlerMetadataIdList *>(opclasses_parse_handler)->GetMdIdArray();
  distr_opclasses->AddRef();

  m_dxl_node = GPOS_NEW(m_mp)
      CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLLogicalCTAS(m_mp, m_mdid, m_mdname_schema, m_mdname, dxl_column_descr_array,
