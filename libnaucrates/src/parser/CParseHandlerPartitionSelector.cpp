...
//
//---------------------------------------------------------------------------
CParseHandlerPartitionSelector::CParseHandlerPartitionSelector(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerPartitionSelector::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPhysicalPartitionSelector), element_local_name)) {
    ...
    if (nullptr != m_rel_mdid) {
      ...
      m_rel_mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
          m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenRelationMdid, EdxltokenPhysicalPartitionSelector);

      // parse selector id
      m_selector_id = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(m_parse_handler_mgr->GetDXLMemoryManager(),
                                                                          attrs, EdxltokenPhysicalPartitionSelectorId,
                                                                          EdxltokenPhysicalPartitionSelector);

      // parse scan id
      m_scan_id = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
                                                                      EdxltokenPhysicalPartitionSelectorScanId,
                                                                      EdxltokenPhysicalPartitionSelector);

      // parse partitions
      m_partitions = CDXLOperatorFactory::ExtractConvertValuesToArray(
          m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenPartitions, EdxltokenPhysicalPartitionSelector);
      ...
      this->Append(proj_list_parse_handler);
             ...
             XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarPartFilterExpr), element_local_name)) {
...
//
//---------------------------------------------------------------------------
void CParseHandlerPartitionSelector::EndElement(const XMLCh *const,  // element_uri,
                                                const XMLCh *const element_local_name,
                                                const XMLCh *const  // element_qname
) {
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarResidualFilter), element_local_name) ||
      0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarPartFilterExpr), element_local_name)) {
    return;
  }
  ...
  CDXLPhysicalPartitionSelector *dxl_op =
      GPOS_NEW(m_mp) CDXLPhysicalPartitionSelector(m_mp, m_rel_mdid, m_selector_id, m_scan_id, m_partitions);
  ...
  if (4 == this->Length()) {
    CParseHandlerPhysicalOp *child_parse_handler = dynamic_cast<CParseHandlerPhysicalOp *>((*this)[this->Length() - 1]);
    AddChildFromParseHandler(child_parse_handler);
  }
