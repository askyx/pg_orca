...
//
//---------------------------------------------------------------------------
CParseHandlerExtStats::CParseHandlerExtStats(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerExtStats::StartElement(const XMLCh *const,  // element_uri,
                                         const XMLCh *const element_local_name GPOS_UNUSED,
                                         const XMLCh *const,  // element_qname,
                                         const Attributes &attrs GPOS_UNUSED) {
  if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenExtendedStats), element_local_name)) {
    CWStringDynamic *str =
  ...
  m_mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
                                                              EdxltokenMdid, EdxltokenExtendedStats);

  const XMLCh *parsed_stat_name = CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenName, EdxltokenExtendedStats);
  ...
  CParseHandlerBase *ndistincts_parse_handler = CParseHandlerFactory::GetParseHandler(
      m_mp, CDXLTokens::XmlstrToken(EdxltokenMVNDistinctList), m_parse_handler_mgr, this);
  m_parse_handler_mgr->ActivateParseHandler(ndistincts_parse_handler);

  this->Append(ndistincts_parse_handler);

  CParseHandlerBase *dependencies_parse_handler = CParseHandlerFactory::GetParseHandler(
      m_mp, CDXLTokens::XmlstrToken(EdxltokenMVDependencyList), m_parse_handler_mgr, this);
...
//
//---------------------------------------------------------------------------
void CParseHandlerExtStats::EndElement(const XMLCh *const,  // element_uri,
  ...
  m_mdid->AddRef();
  m_imd_obj = GPOS_NEW(m_mp) CDXLExtStats(m_mp, m_mdid, m_mdname, dependencies_parse_handler->GetDependencies(),
