...
//
//---------------------------------------------------------------------------
CParseHandlerDirectDispatchInfo::CParseHandlerDirectDispatchInfo(CMemoryPool *mp,
...
//
//---------------------------------------------------------------------------
void CParseHandlerDirectDispatchInfo::StartElement(const XMLCh *const,  // element_uri,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenDirectDispatchInfo), element_local_name)) {
    m_dispatch_is_raw = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenDirectDispatchIsRaw, EdxltokenDirectDispatchInfo,
        true,  // optional
        false  // default
    );
    ...
    m_datum_array_combination = GPOS_NEW(m_mp) CDXLDatum2dArray(m_mp);
             ...
             XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenDirectDispatchKeyValue), element_local_name)) {
    CRefCount::SafeRelease(m_dxl_datum_array);
    ...
    CDXLDatum *dxl_datum = CDXLOperatorFactory::GetDatumVal(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
...
//
//---------------------------------------------------------------------------
void CParseHandlerDirectDispatchInfo::EndElement(const XMLCh *const,  // element_uri,
                                                 const XMLCh *const element_local_name,
                                                 const XMLCh *const  // element_qname
) {
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenDirectDispatchInfo), element_local_name)) {
...
//
//---------------------------------------------------------------------------
CDXLDirectDispatchInfo *CParseHandlerDirectDispatchInfo::GetDXLDirectDispatchInfo() const {
