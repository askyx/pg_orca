...
//
//---------------------------------------------------------------------------
CParseHandlerScalarArrayRef::CParseHandlerScalarArrayRef(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerScalarArrayRef::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarArrayRef), element_local_name)) {
    ...
    IMDId *elem_type_mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenArrayElementType, EdxltokenScalarArrayRef);
    IMDId *array_type_mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenArrayType, EdxltokenScalarArrayRef);
    IMDId *return_type_mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenTypeId, EdxltokenScalarArrayRef);
    INT type_modifier = CDXLOperatorFactory::ExtractConvertAttrValueToInt(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenTypeMod, EdxltokenScalarArrayRef, true,
        default_type_modifier);

    m_dxl_node = GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarArrayRef(m_mp, elem_type_mdid, type_modifier,
                                                                                 array_type_mdid, return_type_mdid));
  } else if (0 ==
             XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarArrayRefIndexList), element_local_name)) {
    GPOS_ASSERT(nullptr != m_dxl_node);
    GPOS_ASSERT(2 > m_parse_index_lists);

    // parse index list
    CParseHandlerBase *child_parse_handler = CParseHandlerFactory::GetParseHandler(
        m_mp, CDXLTokens::XmlstrToken(EdxltokenScalarArrayRefIndexList), m_parse_handler_mgr, this);
    ...
    m_parsing_ref_expr = true;
             ...
             XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarArrayRefAssignExpr), element_local_name)) {
    GPOS_ASSERT(nullptr != m_dxl_node);
    GPOS_ASSERT(2 == m_parse_index_lists);
    GPOS_ASSERT(!m_parsing_ref_expr);
    GPOS_ASSERT(!m_parsing_assign_expr);
...
//
//---------------------------------------------------------------------------
void CParseHandlerScalarArrayRef::EndElement(const XMLCh *const,  // element_uri,
