...
//
//---------------------------------------------------------------------------
CParseHandlerAppend::CParseHandlerAppend(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
                                         CParseHandlerBase *parse_handler_root)
    : CParseHandlerPhysicalOp(mp, parse_handler_mgr, parse_handler_root), m_dxl_op(nullptr) {}
...
//
//---------------------------------------------------------------------------
void CParseHandlerAppend::SetupInitialHandlers(const Attributes &attrs) {
  // seeing a result tag
  GPOS_ASSERT(m_dxl_op == nullptr && "Append dxl node should not have been created yet");
  GPOS_ASSERT(this->Length() == 0 && "No handlers should have been added yet");

  m_dxl_op =
      (CDXLPhysicalAppend *)CDXLOperatorFactory::MakeDXLAppend(m_parse_handler_mgr->GetDXLMemoryManager(), attrs);
...
//
//---------------------------------------------------------------------------
void CParseHandlerAppend::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
...
//
//---------------------------------------------------------------------------
void CParseHandlerAppend::EndElement(const XMLCh *const,  // element_uri,
  ...
  CParseHandlerProperties *prop_parse_handler = dynamic_cast<CParseHandlerProperties *>((*this)[child_index++]);
  CParseHandlerProjList *proj_list_parse_handler = dynamic_cast<CParseHandlerProjList *>((*this)[child_index++]);
  CParseHandlerFilter *filter_parse_handler = dynamic_cast<CParseHandlerFilter *>((*this)[child_index++]);
  ...
  for (; child_index < length; ++child_index) {
    CParseHandlerPhysicalOp *child_parse_handler = dynamic_cast<CParseHandlerPhysicalOp *>((*this)[child_index]);
