...
//
//---------------------------------------------------------------------------
CParseHandlerWindowSpec::CParseHandlerWindowSpec(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerWindowSpec::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenWindowSpec), element_local_name)) {
    ...
    if (nullptr != xml_alias) {
      CWStringDynamic *str_alias =
          CDXLUtils::CreateDynamicStringFromXMLChArray(m_parse_handler_mgr->GetDXLMemoryManager(), xml_alias);
      m_mdname = GPOS_NEW(m_mp) CMDName(m_mp, str_alias);
      GPOS_DELETE(str_alias);
    }

    const XMLCh *xml_part_cols =
    ...
    m_part_by_colid_array = CDXLOperatorFactory::ExtractIntsToUlongArray(
        m_parse_handler_mgr->GetDXLMemoryManager(), xml_part_cols, EdxltokenPartKeys, EdxltokenPhysicalWindow);
...
//
//---------------------------------------------------------------------------
void CParseHandlerWindowSpec::EndElement(const XMLCh *const,  // element_uri,
  ...
  m_dxl_window_spec_gen =
      GPOS_NEW(m_mp) CDXLWindowSpec(m_mp, m_part_by_colid_array, m_mdname, sort_col_list_dxlnode, window_frame);
