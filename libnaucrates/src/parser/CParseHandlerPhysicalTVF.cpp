...
//
//---------------------------------------------------------------------------
CParseHandlerPhysicalTVF::CParseHandlerPhysicalTVF(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerPhysicalTVF::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPhysicalTVF), element_local_name)) {
    ...
    // parse function name
    const XMLCh *xml_str_func_name = CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenName, EdxltokenPhysicalTVF);
    ...
    m_return_type_mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenTypeId, EdxltokenPhysicalTVF);
...
//
//---------------------------------------------------------------------------
void CParseHandlerPhysicalTVF::EndElement(const XMLCh *const,  // element_uri,
  ...
  CDXLPhysicalTVF *dxl_op = GPOS_NEW(m_mp) CDXLPhysicalTVF(m_mp, m_func_mdid, m_return_type_mdid, m_pstr);
