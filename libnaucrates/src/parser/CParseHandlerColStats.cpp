...
//
//---------------------------------------------------------------------------
CParseHandlerColStats::CParseHandlerColStats(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerColStats::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenColumnStats), element_local_name)) {
    ...
    // parse column name
    const XMLCh *parsed_column_name = CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenName, EdxltokenColumnStats);
    ...
    m_width = CDXLOperatorFactory::ExtractConvertAttrValueToDouble(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
                                                                   EdxltokenWidth, EdxltokenColumnStats);
    ...
    if (nullptr != parsed_null_freq) {
      m_null_freq = CDXLOperatorFactory::ConvertAttrValueToDouble(
          m_parse_handler_mgr->GetDXLMemoryManager(), parsed_null_freq, EdxltokenColNullFreq, EdxltokenColumnStats);
    }
    ...
    if (nullptr != parsed_distinct_remaining) {
      m_distinct_remaining = CDXLOperatorFactory::ConvertAttrValueToDouble(m_parse_handler_mgr->GetDXLMemoryManager(),
                                                                           parsed_distinct_remaining,
                                                                           EdxltokenColNdvRemain, EdxltokenColumnStats);
    }
    ...
    if (nullptr != parsed_freq_remaining) {
      m_freq_remaining = CDXLOperatorFactory::ConvertAttrValueToDouble(m_parse_handler_mgr->GetDXLMemoryManager(),
    ...
    if (nullptr != parsed_is_column_stats_missing) {
      m_is_column_stats_missing = CDXLOperatorFactory::ConvertAttrValueToBool(
          m_parse_handler_mgr->GetDXLMemoryManager(), parsed_is_column_stats_missing, EdxltokenColStatsMissing,
          EdxltokenColumnStats);
    }
...
//
//---------------------------------------------------------------------------
void CParseHandlerColStats::EndElement(const XMLCh *const,  // element_uri,
  ...
  m_imd_obj = GPOS_NEW(m_mp) CDXLColStats(m_mp, m_mdid, m_md_name, m_width, m_null_freq, m_distinct_remaining,
