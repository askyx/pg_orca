...
//
//---------------------------------------------------------------------------
CParseHandlerFilter::CParseHandlerFilter(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerFilter::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarFilter), element_local_name)) {
    ...
    m_dxl_node = GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarOneTimeFilter(m_mp));
             ...
             XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarRecheckCondFilter), element_local_name)) {
    // start the filter
...
//
//---------------------------------------------------------------------------
void CParseHandlerFilter::EndElement(const XMLCh *const,  // element_uri,
