...
//
//---------------------------------------------------------------------------
CParseHandlerLogicalConstTable::CParseHandlerLogicalConstTable(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerLogicalConstTable::StartElement(const XMLCh *const,  // element_uri,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenLogicalConstTable), element_local_name)) {
    ...
    GPOS_ASSERT(nullptr != m_const_tuples_datum_array);  // we must have already seen a logical const table
    ...
    CDXLDatum *dxl_datum =
        CDXLOperatorFactory::GetDatumVal(m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenScalarConstValue);
...
//
//---------------------------------------------------------------------------
void CParseHandlerLogicalConstTable::EndElement(const XMLCh *const,  // element_uri,
                                                const XMLCh *const element_local_name,
                                                const XMLCh *const  // element_qname
) {
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenLogicalConstTable), element_local_name)) {
    ...
    CDXLLogicalConstTable *lg_const_table_get_dxl_op =
        GPOS_NEW(m_mp) CDXLLogicalConstTable(m_mp, dxl_col_descr_array, m_const_tuples_datum_array);
    ...
    m_dxl_datum_array = nullptr;  // intialize for the parsing the next const tuple (if needed)
  } else if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenDatum), element_local_name)) {
    CWStringDynamic *str =
        CDXLUtils::CreateDynamicStringFromXMLChArray(m_parse_handler_mgr->GetDXLMemoryManager(), element_local_name);
    GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiDXLUnexpectedTag, str->GetBuffer());
