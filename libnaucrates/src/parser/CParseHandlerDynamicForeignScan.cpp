...
//
//---------------------------------------------------------------------------
CParseHandlerDynamicForeignScan::CParseHandlerDynamicForeignScan(CMemoryPool *mp,
...
//
//---------------------------------------------------------------------------
void CParseHandlerDynamicForeignScan::StartElement(const XMLCh *const,  // element_uri,
                                                   const XMLCh *const element_local_name,
                                                   const XMLCh *const,  // element_qname
                                                   const Attributes &attrs) {
  if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPhysicalDynamicForeignScan), element_local_name)) {
    CWStringDynamic *str =
  ...
  m_selector_ids = CDXLOperatorFactory::ExtractConvertValuesToArray(
      m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenSelectorIds, EdxltokenPhysicalDynamicForeignScan);

  m_foreign_server_oid =
      CDXLOperatorFactory::ExtractConvertAttrValueToOid(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
...
//
//---------------------------------------------------------------------------
void CParseHandlerDynamicForeignScan::EndElement(const XMLCh *const,  // element_uri,
                                                 const XMLCh *const element_local_name,
                                                 const XMLCh *const  // element_qname
) {
  if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPhysicalDynamicForeignScan), element_local_name)) {
  ...
  CParseHandlerMetadataIdList *partition_mdids_parse_handler = dynamic_cast<CParseHandlerMetadataIdList *>((*this)[3]);
  CParseHandlerTableDescr *table_descr_parse_handler = dynamic_cast<CParseHandlerTableDescr *>((*this)[4]);
  ...
  CDXLPhysicalDynamicForeignScan *dxl_op = GPOS_NEW(m_mp)
      CDXLPhysicalDynamicForeignScan(m_mp, table_descr, mdid_partitions_array, m_selector_ids, m_foreign_server_oid);
