...
//
//---------------------------------------------------------------------------
CParseHandlerIndexScan::CParseHandlerIndexScan(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
                                               CParseHandlerBase *parse_handler_root)
    : CParseHandlerPhysicalOp(mp, parse_handler_mgr, parse_handler_root), m_index_scan_dir(EdxlisdSentinel) {}
...
//
//---------------------------------------------------------------------------
void CParseHandlerIndexScan::StartElement(const XMLCh *const,  // element_uri,
...
//
//---------------------------------------------------------------------------
void CParseHandlerIndexScan::EndElement(const XMLCh *const,  // element_uri,
                                        const XMLCh *const element_local_name,
                                        const XMLCh *const  // element_qname
) {
...
//
//---------------------------------------------------------------------------
void CParseHandlerIndexScan::StartElementHelper(const XMLCh *const element_local_name, const Attributes &attrs,
  ...
  m_index_scan_dir =
      CDXLOperatorFactory::ParseIndexScanDirection(index_scan_direction, CDXLTokens::GetDXLTokenStr(token_type));
  ...
  CParseHandlerBase *partition_mdids_parse_handler = CParseHandlerFactory::GetParseHandler(
...
//
//---------------------------------------------------------------------------
void CParseHandlerIndexScan::EndElementHelper(const XMLCh *const element_local_name, Edxltoken token_type,
  ...
  CParseHandlerMetadataIdList *partition_mdids_parse_handler = dynamic_cast<CParseHandlerMetadataIdList *>((*this)[4]);
  CParseHandlerIndexDescr *index_descr_parse_handler = dynamic_cast<CParseHandlerIndexDescr *>((*this)[5]);
  CParseHandlerTableDescr *table_descr_parse_handler = dynamic_cast<CParseHandlerTableDescr *>((*this)[6]);
  ...
  if (EdxltokenPhysicalIndexOnlyScan == token_type) {
    dxl_op = GPOS_NEW(m_mp) CDXLPhysicalIndexOnlyScan(m_mp, dxl_table_descr, dxl_index_descr, m_index_scan_dir);
    m_dxl_node = GPOS_NEW(m_mp) CDXLNode(m_mp, dxl_op);
  } else if (EdxltokenPhysicalIndexScan == token_type) {
    dxl_op = GPOS_NEW(m_mp) CDXLPhysicalIndexScan(m_mp, dxl_table_descr, dxl_index_descr, m_index_scan_dir);
    m_dxl_node = GPOS_NEW(m_mp) CDXLNode(m_mp, dxl_op);
    ...
    dxl_op = GPOS_NEW(m_mp) CDXLPhysicalDynamicIndexOnlyScan(m_mp, dxl_table_descr, dxl_index_descr, m_index_scan_dir,
    ...
    IMdIdArray *mdid_partitions_array = partition_mdids_parse_handler->GetMdIdArray();
    mdid_partitions_array->AddRef();
    dxl_op = GPOS_NEW(m_mp) CDXLPhysicalDynamicIndexScan(m_mp, dxl_table_descr, dxl_index_descr, m_index_scan_dir,
                                                         mdid_partitions_array, selector_ids);
    m_dxl_node = GPOS_NEW(m_mp) CDXLNode(m_mp, dxl_op);
  }
