...
//
//---------------------------------------------------------------------------
CParseHandlerStatsDerivedColumn::CParseHandlerStatsDerivedColumn(CMemoryPool *mp,
...
//
//---------------------------------------------------------------------------
void CParseHandlerStatsDerivedColumn::StartElement(const XMLCh *const element_uri,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenStatsDerivedColumn), element_local_name)) {
    ...
    // parse column id
    m_colid = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
                                                                  EdxltokenColId, EdxltokenStatsDerivedColumn);

    // parse column width
    m_width = CDXLOperatorFactory::ExtractConvertAttrValueToDouble(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
                                                                   EdxltokenWidth, EdxltokenStatsDerivedColumn);
...
//
//---------------------------------------------------------------------------
void CParseHandlerStatsDerivedColumn::EndElement(const XMLCh *const,  // element_uri,
                                                 const XMLCh *const element_local_name,
  ...
  m_dxl_stats_derived_col = GPOS_NEW(m_mp) CDXLStatsDerivedColumn(m_colid, m_width, m_null_freq, m_distinct_remaining,
