...
//
//---------------------------------------------------------------------------
CParseHandlerMDIndex::CParseHandlerMDIndex(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerMDIndex::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPartitions), element_local_name)) {
    ...
    m_parse_handler_mgr->ActivateParseHandler(m_child_indexes_parse_handler);
    this->Append(m_child_indexes_parse_handler);
    m_child_indexes_parse_handler->startElement(element_uri, element_local_name, element_qname, attrs);
  ...
  // parse index name
  const XMLCh *parsed_column_name = CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenName, EdxltokenIndex);
  CWStringDynamic *column_name =
      CDXLUtils::CreateDynamicStringFromXMLChArray(m_parse_handler_mgr->GetDXLMemoryManager(), parsed_column_name);
  ...
  m_clustered = CDXLOperatorFactory::ExtractConvertAttrValueToBool(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
                                                                   EdxltokenIndexClustered, EdxltokenIndex);

  // parse index access method ordering
  m_amcanorder = CDXLOperatorFactory::ExtractConvertAttrValueToBool(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
  ...
  if (nullptr != xmlszItemType) {
    m_mdid_item_type = CDXLOperatorFactory::MakeMdIdFromStr(m_parse_handler_mgr->GetDXLMemoryManager(), xmlszItemType,
  ...
  m_index_key_cols_array = CDXLOperatorFactory::ExtractIntsToUlongArray(
      m_parse_handler_mgr->GetDXLMemoryManager(), xmlszIndexKeys, EdxltokenIndexKeyCols, EdxltokenIndex);
  ...
  m_included_cols_array = CDXLOperatorFactory::ExtractIntsToUlongArray(
      m_parse_handler_mgr->GetDXLMemoryManager(), xmlszIndexIncludedCols, EdxltokenIndexIncludedCols, EdxltokenIndex);

  // parse index returnable column information
  const XMLCh *xmlszIndexReturnableCols =
      CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenIndexReturnableCols, EdxltokenIndex);
  m_returnable_cols_array =
      CDXLOperatorFactory::ExtractIntsToUlongArray(m_parse_handler_mgr->GetDXLMemoryManager(), xmlszIndexReturnableCols,
                                                   EdxltokenIndexReturnableCols, EdxltokenIndex);

  // extract index keys sort directions
  const XMLCh *xmlszIndexKeysSortDirections =
      CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenIndexKeysSortDirection, EdxltokenIndex, true);
  m_sort_direction = CDXLOperatorFactory::ExtractConvertBooleanListToULongArray(
      m_parse_handler_mgr->GetDXLMemoryManager(), xmlszIndexKeysSortDirections,
      CDXLTokens::XmlstrToken(EdxltokenIndexKeySortDESC), CDXLTokens::XmlstrToken(EdxltokenIndexKeySortASC),
      m_index_key_cols_array->Size());

  // extract index keys nulls directions
  const XMLCh *xmlszIndexKeysNullsDirections =
      CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenIndexKeysNullsDirection, EdxltokenIndex, true);
  m_nulls_direction = CDXLOperatorFactory::ExtractConvertBooleanListToULongArray(
      m_parse_handler_mgr->GetDXLMemoryManager(), xmlszIndexKeysNullsDirections,
      CDXLTokens::XmlstrToken(EdxltokenIndexKeyNullsFirst), CDXLTokens::XmlstrToken(EdxltokenIndexKeyNullsLast),
      m_index_key_cols_array->Size());
...
//
//---------------------------------------------------------------------------
void CParseHandlerMDIndex::EndElement(const XMLCh *const,  // element_uri,
  ...
  if (nullptr != m_child_indexes_parse_handler) {
    is_partitioned = true;
    child_indexes = dynamic_cast<CParseHandlerMetadataIdList *>(m_child_indexes_parse_handler)->GetMdIdArray();
    child_indexes->AddRef();
  }

  m_imd_obj = GPOS_NEW(m_mp)
      CMDIndexGPDB(m_mp, m_mdid, m_mdname, m_clustered, is_partitioned, m_amcanorder, m_index_type, m_mdid_item_type,
                   m_index_key_cols_array, m_included_cols_array, m_returnable_cols_array, mdid_opfamilies_array,
