...
//
//---------------------------------------------------------------------------
CParseHandlerLogicalSetOp::CParseHandlerLogicalSetOp(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerLogicalSetOp::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
  ...
  if (0 == this->Length()) {
    ...
    m_input_colids_arrays = CDXLOperatorFactory::ExtractConvertUlongTo2DArray(
        m_parse_handler_mgr->GetDXLMemoryManager(), input_colids_array_str, EdxltokenInputCols,
    ...
    m_cast_across_input_req = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenCastAcrossInputs, EdxltokenLogicalSetOperation);
...
//
//---------------------------------------------------------------------------
EdxlSetOpType CParseHandlerLogicalSetOp::GetSetOpType(const XMLCh *const element_local_name) {
...
//
//---------------------------------------------------------------------------
void CParseHandlerLogicalSetOp::EndElement(const XMLCh *const,  // element_uri,
  ...
  CDXLLogicalSetOp *dxl_op = GPOS_NEW(m_mp)
      CDXLLogicalSetOp(m_mp, setop_type, cold_descr_dxl_array, m_input_colids_arrays, m_cast_across_input_req);
