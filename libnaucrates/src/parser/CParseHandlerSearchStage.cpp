...
//
//---------------------------------------------------------------------------
CParseHandlerSearchStage::CParseHandlerSearchStage(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerSearchStage::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenSearchStage), element_local_name)) {
    ...
    m_cost_threshold = CCost(CDXLOperatorFactory::ConvertAttrValueToDouble(
        m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_cost, EdxltokenCostThreshold, EdxltokenSearchStage));
    ...
    m_time_threshold = CDXLOperatorFactory::ConvertAttrValueToUlong(
        m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_time, EdxltokenTimeThreshold, EdxltokenSearchStage);
    ...
    CParseHandlerBase *xform_set_parse_handler =
        CParseHandlerFactory::GetParseHandler(m_mp, CDXLTokens::XmlstrToken(EdxltokenXform), m_parse_handler_mgr, this);
...
//
//---------------------------------------------------------------------------
void CParseHandlerSearchStage::EndElement(const XMLCh *const,  // element_uri,
  ...
  for (ULONG idx = 0; idx < size; idx++) {
    ...
    BOOL fSet GPOS_ASSERTS_ONLY = m_xforms->ExchangeSet(xform_set_parse_handler->GetXform()->Exfid());
    GPOS_ASSERT(!fSet);
  }
