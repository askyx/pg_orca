...
//
//---------------------------------------------------------------------------
CParseHandlerAssert::CParseHandlerAssert(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerAssert::StartElement(const XMLCh *const,  // element_uri,
  ...
  CHAR *error_code = CDXLOperatorFactory::ExtractConvertAttrValueToSz(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
                                                                      EdxltokenErrorCode, EdxltokenPhysicalAssert);
  if (nullptr == error_code || GPOS_SQLSTATE_LENGTH != clib::Strlen(error_code)) {
  ...
  CParseHandlerBase *assert_pred_parse_handler = CParseHandlerFactory::GetParseHandler(
      m_mp, CDXLTokens::XmlstrToken(EdxltokenScalarAssertConstraintList), m_parse_handler_mgr, this);
...
//
//---------------------------------------------------------------------------
void CParseHandlerAssert::EndElement(const XMLCh *const,  // element_uri,
                                     const XMLCh *const element_local_name,
                                     const XMLCh *const  // element_qname
) {
  if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPhysicalAssert), element_local_name)) {
