...
//
//---------------------------------------------------------------------------
CParseHandlerScalarSubqueryQuantified::CParseHandlerScalarSubqueryQuantified(CMemoryPool *mp,
                                                                             CParseHandlerManager *parse_handler_mgr,
                                                                             CParseHandlerBase *parse_handler_root)
    : CParseHandlerScalarOp(mp, parse_handler_mgr, parse_handler_root), m_dxl_op(nullptr) {}
...
//
//---------------------------------------------------------------------------
void CParseHandlerScalarSubqueryQuantified::StartElement(const XMLCh *const,  // element_uri,
  ...
  // parse operator name
  const XMLCh *xmlszScalarOpName = CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenOpName, dxl_token);

  CWStringDynamic *op_name_str =
      CDXLUtils::CreateDynamicStringFromXMLChArray(m_parse_handler_mgr->GetDXLMemoryManager(), xmlszScalarOpName);
  CMDName *md_op_name = GPOS_NEW(m_mp) CMDName(m_mp, op_name_str);
  GPOS_DELETE(op_name_str);

  // parse column id
  ULONG colid = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
...
//
//---------------------------------------------------------------------------
void CParseHandlerScalarSubqueryQuantified::EndElement(const XMLCh *const,  // element_uri,
                                                       const XMLCh *const element_local_name,
                                                       const XMLCh *const  // element_qname
) {
