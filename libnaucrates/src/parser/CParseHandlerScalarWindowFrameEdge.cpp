...
//
//---------------------------------------------------------------------------
CParseHandlerScalarWindowFrameEdge::CParseHandlerScalarWindowFrameEdge(CMemoryPool *mp,
...
//
//---------------------------------------------------------------------------
void CParseHandlerScalarWindowFrameEdge::StartElement(const XMLCh *const element_uri,
                                                      const XMLCh *const element_local_name,
                                                      const XMLCh *const element_qname, const Attributes &attrs) {
  if (0 ==
      XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarWindowFrameLeadingEdge), element_local_name)) {
    ...
    m_dxl_node = GPOS_NEW(m_mp)
        CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarWindowFrameEdge(m_mp, true /*fLeading*/, dxl_frame_bound));
  } else if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarWindowFrameTrailingEdge),
                                           element_local_name)) {
    GPOS_ASSERT(nullptr == m_dxl_node);
    EdxlFrameBoundary dxl_frame_bound =
        CDXLOperatorFactory::ParseDXLFrameBoundary(attrs, EdxltokenWindowTrailingBoundary);
    m_dxl_node = GPOS_NEW(m_mp)
        CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarWindowFrameEdge(m_mp, false /*fLeading*/, dxl_frame_bound));
  } else {
...
//
//---------------------------------------------------------------------------
void CParseHandlerScalarWindowFrameEdge::EndElement(const XMLCh *const,  // element_uri,
                                                    const XMLCh *const element_local_name,
                                                    const XMLCh *const  // element_qname
) {
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarWindowFrameLeadingEdge),
                                    element_local_name) ||
      0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarWindowFrameTrailingEdge),
                                    element_local_name)) {
