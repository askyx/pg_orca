...
//
//---------------------------------------------------------------------------
CParseHandlerMDGPDBScalarOp::CParseHandlerMDGPDBScalarOp(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerMDGPDBScalarOp::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOp), element_local_name)) {
    // parse operator name
    const XMLCh *xml_str_op_name = CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenName, EdxltokenGPDBScalarOp);

    CWStringDynamic *str_opname =
        CDXLUtils::CreateDynamicStringFromXMLChArray(m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_op_name);
    ...
    const XMLCh *xml_str_comp_type =
        CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenGPDBScalarOpCmpType, EdxltokenGPDBScalarOp);
    ...
    if (nullptr != xml_str_returns_null_on_null_input) {
      m_returns_null_on_null_input = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
          m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenReturnsNullOnNullInput, EdxltokenGPDBScalarOp);
    }

    // ndv-preserving property is optional
    m_is_ndv_preserving = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenIsNDVPreserving, EdxltokenGPDBScalarOp,
        true,  // is optional
        false  // default value
    );
  } else if (0 ==
             XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOpLeftTypeId), element_local_name)) {
    // parse left operand's type
    GPOS_ASSERT(nullptr != m_mdname);

    m_mdid_type_left = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenMdid, EdxltokenGPDBScalarOpLeftTypeId);
  } else if (0 ==
             XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOpRightTypeId), element_local_name)) {
    // parse right operand's type
    GPOS_ASSERT(nullptr != m_mdname);

    m_mdid_type_right = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenMdid, EdxltokenGPDBScalarOpRightTypeId);
  } else if (0 ==
             XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOpResultTypeId), element_local_name)) {
    // parse result type
    GPOS_ASSERT(nullptr != m_mdname);

    m_mdid_type_result = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenMdid, EdxltokenGPDBScalarOpResultTypeId);
  } else if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOpFuncId), element_local_name)) {
    // parse op func id
    GPOS_ASSERT(nullptr != m_mdname);

    m_func_mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
                                                                     EdxltokenMdid, EdxltokenGPDBScalarOpFuncId);
  } else if (0 ==
             XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOpCommOpId), element_local_name)) {
    // parse commutator operator
    GPOS_ASSERT(nullptr != m_mdname);

    m_mdid_commute_opr = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenMdid, EdxltokenGPDBScalarOpCommOpId);
  } else if (0 ==
             XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOpInverseOpId), element_local_name)) {
    // parse inverse operator id
    GPOS_ASSERT(nullptr != m_mdname);

    m_mdid_inverse_opr = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenMdid, EdxltokenGPDBScalarOpInverseOpId);
    ...
    CParseHandlerBase *opfamilies_list_parse_handler = CParseHandlerFactory::GetParseHandler(
    ...
    opfamilies_list_parse_handler->startElement(element_uri, element_local_name, element_qname, attrs);
             ...
             XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOpHashOpfamily), element_local_name)) {
    // parse inverse operator id
    GPOS_ASSERT(nullptr != m_mdname);

    m_mdid_hash_opfamily = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenMdid, EdxltokenGPDBScalarOpHashOpfamily);
  } else if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOpLegacyHashOpfamily),
                                           element_local_name)) {
    // parse inverse operator id
    GPOS_ASSERT(nullptr != m_mdname);

    m_mdid_legacy_hash_opfamily = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenMdid, EdxltokenGPDBScalarOpLegacyHashOpfamily);
  } else {
    CWStringDynamic *str =
...
//
//---------------------------------------------------------------------------
void CParseHandlerMDGPDBScalarOp::EndElement(const XMLCh *const,  // element_uri,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOp), element_local_name)) {
    ...
    m_imd_obj = GPOS_NEW(m_mp)
        CMDScalarOpGPDB(m_mp, m_mdid, m_mdname, m_mdid_type_left, m_mdid_type_right, m_mdid_type_result, m_func_mdid,
                        m_mdid_commute_opr, m_mdid_inverse_opr, m_comparision_type, m_returns_null_on_null_input,
                        mdid_opfamilies_array, m_mdid_hash_opfamily, m_mdid_legacy_hash_opfamily, m_is_ndv_preserving);
...
//
//---------------------------------------------------------------------------
BOOL CParseHandlerMDGPDBScalarOp::IsSupportedChildElem(const XMLCh *const xml_str) {
