...
//
//---------------------------------------------------------------------------
CParseHandlerOptimizerConfig::CParseHandlerOptimizerConfig(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
                                                           CParseHandlerBase *parse_handler_root)
    : CParseHandlerBase(mp, parse_handler_mgr, parse_handler_root), m_pbs(nullptr), m_optimizer_config(nullptr) {}
...
//
//---------------------------------------------------------------------------
void CParseHandlerOptimizerConfig::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenHint), element_local_name)) {
    ...
    CParseHandlerBase *pphHint =
        CParseHandlerFactory::GetParseHandler(m_mp, CDXLTokens::XmlstrToken(EdxltokenHint), m_parse_handler_mgr, this);
    m_parse_handler_mgr->ActivateParseHandler(pphHint);
    pphHint->startElement(element_uri, element_local_name, element_qname, attrs);
    this->Append(pphHint);
    return;
  } else if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPlanHint), element_local_name)) {
    // install a parse handler for the plan hint config
    CParseHandlerBase *pphHint = CParseHandlerFactory::GetParseHandler(m_mp, CDXLTokens::XmlstrToken(EdxltokenPlanHint),
...
//
//---------------------------------------------------------------------------
void CParseHandlerOptimizerConfig::EndElement(const XMLCh *const,  // element_uri,
...
#define MAX_CONFIG_FIELDS 8
  ...
  CParseHandlerPlanHint *pphHint = dynamic_cast<CParseHandlerPlanHint *>((*this)[this->Length() - 2]);
  ...
  m_optimizer_config =
      GPOS_NEW(m_mp) COptimizerConfig(pec, stats_config, pcteconfig, pcm, phint, pplanhint, pwindowoidsGPDB);
...
//
//---------------------------------------------------------------------------
EDxlParseHandlerType CParseHandlerOptimizerConfig::GetParseHandlerType() const {
...
//
//---------------------------------------------------------------------------
CBitSet *CParseHandlerOptimizerConfig::Pbs() const {
  return m_pbs;
}
...
//
//---------------------------------------------------------------------------
COptimizerConfig *CParseHandlerOptimizerConfig::GetOptimizerConfig() const {
