...
//
//---------------------------------------------------------------------------
CParseHandlerLogicalJoin::CParseHandlerLogicalJoin(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerLogicalJoin::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenLogicalJoin), element_local_name)) {
    if (nullptr == m_dxl_node) {
      ...
      CDXLLogicalJoin *pdxlopJoin =
          (CDXLLogicalJoin *)CDXLOperatorFactory::MakeLogicalJoin(m_parse_handler_mgr->GetDXLMemoryManager(), attrs);
...
//
//---------------------------------------------------------------------------
void CParseHandlerLogicalJoin::EndElement(const XMLCh *const,  // element_uri,
