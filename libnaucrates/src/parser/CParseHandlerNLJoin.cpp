...
//
//---------------------------------------------------------------------------
CParseHandlerNLJoin::CParseHandlerNLJoin(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerNLJoin::StartElement(const XMLCh *const,  // element_uri,
  ...
  m_dxl_op =
      (CDXLPhysicalNLJoin *)CDXLOperatorFactory::MakeDXLNLJoin(m_parse_handler_mgr->GetDXLMemoryManager(), attrs);
...
//
//---------------------------------------------------------------------------
void CParseHandlerNLJoin::EndElement(const XMLCh *const,  // element_uri,
                                     const XMLCh *const element_local_name,
                                     const XMLCh *const  // element_qname
) {
  if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPhysicalNLJoin), element_local_name)) {
  ...
  if (m_dxl_op->NestParamsExists()) {
    CParseHandlerNLJIndexParamList *nest_params_parse_handler =
        dynamic_cast<CParseHandlerNLJIndexParamList *>((*this)[EdxlParseHandlerNLJIndexNestLoopParams]);
