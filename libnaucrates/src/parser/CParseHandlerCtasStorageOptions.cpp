...
//
//---------------------------------------------------------------------------
CParseHandlerCtasStorageOptions::CParseHandlerCtasStorageOptions(CMemoryPool *mp,
...
//
//---------------------------------------------------------------------------
void CParseHandlerCtasStorageOptions::StartElement(const XMLCh *const,  // element_uri
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenCTASOptions), element_local_name)) {
    ...
    if (nullptr != xml_str_tablespace) {
      m_mdname_tablespace =
          CDXLUtils::CreateMDNameFromXMLChar(m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_tablespace);
    }
    ...
    ULONG ctas_option_type = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenCtasOptionType, EdxltokenCTASOption);
    ...
    BOOL is_null = CDXLOperatorFactory::ExtractConvertAttrValueToBool(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
                                                                      EdxltokenIsNull, EdxltokenCTASOption);
...
//
//---------------------------------------------------------------------------
void CParseHandlerCtasStorageOptions::EndElement(const XMLCh *const,  // element_uri,
                                                 const XMLCh *const element_local_name,
                                                 const XMLCh *const  // element_qname
) {
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenCTASOptions), element_local_name)) {
    m_dxl_ctas_storage_option = GPOS_NEW(m_mp)
        CDXLCtasStorageOptions(m_mdname_tablespace, m_ctas_on_commit_action, m_ctas_storage_option_array);
    // deactivate handler
...
//
//---------------------------------------------------------------------------
CDXLCtasStorageOptions *CParseHandlerCtasStorageOptions::GetDxlCtasStorageOption() const {
