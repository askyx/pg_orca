...
//
//---------------------------------------------------------------------------
CParseHandlerLogicalTVF::CParseHandlerLogicalTVF(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerLogicalTVF::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenLogicalTVF), element_local_name)) {
    ...
    m_func_mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
                                                                     EdxltokenFuncId, EdxltokenLogicalTVF);

    // parse function name
    const XMLCh *func_name = CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenName, EdxltokenLogicalTVF);

    CWStringDynamic *func_name_str =
        CDXLUtils::CreateDynamicStringFromXMLChArray(m_parse_handler_mgr->GetDXLMemoryManager(), func_name);
    ...
    m_return_type_mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenTypeId, EdxltokenLogicalTVF);
...
//
//---------------------------------------------------------------------------
void CParseHandlerLogicalTVF::EndElement(const XMLCh *const,  // element_uri,
  ...
  CDXLLogicalTVF *lg_tvf_op =
      GPOS_NEW(m_mp) CDXLLogicalTVF(m_mp, m_func_mdid, m_return_type_mdid, m_mdname, cold_descr_dxl_array);
