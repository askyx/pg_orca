...
CParseHandlerExtStatsNDistinctList::CParseHandlerExtStatsNDistinctList(CMemoryPool *mp,
                                                                       CParseHandlerManager *parse_handler_mgr,
                                                                       CParseHandlerBase *parse_handler_base)
    : CParseHandlerBase(mp, parse_handler_mgr, parse_handler_base), m_ndistincts(nullptr) {}
...
//
//---------------------------------------------------------------------------
CMDNDistinctArray *CParseHandlerExtStatsNDistinctList::GetNDistinctList() const {
  return m_ndistincts;
}
...
//
//---------------------------------------------------------------------------
void CParseHandlerExtStatsNDistinctList::StartElement(const XMLCh *const element_uri,
                                                      const XMLCh *const element_local_name,
                                                      const XMLCh *const element_qname, const Attributes &attrs) {
  if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenMVNDistinctList), element_local_name) &&
      0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenMVNDistinct), element_local_name)) {
    CWStringDynamic *str =
        CDXLUtils::CreateDynamicStringFromXMLChArray(m_parse_handler_mgr->GetDXLMemoryManager(), element_local_name);
    GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiDXLUnexpectedTag, str->GetBuffer());
  } else if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenMVNDistinct), element_local_name)) {
    // new ndistinct
    CParseHandlerBase *parse_handler_base_ndistinct = CParseHandlerFactory::GetParseHandler(
        m_mp, CDXLTokens::XmlstrToken(EdxltokenMVNDistinct), m_parse_handler_mgr, this);
    this->Append(parse_handler_base_ndistinct);

    m_parse_handler_mgr->ActivateParseHandler(parse_handler_base_ndistinct);
    parse_handler_base_ndistinct->startElement(element_uri, element_local_name, element_qname, attrs);
  }
}
...
//
//---------------------------------------------------------------------------
void CParseHandlerExtStatsNDistinctList::EndElement(const XMLCh *const,  // element_uri,
                                                    const XMLCh *const element_local_name,
