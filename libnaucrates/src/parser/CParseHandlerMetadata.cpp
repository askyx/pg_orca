...
//
//---------------------------------------------------------------------------
CParseHandlerMetadata::CParseHandlerMetadata(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
EDxlParseHandlerType CParseHandlerMetadata::GetParseHandlerType() const {
...
//
//---------------------------------------------------------------------------
IMDCacheObjectArray *CParseHandlerMetadata::GetMdIdCachedObjArray() {
...
//
//---------------------------------------------------------------------------
IMdIdArray *CParseHandlerMetadata::GetMdIdArray() {
  return m_mdid_array;
}
...
//
//---------------------------------------------------------------------------
CSystemIdArray *CParseHandlerMetadata::GetSysidPtrArray() {
...
//
//---------------------------------------------------------------------------
void CParseHandlerMetadata::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
...
//
//---------------------------------------------------------------------------
void CParseHandlerMetadata::EndElement(const XMLCh *const,  // element_uri,
...
//
//---------------------------------------------------------------------------
CSystemIdArray *CParseHandlerMetadata::GetSrcSysIdArray(const Attributes &attrs, Edxltoken dxl_token_attr,
  ...
  while (nullptr != (xml_str_sys_id = xml_str_tokenizer.nextToken())) {
    ...
    ULONG type = CDXLOperatorFactory::ConvertAttrValueToUlong(m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_type,
                                                              dxl_token_attr, dxl_token_element);

    XMLCh *xml_str_name = xml_str_sys_id_tokenizer.nextToken();
    CWStringDynamic *str_name =
        CDXLUtils::CreateDynamicStringFromXMLChArray(m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_name);

    src_sys_id_array->Append(GPOS_NEW(m_mp)
                                 CSystemId((IMDId::EMDIdType)type, str_name->GetBuffer(), str_name->Length()));
