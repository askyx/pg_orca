...
//
//---------------------------------------------------------------------------
CMDNDistinct *CParseHandlerExtStatsNDistinct::GetNDistinctAt() const {
...
//
//---------------------------------------------------------------------------
void CParseHandlerExtStatsNDistinct::StartElement(const XMLCh *const,  // element_uri,
                                                  const XMLCh *const element_local_name,
                                                  const XMLCh *const,  // element_qname,
                                                  const Attributes &attrs) {
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenMVNDistinct), element_local_name)) {
    // parse degree and relationship values
    m_ndistinct = CDXLOperatorFactory::ExtractConvertAttrValueToDouble(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenMVNDistinct, EdxltokenMVNDistinct);
    m_attnos = CDXLOperatorFactory::ExtractConvertValuesToIntBitSet(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
                                                                    EdxltokenAttno, EdxltokenMVNDistinct);
  } else {
    CWStringDynamic *str =
...
//
//---------------------------------------------------------------------------
void CParseHandlerExtStatsNDistinct::EndElement(const XMLCh *const,  // element_uri,
                                                const XMLCh *const element_local_name,
                                                const XMLCh *const  // element_qname
) {
  if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenMVNDistinct), element_local_name)) {
  ...
  m_attnos->AddRef();
  m_ndistinct_md = GPOS_NEW(m_mp) CMDNDistinct(m_mp, m_ndistinct.Get(), m_attnos);
