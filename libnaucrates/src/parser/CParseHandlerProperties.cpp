...
//
//---------------------------------------------------------------------------
CParseHandlerProperties::CParseHandlerProperties(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
CDXLPhysicalProperties *CParseHandlerProperties::GetProperties() const {
...
//
//---------------------------------------------------------------------------
void CParseHandlerProperties::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenProperties), element_local_name)) {
    ...
    CParseHandlerBase *parse_handler_root =
        CParseHandlerFactory::GetParseHandler(m_mp, CDXLTokens::XmlstrToken(EdxltokenCost), m_parse_handler_mgr, this);
    m_parse_handler_mgr->ActivateParseHandler(parse_handler_root);

    // store parse handler
    this->Append(parse_handler_root);
  } else if (0 ==
             XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenStatsDerivedRelation), element_local_name)) {
    ...
    CParseHandlerBase *parse_handler_stats = CParseHandlerFactory::GetParseHandler(
        m_mp, CDXLTokens::XmlstrToken(EdxltokenStatsDerivedRelation), m_parse_handler_mgr, this);
...
//
//---------------------------------------------------------------------------
void CParseHandlerProperties::EndElement(const XMLCh *const,  // element_uri,
