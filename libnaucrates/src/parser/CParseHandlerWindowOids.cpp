...
CParseHandlerWindowOids::CParseHandlerWindowOids(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
                                                 CParseHandlerBase *parse_handler_root)
    : CParseHandlerBase(mp, parse_handler_mgr, parse_handler_root), m_window_oids(nullptr) {}

CParseHandlerWindowOids::~CParseHandlerWindowOids() {
  CRefCount::SafeRelease(m_window_oids);
}

void CParseHandlerWindowOids::StartElement(const XMLCh *const,  // element_uri,
  ...
  OID row_number_oid = CDXLOperatorFactory::ExtractConvertAttrValueToOid(
      m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenOidRowNumber, EdxltokenWindowOids);
  OID rank_oid = CDXLOperatorFactory::ExtractConvertAttrValueToOid(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
                                                                   EdxltokenOidRank, EdxltokenWindowOids);

  m_window_oids = GPOS_NEW(m_mp) CWindowOids(row_number_oid, rank_oid);
}

// invoked by Xerces to process a closing tag
void CParseHandlerWindowOids::EndElement(const XMLCh *const,  // element_uri,
                                         const XMLCh *const element_local_name,
                                         const XMLCh *const  // element_qname
) {
  if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenWindowOids), element_local_name)) {
...
EDxlParseHandlerType CParseHandlerWindowOids::GetParseHandlerType() const {
  return EdxlphWindowOids;
}

CWindowOids *CParseHandlerWindowOids::GetWindowOids() const {
