...
//
//---------------------------------------------------------------------------
CParseHandlerScalarAggref::CParseHandlerScalarAggref(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerScalarAggref::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarAggref), element_local_name)) {
    ...
    CDXLScalarAggref *dxl_op =
        (CDXLScalarAggref *)CDXLOperatorFactory::MakeDXLAggFunc(m_parse_handler_mgr->GetDXLMemoryManager(), attrs);

    // construct node from the created scalar AggRef
    m_dxl_node = GPOS_NEW(m_mp) CDXLNode(m_mp, dxl_op);
  } else if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarValuesList), element_local_name)) {
...
//
//---------------------------------------------------------------------------
void CParseHandlerScalarAggref::EndElement(const XMLCh *const,  // element_uri,
  ...
  AddChildFromParseHandler(dynamic_cast<CParseHandlerScalarValuesList *>((*this)[EdxlParseHandlerAggrefIndexArgs]));

  AddChildFromParseHandler(
      dynamic_cast<CParseHandlerScalarValuesList *>((*this)[EdxlParseHandlerAggrefIndexDirectArgs]));

  AddChildFromParseHandler(dynamic_cast<CParseHandlerScalarValuesList *>((*this)[EdxlParseHandlerAggrefIndexOrder]));

  AddChildFromParseHandler(dynamic_cast<CParseHandlerScalarValuesList *>((*this)[EdxlParseHandlerAggrefIndexDistinct]));
