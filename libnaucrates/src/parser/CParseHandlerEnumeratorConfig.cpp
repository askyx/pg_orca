...
//
//---------------------------------------------------------------------------
CParseHandlerEnumeratorConfig::CParseHandlerEnumeratorConfig(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
                                                             CParseHandlerBase *parse_handler_root)
    : CParseHandlerBase(mp, parse_handler_mgr, parse_handler_root), m_enumerator_cfg(nullptr) {}
...
//
//---------------------------------------------------------------------------
void CParseHandlerEnumeratorConfig::StartElement(const XMLCh *const,  // element_uri,
  ...
  ULLONG plan_id = CDXLOperatorFactory::ExtractConvertAttrValueToUllong(
      m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenPlanId, EdxltokenOptimizerConfig);
  ULLONG num_of_plan_samples = CDXLOperatorFactory::ExtractConvertAttrValueToUllong(
      m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenPlanSamples, EdxltokenOptimizerConfig);
  CDouble cost_threshold = CDXLOperatorFactory::ExtractConvertAttrValueToDouble(
      m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenCostThreshold, EdxltokenOptimizerConfig);
...
//
//---------------------------------------------------------------------------
void CParseHandlerEnumeratorConfig::EndElement(const XMLCh *const,  // element_uri,
                                               const XMLCh *const element_local_name,
                                               const XMLCh *const  // element_qname
) {
  if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenEnumeratorConfig), element_local_name)) {
...
//
//---------------------------------------------------------------------------
EDxlParseHandlerType CParseHandlerEnumeratorConfig::GetParseHandlerType() const {
...
//
//---------------------------------------------------------------------------
CEnumeratorConfig *CParseHandlerEnumeratorConfig::GetEnumeratorCfg() const {
