...
//
//---------------------------------------------------------------------------
CParseHandlerMDRelation::CParseHandlerMDRelation(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
...
//
//---------------------------------------------------------------------------
void CParseHandlerMDRelation::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
  ...
  if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPartitions), element_local_name)) {
    // parse handler for child_partitions
    m_child_partitions_parse_handler = CParseHandlerFactory::GetParseHandler(
        m_mp, CDXLTokens::XmlstrToken(EdxltokenMetadataIdList), m_parse_handler_mgr, this);
    m_parse_handler_mgr->ActivateParseHandler(m_child_partitions_parse_handler);
    this->Append(m_child_partitions_parse_handler);
    m_child_partitions_parse_handler->startElement(element_uri, element_local_name, element_qname, attrs);
  ...
  m_is_temp_table = CDXLOperatorFactory::ExtractConvertAttrValueToBool(m_parse_handler_mgr->GetDXLMemoryManager(),
                                                                       attrs, EdxltokenRelTemporary, EdxltokenRelation);

  // parse rows
  const XMLCh *xml_rows = CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenRows, EdxltokenRelation, true);

  if (nullptr != xml_rows) {
    m_rows = CDouble(CDXLOperatorFactory::ConvertAttrValueToDouble(m_parse_handler_mgr->GetDXLMemoryManager(), xml_rows,
                                                                   EdxltokenRows, EdxltokenStatsDerivedRelation));
  } else {
    m_rows = -1;
  }
  ...
  if (IMDRelation::ErelstorageAppendOnlyCols == m_rel_storage_type ||
    ...
    m_rel_ao_version = (IMDRelation::Erelaoversion)CDXLOperatorFactory::ExtractConvertAttrValueToUlong(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenRelAppendOnlyVersion, EdxltokenRelation, true,
  ...
  if (nullptr != xmlszPartColumns) {
    m_partition_cols_array = CDXLOperatorFactory::ExtractIntsToUlongArray(
        m_parse_handler_mgr->GetDXLMemoryManager(), xmlszPartColumns, EdxltokenPartKeys, EdxltokenRelation);
  }
  ...
  if (nullptr != xmlszPartTypes) {
    m_str_part_types_array = CDXLOperatorFactory::ExtractConvertPartitionTypeToArray(
        m_parse_handler_mgr->GetDXLMemoryManager(), xmlszPartTypes, EdxltokenPartTypes, EdxltokenRelation);
  }
  ...
  if (nullptr != xmlszConvertHashToRandom) {
    m_convert_hash_to_random = CDXLOperatorFactory::ConvertAttrValueToBool(
        m_parse_handler_mgr->GetDXLMemoryManager(), xmlszConvertHashToRandom, EdxltokenConvertHashToRandom,
        EdxltokenRelation);
  }
...
//
//---------------------------------------------------------------------------
void CParseHandlerMDRelation::EndElement(const XMLCh *const,  // element_uri,
  ...
  CParseHandlerMetadataIdList *pphMdidlCheckConstraints = dynamic_cast<CParseHandlerMetadataIdList *>((*this)[2]);
  ...
  if (m_rel_distr_policy == IMDRelation::EreldistrHash && m_opfamilies_parse_handler != nullptr) {
  ...
  if (nullptr != m_child_partitions_parse_handler) {
    child_partitions = dynamic_cast<CParseHandlerMetadataIdList *>(m_child_partitions_parse_handler)->GetMdIdArray();
    child_partitions->AddRef();
  }

  m_imd_obj = GPOS_NEW(m_mp)
      CMDRelationGPDB(m_mp, m_mdid, m_mdname, m_is_temp_table, m_rel_storage_type, m_rel_ao_version, m_rel_distr_policy,
                      md_col_array, m_distr_col_array, distr_opfamilies, m_partition_cols_array, m_str_part_types_array,
                      child_partitions, m_convert_hash_to_random, m_key_sets_arrays, md_index_info_array,
                      mdid_check_constraint_array, m_part_constraint, m_foreign_server, m_rows);
...
//
//---------------------------------------------------------------------------
void CParseHandlerMDRelation::ParseRelationAttributes(const Attributes &attrs, Edxltoken dxl_token_element) {
  // parse table name
  ...
  CWStringDynamic *str_table_name =
      CDXLUtils::CreateDynamicStringFromXMLChArray(m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_table_name);
  ...
  m_mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
                                                              EdxltokenMdid, dxl_token_element);
  ...
  if (m_rel_distr_policy == IMDRelation::EreldistrHash) {
    ...
    m_distr_col_array = CDXLOperatorFactory::ExtractIntsToUlongArray(
        m_parse_handler_mgr->GetDXLMemoryManager(), rel_distr_cols_xml, EdxltokenDistrColumns, dxl_token_element);
  }
  ...
  if (nullptr != xml_str_keys) {
    m_key_sets_arrays = CDXLOperatorFactory::ExtractConvertUlongTo2DArray(
        m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_keys, EdxltokenKeys, dxl_token_element);
  } else {
    // construct an empty keyset
    m_key_sets_arrays = GPOS_NEW(m_mp) ULongPtr2dArray(m_mp);
  }

  const XMLCh *xmlszForeignServer = attrs.getValue(CDXLTokens::XmlstrToken(EdxltokenRelForeignServer));
  if (nullptr != xmlszForeignServer) {
    m_foreign_server = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
        m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenRelForeignServer, dxl_token_element);
  }
}
...
//
//---------------------------------------------------------------------------
void CParseHandlerMDRelation::ParseChildNodes() {
  // parse handler for check constraints
