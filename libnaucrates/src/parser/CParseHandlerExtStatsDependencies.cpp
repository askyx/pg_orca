...
CParseHandlerExtStatsDependencies::CParseHandlerExtStatsDependencies(CMemoryPool *mp,
                                                                     CParseHandlerManager *parse_handler_mgr,
                                                                     CParseHandlerBase *parse_handler_base)
    : CParseHandlerBase(mp, parse_handler_mgr, parse_handler_base), m_dependencies(nullptr) {}
...
//
//---------------------------------------------------------------------------
CMDDependencyArray *CParseHandlerExtStatsDependencies::GetDependencies() const {
...
//
//---------------------------------------------------------------------------
void CParseHandlerExtStatsDependencies::StartElement(const XMLCh *const element_uri,
                                                     const XMLCh *const element_local_name,
                                                     const XMLCh *const element_qname, const Attributes &attrs) {
  if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenMVDependencyList), element_local_name) &&
      0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenMVDependency), element_local_name)) {
    CWStringDynamic *str =
        CDXLUtils::CreateDynamicStringFromXMLChArray(m_parse_handler_mgr->GetDXLMemoryManager(), element_local_name);
    GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiDXLUnexpectedTag, str->GetBuffer());
  } else if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenMVDependency), element_local_name)) {
    // new bucket
    CParseHandlerBase *parse_handler_base_dependency = CParseHandlerFactory::GetParseHandler(
        m_mp, CDXLTokens::XmlstrToken(EdxltokenMVDependency), m_parse_handler_mgr, this);
    this->Append(parse_handler_base_dependency);

    m_parse_handler_mgr->ActivateParseHandler(parse_handler_base_dependency);
    parse_handler_base_dependency->startElement(element_uri, element_local_name, element_qname, attrs);
  }
}
...
//
//---------------------------------------------------------------------------
void CParseHandlerExtStatsDependencies::EndElement(const XMLCh *const,  // element_uri,
                                                   const XMLCh *const element_local_name,
