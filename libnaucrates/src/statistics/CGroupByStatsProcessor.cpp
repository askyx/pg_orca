...
CStatistics *CGroupByStatsProcessor::CalcGroupByStats(CMemoryPool *mp, const CStatistics *input_stats,
  ...
  if (input_stats->IsEmpty()) {
    ...
    CHistogram::AddDummyHistogramAndWidthInfo(mp, col_factory, col_histogram_mapping, col_width_mapping, aggs,
                                              true /* is_empty */);
    CHistogram::AddDummyHistogramAndWidthInfo(mp, col_factory, col_histogram_mapping, col_width_mapping, GCs,
                                              true /* is_empty */);

    agg_stats = GPOS_NEW(mp) CStatistics(mp, col_histogram_mapping, col_width_mapping, agg_rows, true /* is_empty */);
  } else {
    ...
    CColRefSet *groupby_cols_for_stats = CStatisticsUtils::MakeGroupByColsForStats(mp, GCs, computed_groupby_cols);
    ...
    CStatisticsUtils::AddGrpColStats(mp, input_stats, groupby_cols_for_stats, col_histogram_mapping, col_width_mapping);
    CStatisticsUtils::AddGrpColStats(mp, input_stats, computed_groupby_cols, col_histogram_mapping, col_width_mapping);

    const CStatisticsConfig *stats_config = input_stats->GetStatsConfig();

    CDoubleArray *NDVs =
        CStatisticsUtils::ExtractNDVForGrpCols(mp, stats_config, input_stats, groupby_cols_for_stats, keys);
    ...
    agg_stats =
        GPOS_NEW(mp) CStatistics(mp, col_histogram_mapping, col_width_mapping, agg_rows, input_stats->IsEmpty());
  }
  ...
  CStatisticsUtils::ComputeCardUpperBounds(mp, input_stats, agg_stats, agg_rows,
