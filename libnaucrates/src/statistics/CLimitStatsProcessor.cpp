...
CStatistics *CLimitStatsProcessor::CalcLimitStats(CMemoryPool *mp, const CStatistics *input_stats,
  ...
  CStatistics *pstatsLimit = GPOS_NEW(mp) CStatistics(mp, colid_histogram, input_stats->CopyWidths(mp), limit_rows,
  ...
  CStatisticsUtils::ComputeCardUpperBounds(mp, input_stats, pstatsLimit, limit_rows,
