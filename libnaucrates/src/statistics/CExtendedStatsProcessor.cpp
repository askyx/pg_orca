 ...
 *     dependencies.c.
 */
CMDExtStatsInfo *choose_best_statistics(CMemoryPool *mp, CMDExtStatsInfoArray *md_statsinfo_array, CBitSet *attnums,
 ...
 *     dependencies.c.
 */
static bool dependency_is_fully_matched(CMDDependency *dependency, CBitSet *attnums) {
  /*
...
void CExtendedStatsProcessor::ApplyCorrelatedStatsToScaleFactorFilterCalculation(
    CDoubleArray *scale_factors, CStatsPredConj *conjunctive_pred_stats, const IMDExtStatsInfo *md_statsinfo,
    UlongToIntMap *colid_to_attno_mapping, CMemoryPool *mp, UlongToHistogramMap *result_histograms) {
  ...
  stat = choose_best_statistics(mp, md_statsinfo->GetExtStatInfoArray(), clauses_attnums,
  ...
  CMDIdGPDB *pmdid = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidExtStats, stat->GetStatOid());
  ...
  while (true) {
     ...
     */
    s1 *= (dependency->GetDegree().Get() + (1 - dependency->GetDegree().Get()) * s2);
  }
...
//---------------------------------------------------------------------------
bool CExtendedStatsProcessor::ApplyCorrelatedStatsToNDistinctCalculation(CMemoryPool *mp,
