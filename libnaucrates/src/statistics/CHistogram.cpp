...
// ctor
CHistogram::CHistogram(CMemoryPool *mp, CBucketArray *histogram_buckets, bool is_well_defined)
    : m_mp(mp),
...
// ctor
CHistogram::CHistogram(CMemoryPool *mp, CBucketArray *histogram_buckets, bool is_well_defined, CDouble null_freq,
                       CDouble distinct_remaining, CDouble freq_remaining, bool is_col_stats_missing)
    : m_mp(mp),
      m_histogram_buckets(histogram_buckets),
      m_is_well_defined(is_well_defined),
...
void CHistogram::SetNullFrequency(CDouble null_freq) {
...
IOstream &CHistogram::OsPrint(IOstream &os) const {
  ...
  uint32_t num_buckets = m_histogram_buckets->Size();
...
CHistogram *CHistogram::MakeHistogramLessThanOrLessThanEqualFilter(CStatsPred::EStatsCmpType stats_cmp_type,
                                                                   CPoint *point) const {
  ...
  const uint32_t num_buckets = m_histogram_buckets->Size();
  ...
  for (uint32_t bucket_index = 0; bucket_index < num_buckets; bucket_index++) {
    ...
    if (bucket->IsBefore(point)) {
      ...
      CBucket *last_bucket =
          bucket->MakeBucketScaleUpper(m_mp, point, CStatsPred::EstatscmptLEq == stats_cmp_type /*include_upper*/);
...
CBucketArray *CHistogram::MakeBucketsWithInequalityFilter(CPoint *point) const {
...
CHistogram *CHistogram::MakeHistogramInequalityFilter(CPoint *point) const {
  GPOS_ASSERT(nullptr != point);
  ...
  return GPOS_NEW(m_mp)
      CHistogram(m_mp, histogram_buckets, true /*is_well_defined*/, null_freq, m_distinct_remaining, m_freq_remaining);
}

// construct new histogram with IDF filter
CHistogram *CHistogram::MakeHistogramIDFFilter(CPoint *point) const {
  GPOS_ASSERT(nullptr != point);
...
CBucketArray *CHistogram::MakeBucketsWithEqualityFilter(CPoint *point) const {
  GPOS_ASSERT(nullptr != point);
...
CHistogram *CHistogram::MakeHistogramEqualFilter(CPoint *point) const {
  GPOS_ASSERT(nullptr != point);

  if (point->GetDatum()->IsNull()) {
    return GPOS_NEW(m_mp) CHistogram(m_mp, GPOS_NEW(m_mp) CBucketArray(m_mp), true /* is_well_defined */, m_null_freq,
                                     DefaultNDVRemain, DefaultNDVFreqRemain);
  }
...
CHistogram *CHistogram::MakeHistogramINDFFilter(CPoint *point) const {
  GPOS_ASSERT(nullptr != point);

  CBucketArray *histogram_buckets = MakeBucketsWithEqualityFilter(point);
  const uint32_t num_of_buckets = histogram_buckets->Size();
  CDouble null_freq(0.0);
  if (point->GetDatum()->IsNull()) {
  ...
  return GPOS_NEW(m_mp) CHistogram(m_mp, histogram_buckets, true /* is_well_defined */, null_freq,
...
CHistogram *CHistogram::MakeHistogramGreaterThanOrGreaterThanEqualFilter(CStatsPred::EStatsCmpType stats_cmp_type,
                                                                         CPoint *point) const {
...
CDouble CHistogram::GetFrequency() const {
  CDouble frequency(0.0);
  const uint32_t num_of_buckets = m_histogram_buckets->Size();
  for (uint32_t bucket_index = 0; bucket_index < num_of_buckets; bucket_index++) {
    CBucket *bucket = (*m_histogram_buckets)[bucket_index];
...
CDouble CHistogram::GetNumDistinct() const {
...
void CHistogram::CapNDVs(CDouble rows) {
  const uint32_t num_of_buckets = m_histogram_buckets->Size();
...
CBucketArray *CHistogram::DeepCopyHistogramBuckets(CMemoryPool *mp, const CBucketArray *buckets) {
  CBucketArray *histogram_buckets = GPOS_NEW(mp) CBucketArray(mp, buckets->Size());
  for (uint32_t ul = 0; ul < buckets->Size(); ul++) {
...
bool CHistogram::IsNormalized() const {
  CDouble frequency = GetFrequency();

  return (frequency > CDouble(1.0) - CStatistics::Epsilon && frequency < CDouble(1.0) + CStatistics::Epsilon);
}
...
bool CHistogram::IsValid() const {
  ...
  if (IsHistogramForTextRelatedTypes()) {
    ...
    for (uint32_t bucket_index = 1; bucket_index < m_histogram_buckets->Size(); bucket_index++) {
      CBucket *bucket = (*m_histogram_buckets)[bucket_index];
...
CHistogram *CHistogram::MakeHistogramFilterNormalize(CStatsPred::EStatsCmpType stats_cmp_type, CPoint *point,
...
CHistogram *CHistogram::MakeJoinHistogramNormalize(CStatsPred::EStatsCmpType stats_cmp_type, CDouble rows,
  ...
  *scale_factor = result_histogram->NormalizeHistogram();
  ...
  if (CStatsPred::EstatscmptINDF == stats_cmp_type) {
    ...
    *scale_factor = std::max(CDouble(1.0), cartesian_product_num_rows / expected_num_rows_INDF);
  }
...
CDouble CHistogram::GetInequalityJoinScaleFactor(CDouble rows, const CHistogram *other_histogram,
  ...
  CHistogram *equijoin_histogram =
      MakeJoinHistogramNormalize(CStatsPred::EstatscmptEq, rows, other_histogram, rows_other, &scale_factor);
...
// output histogram
CHistogram *CHistogram::MakeLASJHistogramNormalize(CStatsPred::EStatsCmpType stats_cmp_type, CDouble rows,
...
CHistogram *CHistogram::MakeHistogramFilter(CStatsPred::EStatsCmpType stats_cmp_type, CPoint *point) const {
...
CHistogram *CHistogram::MakeJoinHistogram(CStatsPred::EStatsCmpType stats_cmp_type, const CHistogram *histogram) const {
...
CHistogram *CHistogram::MakeLASJHistogram(CStatsPred::EStatsCmpType stats_cmp_type, const CHistogram *histogram) const {
  ...
  uint32_t idx1 = 0;  // index into this histogram
  uint32_t idx2 = 0;  // index into other histogram

  CBucket *candidate_bucket = nullptr;

  const uint32_t buckets1 = GetNumBuckets();
  const uint32_t buckets2 = histogram->GetNumBuckets();
  ...
  return GPOS_NEW(m_mp)
      CHistogram(m_mp, new_buckets, true /*is_well_defined*/, null_freq, m_distinct_remaining, m_freq_remaining);
}
...
CDouble CHistogram::NormalizeHistogram() {
  ...
  if (scale_factor != double(1.0)) {
    ...
    // and modify individual buckets.
    CBucketArray *histogram_buckets = DeepCopyHistogramBuckets(m_mp, m_histogram_buckets);
    for (uint32_t ul = 0; ul < histogram_buckets->Size(); ul++) {
      CBucket *bucket = (*histogram_buckets)[ul];
...
CHistogram *CHistogram::CopyHistogram() const {
  m_histogram_buckets->AddRef();
  CHistogram *histogram_copy = GPOS_NEW(m_mp)
      CHistogram(m_mp, m_histogram_buckets, m_is_well_defined, m_null_freq, m_distinct_remaining, m_freq_remaining);
  if (WereNDVsScaled()) {
...
bool CHistogram::IsOpSupportedForTextFilter(CStatsPred::EStatsCmpType stats_cmp_type) {
...
bool CHistogram::IsOpSupportedForFilter(CStatsPred::EStatsCmpType stats_cmp_type) {
...
bool CHistogram::ContainsOnlySingletonBuckets() const {
  for (uint32_t ul = 0; ul < m_histogram_buckets->Size(); ++ul) {
...
bool CHistogram::JoinPredCmpTypeIsSupported(CStatsPred::EStatsCmpType stats_cmp_type) {
  return (CStatsPred::EstatscmptOther != stats_cmp_type);
}

// construct a new histogram with equality join
CHistogram *CHistogram::MakeJoinHistogramEqualityFilter(const CHistogram *histogram) const {
  uint32_t idx1 = 0;  // index on buckets from this histogram
  uint32_t idx2 = 0;  // index on buckets from other histogram
  ...
  while (idx1 < buckets1 && idx2 < buckets2) {
    ...
    if (bucket1->Intersects(bucket2)) {
      ...
      int32_t res = CBucket::CompareUpperBounds(bucket1, bucket2);
      if (0 == res) {
        // both ubs are equal
        idx1++;
        idx2++;
  ...
  ComputeJoinNDVRemainInfo(this, histogram, join_buckets, hist1_buckets_freq, hist2_buckets_freq, &distinct_remaining,
                           &freq_remaining);

  return GPOS_NEW(m_mp)
      CHistogram(m_mp, join_buckets, true /*is_well_defined*/, 0.0 /*null_freq*/, distinct_remaining, freq_remaining);
}

// construct a new histogram for NDV based cardinality estimation
CHistogram *CHistogram::MakeNDVBasedJoinHistogramEqualityFilter(const CHistogram *histogram) const {
  CDouble distinct_remaining(0.0);
  CDouble freq_remaining(0.0);
  CBucketArray *join_buckets = GPOS_NEW(m_mp) CBucketArray(m_mp);
...
CHistogram *CHistogram::MakeJoinHistogramINDFFilter(const CHistogram *histogram) const {
  ...
  CDouble null_freq =
      GetNullFreq() * histogram->GetNullFreq() * double(1.0) / std::max(GetNumDistinct(), histogram->GetNumDistinct());
...
bool CHistogram::CanComputeJoinNDVRemain(const CHistogram *histogram1, const CHistogram *histogram2) {
...
void CHistogram::ComputeJoinNDVRemainInfo(const CHistogram *histogram1, const CHistogram *histogram2,
                                          const CBucketArray *join_buckets, CDouble hist1_buckets_freq,
                                          CDouble hist2_buckets_freq, CDouble *result_distinct_remain,
  ...
  *result_freq_remain = 0.0;
  ...
  CDouble freq_remain_join =
      freq_remain1 * freq_remain2 / std::max(histogram1->GetDistinctRemain(), histogram2->GetDistinctRemain());
  freq_remain_join = freq_remain_join +
                     freq_remain1 * freq_non_join_buckets2 / std::max(num_distinct_non_null1, NDVs_non_join_buckets2);
  freq_remain_join = freq_remain_join +
                     freq_remain2 * freq_non_join_buckets1 / std::max(num_distinct_non_null2, NDVs_non_join_buckets1);
...
CHistogram *CHistogram::MakeGroupByHistogramNormalize(CDouble,  // rows,
  ...
  for (uint32_t ul = 0; ul < num_of_buckets; ul++) {
    ...
    CBucket *new_bucket = GPOS_NEW(m_mp) CBucket(lower_bound, upper_bound, true /* fClosedLower */, is_upper_closed,
  ...
  CHistogram *result_histogram = GPOS_NEW(m_mp)
      CHistogram(m_mp, new_buckets, true /*is_well_defined*/, new_null_freq, m_distinct_remaining, freq_remaining);
...
CHistogram *CHistogram::MakeUnionAllHistogramNormalize(CDouble rows, const CHistogram *histogram,
                                                       CDouble rows_other) const {
  GPOS_ASSERT(nullptr != histogram);
  ...
  while (nullptr != bucket1 && nullptr != bucket2) {
    if (bucket1->IsBefore(bucket2)) {
      ...
      CBucket *merge_bucket =
          bucket1->SplitAndMergeBuckets(m_mp, bucket2, rows, rows_other, &bucket1_new, &bucket2_new, &result_rows);
  ...
  const uint32_t num_buckets1 = GetNumBuckets();
  const uint32_t num_buckets2 = histogram->GetNumBuckets();
  ...
  AddBuckets(m_mp, histogram->m_histogram_buckets, new_buckets, rows_other, rows_new, idx2, num_buckets2);

  // add any leftover buckets from this histogram
  AddBuckets(m_mp, m_histogram_buckets, new_buckets, rows, rows_new, idx1, num_buckets1);
  ...
  uint32_t max_num_buckets = COptCtxt::PoctxtFromTLS()->GetOptimizerConfig()->GetStatsConf()->UlMaxStatsBuckets();
  uint32_t desired_num_buckets = std::max((uint32_t)max_num_buckets, std::max(num_buckets1, num_buckets2));
  CBucketArray *result_buckets = CombineBuckets(m_mp, new_buckets, desired_num_buckets);
  CHistogram *result_histogram = GPOS_NEW(m_mp)
      CHistogram(m_mp, result_buckets, true /*is_well_defined*/, new_null_freq, distinct_remaining, freq_remaining);
...
uint32_t CHistogram::AddResidualUnionAllBucket(CBucketArray *histogram_buckets, CBucket *bucket, CDouble rows_old,
...
void CHistogram::AddBuckets(CMemoryPool *mp, const CBucketArray *src_buckets, CBucketArray *dest_buckets,
                            CDouble rows_old, CDouble rows_new, uint32_t begin, uint32_t end) {
  GPOS_ASSERT(nullptr != src_buckets);
  GPOS_ASSERT(nullptr != dest_buckets);
  GPOS_ASSERT(begin <= end);
  GPOS_ASSERT(end <= src_buckets->Size());

  for (uint32_t ul = begin; ul < end; ul++) {
...
// col < 6  ==>  100 * (.2 + .25 * .4)  = 100 * .5 * .6   = 30 rows
CBucketArray *CHistogram::CombineBuckets(CMemoryPool *mp, CBucketArray *buckets, uint32_t desired_num_buckets) {
  ...
  uint32_t bucketsToCombine = buckets->Size() - desired_num_buckets;
  CKHeap<SAdjBucketBoundaryArray, SAdjBucketBoundary> *boundary_factors =
      GPOS_NEW(mp) CKHeap<SAdjBucketBoundaryArray, SAdjBucketBoundary>(mp, bucketsToCombine);

  // loop over the bucket boundaries and determine how much information
  // we would lose by eliminating them
  for (uint32_t ul = 0; ul < buckets->Size() - 1; ++ul) {
    ...
    if (bucket1->GetUpperBound()->Equals(bucket2->GetLowerBound()) &&
      ...
      CDouble width1 =
          bucket1->GetUpperBound()->Width(bucket1->GetLowerBound(), bucket1->IsLowerClosed(), bucket1->IsUpperClosed());
      CDouble freq2 = bucket2->GetFrequency();
      CDouble ndv2 = bucket2->GetNumDistinct();
      CDouble width2 =
          bucket2->GetUpperBound()->Width(bucket2->GetLowerBound(), bucket2->IsLowerClosed(), bucket2->IsUpperClosed());
      ...
      SAdjBucketBoundary *elem = GPOS_NEW(mp) SAdjBucketBoundary(ul, factor1 + factor2);
  ...
  while (nullptr != (candidate_to_remove = boundary_factors->RemoveBestElement())) {
  ...
  for (uint32_t ul = 0; ul < buckets->Size(); ++ul) {
    ...
    if (end_bucket_ix > ul) {
      ...
      merged = GPOS_NEW(mp) CBucket(bucket1->GetLowerBound(), bucket2->GetUpperBound(), bucket1->IsLowerClosed(),
                                    bucket2->IsUpperClosed(), merged_frequencies, merged_ndvs);
  ...
  //	currently isn't a high prioritiy and hitting it hides more serious issues
		...
		for (uint32_t ul = 0; ul < result_buckets->Size(); ++ul)
		{
  ...
  GPOS_ASSERT(result_buckets->Size() == desired_num_buckets || result_buckets->Size() == desired_num_buckets + 1);
...
void CHistogram::CleanupResidualBucket(CBucket *bucket, bool bucket_is_residual) {
...
CBucket *CHistogram::GetNextBucket(const CHistogram *histogram, CBucket *new_bucket, bool *result_bucket_is_residual,
...
CHistogram *CHistogram::MakeUnionHistogramNormalize(CDouble rows, const CHistogram *other_histogram, CDouble rows_other,
  ...
  while (nullptr != bucket1 && nullptr != bucket2) {
    if (bucket1->IsBefore(bucket2)) {
      ...
      merge_bucket = bucket1->SplitAndMergeBuckets(
          m_mp, bucket2, rows, rows_other, &bucket1_new, &bucket2_new, &result_rows, false /* is_union_all */
      );
  ...
  const uint32_t num_buckets1 = GetNumBuckets();
  const uint32_t num_buckets2 = other_histogram->GetNumBuckets();
  ...
  idx2 =
      AddResidualUnionBucket(histogram_buckets, bucket2, rows_other, bucket2_is_residual, idx2, num_tuples_per_bucket);

  CleanupResidualBucket(bucket1, bucket1_is_residual);
  CleanupResidualBucket(bucket2, bucket2_is_residual);

  // add any leftover buckets from other histogram
  AddBuckets(m_mp, other_histogram->m_histogram_buckets, histogram_buckets, rows_other, num_tuples_per_bucket, idx2,
             num_buckets2);

  // add any leftover buckets from this histogram
  AddBuckets(m_mp, m_histogram_buckets, histogram_buckets, rows, num_tuples_per_bucket, idx1, num_buckets1);
  ...
  for (uint32_t ul = 0; ul < num_tuples_per_bucket->Size(); ++ul) {
  ...
  *num_output_rows = std::max(CStatistics::MinRows, total_output_rows);
  ...
  CHistogram *result_histogram =
      GPOS_NEW(m_mp) CHistogram(m_mp, result_buckets, true /* is_well_defined */, null_freq, num_NDV_remain,
                                NDV_remain_freq, false /* is_col_stats_missing */
      );
...
uint32_t CHistogram::AddResidualUnionBucket(CBucketArray *histogram_buckets, CBucket *bucket, CDouble rows,
                                            bool bucket_is_residual, uint32_t index,
...
// add buckets from one array to another
void CHistogram::AddBuckets(CMemoryPool *mp, const CBucketArray *src_buckets, CBucketArray *dest_buckets, CDouble rows,
...
CBucket *CHistogram::operator[](uint32_t pos) const {
...
CDXLStatsDerivedColumn *CHistogram::TranslateToDXLDerivedColumnStats(CMDAccessor *md_accessor, uint32_t colid,
                                                                     CDouble width) const {
  ...
  for (uint32_t ul = 0; ul < num_of_buckets; ul++) {
    ...
    CDXLBucket *dxl_bucket = GPOS_NEW(m_mp)
        CDXLBucket(dxl_datum_lower, dxl_datum_upper, bucket->IsLowerClosed(), bucket->IsUpperClosed(), freq, distinct);

    dxl_stats_bucket_array->Append(dxl_bucket);
  }

  return GPOS_NEW(m_mp)
      CDXLStatsDerivedColumn(colid, width, m_null_freq, m_distinct_remaining, m_freq_remaining, dxl_stats_bucket_array);
}

// randomly pick a bucket index based on bucket frequency values
uint32_t CHistogram::GetRandomBucketIndex(uint32_t *seed) const {
  const uint32_t size = m_histogram_buckets->Size();
  GPOS_ASSERT(0 < size);
...
void CHistogram::GetSampleRate(double left, double right, double *sample_rate, uint32_t index) {
...
void CHistogram::ComputeSkew() {
  m_skew_was_measured = true;

  if (0 == m_histogram_buckets->Size() || !(*m_histogram_buckets)[0]->CanSample()) {
    return;
  }
  ...
  uint32_t bucket_index = m_histogram_buckets->Size() - 1;
  // start with bucket lower boundary
  uint32_t sample_index = 0;
  CDouble accumulated_freq = 0;
  for (uint32_t ul = 0; ul < GPOPT_SKEW_SAMPLE_SIZE; ul++) {
    CBucket *bucket = (*m_histogram_buckets)[bucket_index];
    ...
    if (filling_rate >= accumulated_freq + bucket->GetFrequency() && bucket_index > 0) {
      accumulated_freq = accumulated_freq + bucket->GetFrequency();
      bucket_index--;
      sample_index = 0;
    }
...
CHistogram *CHistogram::MakeDefaultHistogram(CMemoryPool *mp, CColRef *col_ref, bool is_empty) {
...
CHistogram *CHistogram::MakeDefaultBoolHistogram(CMemoryPool *mp) {
  ...
  return GPOS_NEW(mp) CHistogram(mp, buckets, true /* is_well_defined */, null_freq, distinct_remaining, freq_remaining,
                                 true /*is_col_stats_missing */
  );
}

// check if the join cardinality estimation can be done based on NDV alone
bool CHistogram::NeedsNDVBasedCardEstimationForEq(const CHistogram *histogram) {
  GPOS_ASSERT(nullptr != histogram);
  ...
  if (IMDType::EtiInt2 == type_info || IMDType::EtiInt4 == type_info || IMDType::EtiInt8 == type_info ||
      IMDType::EtiBool == type_info || IMDType::EtiOid == type_info) {
    return false;
  }
...
void CHistogram::AddHistograms(CMemoryPool *mp, UlongToHistogramMap *src_histograms,
...
void CHistogram::AddDummyHistogramAndWidthInfo(CMemoryPool *mp, CColumnFactory *col_factory,
  ...
  for (auto colid : columns) {
    ...
    if (nullptr == output_histograms->Find(&colid)) {
      ...
      output_histograms->Insert(GPOS_NEW(mp) uint32_t(colid), histogram);
    }

    if (nullptr == output_col_widths->Find(&colid)) {
      CDouble width = CStatisticsUtils::DefaultColumnWidth(col_ref->RetrieveType());
      output_col_widths->Insert(GPOS_NEW(mp) uint32_t(colid), GPOS_NEW(mp) CDouble(width));
    }
  }
}

//	add empty histogram for the columns in the input histogram
void CHistogram::AddEmptyHistogram(CMemoryPool *mp, UlongToHistogramMap *output_histograms,
...
bool CHistogram::IsHistogramForTextRelatedTypes() const {
