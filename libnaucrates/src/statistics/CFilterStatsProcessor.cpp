...
IStatistics *CFilterStatsProcessor::MakeStatsFilterForScalarExpr(
  ...
  IStatistics *result_stats =
      CFilterStatsProcessor::MakeStatsFilter(mp, dynamic_cast<CStatistics *>(child_stats), pred_stats, do_cap_NDVs);
  pred_stats->Release();

  if (exprhdl.HasOuterRefs() && 0 < all_outer_stats->Size()) {
    // derive stats based on outer references
    IStatistics *stats = CJoinStatsProcessor::DeriveStatsWithOuterRefs(mp, exprhdl, outer_refs_scalar_expr,
                                                                       result_stats, all_outer_stats);
...
CDouble CFilterStatsProcessor::SelectivityOfPredicate(CMemoryPool *mp, CExpression *pred, CTableDescriptor *ptabdesc,
  ...
  uint32_t num_outer_ref_preds = 0;
  ...
  IStatistics *base_table_stats =
      md_accessor->Pstats(mp, ptabdesc->MDId(), used_local_col_refs, dummy_width_set, stats_config);

  // derive stats based on local filter
  IStatistics *result_stats =
      CFilterStatsProcessor::MakeStatsFilter(mp, dynamic_cast<CStatistics *>(base_table_stats), pred_stats, false);
  ...
  if (nullptr != expr_with_outer_refs) {
    ...
    const uint32_t size = outer_ref_exprs->Size();
  ...
  if (1 < num_outer_ref_preds) {
    ...
    result = std::min(
        result.Get() / CScaleFactorUtils::DampedFilterScaleFactor(stats_config, num_outer_ref_preds).Get(), 1.0);
...
CStatistics *CFilterStatsProcessor::MakeStatsFilter(CMemoryPool *mp, const CStatistics *input_stats,
  ...
  if (input_stats->IsEmpty()) {
    ...
    histograms_new = MakeHistHashMapConjOrDisjFilter(mp, stats_config, histograms_copy, input_rows, base_pred_stats,
  ...
  CStatistics *filter_stats =
      GPOS_NEW(mp) CStatistics(mp, histograms_new, input_stats->CopyWidths(mp), rows_filter, input_stats->IsEmpty(),
  ...
  CStatisticsUtils::ComputeCardUpperBounds(mp, input_stats, filter_stats, rows_filter,
...
UlongToHistogramMap *CFilterStatsProcessor::MakeHistHashMapConjOrDisjFilter(
    CMemoryPool *mp, const CStatisticsConfig *stats_config, UlongToHistogramMap *input_histograms, CDouble input_rows,
    CStatsPred *pred_stats, CDouble *scale_factor, const CStatistics *input_stats) {
  ...
  if (CStatsPred::EsptConj == pred_stats->GetPredStatsType()) {
    ...
    return MakeHistHashMapConjFilter(mp, stats_config, input_histograms, input_rows, conjunctive_pred_stats,
                                     scale_factor, input_stats);
  }

  CStatsPredDisj *disjunctive_pred_stats = CStatsPredDisj::ConvertPredStats(pred_stats);
  result_histograms = MakeHistHashMapDisjFilter(mp, stats_config, input_histograms, input_rows, disjunctive_pred_stats,
                                                scale_factor, input_stats);
...
UlongToHistogramMap *CFilterStatsProcessor::MakeHistHashMapConjFilter(
    CMemoryPool *mp, const CStatisticsConfig *stats_config, UlongToHistogramMap *input_histograms, CDouble input_rows,
    CStatsPredConj *conjunctive_pred_stats, CDouble *scale_factor, const CStatistics *input_stats) {
  GPOS_ASSERT(nullptr != stats_config);
  GPOS_ASSERT(nullptr != input_histograms);
  ...
  CExtendedStatsProcessor::ApplyCorrelatedStatsToScaleFactorFilterCalculation(
      scale_factors, conjunctive_pred_stats, input_stats->GetExtStatsInfo(), input_stats->GetColidToAttnoMapping(), mp,
      result_histograms);

  // properties of last seen column
  CDouble last_scale_factor(1.0);
  uint32_t last_colid = UINT32_MAX;
  ...
  for (uint32_t ul = 0; ul < filters; ul++) {
    ...
    uint32_t colid = child_pred_stats->GetColId();
    ...
    if (CStatsPred::EsptDisj != child_pred_stats->GetPredStatsType()) {
      ...
      result_histogram =
          MakeHistSimpleFilter(mp, child_pred_stats, filter_colids, hist_before, &last_scale_factor, &last_colid);
      ...
      UlongToHistogramMap *disjunctive_histograms_after =
          MakeHistHashMapDisjFilter(mp, stats_config, result_histograms, num_disj_input_rows, disjunctive_pred_stats,
                                    &disjunctive_scale_factor, input_stats);
...
UlongToHistogramMap *CFilterStatsProcessor::MakeHistHashMapDisjFilter(
    CMemoryPool *mp, const CStatisticsConfig *stats_config, UlongToHistogramMap *input_histograms, CDouble input_rows,
    CStatsPredDisj *disjunctive_pred_stats, CDouble *scale_factor, const CStatistics *input_stats) {
  GPOS_ASSERT(nullptr != stats_config);
  GPOS_ASSERT(nullptr != input_histograms);
  ...
  uint32_t previous_colid = UINT32_MAX;
  ...
  // iterate over filters and update corresponding histograms
  const uint32_t filters = disjunctive_pred_stats->GetNumPreds();
  for (uint32_t ul = 0; ul < filters; ul++) {
    ...
    if (IsNewStatsColumn(colid, previous_colid)) {
      ...
      CStatisticsUtils::UpdateDisjStatistics(mp, non_updatable_cols, input_rows, cumulative_rows, previous_histogram,
    ...
    bool is_colid_present = (UINT32_MAX != colid);
    ...
    if (is_pred_simple) {
      ...
      disjunctive_child_col_histogram =
          MakeHistSimpleFilter(mp, child_pred_stats, filter_colids, histogram, &child_scale_factor, &previous_colid);
      ...
      child_histograms = MakeHistHashMapConjOrDisjFilter(mp, stats_config, input_histograms, input_rows,
                                                         child_pred_stats, &child_scale_factor, input_stats);

      GPOS_ASSERT_IMP(CStatsPred::EsptDisj == child_pred_stats->GetPredStatsType(), UINT32_MAX != colid);
    ...
    if (is_colid_present) {
      ...
      if (nullptr == previous_histogram) {
        ...
        new_histogram = previous_histogram->MakeUnionHistogramNormalize(
            cumulative_rows, disjunctive_child_col_histogram, num_rows_disj_child, &output_rows);
      ...
      CDouble current_rows_estimate =
          input_rows / CScaleFactorUtils::CalcScaleFactorCumulativeDisj(stats_config, scale_factors, input_rows);
      UlongToHistogramMap *merged_histograms = CStatisticsUtils::MergeHistogramMapsForDisjPreds(
          mp, non_updatable_cols, disjunctive_result_histograms, child_histograms, current_rows_estimate,
          num_rows_disj_child);
...
//	an AND/OR predicate
CHistogram *CFilterStatsProcessor::MakeHistSimpleFilter(CMemoryPool *mp, CStatsPred *pred_stats, CBitSet *filter_colids,
  ...
  if (CStatsPred::EsptArrayCmp == pred_stats->GetPredStatsType()) {
    ...
    return MakeHistArrayCmpAnyFilter(mp, arraycmp_pred_stats, filter_colids, hist_before, last_scale_factor,
                                     target_last_colid);
  }

  CStatsPredUnsupported *unsupported_pred_stats = CStatsPredUnsupported::ConvertPredStats(pred_stats);

  return MakeHistUnsupportedPred(unsupported_pred_stats, filter_colids, hist_before, last_scale_factor,
                                 target_last_colid);
}

// create a new histograms after applying the point filter
CHistogram *CFilterStatsProcessor::MakeHistPointFilter(CStatsPredPoint *pred_stats, CBitSet *filter_colids,
                                                       CHistogram *hist_before, CDouble *last_scale_factor,
                                                       uint32_t *target_last_colid) {
  GPOS_ASSERT(nullptr != pred_stats);
  ...
  const uint32_t colid = pred_stats->GetColId();
  ...
  CHistogram *result_histogram =
      hist_before->MakeHistogramFilterNormalize(pred_stats->GetCmpType(), point, &local_scale_factor);
...
CHistogram *CFilterStatsProcessor::MakeHistUnsupportedPred(CStatsPredUnsupported *pred_stats, CBitSet *filter_colids,
                                                           CHistogram *hist_before, CDouble *last_scale_factor,
                                                           uint32_t *target_last_colid) {
  GPOS_ASSERT(nullptr != pred_stats);
  GPOS_ASSERT(nullptr != filter_colids);
...
CHistogram *CFilterStatsProcessor::MakeHistLikeFilter(CStatsPredLike *pred_stats, CBitSet *filter_colids,
                                                      CHistogram *hist_before, CDouble *last_scale_factor,
                                                      uint32_t *target_last_colid) {
  GPOS_ASSERT(nullptr != pred_stats);
  GPOS_ASSERT(nullptr != filter_colids);
...
CHistogram *CFilterStatsProcessor::MakeHistArrayCmpAnyFilter(CMemoryPool *mp, CStatsPredArrayCmp *pred_stats,
                                                             CBitSet *filter_colids, CHistogram *base_histogram,
  ...
  for (uint32_t ul = 0; ul < points->Size(); ++ul) {
  ...
  CBucketArray *dummy_histogram_buckets = CHistogram::DeepCopyHistogramBuckets(mp, base_histogram->GetBuckets());
  uint32_t point_iter = 0;
  uint32_t ndv_remain = 0;
  for (uint32_t bucket_iter = 0; bucket_iter < dummy_histogram_buckets->Size(); ++bucket_iter) {
    ...
    while (point_iter < deduped_points->Size() && bucket->IsBefore((*deduped_points)[point_iter])) {
      ndv_remain++;
      point_iter++;
    }
    ...
    while (point_iter < deduped_points->Size() && bucket->Contains((*deduped_points)[point_iter])) {
      ndv++;
      point_iter++;
    }
  ...
  CHistogram *dummy_histogram =
      GPOS_NEW(mp) CHistogram(mp, dummy_histogram_buckets, true /* is_well_defined */, CDouble(0.0) /* null_freq */,
...
bool CFilterStatsProcessor::IsNewStatsColumn(uint32_t colid, uint32_t last_colid) {
  return (UINT32_MAX == colid || colid != last_colid);
}
