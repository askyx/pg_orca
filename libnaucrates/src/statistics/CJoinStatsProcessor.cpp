...
void CJoinStatsProcessor::JoinHistograms(CMemoryPool *mp, const CHistogram *histogram1, const CHistogram *histogram2,
  ...
  if (IStatistics::EsjtLeftAntiSemiJoin == join_type) {
    CLeftAntiSemiJoinStatsProcessor::JoinHistogramsLASJ(histogram1, histogram2, join_pred_stats, num_rows1, num_rows2,
                                                        result_hist1, result_hist2, scale_factor, is_input_empty,
                                                        join_type, DoIgnoreLASJHistComputation);
  ...
  if (empty_histograms) {
    ...
    *scale_factor = std::min(num_rows1, num_rows2);
    ...
    CHistogram *join_histogram =
        histogram1->MakeJoinHistogramNormalize(stats_cmp_type, num_rows1, histogram2, num_rows2, scale_factor);
...
IStatistics *CJoinStatsProcessor::CalcAllJoinStats(CMemoryPool *mp, IStatisticsArray *statistics_array,
  ...
  for (uint32_t i = 1; i < num_stats; i++) {
    ...
    if (nullptr != unsupported_pred_stats) {
      ...
      IStatistics *stats_after_join_filter = CFilterStatsProcessor::MakeStatsFilter(
          mp, dynamic_cast<CStatistics *>(stats), unsupported_pred_stats, false /* do_cap_NDVs */);
...
CStatistics *CJoinStatsProcessor::SetResultingJoinStats(CMemoryPool *mp, CStatisticsConfig *stats_config,
  ...
  for (uint32_t i = 0; i < num_join_conds; i++) {
    ...
    JoinHistograms(mp, outer_histogram, inner_histogram, pred_info, outer_stats->Rows(), inner_side_stats->Rows(),
                   &outer_histogram_after, &inner_histogram_after, &local_scale_factor, is_input_empty, join_type,
                   DoIgnoreLASJHistComputation);

    output_is_empty = JoinStatsAreEmpty(outer_stats->IsEmpty(), output_is_empty, outer_histogram, inner_histogram,
                                        outer_histogram_after, join_type);
    ...
    join_conds_scale_factors->Append(
        GPOS_NEW(mp) CScaleFactorUtils::SJoinCondition(local_scale_factor, mdid_pair, both_dist_keys));
  }

  num_join_rows = CStatistics::MinRows;
  if (!output_is_empty) {
    num_join_rows = CalcJoinCardinality(mp, stats_config, outer_stats->Rows(), inner_side_stats->Rows(),
  ...
  CStatistics *join_stats =
      GPOS_NEW(mp) CStatistics(mp, result_col_hist_mapping, col_width_mapping_result, num_join_rows, output_is_empty,
  ...
  CStatisticsUtils::ComputeCardUpperBounds(mp, outer_stats, join_stats, num_join_rows,
                                           CStatistics::EcbmMin /* card_bounding_method */);
  if (!semi_join) {
    CStatisticsUtils::ComputeCardUpperBounds(mp, inner_side_stats, join_stats, num_join_rows,
                                             CStatistics::EcbmMin /* card_bounding_method */);
  }
...
CDouble CJoinStatsProcessor::CalcJoinCardinality(CMemoryPool *mp, CStatisticsConfig *stats_config,
                                                 CDouble left_num_rows, CDouble right_num_rows,
                                                 CScaleFactorUtils::SJoinConditionArray *join_conds_scale_factors,
                                                 IStatistics::EStatsJoinType join_type) {
  GPOS_ASSERT(nullptr != stats_config);
  GPOS_ASSERT(nullptr != join_conds_scale_factors);
  CDouble limit_for_result_scale_factor(std::max(left_num_rows.Get(), right_num_rows.Get()));
...
IStatistics *CJoinStatsProcessor::DeriveJoinStats(CMemoryPool *mp, CExpressionHandle &exprhdl,
  ...
  GPOS_ASSERT(COperator::EopLogicalLeftOuterJoin == op_id || COperator::EopLogicalInnerJoin == op_id ||
              COperator::EopLogicalNAryJoin == op_id || COperator::EopLogicalFullOuterJoin == op_id ||
...
IStatistics *CJoinStatsProcessor::DeriveStatsWithOuterRefs(
    CMemoryPool *mp,
