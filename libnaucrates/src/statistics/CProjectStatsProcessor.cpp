...
CStatistics *CProjectStatsProcessor::CalcProjStats(CMemoryPool *mp, const CStatistics *input_stats,
  ...
  for (auto colid : projection_colids) {
    ...
    if (nullptr == histogram) {
      ...
      if (0 == proj_col_bucket->Size() && IMDType::EtiBool == colref->RetrieveType()->GetDatumType()) {
        ...
        proj_col_histogram = GPOS_NEW(mp) CHistogram(mp, proj_col_bucket, is_well_defined, null_freq,
  ...
  CStatistics *projection_stats =
      GPOS_NEW(mp) CStatistics(mp, histograms_new, colid_width_mapping, input_rows, input_stats->IsEmpty(),
  ...
  CStatisticsUtils::ComputeCardUpperBounds(mp, input_stats, projection_stats, input_rows,
