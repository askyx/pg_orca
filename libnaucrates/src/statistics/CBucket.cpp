...
//
//---------------------------------------------------------------------------
CBucket::CBucket(CPoint *bucket_lower_bound, CPoint *bucket_upper_bound, bool is_lower_closed, bool is_upper_closed,
...
//
//---------------------------------------------------------------------------
bool CBucket::Contains(const CPoint *point) const {
...
//
//---------------------------------------------------------------------------
bool CBucket::IsBefore(const CPoint *point) const {
...
//
//---------------------------------------------------------------------------
bool CBucket::IsAfter(const CPoint *point) const {
  GPOS_ASSERT(nullptr != point);
...
//
//---------------------------------------------------------------------------
CDouble CBucket::GetOverlapPercentage(const CPoint *point, bool include_point) const {
  ...
  if (this->m_bucket_lower_bound->Equals(point) && include_point) {
    ...
    // point.
    return CDouble(1.0) - std::min(double(1.0), (CDouble(1.0) / m_distinct).Get());
  }
...
//
//---------------------------------------------------------------------------
IOstream &CBucket::OsPrint(IOstream &os) const {
  os << "CBucket(";
...
//---------------------------------------------------------------------------
CBucket *CBucket::MakeBucketGreaterThan(CMemoryPool *mp, CPoint *point) const {
...
//
//---------------------------------------------------------------------------
CBucket *CBucket::MakeBucketScaleUpper(CMemoryPool *mp, CPoint *point_upper_new, bool include_upper) const {
  GPOS_ASSERT(mp);
  ...
  return GPOS_NEW(mp) CBucket(this->m_bucket_lower_bound, point_upper_new, this->m_is_lower_closed, include_upper,
...
//
//---------------------------------------------------------------------------
CBucket *CBucket::MakeBucketScaleLower(CMemoryPool *mp, CPoint *point_lower_new, bool include_lower) const {
  GPOS_ASSERT(mp);
  ...
  return GPOS_NEW(mp) CBucket(point_lower_new, this->m_bucket_upper_bound, include_lower, this->m_is_upper_closed,
                              frequency_new, distinct_new);
}
...
//
//---------------------------------------------------------------------------
CBucket *CBucket::MakeBucketSingleton(CMemoryPool *mp, CPoint *point_singleton) const {
  GPOS_ASSERT(mp);
  ...
  return GPOS_NEW(mp) CBucket(point_singleton, point_singleton, true /* is_lower_closed */, true /* is_upper_closed */,
                              frequency_new, distinct_new);
}
...
//
//---------------------------------------------------------------------------
CBucket *CBucket::MakeBucketCopy(CMemoryPool *mp) {
  // reuse the points
  m_bucket_lower_bound->AddRef();
  m_bucket_upper_bound->AddRef();

  return GPOS_NEW(mp) CBucket(m_bucket_lower_bound, m_bucket_upper_bound, m_is_lower_closed, m_is_upper_closed,
                              m_frequency, m_distinct);
}

bool CBucket::Equals(const CBucket *bucket) const {
  GPOS_ASSERT(bucket != nullptr);
  if (this->GetLowerBound()->Equals(bucket->GetLowerBound()) && this->IsLowerClosed() == bucket->IsLowerClosed() &&
      this->GetUpperBound()->Equals(bucket->GetUpperBound()) && this->IsUpperClosed() == bucket->IsUpperClosed() &&
      this->GetFrequency() == bucket->GetFrequency() && this->GetNumDistinct() == bucket->GetNumDistinct()) {
    return true;
  }
  return false;
}
...
//---------------------------------------------------------------------------
CBucket *CBucket::MakeBucketUpdateFrequency(CMemoryPool *mp, CDouble rows_old, CDouble rows_new) {
  // reuse the points
  m_bucket_lower_bound->AddRef();
  m_bucket_upper_bound->AddRef();
...
//
//---------------------------------------------------------------------------
int32_t CBucket::CompareLowerBounds(const CBucket *bucket1, const CBucket *bucket2) {
...
//---------------------------------------------------------------------------
int32_t CBucket::CompareLowerBoundToUpperBound(const CBucket *bucket1, const CBucket *bucket2) {
...
//
//---------------------------------------------------------------------------
int32_t CBucket::CompareUpperBounds(const CBucket *bucket1, const CBucket *bucket2) {
  GPOS_ASSERT(nullptr != bucket1);
  GPOS_ASSERT(nullptr != bucket2);
...
//
//---------------------------------------------------------------------------
bool CBucket::Intersects(const CBucket *bucket) const {
...
//
//---------------------------------------------------------------------------
bool CBucket::Subsumes(const CBucket *bucket) const {
  // both are singletons
  if (this->IsSingleton() && bucket->IsSingleton()) {
    return GetLowerBound()->Equals(bucket->GetLowerBound());
  }
  ...
  int32_t lower_bounds_comparison = CompareLowerBounds(this, bucket);
  int32_t upper_bounds_comparison = CompareUpperBounds(this, bucket);
...
//---------------------------------------------------------------------------
CBucket *CBucket::MakeBucketIntersect(CMemoryPool *mp, CBucket *bucket, CDouble *result_freq_intersect1,
  ...
  // discarded
  CDouble distinct_new(std::min(ratio1.Get() * m_distinct.Get(), ratio2.Get() * bucket->m_distinct.Get()));
  ...
  CDouble distinct_max(std::max(ratio1.Get() * m_distinct.Get(), ratio2.Get() * bucket->GetNumDistinct().Get()));
  CDouble frequency_new(distinct_max == CDouble(0) ? 0
                                                   : freq_intersect1 * freq_intersect2 * double(1.0) / distinct_max);
  ...
  *result_freq_intersect2 = freq_intersect2;
  ...
  return GPOS_NEW(mp)
      CBucket(lower_new, upper_new, lower_new_is_closed, upper_new_is_closed, frequency_new, distinct_new);
}
...
//
//---------------------------------------------------------------------------
void CBucket::Difference(CMemoryPool *mp, CBucket *bucket_other, CBucket **result_bucket_lower,
  ...
  if (this->GetLowerBound()->IsLessThan(bucket_other->GetLowerBound())) {
    *result_bucket_lower =
        this->MakeBucketScaleUpper(mp, bucket_other->GetLowerBound(), !bucket_other->IsLowerClosed());
  }
  ...
  if (bucket_other->GetUpperBound()->IsLessThan(this->GetUpperBound())) {
    *result_bucket_upper =
        this->MakeBucketScaleLower(mp, bucket_other->GetUpperBound(), !bucket_other->IsUpperClosed());
  }
...
//
//---------------------------------------------------------------------------
bool CBucket::IsBefore(const CBucket *bucket) const {
...
//
//---------------------------------------------------------------------------
bool CBucket::IsAfter(const CBucket *bucket) const {
  if (this->Intersects(bucket)) {
    return false;
  }
...
CBucket *CBucket::SplitAndMergeBuckets(
    ...
    if ((!minLower->Equals(maxLower) && this->GetLowerBound()->Equals(minLower)) ||
      ...
      *result_rows = rows;
      ...
      lower_third = GPOS_NEW(mp)
          CBucket(this->GetLowerBound(), maxLower, this->IsLowerClosed(), include_upper, lower_freq, lower_ndv);
      ...
      *result_rows = rows_other;
      ...
      maxLower->AddRef();
      lower_third = GPOS_NEW(mp) CBucket(bucket_other->GetLowerBound(), maxLower, bucket_other->IsLowerClosed(),
  ...
  if (!minUpper->Equals(maxUpper)) {
    ...
    // return [10,20) as upper_third
      ...
      GPOS_ASSERT(this_overlap * this->GetFrequency() + upper_third->GetFrequency() <=
      ...
      GPOS_ASSERT(bucket_other_overlap * bucket_other->GetFrequency() + upper_third->GetFrequency() <=
  ...
  CDouble max_merged_ndv = (CDouble)UINT32_MAX;
...
//
//---------------------------------------------------------------------------
CDouble CBucket::GetSample(double ratio) const {
...
//
//---------------------------------------------------------------------------
CBucket *CBucket::MakeBucketSingleton(CMemoryPool *mp, IDatum *datum) {
  GPOS_ASSERT(nullptr != datum);

  datum->AddRef();
  datum->AddRef();

  return GPOS_NEW(mp) CBucket(GPOS_NEW(mp) CPoint(datum), GPOS_NEW(mp) CPoint(datum), true /* is_lower_closed */,
                              true /* is_upper_closed */, CDouble(1.0), CDouble(1.0));
}
