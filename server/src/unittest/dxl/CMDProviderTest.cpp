...
CMDProviderTest::EresUnittest() {
  CUnittest rgut[] = {
      ...
      GPOS_UNITTEST_FUNC_THROW(CMDProviderTest::EresUnittest_Negative, gpdxl::ExmaMD, gpdxl::ExmiMDCacheEntryNotFound),
  };
...
CMDProviderTest::EresUnittest_Basic() {
  ...
  IMDCacheObjectArray *mdcache_obj_array =
      CDXLUtils::ParseDXLToIMDObjectArray(mp, dxl_string, nullptr /*xsd_file_path*/);
...
//
//---------------------------------------------------------------------------
void CMDProviderTest::TestMDLookup(CMemoryPool *mp, IMDProvider *pmdp) {
  CAutoMDAccessor amda(mp, pmdp, CTestUtils::m_sysidDefault, CMDCache::Pcache());

  // lookup different objects
  CMDIdGPDB *pmdid1 =
      GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID, 1 /* major version */, 1 /* minor version */);
  CMDIdGPDB *pmdid2 =
      GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID, 12 /* version */, 1 /* minor version */);
  ...
  IMDCacheObject *pimdobj1 = CDXLUtils::ParseDXLToIMDIdCacheObj(mp, pstrMDObject1, nullptr);

  IMDCacheObject *pimdobj2 = CDXLUtils::ParseDXLToIMDIdCacheObj(mp, pstrMDObject2, nullptr);

  GPOS_UNITTEST_ASSERT(nullptr != pimdobj1 && pmdid1->Equals(pimdobj1->MDId()));
  GPOS_UNITTEST_ASSERT(nullptr != pimdobj2 && pmdid2->Equals(pimdobj2->MDId()));
...
CMDProviderTest::EresUnittest_Stats() {
...

  {
    ...
    CMDIdRelStats *rel_stats_mdid =
        GPOS_NEW(mp) CMDIdRelStats(GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID, 1, 1));

    CWStringBase *pstrRelStats = pmdpFile->GetMDObjDXLStr(mp, amda.Pmda(), rel_stats_mdid);
    GPOS_UNITTEST_ASSERT(nullptr != pstrRelStats);
    IMDCacheObject *pmdobjRelStats = CDXLUtils::ParseDXLToIMDIdCacheObj(mp, pstrRelStats, nullptr);
    GPOS_UNITTEST_ASSERT(nullptr != pmdobjRelStats);

    CMDIdColStats *mdid_col_stats = GPOS_NEW(mp)
        CMDIdColStats(GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID, 1, 1), 1 /* attno */);

    CWStringBase *pstrColStats = pmdpFile->GetMDObjDXLStr(mp, amda.Pmda(), mdid_col_stats);
    GPOS_UNITTEST_ASSERT(nullptr != pstrColStats);
    IMDCacheObject *pmdobjColStats = CDXLUtils::ParseDXLToIMDIdCacheObj(mp, pstrColStats, nullptr);
...
CMDProviderTest::EresUnittest_Negative() {
...

  {
    ...
    CMDIdGPDB *mdid = GPOS_NEW(mp)
        CMDIdGPDB(IMDId::EmdidGeneral, GPOPT_MDCACHE_TEST_OID, 15 /* major version */, 1 /* minor version */);
