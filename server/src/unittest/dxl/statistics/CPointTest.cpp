...
CPointTest::EresUnittest_CPointInt4() {
  ...
  // should be 1.0
  GPOS_UNITTEST_ASSERT_MSG(0.99 < dDistance && dDistance < 1.01, "incorrect distance calculation");
...
CPointTest::EresUnittest_CPointBool() {
  ...
  // true == true
  GPOS_UNITTEST_ASSERT_MSG(point1->Equals(point1), "true must be equal to true");

  // true != false
  GPOS_UNITTEST_ASSERT_MSG(point1->IsNotEqual(point2), "true must not be equal to false");
