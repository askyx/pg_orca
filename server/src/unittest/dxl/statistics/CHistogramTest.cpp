...
CHistogramTest::EresUnittest() {
  ...
  CUnittest rgutSharedOptCtxt[] = {GPOS_UNITTEST_FUNC(CHistogramTest::EresUnittest_CHistogramInt4),
                                   ...
                                   GPOS_UNITTEST_FUNC(CHistogramTest::EresUnittest_MergeUnionDoubleLessThanEpsilon)};
...
CHistogramTest::EresUnittest_CHistogramInt4() {
  ...
  CHistogram *phist4 = histogram->MakeHistogramFilterNormalize(CStatsPred::EstatscmptEq, ppoint4, &scale_factor);
  ...
  CHistogram *phist8 = phist7->MakeHistogramFilter(CStatsPred::EstatscmptEq, point3);
  GPOS_UNITTEST_ASSERT(fabs((phist8->GetFrequency() - 0.2).Get()) < CStatistics::Epsilon);
  GPOS_UNITTEST_ASSERT(fabs((phist8->GetNumDistinct() - 1.0).Get()) < CStatistics::Epsilon);

  // greater than, hitting remaining tuples
  CHistogram *phist9 = phist7->MakeHistogramFilter(CStatsPred::EstatscmptG, point1);
  CCardinalityTestUtils::PrintHist(mp, "phist9", phist9);
  GPOS_UNITTEST_ASSERT(fabs((phist9->GetFrequency() - 0.26).Get()) < CStatistics::Epsilon);
  GPOS_UNITTEST_ASSERT(fabs((phist9->GetNumDistinct() - 1.8).Get()) < CStatistics::Epsilon);
  ...
  GPOS_UNITTEST_ASSERT(fabs((phist10->GetDistinctRemain() - 2.0).Get()) < CStatistics::Epsilon);
  GPOS_UNITTEST_ASSERT(fabs((phist10->GetFreqRemain() - 0.08).Get()) < CStatistics::Epsilon);
...
CHistogram *CHistogramTest::PhistExampleInt4Remain(CMemoryPool *mp) {
...
CHistogramTest::EresUnittest_MergeUnionDoubleLessThanEpsilon() {
  ...
  CPoint *ppLower1 = CCardinalityTestUtils::PpointDouble(mp, GPDB_FLOAT8, CDouble(631.82140700000002));
  CPoint *ppUpper1 = CCardinalityTestUtils::PpointDouble(mp, GPDB_FLOAT8, CDouble(631.82140700000002));

  CBucket *bucket1 = GPOS_NEW(mp)
      CBucket(ppLower1, ppUpper1, true /* is_lower_closed */, true /*is_upper_closed*/, CDouble(0.2), CDouble(50));

  // (631.82140500000003, 645.05197699999997)
  CPoint *ppLower2 = CCardinalityTestUtils::PpointDouble(mp, GPDB_FLOAT8, CDouble(631.82140500000003));
  CPoint *ppUpper2 = CCardinalityTestUtils::PpointDouble(mp, GPDB_FLOAT8, CDouble(645.05197699999997));
