...
CBucket *CCardinalityTestUtils::PbucketIntegerClosedLowerBound(CMemoryPool *mp, INT iLower, INT iUpper,
...
CBucket *CCardinalityTestUtils::PbucketInteger(CMemoryPool *mp, INT iLower, INT iUpper, BOOL is_lower_closed,
...
CBucket *CCardinalityTestUtils::PbucketSingletonBoolVal(CMemoryPool *mp, BOOL fValue, CDouble frequency) {
...
CHistogram *CCardinalityTestUtils::PhistInt4Remain(CMemoryPool *mp, ULONG num_of_buckets, CDouble dNDVPerBucket,
                                                   BOOL fNullFreq, CDouble num_NDV_remain) {
...
CHistogram *CCardinalityTestUtils::PhistExampleInt4(CMemoryPool *mp) {
...
CHistogram *CCardinalityTestUtils::PhistExampleBool(CMemoryPool *mp) {
  CBucketArray *histogram_buckets = GPOS_NEW(mp) CBucketArray(mp);
...
CPoint *CCardinalityTestUtils::PpointGeneric(CMemoryPool *mp, OID oid, CWStringDynamic *pstrEncodedValue, LINT value) {
...
CPoint *CCardinalityTestUtils::PpointNumeric(CMemoryPool *mp, CWStringDynamic *pstrEncodedValue, CDouble value) {
  CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
  ...
  CDXLDatumStatsDoubleMappable *dxl_datum = GPOS_NEW(mp)
      CDXLDatumStatsDoubleMappable(mp, mdid, default_type_modifier, false /*is_const_null*/, data, ulbaSize, value);
...
// helper function to generate a point from an encoded value of specific datatype
CPoint *CCardinalityTestUtils::PpointDouble(CMemoryPool *mp, OID oid, CDouble value) {
  CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
...
void CCardinalityTestUtils::PrintBucket(CMemoryPool *mp, const char *pcPrefix, const CBucket *bucket) {
...
void CCardinalityTestUtils::PrintHist(CMemoryPool *mp, const char *pcPrefix, const CHistogram *histogram) {
  CWStringDynamic str(mp);
  COstreamString oss(&str);
...
void CCardinalityTestUtils::PrintStats(CMemoryPool *mp, const IStatistics *stats) {
  CWStringDynamic str(mp);
  COstreamString oss(&str);
