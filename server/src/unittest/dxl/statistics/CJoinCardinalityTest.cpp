...
CJoinCardinalityTest::EresUnittest_JoinNDVRemain() {
  ...
  for (ULONG ul1 = 0; ul1 < ulHist; ul1++) {
    ...
    CHistogram *histogram =
        CCardinalityTestUtils::PhistInt4Remain(mp, num_of_buckets, dNDVPerBucket, fNullFreq, num_NDV_remain);
    BOOL result GPOS_ASSERTS_ONLY = col_histogram_mapping->Insert(GPOS_NEW(mp) ULONG(ul1), histogram);
  ...
  for (ULONG ul2 = 0; ul2 < ulTestCases && (GPOS_FAILED != eres); ul2++) {
    ...
    CDouble dDiffNDVJoin(
        fabs((dNDVBucketsJoin - CStatisticsUtils::GetNumDistinct(join_histogram->GetBuckets())).Get()));
...
CJoinCardinalityTest::EresUnittest_Join() {
  ...
  SStatsJoinSTestCase rgstatsjointc[] = {
      {"../data/dxl/statistics/Join-Statistics-Input.xml", "../data/dxl/statistics/Join-Statistics-Output.xml", false,
       PdrgpstatspredjoinMultiplePredicates},
      {"../data/dxl/statistics/Join-Statistics-Input-Null-Bucket.xml",
       "../data/dxl/statistics/Join-Statistics-Output-Null-Bucket.xml", false, PdrgpstatspredjoinNullableCols},
      {"../data/dxl/statistics/LOJ-Input.xml", "../data/dxl/statistics/LOJ-Output.xml", true,
       PdrgpstatspredjoinNullableCols},
      {"../data/dxl/statistics/Join-Statistics-Input-Only-Nulls.xml",
       "../data/dxl/statistics/Join-Statistics-Output-Only-Nulls.xml", false, PdrgpstatspredjoinNullableCols},
      {"../data/dxl/statistics/Join-Statistics-Input-Only-Nulls.xml",
       "../data/dxl/statistics/Join-Statistics-Output-LOJ-Only-Nulls.xml", true, PdrgpstatspredjoinNullableCols},
      {"../data/dxl/statistics/Join-Statistics-DDistinct-Input.xml",
       "../data/dxl/statistics/Join-Statistics-DDistinct-Output.xml", false, PdrgpstatspredjoinSingleJoinPredicate},
  };
  ...
  for (ULONG ul = 0; ul < cols->Size(); ul++) {
    ...
    if (nullptr == col_factory->LookupColRef(id)) {
      ...
      (void)col_factory->PcrCreate(pmdtypeint4, default_type_modifier, true /*mark_as_used*/, nullptr, ul /* attno */,
                                   false /*IsNullable*/, id, CName(&str), false /*IsDistCol*/, false);
    }
  }
  cols->Release();
...
CStatsPredJoinArray *CJoinCardinalityTest::PdrgpstatspredjoinSingleJoinPredicate(CMemoryPool *mp) {
...
CStatsPredJoinArray *CJoinCardinalityTest::PdrgpstatspredjoinMultiplePredicates(CMemoryPool *mp) {
  CStatsPredJoinArray *join_preds_stats = GPOS_NEW(mp) CStatsPredJoinArray(mp);
...
CStatsPredJoinArray *CJoinCardinalityTest::PdrgpstatspredjoinNullableCols(CMemoryPool *mp) {
  CStatsPredJoinArray *join_preds_stats = GPOS_NEW(mp) CStatsPredJoinArray(mp);
