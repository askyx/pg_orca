...
CStatisticsTest::EresUnittest_UnionAll() {
  ...
  for (ULONG i = 0; i < ulTestCases; i++) {
    ...
    CStatistics *pstatsOutput = CUnionAllStatsProcessor::CreateStatsForUnionAll(
        mp, pstats1, pstats2, pdrgpulColIdOutput, pdrgpulColIdInput1, pdrgpulColIdInput2);
...
CStatisticsTest::EresUnittest_GbAggWithRepeatedGbCols() {
  ...
  CExpression *pexprGbAgg1 = CUtils::PexprLogicalGbAggGlobal(
      mp, pdrgpcr1, pexpr, GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp)));
  ...
  CExpression *pexprGbAgg2 = CUtils::PexprLogicalGbAggGlobal(
      mp, pdrgpcr2, pexpr, GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp)));
  ...
  CExpression *pexprGbAgg3 = CUtils::PexprLogicalGbAggGlobal(
      mp, pdrgpcr3, pexprPrj, GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp)));
...
CHistogram *CStatisticsTest::PhistExampleInt4Dim(CMemoryPool *mp) {
...
CTableDescriptor *CStatisticsTest::PtabdescTwoColumnSource(CMemoryPool *mp, const CName &nameTable,
                                                           const IMDTypeInt4 *pmdtype, const CWStringConst &strColA,
                                                           const CWStringConst &strColB) {
  CTableDescriptor *ptabdesc = GPOS_NEW(mp)
      CTableDescriptor(mp, GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_TEST_REL_OID1, 1, 1), nameTable,
  ...
  for (ULONG i = 0; i < 2; i++) {
    ...
    CColumnDescriptor *pcoldesc =
        GPOS_NEW(mp) CColumnDescriptor(mp, pmdtype, default_type_modifier, nameColumn, i + 1, false /*is_nullable*/
        );
...
CStatisticsTest::EresUnittest_CStatisticsBasic() {
  ...
  CTableDescriptor *ptabdesc = PtabdescTwoColumnSource(mp, CName(&strRelAlias), pmdtypeint4, strColA, strColB);
  ...
  if (nullptr == col_factory->LookupColRef(1 /*id*/)) {
    ...
    (void)col_factory->PcrCreate(pmdtypeint4, default_type_modifier, true /*mark_as_used*/, nullptr, 0 /* attno */,
                                 false /*IsNullable*/, 1 /* id */, CName(&strColA), pexprGet->Pop()->UlOpId(),
                                 false /*IsDistCol*/
    );
  }

  if (nullptr == col_factory->LookupColRef(2 /*id*/)) {
    (void)col_factory->PcrCreate(pmdtypeint4, default_type_modifier, true /*mark_as_used*/, nullptr, 1 /* attno */,
                                 false /*IsNullable*/, 2 /* id */, CName(&strColB), pexprGet->Pop()->UlOpId(),
                                 false /*IsDistCol*/
    );
  }

  if (nullptr == col_factory->LookupColRef(10 /*id*/)) {
    (void)col_factory->PcrCreate(pmdtypeint4, default_type_modifier, true /*mark_as_used*/, nullptr, 2 /* attno */,
                                 false /*IsNullable*/, 10 /* id */, CName(&strColC), pexprGet->Pop()->UlOpId(),
                                 false /*IsDistCol*/
    );
  }
  ...
  CStatistics *stats =
      GPOS_NEW(mp) CStatistics(mp, col_histogram_mapping, colid_width_mapping, 1000.0 /* rows */, false /* is_empty */);
  stats->Rows();
  ...
  CStatistics *pstats2 =
      GPOS_NEW(mp) CStatistics(mp, phmulhist2, phmuldoubleWidth2, 100.0 /* rows */, false /* is_empty */);
  ...
  IStatistics *pstats3 = stats->CalcInnerJoinStats(mp, pstats2, join_preds_stats);
  ...
  CStatistics *pstats4 = CGroupByStatsProcessor::CalcGroupByStats(mp, stats, GCs, aggs, nullptr /*keys*/);

  GPOS_TRACE(GPOS_WSZ_LIT("pstats4 = stats group by"));
  CCardinalityTestUtils::PrintStats(mp, pstats4);

  // LASJ stats
  IStatistics *pstats5 = stats->CalcLASJoinStats(mp, pstats2, join_preds_stats, true /* DoIgnoreLASJHistComputation */);
...
CStatsPredPtrArry *CStatisticsTest::Pdrgpstatspred1(CMemoryPool *mp) {
...
// create a filter clause
CStatsPredPtrArry *CStatisticsTest::Pdrgpstatspred2(CMemoryPool *mp) {
  // contain for filters
  CStatsPredPtrArry *pdrgpstatspred = GPOS_NEW(mp) CStatsPredPtrArry(mp);
...
void CStatisticsTest::StatsFilterInt4(CMemoryPool *mp, ULONG colid, INT iLower, INT iUpper,
                                      CStatsPredPtrArry *pdrgpstatspred) {
  CStatsPredPoint *pstatspred1 =
      GPOS_NEW(mp) CStatsPredPoint(colid, CStatsPred::EstatscmptGEq, CTestUtils::PpointInt4(mp, iLower));

  CStatsPredPoint *pstatspred2 =
      GPOS_NEW(mp) CStatsPredPoint(colid, CStatsPred::EstatscmptL, CTestUtils::PpointInt4(mp, iUpper));
...
void CStatisticsTest::StatsFilterBool(CMemoryPool *mp, ULONG colid, BOOL fValue, CStatsPredPtrArry *pdrgpstatspred) {
  CStatsPredPoint *pstatspred1 =
      GPOS_NEW(mp) CStatsPredPoint(colid, CStatsPred::EstatscmptEq, CTestUtils::PpointBool(mp, fValue));

  pdrgpstatspred->Append(pstatspred1);
}

// create a stats filter on numeric types
void CStatisticsTest::StatsFilterNumeric(CMemoryPool *mp, ULONG colid, CWStringDynamic *pstrLowerEncoded,
  ...
  CStatsPredPoint *pstatspred1 = GPOS_NEW(mp) CStatsPredPoint(
      colid, CStatsPred::EstatscmptGEq, CCardinalityTestUtils::PpointNumeric(mp, pstrLowerEncoded, dValLower));

  CStatsPredPoint *pstatspred2 = GPOS_NEW(mp) CStatsPredPoint(
      colid, CStatsPred::EstatscmptL, CCardinalityTestUtils::PpointNumeric(mp, pstrUpperEncoded, dValUpper));
...
void CStatisticsTest::StatsFilterGeneric(CMemoryPool *mp, ULONG colid, OID oid, CWStringDynamic *pstrLowerEncoded,
                                         CWStringDynamic *pstrUpperEncoded, LINT lValueLower, LINT lValueUpper,
                                         CStatsPredPtrArry *pdrgpstatspred) {
  CStatsPredPoint *pstatspred1 = GPOS_NEW(mp) CStatsPredPoint(
      colid, CStatsPred::EstatscmptGEq, CCardinalityTestUtils::PpointGeneric(mp, oid, pstrLowerEncoded, lValueLower));

  CStatsPredPoint *pstatspred2 = GPOS_NEW(mp) CStatsPredPoint(
      colid, CStatsPred::EstatscmptL, CCardinalityTestUtils::PpointGeneric(mp, oid, pstrUpperEncoded, lValueUpper));
...
CStatisticsTest::EresUnittest_CStatisticsSelectDerivation() {
  ...
  return CTestUtils::EresTranslate(mp, szQuerySelect, szPlanSelect,
...
CStatisticsTest::EresUnittest_CStatisticsCopy() {
  ...
  CStatistics *pstats =
      GPOS_NEW(mp) CStatistics(mp, phmulhist, phmuldoubleWidth, 100.0 /* rows */, false /* is_empty */,
                               ULONG(5) /* relpages */, ULONG(10) /* relallvisible */, CDouble(100.0) /* rebinds */,
                               ULONG(3) /* num predicates */, nullptr, GPOS_NEW(mp) UlongToIntMap(mp));

  IStatistics *stats_copy = pstats->CopyStats(mp);

  UlongToColRefMap *colref_mapping = GPOS_NEW(mp) UlongToColRefMap(mp);
  IStatistics *stats_copy_remap = pstats->CopyStatsWithRemap(mp, colref_mapping, false);

  GPOS_RESULT eres = GPOS_OK;
  if (pstats->Width() != stats_copy->Width() || pstats->NumRebinds() != stats_copy->NumRebinds() ||
      pstats->RelPages() != stats_copy->RelPages() || pstats->RelAllVisible() != stats_copy->RelAllVisible() ||
      pstats->GetNumberOfPredicates() != stats_copy->GetNumberOfPredicates()) {
    eres = GPOS_FAILED;
  }

  if (pstats->Width() != stats_copy_remap->Width() || pstats->NumRebinds() != stats_copy_remap->NumRebinds() ||
      pstats->RelPages() != stats_copy_remap->RelPages() ||
      pstats->RelAllVisible() != stats_copy_remap->RelAllVisible() ||
      pstats->GetNumberOfPredicates() != stats_copy_remap->GetNumberOfPredicates()) {
    eres = GPOS_FAILED;
  }
  stats_copy->Release();
