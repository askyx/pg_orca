...
CDXLUtilsTest::EresUnittest_SerializeQuery() {
  ...
  CQueryToDXLResult *presult = CDXLUtils::ParseQueryToQueryDXLTree(mp, dxl_string, nullptr /*xsd_file_path*/);
  ...
  for (ULONG ulHeaders = 0; ulHeaders < GPOS_ARRAY_SIZE(rgfHeaders); ulHeaders++) {
    for (ULONG ulIndent = 0; ulIndent < GPOS_ARRAY_SIZE(rgfIndentation); ulIndent++) {
      ...
      CDXLUtils::SerializeQuery(mp, oss, presult->CreateDXLNode(), presult->GetOutputColumnsDXLArray(),
                                presult->GetCTEProducerDXLArray(), rgfHeaders[ulHeaders], rgfIndentation[ulIndent]);
      oss << std::endl;
    }
  }
...
CDXLUtilsTest::EresUnittest_SerializePlan() {
  ...
  CDXLNode *node = CDXLUtils::GetPlanDXLNode(mp, dxl_string, nullptr /*xsd_file_path*/, &plan_id, &plan_space_size);
  ...
  for (ULONG ulHeaders = 0; ulHeaders < GPOS_ARRAY_SIZE(rgfHeaders); ulHeaders++) {
    for (ULONG ulIndent = 0; ulIndent < GPOS_ARRAY_SIZE(rgfIndentation); ulIndent++) {
      oss << "Headers: " << rgfHeaders[ulHeaders] << ", indentation: " << rgfIndentation[ulIndent] << std::endl;
      CDXLUtils::SerializePlan(mp, oss, node, plan_id, plan_space_size, rgfHeaders[ulHeaders],
...
CDXLUtilsTest::EresUnittest_Encoding() {
  ...
  const CHAR *sz =
      "{\"{FUNCEXPR :funcid 1967 :funcresulttype 1184 :funcretset false :funcformat 1 :args ({FUNCEXPR :funcid 1191 "
      ":funcresulttype 1184 :funcretset false :funcformat 2 :args ({CONST :consttype 25 :constlen -1 :constbyval false "
      ":constisnull false :constvalue 7 [ 0 0 0 7 110 111 119 ]})} {CONST :consttype 23 :constlen 4 :constbyval true "
      ":constisnull false :constvalue 4 [ 2 0 0 0 0 0 0 0 ]})}\"}";
