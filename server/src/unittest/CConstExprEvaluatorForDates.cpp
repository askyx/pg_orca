...
//
//---------------------------------------------------------------------------
CExpression *CConstExprEvaluatorForDates::PexprEval(CExpression *pexpr) {
  GPOS_UNITTEST_ASSERT(COperator::EopScalarCmp == pexpr->Pop()->Eopid());
  GPOS_UNITTEST_ASSERT(COperator::EopScalarConst == (*pexpr)[0]->Pop()->Eopid());
  GPOS_UNITTEST_ASSERT(COperator::EopScalarConst == (*pexpr)[1]->Pop()->Eopid());

  CScalarConst *popScalarLeft = dynamic_cast<CScalarConst *>((*pexpr)[0]->Pop());

  GPOS_UNITTEST_ASSERT(CMDIdGPDB::m_mdid_date.Equals(popScalarLeft->GetDatum()->MDId()));
  CScalarConst *popScalarRight = dynamic_cast<CScalarConst *>((*pexpr)[1]->Pop());

  GPOS_UNITTEST_ASSERT(CMDIdGPDB::m_mdid_date.Equals(popScalarRight->GetDatum()->MDId()));
