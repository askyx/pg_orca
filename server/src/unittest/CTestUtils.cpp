...
CSystemId CTestUtils::m_sysidDefault(IMDId::EmdidGeneral, GPOS_WSZ_STR_LENGTH("GPDB"));

// XSD path
const CHAR *CTestUtils::m_szXSDPath = "http://greenplum.com/dxl/2010/12/ dxl.xsd";
...
//
//---------------------------------------------------------------------------
CMDProviderMemory *CTestUtils::CTestSetup::PmdpSetupFileBasedProvider() {
...
//
//---------------------------------------------------------------------------
void CTestUtils::InitProviderFile(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
void CTestUtils::DestroyMDProvider() {
  GPOS_UNITTEST_ASSERT(nullptr != m_mp);
...
CTableDescriptor *CTestUtils::PtabdescPlainWithColNameFormat(CMemoryPool *mp, ULONG num_cols, IMDId *mdid,
  ...
  CTableDescriptor *ptabdesc = GPOS_NEW(mp)
      CTableDescriptor(mp, mdid, nameTable,
                       false,  // convert_hash_to_random
                       IMDRelation::EreldistrRandom, IMDRelation::ErelstorageHeap, IMDRelation::GetCurrentAOVersion(),
                       0,   // ulExecuteAsUser
                       -1,  // lockmode
                       2,   // aclmode SELECT
  ...
  for (ULONG i = 0; i < num_cols; i++) {
    ...
    CColumnDescriptor *pcoldescInt =
        GPOS_NEW(mp) CColumnDescriptor(mp, pmdtypeint4, default_type_modifier, nameColumnInt, i + 1, is_nullable);
...
//
//---------------------------------------------------------------------------
CTableDescriptor *CTestUtils::PtabdescPlain(CMemoryPool *mp, ULONG num_cols, IMDId *mdid, const CName &nameTable,
                                            BOOL is_nullable  // define nullable columns
) {
  return PtabdescPlainWithColNameFormat(mp, num_cols, mdid, GPOS_WSZ_LIT("column_%04d"), nameTable, is_nullable);
}
...
//
//---------------------------------------------------------------------------
CTableDescriptor *CTestUtils::PtabdescCreate(CMemoryPool *mp, ULONG num_cols, IMDId *mdid, const CName &nameTable,
                                             BOOL fPartitioned) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalGet(CMemoryPool *mp, CTableDescriptor *ptabdesc,
                                         const CWStringConst *pstrTableAlias) {
  GPOS_UNITTEST_ASSERT(nullptr != ptabdesc);

  CLogicalGet *pop = GPOS_NEW(mp) CLogicalGet(mp, GPOS_NEW(mp) CName(mp, CName(pstrTableAlias)), ptabdesc,
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalGetNullable(CMemoryPool *mp, OID oidTable, const CWStringConst *str_table_name,
                                                 const CWStringConst *pstrTableAlias) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalGet(CMemoryPool *mp, CWStringConst *str_table_name, CWStringConst *pstrTableAlias,
                                         ULONG ulTableId) {
  CTableDescriptor *ptabdesc = PtabdescCreate(
      mp, GPOPT_TEST_REL_WIDTH, GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, ulTableId, 1, 1), CName(str_table_name));
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalGet(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalForeignGet(CMemoryPool *mp) {
  CWStringConst strName(GPOS_WSZ_LIT("ExternalTable"));
  CMDIdGPDB *mdid = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID, 1, 1);
  CTableDescriptor *ptabdesc = PtabdescCreate(mp, 3, mdid, CName(&strName));
  CWStringConst strAlias(GPOS_WSZ_LIT("ExternalTableAlias"));

  return GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalForeignGet(mp, GPOS_NEW(mp) CName(mp, &strAlias), ptabdesc));
}
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalGetPartitioned(CMemoryPool *mp) {
  ULONG ulAttributes = 2;
  CWStringConst strName(GPOS_WSZ_LIT("PartTable"));
  CMDIdGPDB *mdid = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID_PARTITIONED);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalDynamicGetWithIndexes(CMemoryPool *mp) {
  ULONG ulAttributes = 2;
  CWStringConst strName(GPOS_WSZ_LIT("P1"));
  CMDIdGPDB *mdid = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID_PARTITIONED_WITH_INDEXES);
  CTableDescriptor *ptabdesc = PtabdescCreate(mp, ulAttributes, mdid, CName(&strName), true /*fPartitioned*/);
  ...
  return GPOS_NEW(mp)
      CExpression(mp, GPOS_NEW(mp) CLogicalDynamicGet(mp, GPOS_NEW(mp) CName(mp, CName(&strAlias)), ptabdesc,
                                                      0,  // ulPartIndex
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelect(CMemoryPool *mp, CExpression *pexpr) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelect(CMemoryPool *mp, CWStringConst *str_table_name,
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelect(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelectWithContradiction(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelectPartitioned(CMemoryPool *mp) {
  ...
  CExpression *pexprScalar =
      CUtils::PexprScalarEqCmp(mp, CUtils::PexprScalarIdent(mp, pcrPartKey), CUtils::PexprScalarConstInt4(mp, val));
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalAssert(CMemoryPool *mp) {
  ...
  CWStringConst *pstrErrMsg = CXformUtils::PstrErrorMessage(mp, gpos::CException::ExmaSQL,
                                                            gpos::CException::ExmiSQLTest, GPOS_WSZ_LIT("Test msg"));
  CExpression *pexprAssertConstraint =
      GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarAssertConstraint(mp, pstrErrMsg), pexprPredicate);
  ...
  CLogicalAssert *popAssert = GPOS_NEW(mp)
      CLogicalAssert(mp, GPOS_NEW(mp) CException(gpos::CException::ExmaSQL, gpos::CException::ExmiSQLTest));
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::Pexpr3WayJoinPartitioned(CMemoryPool *mp) {
  return PexprLogicalJoin<CLogicalInnerJoin>(mp, PexprLogicalGet(mp), PexprJoinPartitionedOuter<CLogicalInnerJoin>(mp));
}
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::Pexpr4WayJoinPartitioned(CMemoryPool *mp) {
  return PexprLogicalJoin<CLogicalInnerJoin>(mp, PexprLogicalGet(mp), Pexpr3WayJoinPartitioned(mp));
}
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelectWithNestedAnd(CMemoryPool *mp) {
  GPOS_UNITTEST_ASSERT(nullptr != mp);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelectWithNestedOr(CMemoryPool *mp) {
  GPOS_UNITTEST_ASSERT(nullptr != mp);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelectWithEvenNestedNot(CMemoryPool *mp) {
  GPOS_UNITTEST_ASSERT(nullptr != mp);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprScIdentCmpScIdent(CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight,
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprScIdentCmpConst(CMemoryPool *mp, CExpression *pexpr, IMDType::ECmpType cmp_type,
                                              ULONG ulVal) {
  GPOS_UNITTEST_ASSERT(nullptr != pexpr);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelectCmpToConst(CMemoryPool *mp) {
  // generate a get expression
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelectArrayCmp(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelectArrayCmp(CMemoryPool *mp, CScalarArrayCmp::EArrCmpType earrcmptype,
...
CExpression *CTestUtils::PexprLogicalSelectArrayCmp(CMemoryPool *mp, CScalarArrayCmp::EArrCmpType earrcmptype,
  ...
  return CUtils::PexprLogicalSelect(
      mp, pexprGet, CUtils::PexprScalarArrayCmp(mp, earrcmptype, ecmptype, pdrgpexprArrayElems, colref));
}
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelectWithOddNestedNot(CMemoryPool *mp) {
  GPOS_UNITTEST_ASSERT(nullptr != mp);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelectWithNestedAndOrNot(CMemoryPool *mp) {
  GPOS_UNITTEST_ASSERT(nullptr != mp);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSubqueryWithConstTableGet(CMemoryPool *mp, COperator::EOperatorId op_id) {
  GPOS_UNITTEST_ASSERT(nullptr != mp);
  GPOS_UNITTEST_ASSERT(COperator::EopScalarSubqueryAny == op_id || COperator::EopScalarSubqueryAll == op_id);

  CWStringConst strNameR(GPOS_WSZ_LIT("Rel1"));
  CMDIdGPDB *pmdidR = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_TEST_REL_OID1, 1, 1);
  ...
  if (COperator::EopScalarSubqueryAny == op_id) {
    ...
    pexprSubquery = GPOS_NEW(mp) CExpression(
        mp,
        GPOS_NEW(mp)
            CScalarSubqueryAny(mp, GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_EQ_OP), str, pcrInner),
    ...
    pexprSubquery = GPOS_NEW(mp) CExpression(
        mp,
        GPOS_NEW(mp)
            CScalarSubqueryAll(mp, GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_EQ_OP), str, pcrInner),
        pexprConstTableGet, CUtils::PexprScalarIdent(mp, pcrOuter));
  }
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelectWithConstAnySubquery(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelectWithConstAllSubquery(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelectCorrelated(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelectOnOuterJoin(CMemoryPool *mp) {
  ...
  CExpression *pexprOuterJoin =
      GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalLeftOuterJoin(mp), pexprOuter, pexprInner, pexprOuterJoinPred);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelectCorrelated(CMemoryPool *mp, CColRefSet *outer_refs, ULONG ulLevel) {
  ...
  // assemble select
  CExpression *pexprResult = GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalSelect(mp), pexpr, pexprCorrelation);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalProject(CMemoryPool *mp, CExpression *pexpr, CColRef *colref,
                                             CColRef *new_colref) {
  GPOS_UNITTEST_ASSERT(nullptr != pexpr);
  ...
  return GPOS_NEW(mp) CExpression(
      ...
      GPOS_NEW(mp)
          CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp),
                      GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectElement(mp, new_colref),
                                               GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarIdent(mp, colref)))));
}
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalProject(CMemoryPool *mp) {
  // generate a get expression
  CExpression *pexpr = PexprLogicalGet(mp);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalProjectGbAggCorrelated(CMemoryPool *mp) {
  ...
  CExpression *pexprProject = GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalProject(mp), pexprGbAgg, pexprPrjList);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalJoinCorrelated(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalJoinWithPartitionedAndIndexedInnerChild(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalNAryJoin(CMemoryPool *mp, CExpressionArray *pdrgpexpr) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalNAryJoin(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLeftOuterJoinOnNAryJoin(CMemoryPool *mp) {
  ...
  return GPOS_NEW(mp)
      CExpression(mp, GPOS_NEW(mp) CLogicalLeftOuterJoin(mp), pexprNAryJoin, pexprLOJInnerChild, pexprPred);
}

CExpression *CTestUtils::PexprRightOuterJoin(CMemoryPool *mp) {
  ...
  return GPOS_NEW(mp)
      CExpression(mp, GPOS_NEW(mp) CLogicalRightOuterJoin(mp), pexprOuterChild, pexprInnerChild, pexprPred);
}
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprNAryJoinOnLeftOuterJoin(CMemoryPool *mp) {
  CExpression *pexprNAryJoin = PexprLogicalNAryJoin(mp);
  ...
  CExpression *pexprLOJ = GPOS_NEW(mp)
      CExpression(mp, GPOS_NEW(mp) CLogicalLeftOuterJoin(mp), pexprLOJOuterChild, pexprLOJInnerChild, pexprPred);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalLimit(CMemoryPool *mp, CExpression *pexpr, LINT iStart, LINT iRows, BOOL fGlobal,
                                           BOOL fHasCount) {
  GPOS_UNITTEST_ASSERT(nullptr != pexpr);
  ...
  return GPOS_NEW(mp)
      CExpression(mp, GPOS_NEW(mp) CLogicalLimit(mp, pos, fGlobal, fHasCount, false /*fTopLimitUnderDML*/), pexpr,
                  CUtils::PexprScalarConstInt8(mp, iStart), CUtils::PexprScalarConstInt8(mp, iRows));
}
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalLimit(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalGbAggWithSum(CMemoryPool *mp) {
  ...
  CExpression *pexprPrjList = GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pexprProjElem);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalGbAggWithInput(CMemoryPool *mp, CExpression *pexprInput) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalGbAgg(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprPrjElemWithSum(CMemoryPool *mp, CColRef *colref) {
  CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();

  // generate a SUM expression
  CMDIdGPDB *pmdidSumAgg = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_SUM_AGG);
  CWStringConst *pstrAggFunc = GPOS_NEW(mp) CWStringConst(GPOS_WSZ_LIT("sum"));
  CExpression *pexprScalarAgg = CUtils::PexprAggFunc(mp, pmdidSumAgg, pstrAggFunc, colref, false /*is_distinct*/,
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalGbAggOverJoin(CMemoryPool *mp) {
  ...
  return CUtils::PexprLogicalGbAggGlobal(mp, colref_array, pexprJoin,
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalGbAggDedupOverInnerJoin(CMemoryPool *mp) {
  // generate a join expression
  CExpression *pexprJoin = PexprLogicalJoin<CLogicalInnerJoin>(mp);
  ...
  return GPOS_NEW(mp) CExpression(
      ...
      pexprJoin, GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp)));
}
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalGbAggCorrelated(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprConstTableGet(CMemoryPool *mp, ULONG ulElements) {
  ...
  CName name(&strName);
  CColumnDescriptor *pcoldescInt =
      GPOS_NEW(mp) CColumnDescriptor(mp, pmdtypeint4, default_type_modifier, name, 1 /* attno */, false /*IsNullable*/
      );
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprConstTableGet5(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalInsert(CMemoryPool *mp) {
  ...
  return GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalInsert(mp, ptabdesc, colref_array), pexprCTG);
}
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalDelete(CMemoryPool *mp) {
  CWStringConst strName(GPOS_WSZ_LIT("BaseTable"));
  CMDIdGPDB *mdid = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID, 1, 1);
  CTableDescriptor *ptabdesc = PtabdescCreate(mp, 1, mdid, CName(&strName));
  CWStringConst strAlias(GPOS_WSZ_LIT("BaseTableAlias"));
  ...
  return GPOS_NEW(mp)
      CExpression(mp, GPOS_NEW(mp) CLogicalDelete(mp, ptabdesc, colref_array, colref, colref), pexprGet);
}
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalUpdate(CMemoryPool *mp) {
  CWStringConst strName(GPOS_WSZ_LIT("BaseTable"));
  CMDIdGPDB *mdid = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID, 1, 1);
  CTableDescriptor *ptabdesc = PtabdescCreate(mp, 1, mdid, CName(&strName));
  CWStringConst strAlias(GPOS_WSZ_LIT("BaseTableAlias"));
  ...
  return GPOS_NEW(mp) CExpression(
      mp, GPOS_NEW(mp) CLogicalUpdate(mp, ptabdesc, pdrgpcrDelete, pdrgpcrInsert, colref, colref, true), pexprGet);
}
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalDynamicGet(CMemoryPool *mp, CTableDescriptor *ptabdesc,
  ...
  return GPOS_NEW(mp)
      CExpression(mp, GPOS_NEW(mp) CLogicalDynamicGet(mp, GPOS_NEW(mp) CName(mp, CName(pstrTableAlias)), ptabdesc,
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalDynamicGet(CMemoryPool *mp) {
  CWStringConst strName(GPOS_WSZ_LIT("PartTable"));
  CMDIdGPDB *mdid = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID_PARTITIONED, 1, 1);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelectWithEqPredicateOverDynamicGet(CMemoryPool *mp) {
  CWStringConst strName(GPOS_WSZ_LIT("PartTable"));
  CMDIdGPDB *mdid = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID_PARTITIONED, 1, 1);
  CTableDescriptor *ptabdesc = PtabdescCreate(mp, 3, mdid, CName(&strName), true /*fPartitioned*/);
  CWStringConst strAlias(GPOS_WSZ_LIT("PartTableAlias"));
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSelectWithLTPredicateOverDynamicGet(CMemoryPool *mp) {
  CWStringConst strName(GPOS_WSZ_LIT("PartTable"));
  CMDIdGPDB *mdid = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID_PARTITIONED, 1, 1);
  CTableDescriptor *ptabdesc = PtabdescCreate(mp, 3, mdid, CName(&strName), true /*fPartitioned*/);
  CWStringConst strAlias(GPOS_WSZ_LIT("PartTableAlias"));
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalTVFTwoArgs(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalTVFThreeArgs(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalTVFNoArgs(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalTVF(CMemoryPool *mp, ULONG ulArgs) {
  CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();

  const WCHAR *wszFuncName = GPOS_WSZ_LIT("generate_series");

  IMDId *mdid = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT8_GENERATE_SERIES);
  ...
  CColumnDescriptor *pcoldescInt =
      GPOS_NEW(mp) CColumnDescriptor(mp, pmdtypeint8, default_type_modifier, name, 1 /* attno */, false /*IsNullable*/);
  ...
  CLogicalTVF *popTVF = GPOS_NEW(mp) CLogicalTVF(mp, mdid, mdid_return_type, str_func_name, pdrgpcoldesc);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalCTEProducerOverSelect(CMemoryPool *mp, ULONG ulCTEId) {
  ...
  return GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalCTEProducer(mp, ulCTEId, colref_array), pexprSelect);
}
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalCTEProducerOverSelect(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprCTETree(CMemoryPool *mp) {
  ULONG ulCTEId = 0;
  CExpressionArray *pdrgpexpr = GPOS_NEW(mp) CExpressionArray(mp);
  ...
  CExpression *pexprConsumer =
      GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalCTEConsumer(mp, ulCTEId, pdrgpcrConsumer));
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSequence(CMemoryPool *mp, CExpressionArray *pdrgpexpr) {
  GPOS_UNITTEST_ASSERT(nullptr != pdrgpexpr);

  return GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalSequence(mp), pdrgpexpr);
}
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSequence(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalUnion(CMemoryPool *mp, ULONG ulDepth) {
  ...
  if (0 == ulDepth) {
    ...
    for (ULONG i = 0; i < 3; i++) {
      ...
      if (pop->Eopid() == COperator::EopLogicalGet) {
        ...
        GPOS_UNITTEST_ASSERT(COperator::EopLogicalUnion == pop->Eopid());
    ...
    pexpr =
        GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalUnion(mp, pdrgpcrOutput, pdrgpdrgpcrInput), pdrgpexprInput);
  }
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalSequenceProject(CMemoryPool *mp, OID oidFunc, CExpression *pexprInput) {
  CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
  CColumnFactory *col_factory = COptCtxt::PoctxtFromTLS()->Pcf();
  ...
  CLogicalSequenceProject *popSeqProj = GPOS_NEW(mp) CLogicalSequenceProject(
      mp, GPOS_NEW(mp) CDistributionSpecHashed(pdrgpexpr, true /*fNullsCollocated*/), pdrgpos, pdrgwf);
  ...
  CExpression *pexprWinFunc = GPOS_NEW(mp) CExpression(
      mp, GPOS_NEW(mp) CScalarWindowFunc(
              mp, mdid, mdid_return_type, GPOS_NEW(mp) CWStringConst(mp, pmdfunc->Mdname().GetMDName()->GetBuffer()),
              CScalarWindowFunc::EwsImmediate, false /*is_distinct*/, false /*is_star_arg*/, false /*is_simple_agg*/
              ));
  // window function call is not a cast and so does not need a type modifier
  CColRef *pcrComputed =
      col_factory->PcrCreate(md_accessor->RetrieveType(pmdfunc->GetResultTypeMdid()), default_type_modifier);

  CExpression *pexprPrjList = GPOS_NEW(mp) CExpression(
      mp, GPOS_NEW(mp) CScalarProjectList(mp), CUtils::PexprScalarProjectElement(mp, pcrComputed, pexprWinFunc));
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprOneWindowFunction(CMemoryPool *mp) {
  CExpression *pexprGet = PexprLogicalGet(mp);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprTwoWindowFunctions(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CExpressionJoinsArray *CTestUtils::PdrgpexprJoins(CMemoryPool *mp,
    ...
    if (0 == i) {
      ...
      if (fCrossProduct) {
        ...
        pexprJoin = CUtils::PexprLogicalJoin<CLogicalInnerJoin>(
            mp, (*pdrgpexpr)[i - 1], pexpr, CPredicateUtils::PexprConjunction(mp, nullptr)  // generate a constant True
        );
      } else {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprLogicalNAryJoin(CMemoryPool *mp, CWStringConst *pstrRel, ULONG *pulRel, ULONG ulRels,
                                              BOOL fCrossProduct) {
  CExpressionArray *pdrgpexpr = GPOS_NEW(mp) CExpressionArray(mp);
  for (ULONG ul = 0; ul < ulRels; ul++) {
...
//
//---------------------------------------------------------------------------
CQueryContext *CTestUtils::PqcGenerate(CMemoryPool *mp, CExpression *pexpr, CColRefArray *colref_array) {
  // generate required columns
  CColRefSet *pcrs = GPOS_NEW(mp) CColRefSet(mp);
  pcrs->Include(colref_array);

  COrderSpec *pos = GPOS_NEW(mp) COrderSpec(mp);
  CDistributionSpec *pds = GPOS_NEW(mp) CDistributionSpecSingleton(CDistributionSpecSingleton::EstCoordinator);
...
//
//---------------------------------------------------------------------------
CQueryContext *CTestUtils::PqcGenerate(CMemoryPool *mp, CExpression *pexpr) {
  CColRefSet *pcrs = GPOS_NEW(mp) CColRefSet(mp);
  ...
  COrderSpec *pos = GPOS_NEW(mp) COrderSpec(mp);
  pos->Append(GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_LT_OP), pcrsOutput->PcrAny(), COrderSpec::EntFirst);
  ...
  CReqdPropPlan *prpp = GPOS_NEW(mp) CReqdPropPlan(pcrsOutput, peo, ped, per, pepp, pcter);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprScalarNestedPreds(CMemoryPool *mp, CExpression *pexpr,
                                                CScalarBoolOp::EBoolOperator eboolop) {
  GPOS_UNITTEST_ASSERT(nullptr != mp);
  GPOS_UNITTEST_ASSERT(nullptr != pexpr);
  GPOS_UNITTEST_ASSERT(CScalarBoolOp::EboolopAnd == eboolop || CScalarBoolOp::EboolopOr == eboolop ||
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprFindFirstExpressionWithOpId(CExpression *pexpr, COperator::EOperatorId op_id) {
  GPOS_UNITTEST_ASSERT(nullptr != pexpr);
...
//
//---------------------------------------------------------------------------
void CTestUtils::EqualityPredicate(CMemoryPool *mp, CColRefSet *pcrsLeft, CColRefSet *pcrsRight,
...
//
//---------------------------------------------------------------------------
CPoint *CTestUtils::PpointInt2(CMemoryPool *mp, INT i) {
...
//
//---------------------------------------------------------------------------
CPoint *CTestUtils::PpointInt4(CMemoryPool *mp, INT i) {
...
//
//---------------------------------------------------------------------------
CPoint *CTestUtils::PpointInt4NullVal(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CPoint *CTestUtils::PpointInt8(CMemoryPool *mp, INT i) {
...
//
//---------------------------------------------------------------------------
CPoint *CTestUtils::PpointBool(CMemoryPool *mp, BOOL value) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprReadQuery(CMemoryPool *mp, const CHAR *szQueryFileName) {
  CHAR *szQueryDXL = CDXLUtils::Read(mp, szQueryFileName);

  // parse the DXL query tree from the given DXL document
  CQueryToDXLResult *ptroutput = CDXLUtils::ParseQueryToQueryDXLTree(mp, szQueryDXL, nullptr);
  ...
  CExpression *pexprQuery = trdxl2expr.PexprTranslateQuery(
      ptroutput->CreateDXLNode(), ptroutput->GetOutputColumnsDXLArray(), ptroutput->GetCTEProducerDXLArray());
...
//---------------------------------------------------------------------------
GPOS_RESULT
CTestUtils::EresTranslate(CMemoryPool *mp, const CHAR *szQueryFileName, const CHAR *szPlanFileName,
  ...
  CQueryContext *pqc = CQueryContext::PqcGenerate(
      mp, pexprQuery, ptrdxl2expr.PdrgpulOutputColRefs(), ptrdxl2expr.Pdrgpmdname(), true /*fDeriveStats*/
  );
  ...
  CDXLUtils::SerializePlan(mp, osTranslatedPlan, pdxlnPlan, optimizer_config->GetEnumeratorCfg()->GetPlanId(),
                           optimizer_config->GetEnumeratorCfg()->GetPlanSpaceSize(), true /*serialize_header_footer*/,
                           true /*indentation*/);
...
//---------------------------------------------------------------------------
GPOS_RESULT
CTestUtils::EresCompare(IOstream &os, CWStringDynamic *pstrActual, CWStringDynamic *pstrExpected,
                        BOOL fIgnoreMismatch) {
...
CHAR *CTestUtils::ExtractFilenameFromPath(CHAR *file_path) {
...
void CTestUtils::CreateExpectedAndActualFile(CMemoryPool *mp, const CHAR *file_name, CWStringDynamic *strExpected,
  ...
  wos_expected = GPOS_NEW(mp) std::wofstream(
      CUtils::CreateMultiByteCharStringFromWCString(m_mp, const_cast<WCHAR *>(expected_file.GetBuffer())));
  os_expected = GPOS_NEW(mp) COstreamBasic(wos_expected.Value());

  wos_actual = GPOS_NEW(mp)
      std::wofstream(CUtils::CreateMultiByteCharStringFromWCString(m_mp, const_cast<WCHAR *>(actual_file.GetBuffer())));
...
//
//---------------------------------------------------------------------------
BOOL CTestUtils::FPlanMatch(CMemoryPool *mp, IOstream &os, const CDXLNode *pdxlnActual, ULLONG ullPlanIdActual,
                            ULLONG ullPlanSpaceSizeActual, const CDXLNode *pdxlnExpected, ULLONG ullPlanIdExpected,
  ...
  if (nullptr != pdxlnActual && nullptr == pdxlnExpected) {
    ...
    CDXLUtils::SerializePlan(mp, at.Os(), pdxlnActual, ullPlanIdActual, ullPlanSpaceSizeActual,
  ...
  if (nullptr == pdxlnActual && nullptr != pdxlnExpected) {
    ...
    CDXLUtils::SerializePlan(mp, at.Os(), pdxlnExpected, ullPlanIdExpected, ullPlanSpaceSizeExpected,
                             false /*serialize_document_header_footer*/, true /*indentation*/);
    at.Os() << std::endl;
  ...
  CDXLUtils::SerializePlan(mp, osActual, pdxlnActual, 0 /*ullPlanIdActual*/, 0 /*ullPlanSpaceSizeActual*/,
                           false /*serialize_document_header_footer*/, true /*indentation*/);
  GPOS_CHECK_ABORT;

  CWStringDynamic strExpected(mp);
  COstreamString osExpected(&strExpected);
  CDXLUtils::SerializePlan(mp, osExpected, pdxlnExpected, 0 /*ullPlanIdExpected*/, 0 /*ullPlanSpaceSizeExpected*/,
                           false /*serialize_document_header_footer*/, true /*indentation*/);
  GPOS_CHECK_ABORT;
  ...
  if (!result) {
    ...
    CWStringDynamic strActual(mp);
    COstreamString osActual(&strActual);
    CDXLUtils::SerializePlan(mp, osActual, pdxlnActual, ullPlanIdActual, ullPlanSpaceSizeActual,
                             false /*serialize_document_header_footer*/, true /*indentation*/);
    GPOS_CHECK_ABORT;

    CWStringDynamic strExpected(mp);
    COstreamString osExpected(&strExpected);
    CDXLUtils::SerializePlan(mp, osExpected, pdxlnExpected, ullPlanIdExpected, ullPlanSpaceSizeExpected,
                             false /*serialize_document_header_footer*/, true /*indentation*/);
    GPOS_CHECK_ABORT;
...
//
//---------------------------------------------------------------------------
BOOL CTestUtils::FPlanCompare(CMemoryPool *mp, IOstream &os, const CDXLNode *pdxlnActual, ULLONG ullPlanIdActual,
                              ULLONG ullPlanSpaceSizeActual, const CDXLNode *pdxlnExpected, ULLONG ullPlanIdExpected,
                              ULLONG ullPlanSpaceSizeExpected, BOOL fMatchPlans, INT iCmpSpaceSize,
  ...
  if ((0 == iCmpSpaceSize && ullPlanSpaceSizeActual != ullPlanSpaceSizeExpected) ||  // required comparison is equality
      (-1 == iCmpSpaceSize &&
       ullPlanSpaceSizeActual > ullPlanSpaceSizeExpected) ||  // required comparison is (Actual <= Expected)
      (1 == iCmpSpaceSize &&
       ullPlanSpaceSizeActual < ullPlanSpaceSizeExpected)  // required comparison is (Actual >= Expected)
  ) {
  ...
  return FPlanMatch(mp, os, pdxlnActual, ullPlanIdActual, ullPlanSpaceSizeActual, pdxlnExpected, ullPlanIdExpected,
...
//
//---------------------------------------------------------------------------
IntPtrArray *CTestUtils::PdrgpiSegments(CMemoryPool *mp) {
...
//
//---------------------------------------------------------------------------
CHAR *CTestUtils::SzMinidumpFileName(CMemoryPool *mp, const CHAR *file_name) {
...
//---------------------------------------------------------------------------
GPOS_RESULT
CTestUtils::EresRunMinidump(CMemoryPool *mp, CMDAccessor *md_accessor, const CHAR *file_name, ULONG *pulTestCounter,
                            ULONG ulSessionId, ULONG ulCmdId, BOOL fMatchPlans, INT iCmpSpaceSize,
  ...
  pdxlnPlan = CMinidumperUtils::PdxlnExecuteMinidump(mp, md_accessor, pdxlmd, szMinidumpFileName, ulSegments,
                                                     ulSessionId, ulCmdId, optimizer_config, pceeval);
  ...
  {
    CAutoTrace at(mp);
    if (!CTestUtils::FPlanCompare(mp, at.Os(), pdxlnPlan, optimizer_config->GetEnumeratorCfg()->GetPlanId(),
                                  optimizer_config->GetEnumeratorCfg()->GetPlanSpaceSize(), pdxlmd->PdxlnPlan(),
                                  pdxlmd->GetPlanId(), pdxlmd->GetPlanSpaceSize(), fMatchPlans, iCmpSpaceSize,
                                  file_name)) {
      eres = GPOS_FAILED;
    }
  }
...
CTestUtils::EresRunMinidumps(CMemoryPool *,  // pmpInput,
                             const CHAR *rgszFileNames[], ULONG ulTests, ULONG *pulTestCounter, ULONG ulSessionId,
                             ULONG ulCmdId, BOOL fMatchPlans, BOOL fTestSpacePruning,
  ...
  for (ULONG ul = *pulTestCounter; ul < ulTests; ul++) {
    ...
    if (fMatchPlans) {
      ...
      eres = EresRunMinidumpsUsingOneMDFile(mp, rgszFileNames[ul], &rgszFileNames[ul], pulTestCounter, ulSessionId,
                                            ulCmdId, fMatchPlans,
                                            0,  // iCmpSpaceSize
                                            pceeval);
...
//---------------------------------------------------------------------------
GPOS_RESULT
CTestUtils::EresRunMinidumpsUsingOneMDFile(CMemoryPool *mp, const CHAR *szMDFilePath, const CHAR *rgszFileNames[],
  ...
  {  // scope for MD accessor
    ...
    eres = EresRunMinidump(mp, &mda, rgszFileNames[0], pulTestCounter, ulSessionId, ulCmdId, fMatchPlans, iCmpSpaceSize,
                           pceeval);
...
//---------------------------------------------------------------------------
GPOS_RESULT
CTestUtils::EresSamplePlans(const CHAR *rgszFileNames[], ULONG ulTests, ULONG *pulTestCounter, ULONG ulSessionId,
                            ULONG ulCmdId) {
  GPOS_RESULT eres = GPOS_OK;
  ...
  for (ULONG ul = *pulTestCounter; ul < ulTests; ul++) {
    ...
    if (nullptr == optimizer_config) {
      optimizer_config = GPOS_NEW(mp) COptimizerConfig(
          ...
          CStatisticsConfig::PstatsconfDefault(mp), CCTEConfig::PcteconfDefault(mp), ICostModel::PcmDefault(mp),
          CHint::PhintDefault(mp), nullptr, CWindowOids::GetWindowOids(mp));
    } else {
      optimizer_config->AddRef();
    }
...

    {
      ...
      CDXLNode *pdxlnPlan = CMinidumperUtils::PdxlnExecuteMinidump(mp, &mda, pdxlmd, rgszFileNames[ul], ulSegments,
                                                                   ulSessionId, ulCmdId, optimizer_config,
                                                                   nullptr  // pceeval
      );
...

      {
        ...
        at.Os() << "Generated " << optimizer_config->GetEnumeratorCfg()->UlCreatedSamples() << " samples ... "
                << std::endl;

        // print ids of sampled plans
        CWStringDynamic *str =
            CDXLUtils::SerializeSamplePlans(mp, optimizer_config->GetEnumeratorCfg(), true /*fIdent*/);
        at.Os() << str->GetBuffer();
        GPOS_DELETE(str);
        ...
        for (ULONG ul = 0; ul < size; ul++) {
          at.Os() << optimizer_config->GetEnumeratorCfg()->DCostDistrX(ul) << "\t"
                  << optimizer_config->GetEnumeratorCfg()->DCostDistrY(ul) << std::endl;
        }
...
//---------------------------------------------------------------------------
GPOS_RESULT
CTestUtils::EresCheckPlans(const CHAR *rgszFileNames[], ULONG ulTests, ULONG *pulTestCounter, ULONG ulSessionId,
...
//---------------------------------------------------------------------------
GPOS_RESULT
CTestUtils::EresCheckOptimizedPlan(const CHAR *rgszFileNames[], ULONG ulTests, ULONG *pulTestCounter, ULONG ulSessionId,
                                   ULONG ulCmdId, FnDXLPlanChecker *pfdpc, ICostModelParamsArray *pdrgpcp) {
...
//
//---------------------------------------------------------------------------
IDatum *CTestUtils::CreateGenericDatum(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid_type,
  ...
  if (CMDTypeGenericGPDB::IsTimeRelatedType(mdid_type)) {
    dxl_datum = GPOS_NEW(mp) CDXLDatumStatsDoubleMappable(mp, mdid_type, default_type_modifier, false /*is_const_null*/,
                                                          data, ulbaSize, CDouble(value));
  } else if (pmdtype->IsTextRelated()) {
    dxl_datum = GPOS_NEW(mp) CDXLDatumStatsLintMappable(mp, mdid_type, default_type_modifier, false /*is_const_null*/,
                                                        data, ulbaSize, value);
  } else {
    dxl_datum =
        GPOS_NEW(mp) CDXLDatumGeneric(mp, mdid_type, default_type_modifier, false /*is_const_null*/, data, ulbaSize);
  }
...
//
//---------------------------------------------------------------------------
IDatum *CTestUtils::CreateDoubleDatum(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid_type, CDouble value) {
  GPOS_UNITTEST_ASSERT(nullptr != md_accessor);
...
//
//---------------------------------------------------------------------------
CConstraintInterval *CTestUtils::PciGenericInterval(CMemoryPool *mp, CMDAccessor *md_accessor,
  ...
  CRange *prange =
      GPOS_NEW(mp) CRange(mdid, COptCtxt::PoctxtFromTLS()->Pcomp(), pdatumLower, eriLeft, pdatumUpper, eriRight);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprScalarCmpIdentToConstant(CMemoryPool *mp, CExpression *pexpr) {
  GPOS_UNITTEST_ASSERT(nullptr != mp);
  GPOS_UNITTEST_ASSERT(nullptr != pexpr);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprExistsSubquery(CMemoryPool *mp, CExpression *pexprOuter) {
  GPOS_UNITTEST_ASSERT(nullptr != mp);
  ...
  return CSubqueryTestUtils::PexprSubqueryExistential(
      mp, COperator::EopScalarSubqueryExists, pexprOuter, pexprInner, false /* fCorrelated */
  );
}
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexpSubqueryAll(CMemoryPool *mp, CExpression *pexprOuter) {
  GPOS_UNITTEST_ASSERT(nullptr != mp);
  GPOS_UNITTEST_ASSERT(nullptr != pexprOuter);
  ...
  return GPOS_NEW(mp)
      CExpression(mp,
                  GPOS_NEW(mp) CScalarSubqueryAll(mp, GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_EQ_OP),
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexpSubqueryAny(CMemoryPool *mp, CExpression *pexprOuter) {
  GPOS_UNITTEST_ASSERT(nullptr != mp);
  GPOS_UNITTEST_ASSERT(nullptr != pexprOuter);
  ...
  return GPOS_NEW(mp)
      CExpression(mp,
                  GPOS_NEW(mp) CScalarSubqueryAny(mp, GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4_EQ_OP),
                                                  GPOS_NEW(mp) CWStringConst(GPOS_WSZ_LIT("=")), pcrInner),
                  pexprInner, CUtils::PexprScalarIdent(mp, pcrOuter));
}
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprNotExistsSubquery(CMemoryPool *mp, CExpression *pexprOuter) {
  GPOS_UNITTEST_ASSERT(nullptr != mp);
  GPOS_UNITTEST_ASSERT(nullptr != pexprOuter);
  ...
  return CSubqueryTestUtils::PexprSubqueryExistential(
      mp, COperator::EopScalarSubqueryNotExists, pexprOuter, pexprInner, false /* fCorrelated */
  );
}
...
//
//---------------------------------------------------------------------------
const CExpression *CTestUtils::PexprFirst(const CExpression *pexpr, const COperator::EOperatorId op_id) {
  GPOS_CHECK_STACK_SIZE;
  GPOS_UNITTEST_ASSERT(nullptr != pexpr);
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprAnd(CMemoryPool *mp, CExpression *pexprActual, CExpression *pexprExpected) {
...
//
//---------------------------------------------------------------------------
CExpression *CTestUtils::PexprOr(CMemoryPool *mp, CExpression *pexprActual, CExpression *pexprExpected) {
  GPOS_UNITTEST_ASSERT(nullptr != pexprActual);
  GPOS_UNITTEST_ASSERT(nullptr != pexprExpected);
...
//---------------------------------------------------------------------------
GPOS_RESULT
CTestUtils::EresUnittest_RunTests(const CHAR **rgszFileNames, ULONG *pulTestCounter, ULONG ulTests) {
  ...
  GPOS_RESULT eres = EresUnittest_RunTestsWithoutAdditionalTraceFlags(rgszFileNames, pulTestCounter, ulTests,
                                                                      fMatchPlans, fTestSpacePruning);
  return eres;
}

GPOS_RESULT
CTestUtils::EresUnittest_RunTestsWithoutAdditionalTraceFlags(const CHAR **rgszFileNames, ULONG *pulTestCounter,
                                                             ULONG ulTests, BOOL fMatchPlans, BOOL fTestSpacePruning) {
  CAutoMemoryPool amp;
  CMemoryPool *mp = amp.Pmp();

  GPOS_RESULT eres = CTestUtils::EresRunMinidumps(mp, rgszFileNames, ulTests, pulTestCounter,
                                                  1,  // ulSessionId
                                                  1,  // ulCmdId
                                                  fMatchPlans, fTestSpacePruning);
...
CColRefSetArray *CTestUtils::createEquivalenceClasses(CMemoryPool *mp, CColRefSet *pcrs, const INT setBoundary[]) {
  INT i = 0;
  ULONG bpIndex = 0;
