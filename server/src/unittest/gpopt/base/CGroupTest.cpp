...
static CExpression *InsertGroupIntoMemo(CMemo *pmemo, CMemoryPool *mp) {
  CExpression *pexprGet = CTestUtils::PexprLogicalGet(mp);
  pexprGet->Pop()->AddRef();
  CGroupExpression *pgexpr = GPOS_NEW(mp)
      CGroupExpression(mp, pexprGet->Pop(), GPOS_NEW(mp) CGroupArray(mp), CXform::ExfInvalid /*exfidOrigin*/,
...
CGroupTest::EresUnittest_FResetStatsOnCGroupWithDuplicateGroup() {
  ...
  // group (1)
  if (pmemo->Pgroup(0)->Pstats() != nullptr || pmemo->Pgroup(1)->Pstats() != nullptr) {
    GPOS_DELETE(pmemo);
    pexprGet1->Release();
    pexprGet2->Release();
    return GPOS_FAILED;
