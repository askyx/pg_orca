...
CDistributionSpecTest::EresUnittest_Random() {
  ...
  GPOS_UNITTEST_ASSERT(pdsRandomDuplicateSensitive->FSatisfies(pdsRandomDuplicateSensitive));
  GPOS_UNITTEST_ASSERT(pdsRandomDuplicateSensitive->Matches(pdsRandomDuplicateSensitive));
  GPOS_UNITTEST_ASSERT(pdsRandomDuplicateSensitive->FSatisfies(pdsRandomNonDuplicateSensitive));
  GPOS_UNITTEST_ASSERT(!pdsRandomNonDuplicateSensitive->FSatisfies(pdsRandomDuplicateSensitive));

  // random and universal
  CDistributionSpecUniversal *pdsUniversal = GPOS_NEW(mp) CDistributionSpecUniversal();
  GPOS_UNITTEST_ASSERT(pdsUniversal->FSatisfies(pdsRandomNonDuplicateSensitive));
  GPOS_UNITTEST_ASSERT(!pdsUniversal->FSatisfies(pdsRandomDuplicateSensitive));
...
CDistributionSpecTest::EresUnittest_Singleton() {
  ...
  CDistributionSpecSingleton *pdssCoordinator =
      GPOS_NEW(mp) CDistributionSpecSingleton(CDistributionSpecSingleton::EstCoordinator);
  ...
  GPOS_UNITTEST_ASSERT(!pdssCoordinator->FSatisfies(pdssSegment) && !pdssSegment->FSatisfies(pdssCoordinator));
...
CDistributionSpecTest::EresUnittest_Hashed() {
  ...
  CColRef *pcrC = col_factory->PcrCreate(pmdtypeint4, default_type_modifier, nameC);
  CColRef *pcrD = col_factory->PcrCreate(pmdtypeint4, default_type_modifier, nameD);
  CColRef *pcrE = col_factory->PcrCreate(pmdtypeint4, default_type_modifier, nameE);
  CColRef *pcrF = col_factory->PcrCreate(pmdtypeint4, default_type_modifier, nameF);
  CColRef *pcrG = col_factory->PcrCreate(pmdtypeint4, default_type_modifier, nameG);
  ...
  poptctxt->MarkDMLQuery(true /*fDMLQuery*/);
  CDistributionSpecHashed *pdshashed4 = GPOS_NEW(mp) CDistributionSpecHashed(prgpexpr3, true /* fNullsCollocated */);

  // pdshashed5: HD{C}, false
  CDistributionSpecHashed *pdshashed5 = GPOS_NEW(mp) CDistributionSpecHashed(prgpexpr4, false /* fNullsCollocated */);

  // pdshashed6: HD{D}, false
  CDistributionSpecHashed *pdshashed6 = GPOS_NEW(mp) CDistributionSpecHashed(prgpexpr5, false /* fNullsCollocated */);
  ...
  GPOS_UNITTEST_ASSERT(true == pdshashed7->PdshashedEquiv()->FNullsColocated());
  ...
  GPOS_UNITTEST_ASSERT(false == pdshashed8->PdshashedEquiv()->FNullsColocated());
  ...
  GPOS_UNITTEST_ASSERT(pdshashed9->PdshashedEquiv()->FMatchHashedDistribution(pdshashed4));
  GPOS_UNITTEST_ASSERT(pdshashed9->PdshashedEquiv()->PdshashedEquiv()->FMatchHashedDistribution(pdshashed2));
  GPOS_UNITTEST_ASSERT(
      pdshashed9->PdshashedEquiv()->PdshashedEquiv()->PdshashedEquiv()->FMatchHashedDistribution(pdshashed5));
  ...
  GPOS_UNITTEST_ASSERT(true == pdshashed10->PdshashedEquiv()->FNullsColocated());
  ...
  GPOS_UNITTEST_ASSERT(true == pdshashed11->PdshashedEquiv()->FNullsColocated());
  ...
  GPOS_UNITTEST_ASSERT(false == pdshashed12->PdshashedEquiv()->FNullsColocated());

  // Test 3+ node Combine
  // This is to ensure Combine doesn't just swap the head and tail node,
  // and it indeed reverses the linked list order
  // pdshashed13: {E}, true
  CDistributionSpecHashed *pdshashed13 = GPOS_NEW(mp) CDistributionSpecHashed(prgpexpr6, true /* fNullsCollocated */);

  // pdshashed14: {F}, true
  CDistributionSpecHashed *pdshashed14 = GPOS_NEW(mp) CDistributionSpecHashed(prgpexpr7, true /* fNullsCollocated */);

  // pdshashed15: {G}, true
  CDistributionSpecHashed *pdshashed15 = GPOS_NEW(mp) CDistributionSpecHashed(prgpexpr8, true /* fNullsCollocated */);

  // Combine {E} true, {F} true
  // pdshashed16: {F} true -> {E} true
  CDistributionSpecHashed *pdshashed16 = pdshashed13->Combine(mp, pdshashed14);

  // Combine {G} true, {F} true -> {E} true
  // pdshashed17: {E} true -> {F} true -> {G} true
  CDistributionSpecHashed *pdshashed17 = pdshashed15->Combine(mp, pdshashed16);
  ...
  GPOS_UNITTEST_ASSERT(pdshashed18->PdshashedEquiv()->FMatchHashedDistribution(pdshashed14));
  GPOS_UNITTEST_ASSERT(pdshashed18->PdshashedEquiv()->PdshashedEquiv()->FMatchHashedDistribution(pdshashed13));
  GPOS_UNITTEST_ASSERT(
      pdshashed18->PdshashedEquiv()->PdshashedEquiv()->PdshashedEquiv()->FMatchHashedDistribution(pdshashed5));
  GPOS_UNITTEST_ASSERT(
      pdshashed18->PdshashedEquiv()->PdshashedEquiv()->PdshashedEquiv()->PdshashedEquiv()->FMatchHashedDistribution(
          pdshashed2));
...
CDistributionSpecTest::EresUnittest_NegativeAny() {
  ...
  pdsany->AppendEnforcers(nullptr /*mp*/, *pexprhdl, nullptr /*prpp*/, nullptr /*pdrgpexpr*/, nullptr /*pexpr*/);
  pdsany->Release();
...
CDistributionSpecTest::EresUnittest_NegativeUniversal() {
  ...
  pdsuniversal->AppendEnforcers(nullptr /*mp*/, *pexprhdl, nullptr /*prpp*/, nullptr /*pdrgpexpr*/, nullptr /*pexpr*/);
  pdsuniversal->Release();
  GPOS_DELETE(pexprhdl);
...
CDistributionSpecTest::EresUnittest_NegativeRandom() {
  ...
  pdsrandom->AppendEnforcers(nullptr /*mp*/, *pexprhdl, nullptr /*prpp*/, nullptr /*pdrgpexpr*/, nullptr /*pexpr*/);
  pdsrandom->Release();
  GPOS_DELETE(pexprhdl);
