...
CMDAccessorTest::EresUnittest() {
  CUnittest rgut[] = {
      ...
      GPOS_UNITTEST_FUNC_THROW(CMDAccessorTest::EresUnittest_Negative, gpdxl::ExmaMD, gpdxl::ExmiMDCacheEntryNotFound),
...
CMDAccessorTest::EresUnittest_Basic() {
  ...
  CMDIdGPDB *pmdidObject1 = GPOS_NEW(mp)
      CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID /* OID */, 1 /* major version */, 1 /* minor version */);
  CMDIdGPDB *pmdidObject2 = GPOS_NEW(mp)
      CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID /* OID */, 12 /* version */, 1 /* minor version */);

  const IMDRelation *pimdrel1 = mda.RetrieveRel(pmdidObject1);

  const IMDRelation *pimdrel2 = mda.RetrieveRel(pmdidObject2);

  GPOS_UNITTEST_ASSERT(pimdrel1->MDId()->Equals(pmdidObject1) && pimdrel2->MDId()->Equals(pmdidObject2));
  ...
  CMDIdGPDB *mdid_type = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_INT4, 1, 0);
  CMDIdGPDB *mdid_op = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_OP_INT4_LT, 1, 0);
  CMDIdGPDB *agg_mdid = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, GPDB_AGG_AVG, 1, 0);
  ...
  const IMDTypeInt4 *pmdtypeint4 GPOS_ASSERTS_ONLY = mda.PtMDType<IMDTypeInt4>(CTestUtils::m_sysidDefault);

  const IMDTypeBool *pmdtypebool GPOS_ASSERTS_ONLY = mda.PtMDType<IMDTypeBool>(CTestUtils::m_sysidDefault);
...
CMDAccessorTest::EresUnittest_Navigate() {
  ...
  CMDIdGPDB *mdid_func = GPOS_NEW(mp)
      CMDIdGPDB(IMDId::EmdidGeneral, GPDB_FUNC_TIMEOFDAY /* OID */, 1 /* major version */, 0 /* minor version */);
  ...
  const IMDScalarOp *md_scalar_op GPOS_ASSERTS_ONLY = mda.RetrieveScOp(pmdidEqOp);
...
CMDAccessorTest::EresUnittest_Indexes() {
  ...
  CMDIdGPDB *rel_mdid =
      GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID, 1 /* major */, 1 /* minor version */);
...
CMDAccessorTest::EresUnittest_CheckConstraint() {
  ...
  // lookup a relation in the MD cache
  CMDIdGPDB *rel_mdid =
      GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_TEST_REL_OID21, 1 /* major */, 1 /* minor version */);
...
CMDAccessorTest::EresUnittest_Cast() {
  ...
  const IMDCast *pmdcastInt2BigInt = mda.Pmdcast(pmdtypeInt->MDId(), pmdtypeBigInt->MDId());

  GPOS_UNITTEST_ASSERT(!pmdcastInt2BigInt->IsBinaryCoercible());
  GPOS_UNITTEST_ASSERT(pmdcastInt2BigInt->GetCastFuncMdId()->IsValid());
  GPOS_UNITTEST_ASSERT(pmdcastInt2BigInt->MdidSrc()->Equals(pmdtypeInt->MDId()));
  GPOS_UNITTEST_ASSERT(pmdcastInt2BigInt->MdidDest()->Equals(pmdtypeBigInt->MDId()));

  const IMDCast *pmdcastInt2Oid = mda.Pmdcast(pmdtypeInt->MDId(), pmdtypeOid->MDId());

  GPOS_UNITTEST_ASSERT(pmdcastInt2Oid->IsBinaryCoercible());
  GPOS_UNITTEST_ASSERT(!pmdcastInt2Oid->GetCastFuncMdId()->IsValid());

  const IMDCast *pmdcastOid2Int = mda.Pmdcast(pmdtypeOid->MDId(), pmdtypeInt->MDId());
...
CMDAccessorTest::EresUnittest_ScCmp() {
  ...
  const IMDScCmp *pmdScEqIntBigInt = mda.Pmdsccmp(pmdtypeInt->MDId(), pmdtypeBigInt->MDId(), IMDType::EcmptEq);

  GPOS_UNITTEST_ASSERT(IMDType::EcmptEq == pmdScEqIntBigInt->ParseCmpType());
  GPOS_UNITTEST_ASSERT(pmdScEqIntBigInt->GetLeftMdid()->Equals(pmdtypeInt->MDId()));
  GPOS_UNITTEST_ASSERT(pmdScEqIntBigInt->GetRightMdid()->Equals(pmdtypeBigInt->MDId()));
...
CMDAccessorTest::EresUnittest_Negative() {
  ...
  CMDIdGPDB *pmdidNonExistingObject = GPOS_NEW(mp)
      CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID /* OID */, 15 /* version */, 1 /* minor version */);
...
//
//---------------------------------------------------------------------------
void *CMDAccessorTest::PvLookupSingleObj(void *pv) {
  GPOS_CHECK_ABORT;
...
//
//---------------------------------------------------------------------------
void *CMDAccessorTest::PvLookupMultipleObj(void *pv) {
  GPOS_CHECK_ABORT;
  ...
  for (ULONG ul = 0; ul < ulNumberOfObjects; ul++) {
    ...
    CMDIdGPDB *mdid = GPOS_NEW(pmdtaskparams->m_mp)
        CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID /*OID*/, 1 /*major*/, ul + 1 /*minor*/);
    (void)md_accessor->RetrieveRel(mdid);
    mdid->Release();
  }
...
//
//---------------------------------------------------------------------------
void *CMDAccessorTest::PvInitMDAAndLookup(void *pv) {
  GPOS_UNITTEST_ASSERT(nullptr != pv);
  ...
  // scope for MD accessor
  {
    ...
    // scope for ATP
    {
      ...
      TaskFuncPtr rgPfuncTask[] = {CMDAccessorTest::PvLookupSingleObj, CMDAccessorTest::PvLookupMultipleObj};

      const ULONG ulNumberOfTaskTypes = GPOS_ARRAY_SIZE(rgPfuncTask);

      // create tasks
      for (ULONG i = 0; i < GPOS_ARRAY_SIZE(rgPtsk); i++) {
        rgPtsk[i] = atp.Create(rgPfuncTask[i % ulNumberOfTaskTypes], &mdtaskparams);
