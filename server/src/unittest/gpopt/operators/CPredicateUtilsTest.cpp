...
CPredicateUtilsTest::EresUnittest_Conjunctions() {
  ...
  CExpression *pexprSingleton = CPredicateUtils::PexprConjunction(mp, nullptr /*pdrgpexpr*/);
...
CPredicateUtilsTest::EresUnittest_Disjunctions() {
  ...
  CExpression *pexprSingleton = CPredicateUtils::PexprDisjunction(mp, nullptr /*pdrgpexpr*/);
  GPOS_UNITTEST_ASSERT(nullptr != pexprSingleton);
  pexprSingleton->Release();
...
CPredicateUtilsTest::EresUnittest_PlainEqualities() {
  ...
  CExpression *pexprScIdentInequality =
      CUtils::PexprScalarCmp(mp, pcrLeft, pcrRight, CWStringConst(GPOS_WSZ_LIT("<")),
...
CPredicateUtilsTest::EresUnittest_Implication() {
  ...
  CMDIdGPDB *pmdid3 = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_TEST_REL_OID3, 1, 1);
  ...
  GPOS_UNITTEST_ASSERT(pdrgpexprNewConjuncts->Size() < pdrgpexprOriginalConjuncts->Size());
