...
//
//---------------------------------------------------------------------------
CReqdPropPlan *CExpressionTest::PrppCreateRequiredProperties(CMemoryPool *mp, CColRefSet *pcrs) {
...
//
//---------------------------------------------------------------------------
CExpression *CExpressionTest::PexprCreateGbyWithColumnFormat(CMemoryPool *mp, const WCHAR *wszColNameFormat) {
  CWStringConst strRelName(GPOS_WSZ_LIT("Rel1"));
  CMDIdGPDB *rel_mdid = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_TEST_REL_OID1, 1, 1);
  CTableDescriptor *ptabdesc =
      CTestUtils::PtabdescPlainWithColNameFormat(mp, 3, rel_mdid, wszColNameFormat, CName(&strRelName), false);
...
CExpressionTest::EresUnittest_BitmapGet() {
  ...
  CMDIdGPDB *rel_mdid = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_MDCACHE_TEST_OID, 1, 1);
  ...
  CExpression *pexprIndexCond = CUtils::PexprScalarEqCmp(mp, pcrFirst, CUtils::PexprScalarConstInt4(mp, 20 /*val*/));
  ...
  CExpression *pexprBitmapIndex = GPOS_NEW(mp)
      CExpression(mp, GPOS_NEW(mp) CScalarBitmapIndexProbe(mp, pindexdesc, ptabdesc, mdid), pexprIndexCond);
  ...
  CExpression *pexprTableCond = CUtils::PexprScalarEqCmp(mp, pcrFirst, CUtils::PexprScalarConstInt4(mp, 20 /*val*/));
  ...
  CWStringConst strExpectedDebugPrint(
      GPOS_WSZ_LIT("+--CLogicalBitmapTableGet , Table Name: (\"MyTable\"), Columns: [\"ColRef_0001\" (1), "
                   "\"ColRef_0002\" (2), \"ColRef_0003\" (3)]\n"
                   "   |--CScalarCmp (=)\n"
  ...
  CExpression *pexprIndexCond2 = CUtils::PexprScalarEqCmp(mp, pcrFirst, CUtils::PexprScalarConstInt4(mp, 20 /*val*/));
  ptabdesc->AddRef();
  CExpression *pexprBitmapIndex2 = GPOS_NEW(mp)
      CExpression(mp, GPOS_NEW(mp) CScalarBitmapIndexProbe(mp, pindexdesc2, ptabdesc, pmdid2), pexprIndexCond2);
  ...
  CExpression *pexprBitmapBoolOp1 =
      GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarBitmapBoolOp(mp, CScalarBitmapBoolOp::EbitmapboolAnd, mdid),
...
CExpressionTest::EresUnittest_Const() {
  ...
  GPOS_UNITTEST_ASSERT(pscalarconstUl->HashValue() == pscalarconstUl2nd->HashValue());
...
CExpressionTest::EresUnittest_ComparisonTypes() {
  ...
  GPOS_UNITTEST_ASSERT(IMDType::EcmptEq == CUtils::ParseCmpType(pmdtype->GetMdidForCmpType(IMDType::EcmptEq)));
  GPOS_UNITTEST_ASSERT(IMDType::EcmptL == CUtils::ParseCmpType(pmdtype->GetMdidForCmpType(IMDType::EcmptL)));
  GPOS_UNITTEST_ASSERT(IMDType::EcmptG == CUtils::ParseCmpType(pmdtype->GetMdidForCmpType(IMDType::EcmptG)));
  ...
  GPOS_UNITTEST_ASSERT(IMDType::EcmptNEq == CUtils::ParseCmpType(pmdscopEq->GetInverseOpMdid()));
  GPOS_UNITTEST_ASSERT(IMDType::EcmptLEq == CUtils::ParseCmpType(pmdscopGT->GetInverseOpMdid()));
  GPOS_UNITTEST_ASSERT(IMDType::EcmptGEq == CUtils::ParseCmpType(pmdscopLT->GetInverseOpMdid()));
...
//
//---------------------------------------------------------------------------
void CExpressionTest::SetupPlanForFValidPlanTest(CMemoryPool *mp, CExpression **ppexprGby, CColRefSet **ppcrs,
                                                 CExpression **ppexprPlan, CReqdPropPlan **pprpp) {
...
CExpressionTest::EresUnittest_FValidPlan_InvalidOrder() {
  ...
  CCTEReq *pcter = GPOS_NEW(mp) CCTEReq(mp);
  CReqdPropPlan *prppIncompatibleOrder = GPOS_NEW(mp) CReqdPropPlan(pcrsGetCopy, peo, ped, per, pepp, pcter);

  // Test that the plan is not valid.
  GPOS_UNITTEST_ASSERT(!pexprPlan->FValidPlan(prppIncompatibleOrder, pdpctxtplan));
  pdpctxtplan->Release();
...
CExpressionTest::EresUnittest_FValidPlan_InvalidDistribution() {
  ...
  CReqdPropPlan *prppIncompatibleDistribution = GPOS_NEW(mp) CReqdPropPlan(pcrsCopy, peo, ped, per, pepp, pcter);

  // Test that the plan is not valid.
  GPOS_UNITTEST_ASSERT(!pexprPlan->FValidPlan(prppIncompatibleDistribution, pdpctxtplan));
  pdpctxtplan->Release();
  pexprPlan->Release();
...
CExpressionTest::EresUnittest_FValidPlan_InvalidRewindability() {
  ...
  CDistributionSpec *pds = GPOS_NEW(mp) CDistributionSpecSingleton(CDistributionSpecSingleton::EstCoordinator);
  CRewindabilitySpec *prs =
      GPOS_NEW(mp) CRewindabilitySpec(CRewindabilitySpec::ErtRewindable, CRewindabilitySpec::EmhtNoMotion);
  CEnfdOrder *peo = GPOS_NEW(mp) CEnfdOrder(pos, CEnfdOrder::EomSatisfy);
  CEnfdDistribution *ped = GPOS_NEW(mp) CEnfdDistribution(pds, CEnfdDistribution::EdmExact);
  CEnfdRewindability *per = GPOS_NEW(mp) CEnfdRewindability(prs, CEnfdRewindability::ErmSatisfy);
  CPartitionPropagationSpec *pps = GPOS_NEW(mp) CPartitionPropagationSpec(mp);
  ...
  CReqdPropPlan *prppIncompatibleRewindability = GPOS_NEW(mp) CReqdPropPlan(pcrsCopy, peo, ped, per, pepp, pcter);
  // Test that the plan is not valid.
  GPOS_UNITTEST_ASSERT(!pexprPlan->FValidPlan(prppIncompatibleRewindability, pdpctxtplan));
  pdpctxtplan->Release();
  pexprPlan->Release();
...
CExpressionTest::EresUnittest_FValidPlan_InvalidCTEs() {
  ...
  CColRefSet *pcrsCopy = GPOS_NEW(mp) CColRefSet(mp, *pcrs);
  CReqdPropPlan *prppIncompatibleCTE = GPOS_NEW(mp) CReqdPropPlan(pcrsCopy, peo, ped, per, pepp, pcter);

  // Test that the plan is not valid.
  GPOS_UNITTEST_ASSERT(!pexprPlan->FValidPlan(prppIncompatibleCTE, pdpctxtplan));
  pdpctxtplan->Release();
  pexprPlan->Release();
...
CExpressionTest::EresUnittest_FValidPlanError() {
  ...
  // Test that in debug mode GPOS_UNITTEST_ASSERT fails for non-physical expressions.
  {
    ...
    CReqdPropPlan *prpp = PrppCreateRequiredProperties(mp, pcrsInvalid);
    IDatum *datum = GPOS_NEW(mp) gpnaucrates::CDatumInt8GPDB(CTestUtils::m_sysidDefault, 1 /*val*/, false /*is_null*/);
...
//---------------------------------------------------------------------------
GPOS_RESULT
CExpressionTest::EresCheckCachedReqdCols(CMemoryPool *mp, CExpression *pexpr, CReqdPropPlan *prppInput) {
...
CExpressionTest::EresUnittest_InvalidSetOp() {
...

  {
    ...
    CMDIdGPDB *pmdid1 = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_TEST_REL_OID1, 1, 1);
    ...
    CMDIdGPDB *pmdid2 = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidRel, GPOPT_TEST_REL_OID2, 1, 1);
...
//
//---------------------------------------------------------------------------
CExpression *CExpressionTest::PexprComplexJoinTree(CMemoryPool *mp) {
  ...
  CExpression *pexprInnerJoin1 = CTestUtils::PexprLogicalJoin<CLogicalInnerJoin>(mp, CTestUtils::PexprLogicalGet(mp),
  ...
  CExpression *pexprLeftJoin = CTestUtils::PexprLogicalJoin<CLogicalLeftOuterJoin>(mp, pexprInnerJoin1, pexprJoinIndex);

  CExpression *pexprOuterJoin = CTestUtils::PexprLogicalJoin<CLogicalLeftOuterJoin>(mp);
  CExpression *pexprInnerJoin4 = CTestUtils::PexprLogicalJoin<CLogicalInnerJoin>(mp, pexprLeftJoin, pexprOuterJoin);

  CExpression *pexprInnerJoin5 = CTestUtils::PexprLogicalJoin<CLogicalInnerJoin>(mp, CTestUtils::PexprLogicalGet(mp),
                                                                                 CTestUtils::PexprLogicalSelect(mp));
  CExpression *pexprTopJoin = CTestUtils::PexprLogicalJoin<CLogicalInnerJoin>(mp, pexprInnerJoin4, pexprInnerJoin5);
