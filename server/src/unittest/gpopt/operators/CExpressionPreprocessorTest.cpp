...
//
//---------------------------------------------------------------------------
BOOL CExpressionPreprocessorTest::FHasSubqueryAll(CExpression *pexpr) {
...
//
//---------------------------------------------------------------------------
BOOL CExpressionPreprocessorTest::FHasSubqueryAny(CExpression *pexpr) {
  GPOS_UNITTEST_ASSERT(nullptr != pexpr);
...
//
//---------------------------------------------------------------------------
BOOL CExpressionPreprocessorTest::FHasSubqueryExists(CExpression *pexpr) {
  GPOS_UNITTEST_ASSERT(nullptr != pexpr);
...
//
//---------------------------------------------------------------------------
BOOL CExpressionPreprocessorTest::FHasSubqueryNotExists(CExpression *pexpr) {
  GPOS_UNITTEST_ASSERT(nullptr != pexpr);
...
//
//---------------------------------------------------------------------------
BOOL CExpressionPreprocessorTest::FHasNoOuterJoin(CExpression *pexpr) {
  COperator::EOperatorId rgeopid[] = {
...
//
//---------------------------------------------------------------------------
BOOL CExpressionPreprocessorTest::HasOuterRefs(CExpression *pexpr) {
...
//
//---------------------------------------------------------------------------
BOOL CExpressionPreprocessorTest::FHasSeqPrj(CExpression *pexpr) {
  COperator::EOperatorId rgeopid[] = {
...
//
//---------------------------------------------------------------------------
BOOL CExpressionPreprocessorTest::FHasIDF(CExpression *pexpr) {
  COperator::EOperatorId rgeopid[] = {
...
CExpressionPreprocessorTest::EresUnittest_PreProcessWindowFunc() {
  ...
  GPOS_UNITTEST_ASSERT(FHasNoOuterJoin(pexprPreprocessed) && "unexpected outer join");
...
//
//---------------------------------------------------------------------------
CExpression *CExpressionPreprocessorTest::PexprJoinHelper(CMemoryPool *mp, CExpression *pexprLOJ, BOOL fCascadedLOJ,
  ...
  if (fIntermediateInnerjoin) {
    ...
    CColRef *pcrLeft = (*pexprBottomJoin)[0]->DeriveOutputColumns()->PcrAny();
  ...
  if (fCascadedLOJ) {
    ...
    pexprResult =
        GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalLeftOuterJoin(mp), pexprBottomJoin, pexprGet, pexprEquality);
  }
...
//
//---------------------------------------------------------------------------
CExpression *CExpressionPreprocessorTest::PexprWindowFuncWithLOJHelper(CMemoryPool *mp, CExpression *pexprLOJ,
...
//
//---------------------------------------------------------------------------
void CExpressionPreprocessorTest::PreprocessOuterJoin(const CHAR *szFilePath, BOOL
#ifdef GPOS_DEBUG
                                                                                  fAllowOuterJoin
#endif  // GPOS_DEBUG
) {
...
#ifdef GPOS_DEBUG
    if (fAllowOuterJoin) {
      GPOS_UNITTEST_ASSERT(!FHasNoOuterJoin(pexprPreprocessed) && "expected outer join");
    } else {
      GPOS_UNITTEST_ASSERT(FHasNoOuterJoin(pexprPreprocessed) && "unexpected outer join");
    }
#endif  // GPOS_DEBUG
    ...
    for (ULONG ulLOJCases = 0; ulLOJCases < 2; ulLOJCases++) {
      ...
      for (ULONG ulPredCases = 0; ulPredCases < 2; ulPredCases++) {
        ...
        for (ULONG ulPredBelowWindowCases = 0; ulPredBelowWindowCases < 2; ulPredBelowWindowCases++) {
          ...
          if (fOuterChildPred) {
            colref = (*pexprLOJ)[0]->DeriveOutputColumns()->PcrAny();
          } else {
            colref = (*pexprLOJ)[1]->DeriveOutputColumns()->PcrAny();
          }

          pexprLOJ = PexprJoinHelper(mp, pexprLOJ, fCascadedLOJ, fIntermediateInnerjoin);

          CExpression *pexprSelect = PexprWindowFuncWithLOJHelper(mp, pexprLOJ, colref, fAddWindowFunction,
...

          {
            CAutoTrace at(mp);
            at.Os() << std::endl
                    << "Test Number: " << ulTestCases << ", WindowFunction: " << fAddWindowFunction
                    << ", IntermediateInnerjoin: " << fIntermediateInnerjoin << ", CascadedLOJ: " << fCascadedLOJ
                    << ", OuterChildPred: " << fOuterChildPred << ", PredBelowWindow: " << fPredBelowWindow;
...
//
//---------------------------------------------------------------------------
void CExpressionPreprocessorTest::PreprocessWinFuncWithOuterRefs(const CHAR *szFilePath, BOOL
#ifdef GPOS_DEBUG
...

  {
    ...
    // read query expression
    CExpression *pexpr = CTestUtils::PexprReadQuery(mp, szFilePath);
    GPOS_UNITTEST_ASSERT(HasOuterRefs(pexpr) && "expected outer references");
    ...
    if (fAllowWinFuncOuterRefs) {
      GPOS_UNITTEST_ASSERT(HasOuterRefs(pexprPreprocessed) && "expected outer references");
    } else {
      GPOS_UNITTEST_ASSERT(!HasOuterRefs(pexprPreprocessed) && "unexpected outer references");
    }
#endif  // GPOS_DEBUG
...
//
//---------------------------------------------------------------------------
void CExpressionPreprocessorTest::PreprocessWinFuncWithDistinctAggs(CMemoryPool *mp, const CHAR *szFilePath,
                                                                    BOOL
#ifdef GPOS_DEBUG
                                                                        fAllowSeqPrj
#endif  // GPOS_DEBUG
...
#ifdef GPOS_DEBUG
  if (fAllowSeqPrj) {
    GPOS_UNITTEST_ASSERT(FHasSeqPrj(pexprPreprocessed) && "expected sequence project");
  } else {
    GPOS_UNITTEST_ASSERT(!FHasSeqPrj(pexprPreprocessed) && "unexpected sequence project");
  }

  if (fAllowIDF) {
    GPOS_UNITTEST_ASSERT(FHasIDF(pexprPreprocessed) && "expected (is distinct from)");
  } else {
    GPOS_UNITTEST_ASSERT(!FHasIDF(pexprPreprocessed) && "unexpected (is distinct from)");
  }
...
CExpressionPreprocessorTest::EresUnittest_UnnestSubqueries() {
  ...
  SUnnestSubqueriesTestCase rgunnesttc[] = {
      ...
      {CTestUtils::PexprScalarCmpIdentToConstant, CTestUtils::PexprScalarCmpIdentToConstant, CScalarBoolOp::EboolopAnd,
       COperator::EopSentinel, false},
  ...
  for (ULONG ul = 0; ul < ulTestCases && (GPOS_OK == eres); ul++) {
    ...
    if (nullptr == pexprFirst || (eboolop == CScalarBoolOp::PopConvert(pexprFirst->Pop())->Eboolop())) {
      eres = GPOS_FAILED;
    }
...
CExpressionPreprocessorTest::EresUnittest_PreProcessNestedScalarSubqueries() {
  ...
  GPOS_UNITTEST_ASSERT(1 == UlScalarSubqs(pexprPreprocessed) &&
...
CExpressionPreprocessorTest::EresUnittest_PreProcessOuterJoin() {
  ...
  GPOS_UNITTEST_ASSERT(FHasNoOuterJoin(pexprPreprocessed1) && "unexpected outer join");
  ...
  GPOS_UNITTEST_ASSERT(FHasNoOuterJoin(pexprPreprocessed2) && "unexpected outer join");
  ...
  GPOS_UNITTEST_ASSERT(FHasNoOuterJoin(pexprPreprocessed3) && "unexpected outer join");
  ...
  GPOS_UNITTEST_ASSERT(FHasNoOuterJoin(pexprPreprocessed4) && "unexpected outer join");
  ...
  GPOS_UNITTEST_ASSERT(!FHasNoOuterJoin(pexprPreprocessed5) && "expected outer join");
  ...
  GPOS_UNITTEST_ASSERT(FHasNoOuterJoin(pexprPreprocessed6) && "unexpected outer join");
...
//
//---------------------------------------------------------------------------
CExpression *CExpressionPreprocessorTest::PexprCreateConjunction(CMemoryPool *mp, CColRefArray *colref_array) {
...
CExpressionPreprocessorTest::EresUnittest_PreProcessOrPrefiltersPartialPush() {
  ...
  CWStringConst strExpectedDebugPrintForSelect(GPOS_WSZ_LIT(
      ...
      "+--CLogicalSelect\n"
      "   |--CLogicalInnerJoin\n"
      "   |  |--CLogicalGet \"BaseTableAlias\" (\"BaseTable\"), Columns: [\"column_0000\" (3), \"column_0001\" (4), "
      "\"column_0002\" (5)] Key sets: {[0]}\n"
      "   |  |--CLogicalGet \"BaseTableAlias\" (\"BaseTable\"), Columns: [\"column_0000\" (0), \"column_0001\" (1), "
      "\"column_0002\" (2)] Key sets: {[0]}\n"
      "   |  +--CScalarCmp (=)\n"
      "   |     |--CScalarIdent \"column_0000\" (3)\n"
      "   |     +--CScalarIdent \"column_0000\" (0)\n"
      "   +--CScalarBoolOp (EboolopOr)\n"
...
CExpressionPreprocessorTest::EresUnittest_CollapseInnerJoinHelper(CMemoryPool *mp, COperator *popJoin,
  ...
  CTestUtils::EqualityPredicate(mp, rgpdprel[0]->GetOutputColumns(), rgpdprel[1]->GetOutputColumns(), pdrgpexpr);
  ...
  CTestUtils::EqualityPredicate(mp, rgpdprel[0]->GetOutputColumns(), rgpdprel[2]->GetOutputColumns(), pdrgpexpr);
  ...
  CTestUtils::EqualityPredicate(mp, rgpdprel[0]->GetOutputColumns(), rgpdprel[3]->GetOutputColumns(), pdrgpexpr);
  popJoin->AddRef();
  ...
  CTestUtils::EqualityPredicate(mp, rgpdprel[0]->GetOutputColumns(), rgpdprel[4]->GetOutputColumns(), pdrgpexpr);
  popJoin->AddRef();
  ...
  CTestUtils::EqualityPredicate(mp, rgpdprel[0]->GetOutputColumns(), rgpdprel[5]->GetOutputColumns(), pdrgpexpr);
  popJoin->AddRef();
  ...
  GPOS_UNITTEST_ASSERT(COperator::EopLogicalNAryJoin == pexprPreprocessed->Pop()->Eopid() &&
  ...
  for (ULONG ul = 0; ul < arity - 1; ul++) {
    ...
    GPOS_UNITTEST_ASSERT(1 == CDrvdPropRelational::GetRelationalProperties(pexprChild->PdpDerive())->GetJoinDepth() &&
                         "unexpected deep join tree below NAryJoin");

    COperator::EOperatorId op_id = pexprChild->Pop()->Eopid();
    GPOS_UNITTEST_ASSERT((COperator::EopLogicalGet == op_id || COperator::EopLogicalSelect == op_id) &&
                         "child operator is expected to be either Get or Select");
    GPOS_UNITTEST_ASSERT_IMP(COperator::EopLogicalSelect == op_id,
...
CExpressionPreprocessorTest::EresUnittest_CollapseInnerJoin() {
  ...
  //	   |  |  |--CLogicalNAryJoin
  //	   |  |  |  |--CLogicalLeftOuterJoin
  //	   |  |  |  |  |--CLogicalGet "Rel3" ("Rel3"), Columns: ["column_0000" (6), "column_0001" (7), "column_0002"
  //(8)] Key sets: {[0]} 	   |  |  |  |  |--CLogicalLeftOuterJoin 	   |  |  |  |  |  |--CLogicalGet "Rel1" ("Rel1"), Columns:
  //["column_0000" (0), "column_0001" (1), "column_0002" (2)] Key sets: {[0]} 	   |  |  |  |  |  |--CLogicalGet "Rel2"
  //("Rel2"), Columns: ["column_0000" (3), "column_0001" (4), "column_0002" (5)] Key sets: {[0]} 	   |  |  |  |  |
  //+--CScalarCmp (=) 	   |  |  |  |  |     |--CScalarIdent "column_0000" (0) 	   |  |  |  |  |     +--CScalarIdent
  //"column_0000" (3) 	   |  |  |  |  +--CScalarCmp (=) 	   |  |  |  |     |--CScalarIdent "column_0000" (0) 	   |  |  |  |
  //+--CScalarIdent "column_0000" (6) 	   |  |  |  |--CLogicalGet "Rel4" ("Rel4"), Columns: ["column_0000" (9),
  //"column_0001" (10), "column_0002" (11)] Key sets: {[0]} 	   |  |  |  +--CScalarCmp (=) 	   |  |  |     |--CScalarIdent
  //"column_0000" (0) 	   |  |  |     +--CScalarIdent "column_0000" (9) 	   |  |  |--CLogicalGet "Rel5" ("Rel5"), Columns:
  //["column_0000" (12), "column_0001" (13), "column_0002" (14)] Key sets: {[0]} 	   |  |  +--CScalarCmp (=) 	   |  |
  //|--CScalarIdent "column_0000" (0) 	   |  |     +--CScalarIdent "column_0000" (12) 	   |  |--CLogicalGet "Rel6" ("Rel6"),
  //Columns: ["column_0000" (15), "column_0001" (16), "column_0002" (17)] Key sets: {[0]} 	   |  +--CScalarCmp (=) 	   |
  //|--CScalarIdent "column_0000" (0) 	   |     +--CScalarIdent "column_0000" (15)
  //	   +--CScalarCmp (=)
  //	      |--CScalarIdent "column_0000" (3)
  //	      +--CScalarConst (1)
...
CExpressionPreprocessorTest::EresUnittest_PreProcessConvert2InPredicate() {
  ...
  GPOS_UNITTEST_ASSERT(3 == CUtils::UlCountOperator(apexprGetWithChildren.Value(), COperator::EopScalarCmp));

  CAutoRef<CExpression> apexprConvert(CExpressionPreprocessor::PexprConvert2In(mp, apexprGetWithChildren.Value()));

  GPOS_UNITTEST_ASSERT(1 == CUtils::UlCountOperator(apexprConvert.Value(), COperator::EopScalarArrayCmp));
  GPOS_UNITTEST_ASSERT(1 == CUtils::UlCountOperator(apexprConvert.Value(), COperator::EopScalarCmp));
  // the OR node should not be removed because there should be an array expression and
  // a scalar identity comparison
  GPOS_UNITTEST_ASSERT(1 == CUtils::UlCountOperator(apexprConvert.Value(), COperator::EopScalarBoolOp));
...
//
//---------------------------------------------------------------------------
CExpression *CExpressionPreprocessorTest::PexprCreateConvertableArray(CMemoryPool *mp, BOOL fCreateInStatement) {
  ...
  GPOS_UNITTEST_ASSERT(2 == CUtils::UlCountOperator(pexpr, COperator::EopScalarCmp));
  return pexpr;
}
...
CExpressionPreprocessorTest::EresUnittest_PreProcessConvertArrayWithEquals() {
  ...
  GPOS_UNITTEST_ASSERT(0 == CUtils::UlCountOperator(apexprInConverted.Value(), COperator::EopScalarCmp));
  GPOS_UNITTEST_ASSERT(7 == CUtils::UlCountOperator(apexprInConverted.Value(), COperator::EopScalarConst));
  GPOS_UNITTEST_ASSERT(1 == CUtils::UlCountOperator(apexprInConverted.Value(), COperator::EopScalarArrayCmp));
  ...
  GPOS_UNITTEST_ASSERT(CScalarArrayCmp::EarrcmpAny == popCmpInArray->Earrcmpt());
  ...
  GPOS_UNITTEST_ASSERT(0 == CUtils::UlCountOperator(apexprNotInConverted.Value(), COperator::EopScalarCmp));
  GPOS_UNITTEST_ASSERT(7 == CUtils::UlCountOperator(apexprNotInConverted.Value(), COperator::EopScalarConst));
  GPOS_UNITTEST_ASSERT(1 == CUtils::UlCountOperator(apexprNotInConverted.Value(), COperator::EopScalarArrayCmp));
  ...
  GPOS_UNITTEST_ASSERT(CScalarArrayCmp::EarrcmpAll == popCmpNotInArray->Earrcmpt());
...
CExpressionPreprocessorTest::EresUnittest_PreProcessConvert2InPredicateDeepExpressionTree() {
  ...
  CAutoRef<CColRefArray> apdrgpcr(apexprGet->DeriveOutputColumns()->Pdrgpcr(mp));
  ...
  GPOS_UNITTEST_ASSERT(5 == CUtils::UlCountOperator(apexprGetWithChildren.Value(), COperator::EopScalarCmp));

  CAutoRef<CExpression> apexprConvert(CExpressionPreprocessor::PexprConvert2In(mp, apexprGetWithChildren.Value()));

  GPOS_UNITTEST_ASSERT(2 == CUtils::UlCountOperator(apexprConvert.Value(), COperator::EopScalarArrayCmp));
  GPOS_UNITTEST_ASSERT(1 == CUtils::UlCountOperator(apexprConvert.Value(), COperator::EopScalarCmp));
