...
CXformTest::EresUnittest_ApplyXforms_CTE() {
  ...
  pexprSelect->AddRef();
  CExpression *pexprAnchor = GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalCTEAnchor(mp, ulCTEId), pexprSelect);
...
//
//---------------------------------------------------------------------------
void CXformTest::ApplyExprXforms(CMemoryPool *mp, IOstream &os, CExpression *pexpr) {
    ...
    if (CXformFactory::Pxff()->IsXformIdUsed((CXform::EXformId)ulXformId)) {
      ...
      if (pxform->FCheckPattern(pexpr) && CXform::FPromising(mp, pxform, pexpr)) {
        if (CXform::ExfExpandNAryJoinMinCard == pxform->Exfid()) {
          GPOS_UNITTEST_ASSERT(COperator::EopLogicalNAryJoin == pexpr->Pop()->Eopid());
...
//
//---------------------------------------------------------------------------
CExpression *CXformTest::PexprStarJoinTree(CMemoryPool *mp, ULONG ulTabs) {
...
//
//---------------------------------------------------------------------------
CExpression *CXformTest::PexprJoinTree(CMemoryPool *mp) {
