...
CDecorrelatorTest::EresUnittest_Decorrelate() {
  ...
  Pfpexpr rgpf[] = {CTestUtils::PexprLogicalGbAggCorrelated, CTestUtils::PexprLogicalSelectCorrelated,
                    CTestUtils::PexprLogicalJoinCorrelated, CTestUtils::PexprLogicalProjectGbAggCorrelated};
  ...
  for (ULONG ulCase = 0; ulCase < GPOS_ARRAY_SIZE(rgpf); ulCase++) {
    ...
    BOOL fSuccess = CDecorrelator::FProcess(mp, pexpr, false /*fEqualityOnly*/, &pexprResult, pdrgpexpr, outerRefs);
