...
const CHAR *rgszPreferHashJoinVersusIndexJoin[] = {"../data/dxl/indexjoin/positive_04.mdp"};
...
GPOS_RESULT
CICGTest::EresUnittest() {
  CUnittest rgut[] = {// keep test for testing partially supported operators/xforms
...
CICGTest::EresUnittest_RunTestsWithoutAdditionalTraceFlags() {
  const CHAR *rgszFileNames[] = {"../data/dxl/minidump/Tpcds-NonPart-Q70a.mdp"};
  return CTestUtils::EresUnittest_RunTestsWithoutAdditionalTraceFlags(
      rgszFileNames, &m_ulTestCounterNoAdditionTraceFlag, GPOS_ARRAY_SIZE(rgszFileNames), true, true);
}
...
CICGTest::EresUnittest_RunUnsupportedMinidumpTests() {
  ...
  CAutoTraceFlag atf2(
      static_cast<size_t>(EopttraceDisableXformBase) + static_cast<size_t>(CXform::ExfDynamicGet2DynamicTableScan),
      true);
  ...
  for (ULONG ul = m_ulUnsupportedTestCounter; ul < ulTests; ul++) {
    ...
    GPOS_TRY {
      ...
      CDXLNode *pdxlnPlan = CMinidumperUtils::PdxlnExecuteMinidump(
          mp, filename, optimizer_config->GetCostModel()->UlHosts() /*ulSegments*/, 1 /*ulSessionId*/, 1, /*ulCmdId*/
    ...
    if (GPOS_FAILED == eres && unmatchedException) {
      ...
      at.Os() << " Expected result: " << unSupportedTestCases[ul].major << "." << unSupportedTestCases[ul].minor
              << std::endl;
...
CICGTest::EresUnittest_NegativeIndexApplyTests() {
  ...
  CAutoTraceFlag atfDTS(
      static_cast<size_t>(EopttraceDisableXformBase) + static_cast<size_t>(CXform::ExfDynamicGet2DynamicTableScan),
      true);
  CAutoTraceFlag atfTS(static_cast<size_t>(EopttraceDisableXformBase) + static_cast<size_t>(CXform::ExfGet2TableScan),
                       true);
  ...
  for (ULONG ul = m_ulNegativeIndexApplyTestCounter; ul < ulTests; ul++) {
    GPOS_TRY {
      ...
      COptimizerConfig *optimizer_config = GPOS_NEW(mp)
          COptimizerConfig(CEnumeratorConfig::GetEnumeratorCfg(mp, 0 /*plan_id*/),
      ...
      CDXLNode *pdxlnPlan = CMinidumperUtils::PdxlnExecuteMinidump(
          mp, rgszNegativeIndexApplyFileNames[ul], GPOPT_TEST_SEGMENTS /*ulSegments*/, 1 /*ulSessionId*/, 1, /*ulCmdId*/
          optimizer_config, nullptr                                                                          /*pceeval*/
      );
      GPOS_CHECK_ABORT;
...
//
//---------------------------------------------------------------------------
BOOL CICGTest::FDXLOpSatisfiesPredicate(CDXLNode *pdxl, FnDXLOpPredicate fdop) {
  using namespace gpdxl;
...
//
//---------------------------------------------------------------------------
BOOL CICGTest::FIsNotIndexJoin(CDXLOperator *dxl_op) {
...
//
//---------------------------------------------------------------------------
BOOL CICGTest::FHasNoIndexJoin(CDXLNode *pdxl) {
...
CICGTest::EresUnittest_PreferHashJoinVersusIndexJoinWhenRiskIsHigh() {
  ...
  ICostModelParams::SCostParam *pcpUnlimited =
      GPOS_NEW(mp) ICostModelParams::SCostParam(CCostModelParamsGPDB::EcpIndexJoinAllowedRiskThreshold,
                                                gpos::ulong_max,  // dVal
                                                0,                // dLowerBound
                                                gpos::ulong_max   // dUpperBound
      );
  pdrgpcpUnlimited->Append(pcpUnlimited);
  GPOS_RESULT eres = CTestUtils::EresCheckOptimizedPlan(rgszPreferHashJoinVersusIndexJoin,
  ...
  eres = CTestUtils::EresCheckOptimizedPlan(rgszPreferHashJoinVersusIndexJoin,
                                            GPOS_ARRAY_SIZE(rgszPreferHashJoinVersusIndexJoin),
