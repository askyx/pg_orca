...
class CConstraintTest {
 ...
 private:
  ...
  static CRangeArray *Pdrgprng(CMemoryPool *mp, IMDId *mdid, const SRangeInfo rgRangeInfo[], ULONG ulRanges);

  static CConstraintInterval *PciFirstInterval(CMemoryPool *mp, IMDId *mdid, CColRef *colref);

  static CConstraintInterval *PciSecondInterval(CMemoryPool *mp, IMDId *mdid, CColRef *colref);

  // interval from scalar comparison
  static GPOS_RESULT EresUnittest_CIntervalFromScalarCmp(CMemoryPool *mp, CMDAccessor *md_accessor, CColRef *colref);

  // generate comparison expression
  static CExpression *PexprScalarCmp(CMemoryPool *mp, CMDAccessor *md_accessor, CColRef *colref,
                                     IMDType::ECmpType cmp_type, LINT val);

  // interval from scalar bool op
  static GPOS_RESULT EresUnittest_CIntervalFromScalarBoolOp(CMemoryPool *mp, CMDAccessor *md_accessor, CColRef *colref);

  // debug print
  static void PrintConstraint(CMemoryPool *mp, CConstraint *pcnstr);

  // build a conjunction
  static CConstraintConjunction *Pcstconjunction(CMemoryPool *mp, IMDId *mdid, CColRef *colref);

  // build a disjunction
  static CConstraintDisjunction *Pcstdisjunction(CMemoryPool *mp, IMDId *mdid, CColRef *colref);
