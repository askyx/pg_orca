...
class CSearchStrategyTest {
 private:
  ...
  using PfnOptimize = void (*)(CMemoryPool *, CExpression *, CSearchStageArray *);
  ...
  static void Optimize(CMemoryPool *mp, Pfpexpr pfnGenerator, CSearchStageArray *search_stage_array,
                       PfnOptimize pfnOptimize);

  static void BuildMemo(CMemoryPool *mp, CExpression *pexprInput, CSearchStageArray *search_stage_array);
