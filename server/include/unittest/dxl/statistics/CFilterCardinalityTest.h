...
class CFilterCardinalityTest {
 ...
 private:
  ...
  static GPOS_RESULT EresUnittest_CStatistics(SStatsFilterSTestCase rgstatsdisjtc[], ULONG ulTestCases);
  ...
  static CStatsPred *PstatspredDisjOverConjMultipleIdenticalCols(CMemoryPool *mp);
  ...
  static CStatsPredPtrArry *PdrgpstatspredInteger(CMemoryPool *mp, ULONG colid,
                                                  CStatsPred::EStatsCmpType stats_cmp_type, INT *piVals, ULONG ulVals);

  // create a numeric predicate on a particular column
  static CStatsPredPtrArry *PdrgppredfilterNumeric(CMemoryPool *mp, ULONG colid, SStatsCmpValElem statsCmpValElem);
  ...
  static GPOS_RESULT EresUnittest_CStatisticsCompare(CMemoryPool *mp, CMDAccessor *md_accessor,
