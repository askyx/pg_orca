...
class CCacheTest {
 private:
  ...
  struct SSimpleObject : public CRefCount {
    ...
    static ULONG UlMyHash(ULONG *const &pvKey) { return *pvKey; }

    // key equality function
    static BOOL FMyEqual(ULONG *const &pvKey, ULONG *const &pvKeySecond);

    // equality for object-based comparison
    BOOL operator==(const SSimpleObject &obj) const { return obj.m_ulKey == m_ulKey; }
  ...
  static void CheckGenerationSanityAfterEviction(CCache<SSimpleObject *, ULONG *> *pCache, ULLONG ullOneElemSize,
  ...
  class CDeepObject : public CRefCount {
   ...
   public:
    ...
    // ctor
    CDeepObject() { m_list.Init(GPOS_OFFSET(SDeepObjectEntry, m_link)); }

    // hashing  function
    static ULONG UlMyHash(CDeepObject::CDeepObjectList *const &plist);

    // key equality function
    static BOOL FMyEqual(CDeepObject::CDeepObjectList *const &pvKey, CDeepObject::CDeepObjectList *const &pvKeySecond);

    // key accessor
    CDeepObjectList *Key() { return &m_list; }
  ...
  using CDeepObjectCacheAccessor = CCacheAccessor<CDeepObject *, CDeepObject::CDeepObjectList *>;
