...
CMessageRepositoryTest::EresUnittest_Basic() {
  ...
  GPOS_UNITTEST_ASSERT(GPOS_MATCH_EX(pmsg->m_exception, CException::ExmaSystem, CException::ExmiOOM));
  ...
  GPOS_TRY {
    ...
    pmsg = CMessageRepository::GetMessageRepository()->LookupMessage(CException(CException::ExmaSystem, 1234567),
                                                                     ElocEnUS_Utf8);
  }
  GPOS_CATCH_EX(exc) {
    GPOS_UNITTEST_ASSERT(GPOS_MATCH_EX(exc, CException::ExmaSystem, CException::ExmiAssert));
    GPOS_RESET_EX;
  }
  GPOS_CATCH_END;
