...
//
//---------------------------------------------------------------------------
BOOL CCacheTest::SSimpleObject::FMyEqual(ULONG *const &pvKey, ULONG *const &pvKeySecond) {
  BOOL fReturn = false;
...
//
//---------------------------------------------------------------------------
BOOL CCacheTest::CDeepObject::FMyEqual(CDeepObject::CDeepObjectList *const &plist,
  ...
  if (nullptr == plist && nullptr == plistSecond) {
    ...
    if (plist->Size() != plistSecond->Size()) {
      ...
      while (nullptr != pdoe) {
        GPOS_UNITTEST_ASSERT(nullptr != pdoeSecond && "Reached a NULL entry in the second list");
...
//
//---------------------------------------------------------------------------
void CCacheTest::CDeepObject::AddEntry(CMemoryPool *mp, ULONG ulKey, ULONG ulVal) {
...
CCacheTest::EresUnittest_Basic() {
  ...
  apcache = CCacheFactory::CreateCache<SSimpleObject *, ULONG *>(fUnique, UNLIMITED_CACHE_QUOTA,
                                                                 SSimpleObject::UlMyHash, SSimpleObject::FMyEqual);
  ...
  // insertion - scope for accessor
  {
    ...
    SSimpleObject *psoReturned GPOS_ASSERTS_ONLY = ca.Insert(&(pso->m_ulKey), pso);

    // release the ownership from pso, but ccacheentry still has the ownership
    pso->Release();

    GPOS_UNITTEST_ASSERT(psoReturned == pso && "Incorrect cache entry was inserted");
    ...
    if (pcache->AllowsDuplicateKeys()) {
      ...
      SSimpleObject *psoReturned GPOS_ASSERTS_ONLY = ca.Insert(&(psoDuplicate->m_ulKey), psoDuplicate);

      GPOS_UNITTEST_ASSERT(psoReturned == pso && "Duplicate insertion must fail");
      GPOS_UNITTEST_ASSERT(1 == pcache->Size());
    ...
    // lookup - scope for accessor
    {
      ...
      GPOS_UNITTEST_ASSERT_IMP(!pcache->AllowsDuplicateKeys(), nullptr != pso && 2 == pso->m_ulValue);
      GPOS_UNITTEST_ASSERT_IMP(pcache->AllowsDuplicateKeys(), nullptr == pso);
    }
    ...
    GPOS_UNITTEST_ASSERT(nullptr != psoReturned && 2 == psoReturned->m_ulValue);
  }
...
CCacheTest::EresUnittest_Refcount() {
  ...
  // Scope of the accessor when we insert
  {
    ...
    GPOS_UNITTEST_ASSERT(3 == pso->RefCount() && "Expected refcount to be 3");
    GPOS_UNITTEST_ASSERT(psoReturned == pso && "Incorrect cache entry was inserted");
  }

  GPOS_UNITTEST_ASSERT(2 == pso->RefCount() && "Expected refcount to be 2 because CCacheAccessor goes out of scope");

  {
    // Create new access for lookup
    CSimpleObjectCacheAccessor ca(pcache);

    GPOS_UNITTEST_ASSERT(2 == pso->RefCount() && "Expected pso and CCacheEntry to have ownership");

    // Ideally Lookup should return valid object. Until CCache evict and no one has reference to it, this object can't
    // be deleted
    ca.Lookup(&(pso->m_ulKey));

    // 1 by CRefCount, 2 by CCacheEntry constructor, 3 by CCache Accessor, 4 by Lookup
    GPOS_UNITTEST_ASSERT(4 == pso->RefCount() &&
    ...
    pso->Release();
    GPOS_UNITTEST_ASSERT(3 == pso->RefCount() && "Expected CCacheEntry, CCacheAccessor and customer to have ownership");
  }
...
CCacheTest::InsertOneElement(CCache<SSimpleObject *, ULONG *> *pCache, ULONG ulKey) {
  ...
  SSimpleObject *pso = nullptr;
  {
    ...
    GPOS_UNITTEST_ASSERT(3 == pso->RefCount() && "Expected pso, cacheentry and cacheaccessor to have ownership");
    // Remove the ownership of pso. Still CCacheEntry has the ownership
    pso->Release();
    GPOS_UNITTEST_ASSERT(2 == pso->RefCount() && "Expected pso and cacheentry to have ownership");
    ulTotalAllocatedSize = mp->TotalAllocatedSize();
  }
  GPOS_UNITTEST_ASSERT(1 == pso->RefCount() && "Expected only cacheentry to have ownership");
...
CCacheTest::ULFillCacheWithoutEviction(CCache<SSimpleObject *, ULONG *> *pCache, ULONG ulKeyStart) {
  ...
  GPOS_UNITTEST_ASSERT((ullOneElemCacheSize > ullInitialCacheSize || ullOldEvictionCounter < ullNewEvictionCounter) &&
  ...
  ULLONG ullSizeBeforeEviction GPOS_ASSERTS_ONLY = pCache->TotalAllocatedSize();
  ...
  GPOS_UNITTEST_ASSERT_IMP(0 == ullInitialCacheSize, ullSizeBeforeEviction == ullExpectedCacheSize &&
...
//---------------------------------------------------------------------------
void CCacheTest::CheckGenerationSanityAfterEviction(CCache<SSimpleObject *, ULONG *> *pCache, ULLONG ullOneElemSize,
                                                    ULONG ulOldGenBeginKey, ULONG ulOldGenEndKey,
  ...
  GPOS_UNITTEST_ASSERT(uloldGenEntryCount + ulNewGenEntryCount >=
...
//---------------------------------------------------------------------------
void CCacheTest::TestEvictionForOneCacheSize(ULLONG ullCacheQuota) {
  CAutoP<CCache<SSimpleObject *, ULONG *> > apCache;
  apCache = CCacheFactory::CreateCache<SSimpleObject *, ULONG *>(false, /* not an unique cache */
                                                                 ullCacheQuota, SSimpleObject::UlMyHash,
  ...
  CheckGenerationSanityAfterEviction(pCache, ullOneElemSize, ulLastKeyFirstGen + 2, ulLastKeySecondGen,
                                     ulLastKeyThirdGen);
...
CCacheTest::EresRemoveDuplicates(CCache<SSimpleObject *, ULONG *> *pcache) {
  for (ULONG i = 0; i < GPOS_CACHE_ELEMENTS; i++) {
    ...
    while (nullptr != pso) {
      ...
      GPOS_UNITTEST_ASSERT(pso->m_ulValue < GPOS_CACHE_DUPLICATES && "Incorrect entry was found");
    ...
    GPOS_UNITTEST_ASSERT(count == GPOS_CACHE_DUPLICATES_TO_DELETE && "Incorrect number of deleted entries");
  }
...
CCacheTest::EresUnittest_DeepObject() {
  ...
  // insertion - scope for accessor
  {
    ...
    GPOS_UNITTEST_ASSERT(nullptr != pdoReturned && "Incorrect cache entry was inserted");
    ...
    if (pcache->AllowsDuplicateKeys()) {
      ...
      CDeepObject *pdoReturned GPOS_ASSERTS_ONLY = ca.Insert(pdoDuplicate->Key(), pdoDuplicate);

      GPOS_UNITTEST_ASSERT(pdoReturned == pdo && "Duplicate insertion must fail");
    ...
    // lookup - scope for accessor
    {
      ...
      GPOS_UNITTEST_ASSERT_IMP(pcache->AllowsDuplicateKeys(), nullptr == pdo);
      GPOS_UNITTEST_ASSERT_IMP(!pcache->AllowsDuplicateKeys(), nullptr != pdo);
    ...
    GPOS_UNITTEST_ASSERT(nullptr != pdoReturned && 3 == CDeepObject::UlMyHash(pdoReturned->Key()));
  }
...
CCacheTest::EresUnittest_Iteration() {
  ...
  for (ULONG i = 0; i < GPOS_CACHE_ELEMENTS; i++) {
    ...
    while (nullptr != pso) {
      ...
      GPOS_UNITTEST_ASSERT(ulDuplicates > pso->m_ulValue && "Incorrect entry was found");

      count++;
      pso = ca.Next();
    }
    GPOS_UNITTEST_ASSERT(count == ulDuplicates && "Incorrect number of duplicates");
  }
...
GPOS_RESULT
CCacheTest::EresUnittest_IterativeDeletion() {
  GPOS_UNITTEST_ASSERT(GPOS_CACHE_DUPLICATES >= GPOS_CACHE_DUPLICATES_TO_DELETE);
  ...
  for (ULONG i = 0; i < GPOS_CACHE_ELEMENTS; i++) {
    ...
    while (nullptr != pso) {
      ...
      GPOS_UNITTEST_ASSERT(pso->m_ulValue >= ulDuplicatesToDelete && "Incorrect entry value was found");
      count++;
      pso = ca.Next();
    }

    GPOS_UNITTEST_ASSERT(count == ulRemaining && "Incorrect number of remaining duplicates");
  }
