...
// This tests the behavior of AppendFormat for unicode characters when the locale settings of the system are
// incompatible AppendFormat uses a C library function, vswprintf(), whose behavior is platform specific. vswprintf()
// returns with error on non-Darwin platforms when the locale is incompatible with unicode string. Hence do not run this
...
CWStringTest::EresUnittest_AppendFormatInvalidLocale() {
  ...
  CWStringDynamic *expected = GPOS_NEW(mp) CWStringDynamic(mp, GPOS_WSZ_LIT("UNKNOWN"));
...
CWStringTest::EresUnittest_AppendFormatLarge() {
  ...
  // append small string
  CWStringDynamic *pstr2 = GPOS_NEW(mp) CWStringDynamic(mp, GPOS_WSZ_LIT("Hello"));
  ...
  // append large string
  CWStringDynamic *pstr3 = GPOS_NEW(mp) CWStringDynamic(mp, GPOS_WSZ_LIT("Hello "));
  ...
  WCHAR w_str[25];
  CWStringStatic *pstr4 = GPOS_NEW(mp) CWStringStatic(w_str, GPOS_ARRAY_SIZE(w_str), GPOS_WSZ_LIT("Hello"));
