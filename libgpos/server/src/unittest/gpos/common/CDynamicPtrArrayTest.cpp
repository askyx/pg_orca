...
CDynamicPtrArrayTest::EresUnittest_Basic() {
  ...
  CDynamicPtrArray<CHAR, CleanupNULL<CHAR>> *pdrg = GPOS_NEW(mp) CDynamicPtrArray<CHAR, CleanupNULL<CHAR>>(mp, 2);
...
CDynamicPtrArrayTest::EresUnittest_PdrgpulSubsequenceIndexes() {
  ...
  // should return NULL
  GPOS_UNITTEST_ASSERT(nullptr == pdrgULONGTarget->IndexesOfSubsequence(pdrgULONGLookup));
...
CDynamicPtrArrayTest::EresUnittest_Sort() {
  ...
  for (ULONG ul = 0; ul < unsortedStringArray->Size(); ul++) {
    GPOS_UNITTEST_ASSERT((*unsortedStringArray)[ul]->Equals((*sortedStringArray)[ul]));
  }
