...
CHashMapTest::EresUnittest_Basic() {
  ...
  using UlongPtrToCharMap =
      CHashMap<ULONG_PTR, CHAR, HashPtr<ULONG_PTR>, gpos::Equals<ULONG_PTR>, CleanupNULL<ULONG_PTR>, CleanupNULL<CHAR>>;

  UlongPtrToCharMap *phm = GPOS_NEW(mp) UlongPtrToCharMap(mp, 128);
  for (ULONG i = 0; i < ulCnt; ++i) {
    BOOL fSuccess GPOS_ASSERTS_ONLY = phm->Insert(&rgul[i], (CHAR *)rgsz[i]);
  ...
  using UlongToUlongMap =
      CHashMap<ULONG, ULONG, HashValue<ULONG>, gpos::Equals<ULONG>, CleanupDelete<ULONG>, CleanupDelete<ULONG>>;
...
CHashMapTest::EresUnittest_Ownership() {
  ...
  using UlongPtrToCharMap = CHashMap<ULONG_PTR, CHAR, HashPtr<ULONG_PTR>, gpos::Equals<ULONG_PTR>,
