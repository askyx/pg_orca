...
void CMemoryPoolTracker::RecordAllocation(SAllocHeader *header) {
  m_memory_pool_statistics.RecordAllocation(header->m_user_size, header->m_alloc_size);
  m_allocations_list.Prepend(header);
}

void CMemoryPoolTracker::RecordFree(SAllocHeader *header) {
...
void CMemoryPoolTracker::TearDown() {
...
void CMemoryPoolTracker::WalkLiveObjects(gpos::IMemoryVisitor *visitor) {
  ...
  while (nullptr != header) {
    ...
    visitor->Visit(user, header->m_user_size, header, header->m_alloc_size, header->m_filename, header->m_line,
                   header->m_serial,
#ifdef GPOS_DEBUG
                   &header->m_stack_desc
#else
