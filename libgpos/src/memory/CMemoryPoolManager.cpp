...
CMemoryPoolManager::CMemoryPoolManager(CMemoryPool *internal, EMemoryPoolType memory_pool_type)
    : m_internal_memory_pool(internal), m_ht_all_pools(nullptr), m_memory_pool_type(memory_pool_type) {
...
CMemoryPool *CMemoryPoolManager::NewMemoryPool() {
  return GPOS_NEW(m_internal_memory_pool) CMemoryPoolTracker();
}

// Release given memory pool
void CMemoryPoolManager::Destroy(CMemoryPool *mp) {
  GPOS_ASSERT(nullptr != m_memory_pool_mgr);
...
void CMemoryPoolManager::DeleteImpl(void *ptr, CMemoryPool::EAllocationType eat) {
...
IOstream &CMemoryPoolManager::OsPrint(IOstream &os) {
...
void CMemoryPoolManager::DestroyMemoryPoolAtShutdown(CMemoryPool *mp) {
  GPOS_ASSERT(nullptr != mp);
...
void CMemoryPoolManager::Cleanup() {
  GPOS_ASSERT(nullptr != m_memory_pool_mgr);
  ...
  if (0 < m_memory_pool_mgr->m_global_memory_pool->TotalAllocatedSize()) {
    ...
    gpos::oswcerr << "Memory leaks detected" << std::endl << *m_memory_pool_mgr->m_global_memory_pool << std::endl;
  }
#endif  // GPOS_DEBUG
  ...
  m_memory_pool_mgr->m_ht_all_pools->DestroyEntries(DestroyMemoryPoolAtShutdown);
  GPOS_DELETE(m_memory_pool_mgr->m_ht_all_pools);
}

// Delete memory pools and release manager
void CMemoryPoolManager::Shutdown() {
  GPOS_ASSERT(nullptr != m_memory_pool_mgr);
