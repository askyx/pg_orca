...
ULONG
CDebugCounter::SDebugCounterKey::HashValue(const CDebugCounter::SDebugCounterKey *key) {
  ULONG result = 0;
  ...
  if (0 < m_instance->m_hashmap->Size()) {
    if (!m_instance->m_is_name_constant_get) {
      ...
      while (iter.Advance()) {
        ...
        wstr.Reset();
        os << "CDebugCounterEvent(qryid, qryname, counter, type, val), " << m_instance->m_qry_number << ", "
           << m_instance->m_qry_name.c_str() << ", " << iter.Key()->m_counter_name.c_str() << ", " << typeString
           << ", ";
...
BOOL CDebugCounter::FindByName(const char *counter_name, SDebugCounterKey **key, SDebugCounterValue **val,
...
// by FindByName()
void CDebugCounter::InsertOrUpdateCounter(SDebugCounterKey *key, SDebugCounterValue *val, BOOL update) {
  if (update) {
...
void CDebugCounter::Bump(const char *counter_name) {
  if (OkToProceed()) {
    ...
    BOOL found = m_instance->FindByName(counter_name, &key, &val, ECounterTypeCount);
...
void CDebugCounter::Add(const char *counter_name, long delta) {
  if (OkToProceed()) {
    ...
    SDebugCounterKey *key = NULL;
    SDebugCounterValue *val = NULL;
    BOOL found = m_instance->FindByName(counter_name, &key, &val, ECounterTypeSum);
...
void CDebugCounter::AddDouble(const char *counter_name, double delta) {
  if (OkToProceed()) {
    ...
    SDebugCounterKey *key = NULL;
    SDebugCounterValue *val = NULL;
    BOOL found = m_instance->FindByName(counter_name, &key, &val, ECounterTypeSumDouble);
...
void CDebugCounter::StartCpuTime(const char *counter_name) {
  if (OkToProceed()) {
    ...
    SDebugCounterKey *key = NULL;
    SDebugCounterValue *val = NULL;
    BOOL found = m_instance->FindByName(counter_name, &key, &val, ECounterTypeCpuTime);
