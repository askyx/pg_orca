...
//
//---------------------------------------------------------------------------
CUnittest::CUnittest(const CHAR *szTitle, ETestType ett, GPOS_RESULT (*pfunc)(void), ULONG major, ULONG minor)
    : m_szTitle(szTitle),
      m_ett(ett),
      m_pfunc(pfunc),
      m_pfuncSubtest(nullptr),
...
//
//---------------------------------------------------------------------------
CUnittest::CUnittest(const CHAR *szTitle, ETestType ett, GPOS_RESULT (*pfuncSubtest)(ULONG), ULONG ulSubtest)
    : m_szTitle(szTitle),
      m_ett(ett),
      m_pfunc(nullptr),
...
//
//---------------------------------------------------------------------------
BOOL CUnittest::Equals(CHAR *sz) const {
...
//
//---------------------------------------------------------------------------
BOOL CUnittest::FThrows(ULONG major, ULONG minor) const {
...
CUnittest::EresExecTest(const CUnittest &ut) {
  ...
  GPOS_TRY_HDL(&errhdl) {
    ...
    eres = ut.m_pfunc != nullptr ? ut.m_pfunc() : ut.m_pfuncSubtest(ut.m_ulSubtest);
...
CUnittest::EresExecute(const CUnittest *rgut, const ULONG cSize) {
  ...
  for (ULONG i = 0; i < cSize; i++) {
    ...
    GPOS_TRACE_FORMAT("Unittest %s...%s.", ut.m_szTitle, (GPOS_OK == eresPart ? "OK" : "*** FAILED ***"));
...
//
//---------------------------------------------------------------------------
void CUnittest::FindTest(CBitVector &bv, ETestType ett, CHAR *szTestName) {
  for (ULONG i = 0; i < CUnittest::m_ulTests; i++) {
    CUnittest &ut = CUnittest::m_rgut[i];

    if ((ut.Ett() == ett && (nullptr == szTestName || ut.Equals(szTestName))) ||
...
//
//---------------------------------------------------------------------------
void CUnittest::SetTraceFlag(const CHAR *szTrace) {
  CHAR *pcEnd = nullptr;
  ...
  // scope of timer
  {
    ...
    for (ULONG i = 0; i < CUnittest::m_ulTests; i++) {
      if (pbv->Get(i)) {
        ...
        GPOS_UNITTEST_ASSERT((GPOS_OK == eres || GPOS_FAILED == eres) && "Unexpected result from unittest");
...
//
//---------------------------------------------------------------------------
void CUnittest::Init(CUnittest *rgut, ULONG ulUtCnt, void (*pfConfig)(), void (*pfCleanup)()) {
  GPOS_UNITTEST_ASSERT(0 == m_ulTests && "Unittest array has already been initialized");
