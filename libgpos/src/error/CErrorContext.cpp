...
//
//---------------------------------------------------------------------------
void CErrorContext::Record(CException &exc, VA_LIST vl) {
  if (m_serializing) {
    return;
  }
...
//
//---------------------------------------------------------------------------
void CErrorContext::AppendErrnoMsg() {
  GPOS_ASSERT(m_pending);
...
//
//---------------------------------------------------------------------------
void CErrorContext::CopyPropErrCtxt(const IErrorContext *err_ctxt) {
  GPOS_ASSERT(!m_pending);
  ...
  // copy error message
  m_static_buffer.Reset();
  m_static_buffer.Append(&(dynamic_cast<const CErrorContext *>(err_ctxt)->m_static_buffer));
