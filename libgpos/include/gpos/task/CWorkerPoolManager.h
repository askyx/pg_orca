...
class CWorkerPoolManager {
 ...
 private:
  ...
  void AddRef() { m_auto_task_proxy_counter++; }

  // decrement AutoTaskProxy reference counter
  void RemoveRef() {
    GPOS_ASSERT(m_auto_task_proxy_counter != 0 && "AutoTaskProxy counter decremented from 0");
 ...
 public:
  ...
  // lookup own worker
  inline CWorker *Self() { return m_single_worker; }

  // dtor
  ~CWorkerPoolManager() { GPOS_ASSERT(nullptr == m_worker_pool_manager && "Worker pool has not been shut down"); }
  ...
  // global accessor
  inline static CWorkerPoolManager *WorkerPoolManager() { return m_worker_pool_manager; }
