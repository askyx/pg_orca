...
class CCache {
 ...
 private:
  ...
  using CCacheHashtableAccessor = CSyncHashtableAccessByKey<CCacheHashTableEntry, K>;
  using CCacheHashtableIter = CSyncHashtableIter<CCacheHashTableEntry, K>;
  using CCacheHashtableIterAccessor = CSyncHashtableAccessByIter<CCacheHashTableEntry, K>;
  ...
  static void DestroyCacheEntryWithRefCountTest(CCacheHashTableEntry *entry) {
    GPOS_ASSERT(nullptr != entry);

    // This assert is valid only when ccache get deleted. At that point nobody should hold a pointer to an object in
    // CCache. If ref count is not 1, then we possibly have a leak.
    GPOS_ASSERT(EXPECTED_REF_COUNT_FOR_DELETE == entry->RefCount() &&
  ...
  static void DestroyCacheEntry(CCacheHashTableEntry *entry) {
    GPOS_ASSERT(nullptr != entry);

    // destroy the object before deleting memory pool. This cover the case where object & cacheentry use same memory
    // pool
 ...
 public:
  ...
  CCache(CMemoryPool *mp, BOOL unique, ULLONG cache_quota, ULONG g_clock_init_counter, HashFuncPtr hash_func,
    ...
    GPOS_ASSERT(nullptr != m_mp && "Cache memory pool could not be initialized");

    GPOS_ASSERT(0 != g_clock_init_counter);

    // initialize hashtable
    m_hash_table.Init(m_mp, CACHE_HT_NUM_OF_BUCKETS, GPOS_OFFSET(CCacheHashTableEntry, m_link_hash),
                      GPOS_OFFSET(CCacheHashTableEntry, m_key), (&CCacheHashTableEntry::m_invalid_key), m_hash_func,
                      m_equal_func);
  ...
  float GetEvictionFactor() { return m_eviction_factor; }
