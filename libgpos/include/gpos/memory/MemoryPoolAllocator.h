...
class MemoryPoolAllocator {
 ...
 public:
  ...
  template <class U>
  explicit MemoryPoolAllocator(const MemoryPoolAllocator<U> &other) : mp_(other.mp_) {}

  value_type *allocate(std::size_t n) {
    size_t bytes = sizeof(value_type) * n;
    void *addr = mp_->NewImpl(bytes, __FILE__, __LINE__, CMemoryPool::EatArray);
    return static_cast<value_type *>(addr);
  }

  void deallocate(value_type *p, std::size_t) { CMemoryPool::DeleteImpl(p, CMemoryPool::EatArray); }

  bool operator==(const MemoryPoolAllocator &rhs) const { return mp_ == rhs.mp_; }

  bool operator!=(const MemoryPoolAllocator &rhs) const { return !(*this == rhs); }
};
