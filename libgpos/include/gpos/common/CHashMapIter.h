...
class CHashMapIter : public CStackObject {
 ...
 public:
  CHashMapIter(const CHashMapIter<K, T, HashFn, EqFn, DestroyKFn, DestroyTFn> &) = delete;

  // ctor
  CHashMapIter(TMap *ptm) : m_map(ptm), m_chain_idx(0), m_key_idx(0) { GPOS_ASSERT(nullptr != ptm); }
