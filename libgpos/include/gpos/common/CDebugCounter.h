...
class CDebugCounter {
  ...
  struct SDebugCounterKey : public CRefCount {
    SDebugCounterKey(const char *counter_name) : m_counter_name(counter_name) {}

    ~SDebugCounterKey() {}

    static ULONG HashValue(const SDebugCounterKey *key);
    static BOOL Equals(const SDebugCounterKey *first, const SDebugCounterKey *second) {
  ...
  struct SDebugCounterValue : public CRefCount {
    SDebugCounterValue(ECounterType typ)
        : m_type(typ), m_counter_val_long(0), m_counter_val_double(0.0), m_cpu_timer(NULL), m_timer_is_running(false) {}
    ~SDebugCounterValue() {
      if (NULL != m_cpu_timer)
 ...
 private:
  ...
  BOOL FindByName(const char *counter_name, SDebugCounterKey **key, SDebugCounterValue **val, enum ECounterType typ);

  // insert or update a key value pair that was generated
  // by FindByName()
  void InsertOrUpdateCounter(SDebugCounterKey *key, SDebugCounterValue *val, BOOL update);
